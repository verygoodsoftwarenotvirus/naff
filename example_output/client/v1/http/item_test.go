		...
		ts := httptest.NewTLSServer(
			http.HandlerFunc(
				func(res http.ResponseWriter, req *http.Request) {
					...
					assert.Equal(t, req.URL.Path, fmt.Sprintf("/api/v1/item/%d", expected.ID), "expected and actual path don't match")
...
func TestV1Client_GetItems(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		ts := httptest.NewTLSServer(
			http.HandlerFunc(
				func(res http.ResponseWriter, req *http.Request) {
					assert.Equal(t, req.URL.Path, "/api/v1/item", "expected and actual path don't match")
					assert.Equal(t, req.Method, http.MethodGet)
					require.NoError(t, json.NewEncoder(res).Encode(expected))
				},
			),
...
func TestV1Client_ArchiveItem(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		ts := httptest.NewTLSServer(
			http.HandlerFunc(
				func(res http.ResponseWriter, req *http.Request) {
					assert.Equal(t, req.URL.Path, fmt.Sprintf("/api/v1/item/%d", expected), "expected and actual path don't match")
