...
func TestService_CreationInputMiddleware(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		ed.On("DecodeRequest", mock.AnythingOfType("*http.Request"), mock.Anything).Return(nil)
		s.encoderDecoder = ed
		mh := &mockHTTPHandler{}
	...
	T.Run("with error decoding request", func(t *testing.T) {
		s := buildTestService(t)
		ed := &mockencoding.EncoderDecoder{}
		ed.On("DecodeRequest", mock.AnythingOfType("*http.Request"), mock.Anything).Return(errors.New("blah"))
		s.encoderDecoder = ed
		mh := &mockHTTPHandler{}
		mh.On("ServeHTTP", mock.Anything, mock.Anything)
		h := s.CreationInputMiddleware(mh)
...
func TestService_RequestIsAuthenticated(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		mh.On("ValidationBearerToken", mock.AnythingOfType("*http.Request")).Return(&models.Token{
	...
	T.Run("with error validating token", func(t *testing.T) {
		s := buildTestService(t)
		mh := &mockOauth2Handler{}
		mh.On("ValidationBearerToken", mock.AnythingOfType("*http.Request")).Return((*models.Token)(nil), errors.New("blah"))
		s.oauth2Handler = mh
		req := buildRequest(t)
		actual, err := s.ExtractOAuth2ClientFromRequest(req.Context(), req)
		assert.Error(t, err)
...
func TestService_OAuth2TokenAuthenticationMiddleware(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		res := httptest.NewRecorder()
		mhh := &mockHTTPHandler{}
		mhh.On("ServeHTTP", mock.Anything, mock.AnythingOfType("*http.Request")).Return()
...
func TestService_fetchOAuth2ClientFromRequest(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		req := buildRequest(t).WithContext(context.WithValue(context.Background(), models.OAuth2ClientKey, expected))
...
func TestService_fetchOAuth2ClientIDFromRequest(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		req := buildRequest(t).WithContext(context.WithValue(context.Background(), clientIDKey, expected.ClientID))
