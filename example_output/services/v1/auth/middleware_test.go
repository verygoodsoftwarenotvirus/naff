...
func TestService_UserLoginInputMiddleware(T *testing.T) {
	...
	T.Run("with error decoding request but valid value attached to form", func(t *testing.T) {
		...
		req, err := http.NewRequest(http.MethodPost, "http://todo.verygoodsoftwarenotvirus.ru", strings.NewReader(form.Encode()))
		require.NoError(t, err)
		require.NotNil(t, req)
		res := httptest.NewRecorder()
...
func TestService_AdminMiddleware(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		require.NotNil(t, req)
		res := httptest.NewRecorder()
		req = req.WithContext(context.WithValue(req.Context(), models.UserKey, &models.User{
			IsAdmin: true,
		}))
		s := buildTestService(t)
		ms := &MockHTTPHandler{}
