...
func TestService_DecodeCookieFromRequest(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		c, err := s.buildAuthCookie(&models.User{
			ID:       1,
			Username: "username",
		})
		require.NoError(t, err)
...
func TestService_WebsocketAuthFunction(T *testing.T) {
	...
	T.Run("with valid oauth2 client", func(t *testing.T) {
		...
		s.oauth2ClientsService.(*mockOAuth2ClientValidator).On("ExtractOAuth2ClientFromRequest", mock.Anything).Return(expected, nil)
	...
	T.Run("with valid cookie", func(t *testing.T) {
		...
		s.oauth2ClientsService.(*mockOAuth2ClientValidator).On("ExtractOAuth2ClientFromRequest", mock.Anything).Return(oac, errors.New("blah"))
		req, err := http.NewRequest(http.MethodGet, "http://todo.verygoodsoftwarenotvirus.ru/testing", nil)
		require.NotNil(t, req)
		require.NoError(t, err)
		c, err := s.buildAuthCookie(&models.User{
...
func TestService_FetchUserFromRequest(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		require.NoError(t, err)
		req.AddCookie(c)
		s.userDB.(*mockmodels.UserDataManager).On("GetUser", mock.Anything, userID).Return(expectedUser, nil)
	...
	T.Run("with error fetching user", func(t *testing.T) {
		...
		s.userDB.(*mockmodels.UserDataManager).On("GetUser", mock.Anything, userID).Return((*models.User)(nil), expectedError)
		actualUser, err := s.FetchUserFromRequest(req.Context(), req)
		assert.Nil(t, actualUser)
		assert.Error(t, err)
	})
...
func TestService_Login(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		s.userDB.(*mockmodels.UserDataManager).On("GetUserByUsername", mock.Anything, expectedUser.Username).Return(expectedUser, nil)
		s.authenticator.(*mockauth.Authenticator).On("ValidateLogin", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(true, nil)
	...
	T.Run("with error fetching login data from request", func(t *testing.T) {
		...
		s.userDB.(*mockmodels.UserDataManager).On("GetUserByUsername", mock.Anything, expectedUser.Username).Return(expectedUser, errors.New("arbitrary"))
		exampleLoginData := &models.UserLoginInput{
			Username:  expectedUser.Username,
			Password:  "password",
			TOTPToken: "123456",
	...
	T.Run("with error encoding error fetching login dataa", func(t *testing.T) {
		s := buildTestService(t)
	...
	T.Run("with invalid login", func(t *testing.T) {
		...
		s.userDB.(*mockmodels.UserDataManager).On("GetUserByUsername", mock.Anything, expectedUser.Username).Return(expectedUser, nil)
		s.authenticator.(*mockauth.Authenticator).On("ValidateLogin", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(false, nil)
		exampleLoginData := &models.UserLoginInput{
			Username:  expectedUser.Username,
			Password:  "password",
			TOTPToken: "123456",
	...
	T.Run("with error validating login", func(t *testing.T) {
		...
		s.userDB.(*mockmodels.UserDataManager).On("GetUserByUsername", mock.Anything, expectedUser.Username).Return(expectedUser, nil)
		s.authenticator.(*mockauth.Authenticator).On("ValidateLogin", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(true, errors.New("blah"))
		exampleLoginData := &models.UserLoginInput{
			Username:  expectedUser.Username,
			Password:  "password",
			TOTPToken: "123456",
	...
	T.Run("with error building cookie", func(t *testing.T) {
		...
		cb.On("Encode", mock.Anything, mock.Anything).Return("", errors.New("blah"))
		s.cookieManager = cb
		expectedUser := &models.User{
			Username: "username",
		}
...
func TestService_fetchLoginDataFromRequest(T *testing.T) {
	...
	T.Run("with DB error fetching user", func(t *testing.T) {
		...
		s.userDB.(*mockmodels.UserDataManager).On("GetUserByUsername", mock.Anything, expectedUser.Username).Return((*models.User)(nil), sql.ErrNoRows)
		req, err := http.NewRequest(http.MethodGet, "http://todo.verygoodsoftwarenotvirus.ru/testing", nil)
		require.NotNil(t, req)
		require.NoError(t, err)
		exampleLoginData := &models.UserLoginInput{
	...
	T.Run("with error fetching user", func(t *testing.T) {
		...
		s.userDB.(*mockmodels.UserDataManager).On("GetUserByUsername", mock.Anything, expectedUser.Username).Return((*models.User)(nil), errors.New("blah"))
		req, err := http.NewRequest(http.MethodGet, "http://todo.verygoodsoftwarenotvirus.ru/testing", nil)
		require.NotNil(t, req)
		require.NoError(t, err)
		exampleLoginData := &models.UserLoginInput{
...
func TestService_validateLogin(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		s.authenticator.(*mockauth.Authenticator).On("ValidateLogin", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(expected, nil)
	...
	T.Run("with too weak a password hash", func(t *testing.T) {
		...
		s.authenticator.(*mockauth.Authenticator).On("ValidateLogin", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(expected, auth.ErrPasswordHashTooWeak)
		s.authenticator.(*mockauth.Authenticator).On("HashPassword", mock.Anything, mock.Anything).Return("blah", nil)
		s.userDB.(*mockmodels.UserDataManager).On("UpdateUser", mock.Anything, mock.Anything).Return(nil)
		actual, err := s.validateLogin(ctx, exampleInput)
		assert.Equal(t, expected, actual)
		assert.NoError(t, err)
	})
	...
	T.Run("with too weak a password hash and error hashing the password", func(t *testing.T) {
		...
		s.authenticator.(*mockauth.Authenticator).On("ValidateLogin", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(true, auth.ErrPasswordHashTooWeak)
		s.authenticator.(*mockauth.Authenticator).On("HashPassword", mock.Anything, mock.Anything).Return("", expectedErr)
		actual, err := s.validateLogin(ctx, exampleInput)
		assert.Equal(t, expected, actual)
		assert.Error(t, err)
	})
	...
	T.Run("with too weak a password hash and error updating user", func(t *testing.T) {
		...
		s.authenticator.(*mockauth.Authenticator).On("ValidateLogin", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(true, auth.ErrPasswordHashTooWeak)
		s.authenticator.(*mockauth.Authenticator).On("HashPassword", mock.Anything, mock.Anything).Return("blah", nil)
		s.userDB.(*mockmodels.UserDataManager).On("UpdateUser", mock.Anything, mock.Anything).Return(expectedErr)
		actual, err := s.validateLogin(ctx, exampleInput)
		assert.Equal(t, expected, actual)
		assert.Error(t, err)
	})
	...
	T.Run("with error validating login", func(t *testing.T) {
		...
		s.authenticator.(*mockauth.Authenticator).On("ValidateLogin", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(expected, expectedErr)
		actual, err := s.validateLogin(ctx, exampleInput)
		assert.Equal(t, expected, actual)
		assert.Error(t, err)
	})
