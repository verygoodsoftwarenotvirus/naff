...
func buildTestService(t *testing.T) *Service {
	...
	service, err := ProvideUsersService(context.Background(), config.AuthSettings{}, noop.ProvideNoopLogger(), mockDB, &mockauth.Authenticator{}, func(req *http.Request) uint64 {
		return 0
	}, &mockencoding.EncoderDecoder{}, ucp, newsman.NewNewsman(nil, nil))
...
func TestProvideUsersService(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		service, err := ProvideUsersService(context.Background(), config.AuthSettings{}, noop.ProvideNoopLogger(), mockDB, &mockauth.Authenticator{}, func(req *http.Request) uint64 {
			return 0
		}, &mockencoding.EncoderDecoder{}, ucp, nil)
	...
	T.Run("with nil userIDFetcher", func(t *testing.T) {
		...
		service, err := ProvideUsersService(context.Background(), config.AuthSettings{}, noop.ProvideNoopLogger(), mockDB, &mockauth.Authenticator{}, nil, &mockencoding.EncoderDecoder{}, ucp, nil)
		assert.Error(t, err)
