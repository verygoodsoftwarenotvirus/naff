...
func TestService_validateCredentialChangeRequest(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		auth.On("ValidateLogin", mock.Anything, expected.HashedPassword, examplePassword, expected.TwoFactorSecret, exampleTOTPToken, expected.Salt).Return(true, nil)
		s.authenticator = auth
		actual, sc := s.validateCredentialChangeRequest(req.Context(), expected.ID, examplePassword, exampleTOTPToken)
	...
	T.Run("with no rows found in database", func(t *testing.T) {
		...
		s.database = mockDB
		actual, sc := s.validateCredentialChangeRequest(context.Background(), expected.ID, examplePassword, exampleTOTPToken)
		assert.Nil(t, actual)
	...
	T.Run("with error validating login", func(t *testing.T) {
		...
		s.database = mockDB
		auth := &mockauth.Authenticator{}
		auth.On("ValidateLogin", mock.Anything, expected.HashedPassword, examplePassword, expected.TwoFactorSecret, exampleTOTPToken, expected.Salt).Return(false, errors.New("blah"))
		s.authenticator = auth
		actual, sc := s.validateCredentialChangeRequest(context.Background(), expected.ID, examplePassword, exampleTOTPToken)
		assert.Nil(t, actual)
		assert.Equal(t, http.StatusInternalServerError, sc)
	...
	T.Run("with invalid login", func(t *testing.T) {
		...
		s.database = mockDB
		auth := &mockauth.Authenticator{}
		auth.On("ValidateLogin", mock.Anything, expected.HashedPassword, examplePassword, expected.TwoFactorSecret, exampleTOTPToken, expected.Salt).Return(false, nil)
		s.authenticator = auth
		actual, sc := s.validateCredentialChangeRequest(context.Background(), expected.ID, examplePassword, exampleTOTPToken)
		assert.Nil(t, actual)
...
func TestService_NewTOTPSecret(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		res, req := httptest.NewRecorder(), buildRequest(t)
		req = req.WithContext(context.WithValue(req.Context(), TOTPSecretRefreshMiddlewareCtxKey, exampleInput))
		...
		s.database = mockDB
		auth := &mockauth.Authenticator{}
		auth.On("ValidateLogin", mock.Anything, exampleUser.HashedPassword, exampleInput.CurrentPassword, exampleUser.TwoFactorSecret, exampleInput.TOTPToken, exampleUser.Salt).Return(true, nil)
		s.authenticator = auth
		ed := &mockencoding.EncoderDecoder{}
		ed.On("EncodeResponse", mock.Anything, mock.Anything).Return(nil)
		s.encoderDecoder = ed
	...
	T.Run("with error validating login", func(t *testing.T) {
		...
		s.database = mockDB
		auth := &mockauth.Authenticator{}
		auth.On("ValidateLogin", mock.Anything, exampleUser.HashedPassword, exampleInput.CurrentPassword, exampleUser.TwoFactorSecret, exampleInput.TOTPToken, exampleUser.Salt).Return(false, errors.New("blah"))
		s.authenticator = auth
		ed := &mockencoding.EncoderDecoder{}
		ed.On("EncodeResponse", mock.Anything, mock.Anything).Return(nil)
		s.encoderDecoder = ed
...
func TestService_UpdatePassword(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		req = req.WithContext(context.WithValue(req.Context(), PasswordChangeMiddlewareCtxKey, exampleInput))
		req = req.WithContext(context.WithValue(req.Context(), models.UserIDKey, exampleUser.ID))
		mockDB := database.BuildMockDatabase()
		mockDB.UserDataManager.On("GetUser", mock.Anything, exampleUser.ID).Return(exampleUser, nil)
		mockDB.UserDataManager.On("UpdateUser", mock.Anything, mock.Anything).Return(nil)
