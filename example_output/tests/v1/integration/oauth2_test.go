...
import (
	...
	http "gitlab.com/verygoodsoftwarenotvirus/todo/client/v1/http"
...
func TestOAuth2Clients(test *testing.T) {
	...
	testClient, err := http.NewClient(context.Background(), premade.ClientID, premade.ClientSecret, todoClient.URL, noop.ProvideNoopLogger(), todoClient.PlainClient(), premade.Scopes, debug)
	...
	test.Run("Deleting", func(T *testing.T) {
		...
		T.Run("should be unable to authorize after being deleted", func(t *testing.T) {
			...
			err = testClient.ArchiveOAuth2Client(tctx, premade.ID)
			assert.NoError(t, err)
			c2, err := http.NewClient(context.Background(), premade.ClientID, premade.ClientSecret, todoClient.URL, noop.ProvideNoopLogger(), buildHTTPClient(), premade.Scopes, true)
	...
	test.Run("Listing", func(T *testing.T) {
		T.Run("should be able to be read in a list", func(t *testing.T) {
			...
			checkValueAndError(t, actual, err)
			assert.True(t, len(actual.Clients)-len(expected) > 0, "expected %d - %d to be > 0", len(actual.Clients), len(expected))
