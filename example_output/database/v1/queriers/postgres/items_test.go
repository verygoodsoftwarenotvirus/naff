...
func TestPostgres_CreateItem(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		expectedQuery := "INSERT INTO items (name,details,belongs_to) VALUES ($1,$2,$3) RETURNING id, created_on"
		p, mockDB := buildTestService(t)
		mockDB.ExpectQuery(formatQueryForSQLMock(expectedQuery)).WithArgs(expected.Name, expected.Details, expected.BelongsTo).WillReturnRows(exampleRows)
	...
	T.Run("with error writing to database", func(t *testing.T) {
		...
		expectedQuery := "INSERT INTO items (name,details,belongs_to) VALUES ($1,$2,$3) RETURNING id, created_on"
		p, mockDB := buildTestService(t)
		mockDB.ExpectQuery(formatQueryForSQLMock(expectedQuery)).WithArgs(example.Name, example.Details, example.BelongsTo).WillReturnError(errors.New("blah"))
		actual, err := p.CreateItem(context.Background(), expectedInput)
		assert.Error(t, err)
		assert.Nil(t, actual)
		assert.NoError(t, mockDB.ExpectationsWereMet(), "not all database expectations were met")
...
func TestPostgres_UpdateItem(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		expectedQuery := "UPDATE items SET name = $1, details = $2, updated_on = extract(epoch FROM NOW()) WHERE belongs_to = $3 AND id = $4 RETURNING updated_on"
		p, mockDB := buildTestService(t)
		mockDB.ExpectQuery(formatQueryForSQLMock(expectedQuery)).WithArgs(expected.Name, expected.Details, expected.BelongsTo, expected.ID).WillReturnRows(exampleRows)
	...
	T.Run("with error writing to database", func(t *testing.T) {
		...
		expectedQuery := "UPDATE items SET name = $1, details = $2, updated_on = extract(epoch FROM NOW()) WHERE belongs_to = $3 AND id = $4 RETURNING updated_on"
		p, mockDB := buildTestService(t)
		mockDB.ExpectQuery(formatQueryForSQLMock(expectedQuery)).WithArgs(example.Name, example.Details, example.BelongsTo, example.ID).WillReturnError(errors.New("blah"))
