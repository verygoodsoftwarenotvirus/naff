...
func TestSqlite_CreateOAuth2Client(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		s, mockDB := buildTestService(t)
		expectedQuery := "INSERT INTO oauth2_clients (name,client_id,client_secret,scopes,redirect_uri,belongs_to) VALUES (?,?,?,?,?,?)"
		mockDB.ExpectExec(formatQueryForSQLMock(expectedQuery)).WithArgs(expected.Name, expected.ClientID, expected.ClientSecret, strings.Join(expected.Scopes, scopesSeparator), expected.RedirectURI, expected.BelongsTo).WillReturnResult(sqlmock.NewResult(int64(expected.ID), 1))
		...
		actual, err := s.CreateOAuth2Client(tctx, expectedInput)
		assert.NoError(t, err)
		assert.Equal(t, expected, actual)
		assert.NoError(t, mockDB.ExpectationsWereMet(), "not all database expectations were met")
	})
...
func TestSqlite_UpdateOAuth2Client(T *testing.T) {
	...
	T.Run("with error writing to database", func(t *testing.T) {
		...
		err := s.UpdateOAuth2Client(tctx, exampleInput)
		assert.Error(t, err)
		assert.NoError(t, mockDB.ExpectationsWereMet(), "not all database expectations were met")
	})
}
