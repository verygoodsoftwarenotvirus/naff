...
func TestMariaDB_UpdateItem(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		m, mockDB := buildTestService(t)
		expectedQuery := "UPDATE items SET name = ?, details = ?, updated_on = UNIX_TIMESTAMP() WHERE belongs_to = ? AND id = ?"
		mockDB.ExpectExec(formatQueryForSQLMock(expectedQuery)).WithArgs(expected.Name, expected.Details, expected.BelongsTo, expected.ID).WillReturnResult(sqlmock.NewResult(1, 1))
	...
	T.Run("with error writing to database", func(t *testing.T) {
		...
		err := m.UpdateItem(context.Background(), example)
		assert.Error(t, err)
		assert.NoError(t, mockDB.ExpectationsWereMet(), "not all database expectations were met")
	})
}
