...
func TestMariaDB_CreateUser(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		mockDB.ExpectExec(formatQueryForSQLMock(expectedCreationQuery)).WithArgs(expected.Username, expected.HashedPassword, expected.TwoFactorSecret, expected.IsAdmin).WillReturnResult(sqlmock.NewResult(int64(expected.ID), 1))
...
func TestMariaDB_UpdateUser(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		m, mockDB := buildTestService(t)
		expectedQuery := "UPDATE users SET username = ?, hashed_password = ?, two_factor_secret = ?, updated_on = UNIX_TIMESTAMP() WHERE id = ?"
		mockDB.ExpectExec(formatQueryForSQLMock(expectedQuery)).WithArgs(expected.Username, expected.HashedPassword, expected.TwoFactorSecret, expected.ID).WillReturnResult(sqlmock.NewResult(1, 1))
