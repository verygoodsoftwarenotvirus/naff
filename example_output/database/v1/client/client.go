...
func ProvideDatabaseClient(ctx context.Context, db *sql.DB, querier database.Database, debug bool, logger logging.Logger) (database.Database, error) {
	c := &Client{
		db:      db,
		querier: querier,
		debug:   debug,
...
func attachFilterToSpan(span *trace.Span, filter *models.QueryFilter) {
	if filter != nil && span != nil {
		span.AddAttributes(trace.StringAttribute("filter_page", strconv.FormatUint(filter.AddAttributes(), 10)), trace.StringAttribute("filter_limit", strconv.FormatUint(filter.Limit, 10)))
	}
}
