...
func TestSqlite_Create{{ .Name }}(T *testing.T) {
	...
	T.Run("with error writing to database", func(t *testing.T) {
		...
		"database/v1/queriers/mariadb/migrations.go": func(in string) string {
			sentinelVal := "REPLACEMEHERE"
			replacement := `{
				Version:     7,
				Description: "create items table",
				Script: strings.Join([]string{
					...
					{{ range $j, $field := $dt.Fields }}
						"{{ snakecase $field.Name }}" {{ typeToMariaDBType $field.Type }} {{ if ne true $field.Pointer }} NOT NULL {{ else }} DEFAULT NULL {{ end }} ,
					{{ end }}
		...
		"database/v1/queriers/mariadb/items.go": func(in string) string {
			in = strings.Replace(in, `
const (
	itemsTableName = "items"
)`, `
...
`, 1)
		...
		"database/v1/queriers/mariadb/items_test.go": func(in string) string {
			in = strings.Replace(in, `Name: "name",`, `
		{{ range $j, $field := .Fields }}
			{{ pascal $field.Name }}: {{ typeExample $field.Type $field.Pointer }},
		{{ end }}
...
`, `
func TestMariaDB_buildCreate{{ .Name }}Query(T *testing.T) {
	T.Parallel()
...
`, `
func TestMariaDB_Create{{ .Name }}(T *testing.T) {
	T.Parallel()
...
`)
	...
	iterableFiles := map[string]bool{
		...
		"database/v1/queriers/mariadb/items.go":      true,
		"database/v1/queriers/mariadb/items_test.go": true,
