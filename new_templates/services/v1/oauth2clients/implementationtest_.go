...
func implementationTestDotGo() *jen.File {
	...
	ret.Add(jen.Func().ID("TestService_AuthorizeScopeHandler").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
		...
		jen.ID("T").Dot(
			...
			jen.ID("exampleClient").Op(":=").Op("&").ID("models").Dot(
				"OAuth2Client",
			).Valuesln(jen.ID("ClientID").Op(":").Lit("blargh"), jen.ID("Scopes").Op(":").Index().ID("string").Valuesln()),
			jen.ID("mockDB").Op(":=").ID("database").Dot(
				"BuildMockDatabase",
			).Call(),
			jen.ID("mockDB").Dot(
	...
	ret.Add(jen.Func().ID("TestService_ClientAuthorizedHandler").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
		...
		jen.ID("T").Dot(
			...
			jen.ID("exampleClient").Op(":=").Op("&").ID("models").Dot(
				"OAuth2Client",
			).Valuesln(jen.ID("ID").Op(":").Lit(1), jen.ID("ClientID").Op(":").Lit("blah"), jen.ID("Scopes").Op(":").Index().ID("string").Valuesln()),
