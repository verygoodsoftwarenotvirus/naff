...
func oauth2ClientsServiceTestDotGo() *jen.File {
	...
	ret.Add(jen.Func().ID("buildTestService").Params(jen.ID("t").Op("*").Qual("testing", "T")).Params(jen.Op("*").ID("Service")).Block(
		...
		jen.ID("service").Op(":=").Op("&").ID("Service").Valuesln(jen.ID("database").Op(":").ID("database").Dot(
			...
			"ProvideNoopLogger",
		).Call(), jen.ID("encoderDecoder").Op(":").Op("&").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/encoding/v1/mock", "EncoderDecoder").Valuesln(), jen.ID("authenticator").Op(":").Op("&").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/auth/v1/mock", "Authenticator").Valuesln(), jen.ID("urlClientIDExtractor").Op(":").Func().Params(jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.ID("uint64")).Block(
			jen.Return().Lit(0),
		), jen.ID("oauth2ClientCounter").Op(":").Op("&").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/metrics/v1/mock", "UnitCounter").Valuesln(), jen.ID("tokenStore").Op(":").ID("tokenStore"), jen.ID("oauth2Handler").Op(":").ID("server")),
	...
	ret.Add(jen.Func().ID("TestProvideOAuth2ClientsService").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
		...
		jen.ID("T").Dot(
			...
			jen.List(jen.ID("service"), jen.ID("err")).Op(":=").ID("ProvideOAuth2ClientsService").Call(jen.Qual("context", "Background").Call(), jen.ID("noop").Dot(
				...
				jen.Return().Lit(0),
			), jen.Op("&").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/encoding/v1/mock", "EncoderDecoder").Valuesln(), jen.ID("ucp")),
			jen.ID("assert").Dot(
				"NoError",
