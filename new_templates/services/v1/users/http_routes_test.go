...
func httpRoutesTestDotGo() *jen.File {
	...
	ret.Add(jen.Func().ID("TestService_validateCredentialChangeRequest").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
		...
		jen.ID("T").Dot(
			...
			jen.ID("auth").Op(":=").Op("&").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/auth/v1/mock", "Authenticator").Valuesln(),
			jen.ID("auth").Dot(
				"On",
	...
	ret.Add(jen.Func().ID("TestService_Create").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
		...
		jen.ID("T").Dot(
			...
			jen.ID("mc").Op(":=").Op("&").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/metrics/v1/mock", "UnitCounter").Valuesln(),
			jen.ID("mc").Dot(
				"On",
