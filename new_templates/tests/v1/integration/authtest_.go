...
`), jen.ID("username"), jen.ID("password"), jen.ID("code")),
		...
		jen.If(
			jen.ID("len").Call(jen.ID("cookies")).Op("==").Lit(1),
		).Block(
	...
	ret.Add(jen.Func().ID("TestAuth").Params(jen.ID("test").Op("*").Qual("testing", "T")).Block(
		...
		jen.ID("test").Dot(
			...
			utils.RequireNoError(jen.ID("t"), jen.Qual("encoding/json", "NewDecoder").Call(jen.ID("res").Dot(
				"Body",
			)).Dot(
				"Decode",
			).Call(jen.ID("ucr"))),
