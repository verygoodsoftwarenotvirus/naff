...
func oauth2TestDotGo() *jen.File {
	...
	ret.Add(jen.Func().ID("TestOAuth2Clients").Params(jen.ID("test").Op("*").Qual("testing", "T")).Block(
		...
		utils.RequireNoError(jen.ID("test"), jen.ID("err"), jen.Lit("error setting up auxiliary client")),
		jen.ID("test").Dot(
			"Run",
		...
		jen.ID("test").Dot(
			...
			jen.ID("T").Dot(
				...
				jen.ID("checkValueAndError").Call(jen.ID("t"), jen.ID("actual"), jen.ID("err")),
				utils.AssertTrue(jen.ID("t"), jen.ID("len").Call(jen.ID("actual").Dot(
					"Clients",
				...
				jen.For(jen.List(jen.ID("_"), jen.ID("oAuth2Client")).Op(":=").Range().ID("expected")).Block(
					...
					utils.AssertTrue(jen.ID("t"), jen.ID("clientFound"), jen.Lit("expected oAuth2Client ID %d to be present in results"), jen.ID("oAuth2Client").Dot(
						"ID",
					)),
				),
