...
func mainDotGo() *jen.File {
	...
	ret.Add(
		jen.Func().ID("main").Params().Block(
			...
			jen.List(jen.ID("cfg"), jen.ID("err")).Op(":=").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/config/v1", "ParseConfigFile").Call(jen.ID("configFilepath")),
			jen.If(jen.ID("err").Op("!=").ID("nil").Op("||").ID("cfg").Op("==").ID("nil")).Block(
				jen.ID("logger").Dot("Fatal").Call(jen.ID("err")),
			),
			jen.Line(),
			jen.Comment("only allow initialization to take so long"),
			jen.List(jen.ID("tctx"), jen.ID("cancel")).Op(":=").Qual("context", "WithTimeout").Call(
				jen.Qual("context", "Background").Call(),
				jen.ID("cfg").Dot("Meta").Dot("StartupDeadline"),
			),
