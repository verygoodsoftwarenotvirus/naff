...
func bcryptTestDotGo() *jen.File {
	...
	ret.Add(jen.Func().ID("TestBcrypt_HashPassword").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
		...
		jen.ID("T").Dot(
			...
			utils.AssertNoError(jen.ID("t"), jen.ID("err"), nil),
			utils.AssertNotEmpty(jen.ID("t"), jen.ID("actual"), nil),
		)),
	),
	)
	ret.Add(jen.Func().ID("TestBcrypt_PasswordMatches").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
		utils.ParallelTest(nil),
		jen.ID("x").Op(":=").ID("auth").Dot("ProvideBcryptAuthenticator").Call(
		...
		jen.ID("T").Dot("Run").Call(jen.Lit("normal usage"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
			...
			utils.AssertTrue(jen.ID("t"), jen.ID("actual"), nil),
		)),
		jen.ID("T").Dot("Run").Call(jen.Lit("when passwords don't match"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
			...
			utils.AssertFalse(jen.ID("t"), jen.ID("actual")),
		)),
	),
	)
	ret.Add(jen.Func().ID("TestBcrypt_PasswordIsAcceptable").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
		...
		jen.ID("T").Dot(
			...
			utils.ParallelTest(nil),
			utils.AssertTrue(jen.ID("t"), jen.ID("x").Dot(
	...
	ret.Add(jen.Func().ID("TestBcrypt_ValidateLogin").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
		...
		jen.ID("T").Dot(
			...
			utils.AssertTrue(jen.ID("t"), jen.ID("valid")),
		)),
		jen.ID("T").Dot(
			"Run",
