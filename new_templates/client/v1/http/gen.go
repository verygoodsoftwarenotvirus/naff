...
func mainDotGo() *jen.File {
	...
	ret.Add(
		jen.Var().Defs(
			jen.Id("ErrNotFound").Op("=").Id("errors").Dot("New").Call(
				jen.Lit("404: not found"),
			),
			jen.Id("ErrUnauthorized").Op("=").Id("errors").Dot("New").Call(
				jen.Lit("401: not authorized"),
			),
		),
		jen.Line(),
	)
	...
	ret.Add(
		...
		jen.Func().Id("NewClient").Params(
			...
			jen.List(
				jen.Id("clientID"), jen.Id("clientSecret"),
			).Id("string"),
			...
			jen.Id("c").Op(":=").Op("&").Id("V1Client").Values(jen.Dict{
				...
				jen.Id("logger"): jen.Qual("logger", "WithName").Call(
					jen.Id("clientName"),
				),
			...
			jen.Id("logger").Dot("WithValue").Call(
				jen.Lit("url"), jen.Id("address").Dot("String").Call(),
			).Dot("Debug").Call(
	...
	ret.Add(
		...
		jen.Func().Id("NewSimpleClient").Params(
			jen.Id("ctx").Qual("context", "Context"), jen.Id("address").Op("*").Qual("net/url", "URL"), jen.Id("debug").Id("bool"),
		).Params(
			jen.Op("*").Id("V1Client"), jen.Id("error"),
		).Block(
	...
	ret.Add(
		...
		jen.Func().Params(jen.Id("c").Op("*").Id("V1Client")).Id("executeRawRequest").Params(
			...
			jen.If(jen.List(
				jen.Id("command"), jen.Id("err"),
			).Op(":=").Qual("http2curl", "GetCurlCommand").Call(
				jen.Id("req"),
			),
			...
			jen.If(jen.Id("c").Dot("Debug")).Block(
				jen.List(
					jen.Id("bdump"), jen.Id("err"),
				).Op(":=").Id("httputil").Dot("DumpResponse").Call(
					jen.Id("res"), jen.Id("true"),
				),
				jen.If(jen.Id("err").Op("==").Id("nil").Op("&&").Id("req").Dot("Method").Op("!=").Qual("net/http", "MethodGet")).Block(
					jen.Id("logger").Op("=").Id("logger").Dot("WithValue").Call(
						jen.Lit("response_body"), jen.Id("string").Call(
							jen.Id("bdump"),
						),
					),
				),
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.If(jen.Id("err").Op("!=").Id("nil")).Block(
				jen.Id("panic").Call(
					jen.Qual("fmt", "Sprintf").Call(
						jen.Lit("user tried to build an invalid URL: %v"), jen.Id("err"),
					),
				),
			),
			jen.Line(),
	...
	ret.Add(
		...
		jen.Func().Params(jen.Id("c").Op("*").Id("V1Client")).Id("IsUp").Params().Params(jen.Id("bool")).Block(
			...
			jen.If(jen.Id("err").Op("!=").Id("nil")).Block(
				jen.Id("c").Dot("logger").Dot("Error").Call(
					jen.Id("err"), jen.Lit("health check"),
				),
				jen.Return().Id("false"),
			),
			jen.Line(),
		...
		jen.Func().Params(jen.Id("c").Op("*").Id("V1Client")).Id("retrieve").Params(
			...
			jen.If(jen.Id("err").Op(":=").Id("argIsNotPointerOrNil").Call(
				...
				jen.Return().Qual("fmt", "Errorf").Call(
					jen.Lit("struct to load must be a pointer: %w"), jen.Id("err"),
				),
			),
			jen.Line(),
			jen.List(
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Id("ctx").Qual("context", "Context"),
			jen.Id("req").Op("*").Qual("net/http", "Request"), jen.Id("out").Interface(),
		).Params(jen.Id("error")).Block(
			jen.If(jen.Id("out").Op("!=").Id("nil")).Block(
				jen.If(jen.List(
					jen.Id("np"), jen.Id("err"),
			...
			jen.List(
				...
				jen.Id("ctx"),
				jen.Id("c").Dot("plainClient"), jen.Id("req"),
			), jen.If(jen.Id("err").Op("!=").Id("nil")).Block(
				jen.Return().Qual("fmt", "Errorf").Call(
					jen.Lit("executing request: %w"), jen.Id("err"),
				),
			),
			jen.If(jen.Id("res").Dot("StatusCode").Op("==").Qual("net/http", "StatusNotFound")).Block(jen.Return().Id("ErrNotFound")),
			jen.If(jen.Id("out").Op("!=").Id("nil")).Block(
				jen.Id("resErr").Op(":=").Id("unmarshalBody").Call(
					jen.Id("res"), jen.Op("&").Id("out"),
				), jen.If(jen.Id("resErr").Op("!=").Id("nil")).Block(
...
func mainTestDotGo() *jen.File {
	...
	ret.Add(
		jen.Func().Id("buildTestClient").Params(
			jen.Id("t").Op("*").Qual("testing", "T"), jen.Id("ts").Op("*").Id("httptest").Dot("Server"),
		).Params(
			jen.Op("*").Id("V1Client"),
		).Block(
	...
	ret.Add(
		jen.Func().Id("TestV1Client_AuthenticatedClient").Params(jen.Id("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("obligatory"), jen.Func().Params(jen.Id("t").Op("*").Qual("testing", "T")).Block(
					...
					jen.Id("assert").Dot("Equal").Call(
						jen.Id("t"), jen.Id("ts").Dot("Client").Call(), jen.Id("actual"), jen.Lit("AuthenticatedClient should return the assigned authedClient"),
					),
				),
			),
		),
		...
		jen.Func().Id("TestV1Client_PlainClient").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("obligatory"), jen.Func().Params(jen.Id("t").Op("*").Qual("testing", "T")).Block(
					...
					jen.Id("assert").Dot("Equal").Call(
						jen.Id("t"), jen.Id("ts").Dot("Client").Call(), jen.Id("actual"), jen.Lit("PlainClient should return the assigned plainClient"),
					),
				),
			),
		),
	...
	ret.Add(
		jen.Func().Id("TestNewClient").Params(
			...
			jen.Id("T").Dot("Parallel").Call(),
			jen.Id("T").Dot("Run").Call(
				jen.Lit("happy path"), jen.Func().Params(jen.Id("t").Op("*").Qual("testing", "T")).Block(
					jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Id("nil"),
					),
					jen.List(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with client but invalid timeout"), jen.Func().Params(jen.Id("t").Op("*").Qual("testing", "T")).Block(
					jen.List(
						jen.Id("c"), jen.Id("err"),
					).Op(":=").Id("NewClient").Call(
						jen.Qual("context", "Background").Call(),
	...
	ret.Add(
		jen.Func().Id("TestV1Client_executeRequest").Params(
			...
			jen.Id("T").Dot("Parallel").Call(),
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with error"), jen.Func().Params(
					jen.Id("t").Op("*").Qual("testing", "T"),
				).Block(
					...
					jen.List(
						...
						jen.Id("expectedMethod"), jen.Id("ts").Dot("URL"), jen.Id("nil"),
					),
					jen.Id("require").Dot("NotNil").Call(
						jen.Id("t"), jen.Id("req"),
					),
	...
	ret.Add(
		jen.Func().Id("TestBuildURL").Params(
			...
			jen.Id("T").Dot("Parallel").Call(),
			jen.Id("T").Dot("Run").Call(
				jen.Lit("various urls"), jen.Func().Params(
					jen.Id("t").Op("*").Qual("testing", "T"),
				).Block(
					jen.Id("t").Dot("Parallel").Call(),
					jen.List(
					...
					jen.For(jen.List(
						...
						jen.Id("assert").Dot("Equal").Call(
							jen.Id("t"), jen.Id("tc").Dot("expectation"), jen.Id("actual"),
						),
					),
				),
			),
		...
		jen.Func().Id("TestV1Client_BuildWebsocketURL").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("happy path"), jen.Func().Params(
					...
					jen.Id("assert").Dot("Equal").Call(
						jen.Id("t"), jen.Id("expected"), jen.Id("actual"),
					),
				),
			),
		),
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("happy path"), jen.Func().Params(
					...
					jen.Id("assert").Dot("Equal").Call(
						...
						jen.Lit("request should be a %s request"), jen.Id("expectedMethod"),
					),
				),
			),
		),
	...
	ret.Add(
		jen.Func().Id("TestV1Client_IsUp").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("happy path"), jen.Func().Params(
					...
					jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Qual("net/http", "HandlerFunc").Call(
							jen.Func().Params(
								jen.Id("res").Qual("net/http", "ResponseWriter"), jen.Id("req").Op("*").Qual("net/http", "Request"),
							).Block(
								jen.Id("assert").Dot("Equal").Call(
									jen.Id("t"), jen.Id("req").Dot("Method"), jen.Qual("net/http", "MethodGet"),
								),
								jen.Id("res").Dot("WriteHeader").Call(
			...
			jen.Line(),
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with bad status code"), jen.Func().Params(
					jen.Id("t").Op("*").Qual("testing", "T"),
				).Block(
					jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Qual("net/http", "HandlerFunc").Call(
		...
		jen.Func().Id("TestV1Client_buildDataRequest").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("happy path"), jen.Func().Params(
					...
					jen.Id("assert").Dot("Equal").Call(
						jen.Id("t"), jen.Id("expectedMethod"), jen.Id("req").Dot("Method"),
					),
				),
			),
		),
	...
	ret.Add(
		jen.Func().Id("TestV1Client_makeRequest").Params(jen.Id("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.Line(),
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with 404"), jen.Func().Params(
					jen.Line(),
					jen.Id("t").Op("*").Qual("testing", "T"),
				).Block(
					jen.Id("ctx").Op(":=").Qual("context", "Background").Call(),
	...
	ret.Add(
		jen.Func().Id("TestV1Client_makeUnauthedDataRequest").Params(jen.Id("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.Line(),
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with 404"), jen.Func().Params(jen.Id("t").Op("*").Qual("testing", "T")).Block(
					jen.Id("ctx").Op(":=").Qual("context", "Background").Call(),
					jen.Id("expectedMethod").Op(":=").Qual("net/http", "MethodPost"),
					jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Qual("net/http", "HandlerFunc").Call(
					...
					jen.Id("assert").Dot("Equal").Call(
						jen.Id("t"), jen.Id("ErrNotFound"), jen.Id("err"),
					),
				),
			),
			jen.Line(),
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with timeout"), jen.Func().Params(
					jen.Id("t").Op("*").Qual("testing", "T"),
				).Block(
					jen.Id("ctx").Op(":=").Qual("context", "Background").Call(),
					jen.Id("expectedMethod").Op(":=").Qual("net/http", "MethodPost"),
			...
			jen.Line(),
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with nil as output"), jen.Func().Params(jen.Id("t").Op("*").Qual("testing", "T")).Block(
					jen.Id("ctx").Op(":=").Qual("context", "Background").Call(),
					jen.Id("expectedMethod").Op(":=").Qual("net/http", "MethodPost"),
					jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Id("nil"),
	...
	ret.Add(
		jen.Func().Id("TestV1Client_retrieve").Params(jen.Id("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.Line(),
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with nil passed in"), jen.Func().Params(jen.Id("t").Op("*").Qual("testing", "T")).Block(
					jen.Id("ctx").Op(":=").Qual("context", "Background").Call(),
					jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Id("nil"),
					),
					...
					jen.Line(),
					jen.Id("err").Op("=").Id("c").Dot("retrieve").Call(
						jen.Id("ctx"), jen.Id("req"), jen.Id("nil"),
					),
					jen.Id("assert").Dot("Error").Call(
						jen.Id("t"), jen.Id("err"),
					),
					...
					jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Qual("net/http", "HandlerFunc").Call(
							jen.Func().Params(
								...
								jen.Id("assert").Dot("Equal").Call(
									jen.Id("t"),
									jen.Id("req").Dot("Method"), jen.Id("expectedMethod"),
								),
								jen.Id("res").Dot("WriteHeader").Call(
									jen.Qual("net/http", "StatusNotFound"),
								),
...
func helpersDotGo() *jen.File {
	...
	ret.Add(
		...
		jen.Func().Id("unmarshalBody").Params(
			jen.Id("res").Op("*").Qual("net/http", "Response"), jen.Id("dest").Interface(),
		).Params(
			jen.Id("error"),
		).Block(
			jen.If(
			...
			jen.List(
				jen.Id("bodyBytes"), jen.Id("err"),
			).Op(":=").Qual("io/ioutil", "ReadAll").
				Call(jen.Id("res").Dot("Body")), jen.If(jen.Id("err").Op("!=").Id("nil")).Block(
				jen.Return().Id("err"),
			),
			jen.If(jen.Id("res").Dot("StatusCode").Op(">=").Qual("net/http", "StatusBadRequest")).Block(
				...
				jen.If(jen.Id("err").Op("=").Qual("encoding/json", "Unmarshal").Call(
					...
					jen.Return().Qual("fmt", "Errorf").Call(
						jen.Lit("unmarshaling error: %w"), jen.Id("err"),
					),
				),
	...
	ret.Add(
		...
		jen.Func().Id("createBodyFromStruct").Params(
			...
			jen.Qual("io", "Reader"), jen.Id("error"),
		).Block(
			jen.List(
				jen.Id("out"), jen.Id("err"),
...
func helpersTestDotGo() *jen.File {
	...
	ret.Add(
		jen.Func().Id("TestArgIsNotPointerOrNil").Params(
			...
			jen.Id("T").Dot("Parallel").Call(),
			jen.Id("T").Dot("Run").Call(
				jen.Lit("expected use"), jen.Func().Params(
					jen.Id("t").Op("*").Qual("testing", "T"),
				).Block(
					jen.Id("err").Op(":=").Id("argIsNotPointerOrNil").Call(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with non-pointer"), jen.Func().Params(
					jen.Id("t").Op("*").Qual("testing", "T"),
				).Block(
					jen.Id("err").Op(":=").Id("argIsNotPointerOrNil").Call(
						jen.Id("testingType").Values(),
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with nil"), jen.Func().Params(
					jen.Id("t").Op("*").Qual("testing", "T"),
				).Block(
					jen.Id("err").Op(":=").Id("argIsNotPointerOrNil").Call(
						jen.Id("nil"),
	...
	ret.Add(
		jen.Func().Id("TestArgIsNotNil").Params(
			...
			jen.Id("T").Dot("Parallel").Call(),
			jen.Id("T").Dot("Run").Call(
				jen.Lit("without nil"), jen.Func().Params(jen.Id("t").Op("*").Qual("testing", "T")).Block(
					jen.List(
						jen.Id("isNil"), jen.Id("err"),
					).Op(":=").Id("argIsNotNil").Call(
						jen.Op("&").Id("testingType").Values(),
					),
					jen.Id("assert").Dot("False").Call(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with non-pointer"), jen.Func().Params(jen.Id("t").Op("*").Qual("testing", "T")).Block(
					jen.List(
						jen.Id("isNil"),
						jen.Id("err"),
					).Op(":=").Id("argIsNotNil").Call(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with nil"), jen.Func().Params(jen.Id("t").Op("*").Qual("testing", "T")).Block(
					jen.List(
						jen.Id("isNil"), jen.Id("err"),
					).Op(":=").Id("argIsNotNil").Call(
						jen.Id("nil"),
	...
	ret.Add(
		jen.Func().Id("TestUnmarshalBody").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with good status but unmarshallable response"), jen.Func().Params(
					jen.Id("t").Op("*").Qual("testing", "T"),
				).Block(
					jen.Id("res").Op(":=").Op("&").Qual("net/http", "Response").Values(
						jen.Id("Body").Op(":").Qual("io/ioutil", "NopCloser").Call(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with an erroneous error code"), jen.Func().Params(
					jen.Id("t").Op("*").Qual("testing", "T"),
				).Block(
					jen.Id("res").Op(":=").Op("&").Qual("net/http", "Response").Values(
						jen.Id("Body").Op(":").Qual("io/ioutil", "NopCloser").Call(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with an erroneous error code and unmarshallable body"), jen.Func().Params(
					jen.Id("t").Op("*").Qual("testing", "T"),
				).Block(
					jen.Id("res").Op(":=").Op("&").Qual("net/http", "Response").Values(
						jen.Id("Body").Op(":").Qual("io/ioutil", "NopCloser").Call(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with nil target variable"), jen.Func().Params(jen.Id("t").Op("*").Qual("testing", "T")).Block(
					jen.Id("err").Op(":=").Id("unmarshalBody").Call(
						jen.Id("nil"),
						jen.Id("nil"),
					),
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with erroneous reader"), jen.Func().Params(jen.Id("t").Op("*").Qual("testing", "T")).Block(
	...
	ret.Add(
		jen.Func().Id("TestCreateBodyFromStruct").Params(jen.Id("T").Op("*").Qual("testing", "T")).Block(
			jen.Id("T").Dot("Parallel").Call(),
			jen.Id("T").Dot("Run").Call(
				jen.Lit("expected use"), jen.Func().Params(jen.Id("t").Op("*").Qual("testing", "T")).Block(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with unmarshallable struct"), jen.Func().Params(
					jen.Id("t").Op("*").Qual("testing", "T"),
				).Block(
...
func oauth2ClientsDotGo() *jen.File {
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Id("ctx").Qual("context", "Context"), jen.Id("id").Id("uint64"),
		).Params(
			jen.Op("*").Qual("net/http", "Request"), jen.Id("error"),
		).Block(
			jen.Id("uri").Op(":=").Id("c").Dot("BuildURL").Call(
				jen.Id("nil"), jen.Id("oauth2ClientsBasePath"), jen.Qual("strconv", "FormatUint").Call(
					jen.Id("id"), jen.Lit(10),
				),
			), jen.Return().Qual("net/http", "NewRequest").Call(
				jen.Qual("net/http", "MethodGet"), jen.Id("uri"), jen.Id("nil"),
			),
		),
		jen.Line(),
	)
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Id("oauth2Client").Op("*").Id("models").Dot("OAuth2Client"), jen.Id("err").Id("error"),
		).Block(
			jen.List(
				jen.Id("req"),
				jen.Id("err"),
			...
			jen.If(jen.Id("err").Op("!=").Id("nil")).Block(
				jen.Return().List(
					jen.Id("nil"),
					jen.Qual("fmt", "Errorf").Call(
						jen.Lit("building request: %w"), jen.Id("err"),
					),
				),
			),
			jen.Id("err").Op("=").Id("c").Dot("retrieve").Call(
	...
	ret.Add(
		jen.Comment(""),
		jen.Line(),
		jen.Func().Params(jen.Id("c").Op("*").Id("V1Client")).Id("BuildGetOAuth2ClientsRequest").Params(
			jen.Id("ctx").Qual("context", "Context"),
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Id("ctx").Qual("context", "Context"), jen.Id("filter").Op("*").Id("models").Dot("QueryFilter"),
		).Params(
			jen.Op("*").Id("models").Dot("OAuth2ClientList"), jen.Id("error"),
		).Block(
			jen.List(
				jen.Id("req"),
				jen.Id("err"),
	...
	ret.Add(
		jen.Comment(""),
		jen.Line(),
		jen.Func().Params(jen.Id("c").Op("*").Id("V1Client")).Id("BuildCreateOAuth2ClientRequest").Params(
			jen.Id("ctx").Qual("context", "Context"),
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Id("ctx").Qual("context", "Context"), jen.Id("cookie").Op("*").Qual("net/http", "Cookie"), jen.Id("input").Op("*").Id("models").Dot("OAuth2ClientCreationInput"),
		).Params(
			jen.Op("*").Id("models").Dot("OAuth2Client"), jen.Id("error"),
		).Block(
			...
			jen.List(
				...
				jen.Id("ctx"), jen.Id("cookie"), jen.Id("input"),
			), jen.If(jen.Id("err").Op("!=").Id("nil")).Block(
				jen.Return().List(
					jen.Id("nil"),
					jen.Id("err"),
			...
			jen.Return().List(jen.Id("oauth2Client"), jen.Id("nil")),
		),
		jen.Line(),
	)
...
func oauth2ClientsTestDotGo() *jen.File {
	...
	ret.Add(
		jen.Func().Id("TestV1Client_CreateOAuth2Client").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with invalid body"), jen.Func().Params(
					jen.Id("t").Op("*").Qual("testing", "T"),
				).Block(
					jen.Id("ctx").Op(":=").Qual("context", "Background").Call(),
					jen.Id("exampleInput").Op(":=").Op("&").Id("models").Dot("OAuth2ClientCreationInput").Values(
		...
		jen.Func().Id("TestV1Client_BuildArchiveOAuth2ClientRequest").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("happy path"), jen.Func().Params(
					...
					jen.Id("expectedMethod").Op(":=").Qual("net/http", "MethodDelete"),
					jen.Id("ctx").Op(":=").Qual("context", "Background").Call(),
					jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Id("nil"),
					),
...
func usersDotGo() *jen.File {
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Id("ctx").Qual("context", "Context"), jen.Id("userID").Id("uint64"),
		).Params(
			jen.Op("*").Qual("net/http", "Request"), jen.Id("error"),
		).Block(
			jen.Id("uri").Op(":=").Id("c").Dot("buildVersionlessURL").Call(
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Id("user").Op("*").Id("models").Dot("User"), jen.Id("err").Id("error"),
		).Block(
			jen.List(
				jen.Id("req"), jen.Id("err"),
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Return().List(jen.Id("users"), jen.Id("err")),
		),
		jen.Line(),
	)
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Return().List(jen.Id("user"), jen.Id("err")),
		),
		jen.Line(),
	)
		...
		jen.Func().Params(
			...
			jen.List(
				...
				jen.Op("&").Id("models").Dot("UserLoginInput").Values(
					jen.Id("Username").Op(":").Id("username"), jen.Id("Password").Op(":").Id("password"),
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Id("ctx").Qual("context", "Context"), jen.List(
				jen.Id("username"), jen.Id("password"), jen.Id("totpToken"),
			).Id("string"),
		).Params(
			jen.Op("*").Qual("net/http", "Cookie"), jen.Id("error"),
		).Block(
			jen.List(
				jen.Id("req"), jen.Id("err"),
			...
			jen.If(jen.Id("err").Op("!=").Id("nil")).Block(
				jen.Return().List(
					jen.Id("nil"), jen.Qual("fmt", "Errorf").Call(
						jen.Lit("encountered error executing login request: %w"), jen.Id("err"),
					),
				),
			),
			jen.If(jen.Id("c").Dot("Debug")).Block(
				...
				jen.If(jen.Id("err").Op("!=").Id("nil")).Block(
					...
					jen.Lit("response"), jen.Id("string").Call(
						jen.Id("b"),
					),
				).Dot("Debug").Call(
...
func usersTestDotGo() *jen.File {
	...
	ret.Add(
		jen.Func().Id("TestV1Client_Login").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("with missing cookie"), jen.Func().Params(
					jen.Id("t").Op("*").Qual("testing", "T"),
				).Block(
					jen.Id("ctx").Op(":=").Qual("context", "Background").Call(),
					jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
...
func webhooksDotGo() *jen.File {
	...
	ret.Add(
		jen.Comment(""),
		jen.Line(),
		jen.Func().Params(jen.Id("c").Op("*").Id("V1Client")).Id("BuildGetWebhookRequest").Params(
			jen.Id("ctx").Qual("context", "Context"),
			jen.Id("id").Id("uint64"),
		).Params(
			jen.Op("*").Qual("net/http", "Request"), jen.Id("error"),
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Id("webhook").Op("*").Id("models").Dot("Webhook"), jen.Id("err").Id("error"),
		).Block(
			jen.List(
				jen.Id("req"),
				jen.Id("err"),
	...
	ret.Add(
		jen.Comment(""),
		jen.Line(),
		jen.Func().Params(jen.Id("c").Op("*").Id("V1Client")).Id("GetWebhooks").Params(
			jen.Id("ctx").Qual("context", "Context"),
			jen.Id("filter").Op("*").Id("models").Dot("QueryFilter"),
		).Params(
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Return().Id("c").Dot("buildDataRequest").Call(
				jen.Qual("net/http", "MethodPut"), jen.Id("uri"), jen.Id("updated"),
			),
		),
	)
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Id("ctx").Qual("context", "Context"), jen.Id("updated").Op("*").Id("models").Dot("Webhook"),
		).Params(
			jen.Id("error"),
		).Block(
			jen.List(
				...
				jen.Id("ctx"), jen.Id("updated"),
			), jen.If(jen.Id("err").Op("!=").Id("nil")).Block(
				jen.Return().Qual("fmt", "Errorf").Call(
					jen.Lit("building request: %w"), jen.Id("err"),
				),
			), jen.Return().Id("c").Dot("executeRequest").Call(
				jen.Id("ctx"), jen.Id("req"), jen.Op("&").Id("updated"),
			),
		),
	)
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Id("uri").Op(":=").Id("c").Dot("BuildURL").Call(
				jen.Id("nil"), jen.Id("webhooksBasePath"), jen.Qual("strconv", "FormatUint").Call(
					jen.Id("id"), jen.Lit(10),
				),
			), jen.Return().Qual("net/http", "NewRequest").Call(
				jen.Qual("net/http", "MethodDelete"), jen.Id("uri"), jen.Id("nil"),
			),
		),
	)
		...
		jen.Func().Id("TestV1Client_BuildGetWebhookRequest").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("happy path"), jen.Func().Params(
					...
					jen.Id("expectedMethod").Op(":=").Qual("net/http", "MethodGet"), jen.Id("ctx").Op(":=").Qual("context", "Background").Call(), jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Id("nil"),
					), jen.Id("c").Op(":=").Id("buildTestClient").Call(
						jen.Id("t"),
						jen.Id("ts"),
						...
						jen.Id("t"), jen.Id("actual").Dot("Method"), jen.Id("expectedMethod"), jen.Lit("request should be a %s request"), jen.Id("expectedMethod"),
					),
				),
			),
		),
		...
		jen.Func().Id("TestV1Client_GetWebhook").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("happy path"), jen.Func().Params(
					...
					jen.Id("expected").Op(":=").Op("&").Id("models").Dot("Webhook").Values(jen.Id("ID").Op(":").Lit(1), jen.Id("Name").Op(":").Lit("example")), jen.Id("ctx").Op(":=").Qual("context", "Background").Call(), jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Qual("net/http", "HandlerFunc").Call(
							jen.Func().Params(
								jen.Id("res").Qual("net/http", "ResponseWriter"), jen.Id("req").Op("*").Qual("net/http", "Request"),
							).Block(
								...
								jen.Id("assert").Dot("Equal").Call(
									jen.Id("t"), jen.Id("req").Dot("URL").Dot("Path"), jen.Qual("fmt", "Sprintf").Call(
										jen.Lit("/api/v1/webhooks/%d"), jen.Id("expected").Dot("ID"),
									), jen.Lit("expected and actual path don't match"),
								), jen.Id("assert").Dot("Equal").Call(
									jen.Id("t"), jen.Id("req").Dot("Method"), jen.Qual("net/http", "MethodGet"),
								), jen.Id("require").Dot("NoError").Call(
						...
						jen.Id("ctx"), jen.Id("expected").Dot("ID"),
					), jen.Id("require").Dot("NotNil").Call(
						jen.Id("t"), jen.Id("actual"),
					), jen.Id("assert").Dot("NoError").Call(
						jen.Id("t"), jen.Id("err"), jen.Lit("no error should be returned"),
		...
		jen.Func().Id("TestV1Client_GetWebhooks").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("happy path"), jen.Func().Params(
					...
					jen.Id("expected").Op(":=").Op("&").Id("models").Dot("WebhookList").Values(jen.Id("Webhooks").Op(":").Index().Id("models").Dot("Webhook").Values(jen.Values(jen.Id("ID").Op(":").Lit(1), jen.Id("Name").Op(":").Lit("example")))), jen.Id("ctx").Op(":=").Qual("context", "Background").Call(), jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Qual("net/http", "HandlerFunc").Call(
							jen.Func().Params(
								jen.Id("res").Qual("net/http", "ResponseWriter"), jen.Id("req").Op("*").Qual("net/http", "Request"),
							).Block(
								jen.Id("assert").Dot("Equal").Call(
									jen.Id("t"), jen.Id("req").Dot("URL").Dot("Path"), jen.Lit("/api/v1/webhooks"), jen.Lit("expected and actual path don't match"),
								), jen.Id("assert").Dot("Equal").Call(
									jen.Id("t"), jen.Id("req").Dot("Method"), jen.Qual("net/http", "MethodGet"),
								), jen.Id("require").Dot("NoError").Call(
									jen.Id("t"), jen.Qual("encoding/json", "NewEncoder").Call(
		...
		jen.Func().Id("TestV1Client_BuildCreateWebhookRequest").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("happy path"), jen.Func().Params(
					...
					jen.Id("expectedMethod").Op(":=").Qual("net/http", "MethodPost"), jen.Id("ctx").Op(":=").Qual("context", "Background").Call(), jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Id("nil"),
					), jen.Id("exampleInput").Op(":=").Op("&").Id("models").Dot("WebhookCreationInput").Values(jen.Id("Name").Op(":").Lit("expected name")), jen.Id("c").Op(":=").Id("buildTestClient").Call(
						jen.Id("t"),
						jen.Id("ts"),
					), jen.List(
						jen.Id("actual"), jen.Id("err"),
					).Op(":=").Id("c").Dot("BuildCreateWebhookRequest").Call(
						jen.Id("ctx"), jen.Id("exampleInput"),
					), jen.Id("require").Dot("NotNil").Call(
						jen.Id("t"), jen.Id("actual"),
					), jen.Id("assert").Dot("NoError").Call(
						jen.Id("t"), jen.Id("err"), jen.Lit("no error should be returned"),
		...
		jen.Func().Id("TestV1Client_CreateWebhook").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("happy path"), jen.Func().Params(
					...
					jen.Id("expected").Op(":=").Op("&").Id("models").Dot("Webhook").Values(jen.Id("ID").Op(":").Lit(1), jen.Id("Name").Op(":").Lit("example")), jen.Id("exampleInput").Op(":=").Op("&").Id("models").Dot("WebhookCreationInput").Values(jen.Id("Name").Op(":").Id("expected").Dot("Name")), jen.Id("ctx").Op(":=").Qual("context", "Background").Call(), jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Qual("net/http", "HandlerFunc").Call(
							jen.Func().Params(
								jen.Id("res").Qual("net/http", "ResponseWriter"), jen.Id("req").Op("*").Qual("net/http", "Request"),
							).Block(
								jen.Id("assert").Dot("Equal").Call(
									...
									jen.Id("t"), jen.Id("req").Dot("Method"), jen.Qual("net/http", "MethodPost"),
								), jen.Var().Id("x").Op("*").Id("models").Dot("WebhookCreationInput"), jen.Id("require").Dot("NoError").Call(
									...
									jen.Id("t"), jen.Id("exampleInput"), jen.Id("x"),
								), jen.Id("require").Dot("NoError").Call(
									jen.Id("t"), jen.Qual("encoding/json", "NewEncoder").Call(
										jen.Id("res"),
									).Dot("Encode").Call(
		...
		jen.Func().Id("TestV1Client_BuildUpdateWebhookRequest").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("happy path"), jen.Func().Params(
					...
					jen.Id("expectedMethod").Op(":=").Qual("net/http", "MethodPut"), jen.Id("ctx").Op(":=").Qual("context", "Background").Call(), jen.Id("exampleInput").Op(":=").Op("&").Id("models").Dot("Webhook").Values(jen.Id("Name").Op(":").Lit("changed name")), jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Id("nil"),
					), jen.Id("c").Op(":=").Id("buildTestClient").Call(
						jen.Id("t"),
						jen.Id("ts"),
						...
						jen.Qual("net/http", "HandlerFunc").Call(
							jen.Func().Params(
								...
								jen.Id("assert").Dot("Equal").Call(
									...
									jen.Id("t"), jen.Id("req").Dot("Method"), jen.Qual("net/http", "MethodPut"),
								), jen.Id("res").Dot("WriteHeader").Call(
									jen.Qual("net/http", "StatusOK"),
								),
							),
		...
		jen.Func().Id("TestV1Client_BuildArchiveWebhookRequest").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("happy path"), jen.Func().Params(
					...
					jen.Id("expectedMethod").Op(":=").Qual("net/http", "MethodDelete"), jen.Id("ctx").Op(":=").Qual("context", "Background").Call(), jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Id("nil"),
					), jen.Id("expectedID").Op(":=").Id("uint64").Call(
						jen.Lit(1),
					), jen.Id("c").Op(":=").Id("buildTestClient").Call(
						...
						jen.Id("t"), jen.Id("actual").Dot("URL"),
					), jen.Id("assert").Dot("True").Call(
						jen.Id("t"), jen.Qual("strings", "HasSuffix").Call(
							jen.Id("actual").Dot("URL").Dot("String").Call(), jen.Qual("fmt", "Sprintf").Call(
								jen.Lit("%d"), jen.Id("expectedID"),
		...
		jen.Func().Id("TestV1Client_ArchiveWebhook").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("happy path"), jen.Func().Params(
					...
					jen.Id("expected").Op(":=").Id("uint64").Call(
						jen.Lit(1),
					), jen.Id("ctx").Op(":=").Qual("context", "Background").Call(), jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Qual("net/http", "HandlerFunc").Call(
							jen.Func().Params(
								jen.Id("res").Qual("net/http", "ResponseWriter"), jen.Id("req").Op("*").Qual("net/http", "Request"),
							).Block(
								jen.Id("assert").Dot("Equal").Call(
									jen.Id("t"), jen.Id("req").Dot("URL").Dot("Path"), jen.Qual("fmt", "Sprintf").Call(
										jen.Lit("/api/v1/webhooks/%d"), jen.Id("expected"),
									), jen.Lit("expected and actual path don't match"),
								), jen.Id("assert").Dot("Equal").Call(
									jen.Id("t"), jen.Id("req").Dot("Method"), jen.Qual("net/http", "MethodDelete"),
								), jen.Id("res").Dot("WriteHeader").Call(
									jen.Qual("net/http", "StatusOK"),
								),
							),
...
func itemsDotGo() *jen.File {
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Id("uri").Op(":=").Id("c").Dot("BuildURL").Call(
				jen.Id("nil"), jen.Id("itemsBasePath"), jen.Qual("strconv", "FormatUint").Call(
					jen.Id("id"), jen.Lit(10),
				),
			), jen.Return().Qual("net/http", "NewRequest").Call(
				jen.Qual("net/http", "MethodGet"), jen.Id("uri"), jen.Id("nil"),
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Id("item").Op("*").Id("models").Dot("Item"), jen.Id("err").Id("error"),
		).Block(
			jen.List(
				jen.Id("req"), jen.Id("err"),
				...
				jen.Id("ctx"), jen.Id("req"), jen.Op("&").Id("item"),
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Id("uri").Op(":=").Id("c").Dot("BuildURL").Call(
				jen.Id("filter").Dot("ToValues").Call(), jen.Id("itemsBasePath"),
			), jen.Return().Qual("net/http", "NewRequest").Call(
				jen.Qual("net/http", "MethodGet"), jen.Id("uri"), jen.Id("nil"),
			),
		),
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Id("items").Op("*").Id("models").Dot("ItemList"), jen.Id("err").Id("error"),
		).Block(
			jen.List(
				jen.Id("req"), jen.Id("err"),
				...
				jen.Id("ctx"), jen.Id("req"), jen.Op("&").Id("items"),
			), jen.Id("retrieveErr").Op("!=").Id("nil"),
			).Block(
				jen.Return().List(jen.Id("nil"), jen.Id("retrieveErr")),
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Id("ctx").Qual("context", "Context"), jen.Id("body").Op("*").Id("models").Dot("ItemCreationInput"),
		).Params(
			jen.Op("*").Qual("net/http", "Request"), jen.Id("error"),
		).Block(
			jen.Id("uri").Op(":=").Id("c").Dot("BuildURL").Call(
				jen.Id("nil"), jen.Id("itemsBasePath"),
			), jen.Return().Id("c").Dot("buildDataRequest").Call(
				jen.Qual("net/http", "MethodPost"), jen.Id("uri"), jen.Id("body"),
			),
		),
		jen.Line(),
	)
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Id("ctx").Qual("context", "Context"), jen.Id("input").Op("*").Id("models").Dot("ItemCreationInput"),
		).Params(
			jen.Id("item").Op("*").Id("models").Dot("Item"), jen.Id("err").Id("error"),
		).Block(
			jen.List(
				...
				jen.Id("ctx"), jen.Id("req"), jen.Op("&").Id("item"),
			), jen.Return().List(jen.Id("item"), jen.Id("err")),
		),
		jen.Line(),
	)
	...
	ret.Add(
		...
		jen.Func().Params(
			...
			jen.Id("ctx").Qual("context", "Context"), jen.Id("updated").Op("*").Id("models").Dot("Item"),
		).Params(
			jen.Op("*").Qual("net/http", "Request"), jen.Id("error"),
		).Block(
			jen.Id("uri").Op(":=").Id("c").Dot("BuildURL").Call(
		...
		jen.Func().Id("TestV1Client_GetItem").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("happy path"), jen.Func().Params(
					...
					jen.Id("expected").Op(":=").Op("&").Id("models").Dot("Item").Values(jen.Id("ID").Op(":").Lit(1), jen.Id("Name").Op(":").Lit("example"), jen.Id("Details").Op(":").Lit("blah")), jen.Id("ctx").Op(":=").Qual("context", "Background").Call(), jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Qual("net/http", "HandlerFunc").Call(
							jen.Func().Params(
								jen.Id("res").Qual("net/http", "ResponseWriter"), jen.Id("req").Op("*").Qual("net/http", "Request"),
							).Block(
								...
								jen.Id("assert").Dot("Equal").Call(
									jen.Id("t"), jen.Id("req").Dot("URL").Dot("Path"), jen.Qual("fmt", "Sprintf").Call(
										jen.Lit("/api/v1/items/%d"), jen.Id("expected").Dot("ID"),
									), jen.Lit("expected and actual path don't match"),
								), jen.Id("assert").Dot("Equal").Call(
									jen.Id("t"), jen.Id("req").Dot("Method"), jen.Qual("net/http", "MethodGet"),
								), jen.Id("require").Dot("NoError").Call(
		...
		jen.Func().Id("TestV1Client_GetItems").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("happy path"), jen.Func().Params(
					...
					jen.Id("expected").Op(":=").Op("&").Id("models").Dot("ItemList").Values(jen.Id("Items").Op(":").Index().Id("models").Dot("Item").Values(jen.Values(jen.Id("ID").Op(":").Lit(1), jen.Id("Name").Op(":").Lit("example"), jen.Id("Details").Op(":").Lit("blah")))), jen.Id("ctx").Op(":=").Qual("context", "Background").Call(), jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Qual("net/http", "HandlerFunc").Call(
							jen.Func().Params(
								jen.Id("res").Qual("net/http", "ResponseWriter"), jen.Id("req").Op("*").Qual("net/http", "Request"),
							).Block(
								jen.Id("assert").Dot("Equal").Call(
									jen.Id("t"), jen.Id("req").Dot("URL").Dot("Path"), jen.Lit("/api/v1/items"), jen.Lit("expected and actual path don't match"),
								), jen.Id("assert").Dot("Equal").Call(
									jen.Id("t"), jen.Id("req").Dot("Method"), jen.Qual("net/http", "MethodGet"),
								), jen.Id("require").Dot("NoError").Call(
									jen.Id("t"), jen.Qual("encoding/json", "NewEncoder").Call(
		...
		jen.Func().Id("TestV1Client_BuildCreateItemRequest").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("happy path"), jen.Func().Params(
					...
					jen.Id("expectedMethod").Op(":=").Qual("net/http", "MethodPost"), jen.Id("ctx").Op(":=").Qual("context", "Background").Call(), jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Id("nil"),
					), jen.Id("exampleInput").Op(":=").Op("&").Id("models").Dot("ItemCreationInput").Values(jen.Id("Name").Op(":").Lit("expected name"), jen.Id("Details").Op(":").Lit("expected details")), jen.Id("c").Op(":=").Id("buildTestClient").Call(
						jen.Id("t"),
						jen.Id("ts"),
					), jen.List(
						jen.Id("actual"), jen.Id("err"),
						...
						jen.Qual("net/http", "HandlerFunc").Call(
							jen.Func().Params(
								...
								jen.Id("assert").Dot("Equal").Call(
									...
									jen.Id("t"), jen.Id("req").Dot("Method"), jen.Qual("net/http", "MethodPost"),
								), jen.Var().Id("x").Op("*").Id("models").Dot("ItemCreationInput"), jen.Id("require").Dot("NoError").Call(
									jen.Id("t"), jen.Qual("encoding/json", "NewDecoder").Call(
										jen.Id("req").Dot("Body"),
									).Dot("Decode").Call(
										jen.Op("&").Id("x"),
		...
		jen.Func().Id("TestV1Client_BuildUpdateItemRequest").Params(
			...
			jen.Id("T").Dot("Run").Call(
				jen.Lit("happy path"), jen.Func().Params(
					...
					jen.Id("expectedMethod").Op(":=").Qual("net/http", "MethodPut"), jen.Id("ctx").Op(":=").Qual("context", "Background").Call(), jen.Id("exampleInput").Op(":=").Op("&").Id("models").Dot("Item").Values(jen.Id("Name").Op(":").Lit("changed name"), jen.Id("Details").Op(":").Lit("changed details")), jen.Id("ts").Op(":=").Id("httptest").Dot("NewTLSServer").Call(
						jen.Id("nil"),
					), jen.Id("c").Op(":=").Id("buildTestClient").Call(
						jen.Id("t"),
						jen.Id("ts"),
					...
					jen.Id("expected").Op(":=").Id("uint64").Call(
						...
						jen.Qual("net/http", "HandlerFunc").Call(
							jen.Func().Params(
								...
								jen.Id("assert").Dot("Equal").Call(
									jen.Id("t"), jen.Id("req").Dot("URL").Dot("Path"), jen.Qual("fmt", "Sprintf").Call(
										jen.Lit("/api/v1/items/%d"), jen.Id("expected"),
									), jen.Lit("expected and actual path don't match"),
								), jen.Id("assert").Dot("Equal").Call(
									jen.Id("t"), jen.Id("req").Dot("Method"), jen.Qual("net/http", "MethodDelete"),
								), jen.Id("res").Dot("WriteHeader").Call(
