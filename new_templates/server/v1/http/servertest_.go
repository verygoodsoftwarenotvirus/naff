...
func serverTestDotGo() *jen.File {
	...
	ret.Add(jen.Func().ID("buildTestServer").Params().Params(jen.Op("*").ID("Server")).Block(
		jen.ID("s").Op(":=").Op("&").ID("Server").Valuesln(jen.ID("DebugMode").Op(":").ID("true"), jen.ID("db").Op(":").ID("database").Dot(
			"BuildMockDatabase",
		).Call(), jen.ID("config").Op(":").ID("config").Dot(
			"ServerSettings",
		).Valuesln(), jen.ID("encoder").Op(":").Op("&").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/encoding/v1/mock", "EncoderDecoder").Valuesln(), jen.ID("httpServer").Op(":").ID("provideHTTPServer").Call(), jen.ID("logger").Op(":").ID("noop").Dot(
			"ProvideNoopLogger",
	...
	ret.Add(jen.Func().ID("TestProvideServer").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
		...
		jen.ID("T").Dot(
			...
			jen.List(jen.ID("actual"), jen.ID("err")).Op(":=").ID("ProvideServer").Call(jen.Qual("context", "Background").Call(), jen.Op("&").ID("config").Dot(
				...
				"ProvideNoopLogger",
			).Call(), jen.Op("&").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/encoding/v1/mock", "EncoderDecoder").Valuesln(), jen.ID("newsman").Dot(
				"NewNewsman",
