...
func adminTestDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("_").ID("suite").Dot("SetupTestSuite").Op("=").Parens(jen.Op("*").ID("adminTestSuite")).Call(jen.ID("nil")),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("adminTestSuite")).ID("TestClient_UpdateUserReputation").Params().Body(
			jen.Var().Defs(
				jen.ID("expectedPath").Op("=").Lit("/api/v1/admin/users/status"),
			),
			...
			jen.ID("s").Dot("Run").Call(
				...
				jen.Func().Params().Body(
					jen.ID("t").Op(":=").ID("s").Dot("T").Call(),
					jen.ID("exampleInput").Op(":=").Op("&").ID("types").Dot("UserReputationUpdateInput").Values(),
					jen.ID("spec").Op(":=").ID("newRequestSpec").Call(
						jen.ID("false"),
						jen.Qual("net/http", "MethodPost"),
						jen.Lit(""),
