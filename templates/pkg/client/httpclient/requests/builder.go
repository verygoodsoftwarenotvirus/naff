...
func builderDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("b").Op("*").ID("Builder")).ID("URL").Params().Params(jen.Op("*").Qual("net/url", "URL")).Body(
			jen.Return().ID("b").Dot("url")),
		jen.Line(),
	)

	code.Add(
		jen.Comment("SetURL provides the client's URL."),
		jen.Line(),
		jen.Func().Params(jen.ID("b").Op("*").ID("Builder")).ID("SetURL").Params(jen.ID("u").Op("*").Qual("net/url", "URL")).Params(jen.ID("error")).Body(
			jen.If(jen.ID("u").Op("==").ID("nil")).Body(
	...
	code.Add(
		jen.Comment("buildUnversionedURL builds a url without the v1 API prefix. It should otherwise be identical to buildRawURL."),
		jen.Line(),
		jen.Func().Params(jen.ID("b").Op("*").ID("Builder")).ID("buildUnversionedURL").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("qp").Qual("net/url", "Values"), jen.ID("parts").Op("...").ID("string")).Params(jen.ID("string")).Body(
			jen.List(jen.ID("_"), jen.ID("span")).Op(":=").ID("b").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
