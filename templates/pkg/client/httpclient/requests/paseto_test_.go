		...
		jen.Func().ID("TestBuilder_BuildAPIClientAuthTokenRequest").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("clientEncoder").Op(":=").Op("&").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/encoding/mock", "ClientEncoder").Values(),
					...
					jen.ID("clientEncoder").Dot("On").Call(
						jen.Lit("Encode"),
						jen.ID("mock").Dot("Anything"),
						jen.ID("mock").Dot("IsType").Call(jen.Op("&").Qual("bytes", "Buffer").Values()),
						jen.ID("exampleInput"),
