...
func clientDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("c").Op("*").ID("Client")).ID("fetchResponseToRequest").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("client").Op("*").Qual("net/http", "Client"), jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.Op("*").Qual("net/http", "Response"), jen.ID("error")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("c").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
