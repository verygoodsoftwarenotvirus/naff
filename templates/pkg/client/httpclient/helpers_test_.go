	...
	code.Add(
		jen.Func().ID("TestArgIsNotPointerOrNil").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					jen.ID("t").Dot("Parallel").Call(),
					jen.ID("err").Op(":=").ID("argIsNotPointerOrNil").Call(jen.Op("&").ID("testingType").Values()),
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					jen.ID("t").Dot("Parallel").Call(),
					jen.ID("err").Op(":=").ID("argIsNotPointerOrNil").Call(jen.ID("testingType").Values()),
					jen.ID("assert").Dot("Error").Call(
						jen.ID("t"),
						jen.ID("err"),
	...
	code.Add(
		jen.Func().ID("TestArgIsNotPointer").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					jen.ID("t").Dot("Parallel").Call(),
					jen.List(jen.ID("notAPointer"), jen.ID("err")).Op(":=").ID("argIsNotPointer").Call(jen.Op("&").ID("testingType").Values()),
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					jen.ID("t").Dot("Parallel").Call(),
					jen.List(jen.ID("notAPointer"), jen.ID("err")).Op(":=").ID("argIsNotPointer").Call(jen.ID("testingType").Values()),
	...
	code.Add(
		jen.Func().ID("TestArgIsNotNil").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					jen.ID("t").Dot("Parallel").Call(),
					jen.List(jen.ID("isNil"), jen.ID("err")).Op(":=").ID("argIsNotNil").Call(jen.Op("&").ID("testingType").Values()),
					jen.ID("assert").Dot("False").Call(
						jen.ID("t"),
						jen.ID("isNil"),
						jen.Lit("expected `false` when a pointer is provided"),
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					jen.ID("t").Dot("Parallel").Call(),
					jen.List(jen.ID("isNil"), jen.ID("err")).Op(":=").ID("argIsNotNil").Call(jen.ID("testingType").Values()),
					jen.ID("assert").Dot("False").Call(
						jen.ID("t"),
						jen.ID("isNil"),
						jen.Lit("expected `true` when a non-pointer is provided"),
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("res").Op(":=").Op("&").Qual("net/http", "Response").Valuesln(jen.ID("Body").Op(":").Qual("io", "NopCloser").Call(jen.Qual("strings", "NewReader").Call(jen.Func().Params().Params(jen.ID("string")).Body(
						jen.List(jen.ID("bs"), jen.ID("err")).Op(":=").Qual("encoding/json", "Marshal").Call(jen.Op("&").ID("types").Dot("ErrorResponse").Values()),
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("rc").Dot("On").Call(
						jen.Lit("Read"),
						jen.ID("mock").Dot("IsType").Call(jen.Index().ID("uint8").Values()),
					).Dot("Return").Call(
						jen.Lit(0),
						jen.ID("expected"),
					),
