	...
	ret.Add(
		jen.Func().ID("TestService_AuthenticationMiddleware").Params(jen.ID("T").PointerTo().Qual("testing", "T")).Block(
			...
			utils.BuildSubTestWithoutContext(
				...
				utils.RequireNotNil(jen.ID("req"), nil),
				jen.Line(),
				jen.ID("s").Dot("AuthenticationMiddleware").Call(jen.True()).Call(jen.ID("h")).Dot("ServeHTTP").Call(jen.ID("res"), jen.ID("req")),
				jen.Line(),
			...
			utils.BuildSubTestWithoutContext(
				...
				utils.RequireNotNil(jen.ID("req"), nil),
				jen.Line(),
				jen.ID("s").Dot("AuthenticationMiddleware").Call(jen.False()).Call(jen.ID("h")).Dot("ServeHTTP").Call(jen.ID("res"), jen.ID("req")),
				jen.Line(),
				utils.AssertEqual(jen.Qual("net/http", "StatusOK"), jen.ID("res").Dot("Code"), nil),
				jen.Line(),
				utils.AssertExpectationsFor("ocv", "mockDB", "h"),
