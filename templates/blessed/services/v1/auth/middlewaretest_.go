	...
	ret.Add(
		jen.Func().ID("TestService_CookieAuthenticationMiddleware").Params(jen.ID("T").ParamPointer().Qual("testing", "T")).Block(
			...
			utils.BuildSubTestWithoutContext(
				...
				jen.ID("exampleUser").Assign().VarPointer().Qual(proj.ModelsV1Package(), "User").Values(jen.ID("Username").MapAssign().Add(utils.FakeUsernameFunc())),
				jen.ID("md").Assign().VarPointer().Qual(proj.ModelsV1Package("mock"), "UserDataManager").Values(),
				jen.ID("md").Dot("On").Call(jen.Lit("GetUser"), jen.Qual("github.com/stretchr/testify/mock", "Anything"), jen.Qual("github.com/stretchr/testify/mock", "Anything")).Dot("Return").Call(jen.Parens(jen.PointerTo().Qual(proj.ModelsV1Package(), "User")).Call(jen.Nil()), jen.Nil()),
				jen.ID("s").Dot("userDB").Equals().ID("md"),
				jen.Line(),
				jen.List(jen.ID("req"), jen.Err()).Assign().Qual("net/http", "NewRequest").Call(jen.Qual("net/http", "MethodPost"), jen.Lit("http://todo.verygoodsoftwarenotvirus.ru"), jen.Nil()),
				jen.Qual("github.com/stretchr/testify/require", "NoError").Call(jen.ID("t"), jen.Err()),
	...
	ret.Add(
		jen.Func().ID("TestService_AuthenticationMiddleware").Params(jen.ID("T").ParamPointer().Qual("testing", "T")).Block(
			...
			utils.BuildSubTestWithoutContext(
				...
				jen.ID("ocv").Assign().VarPointer().ID("mockOAuth2ClientValidator").Values(),
				jen.ID("ocv").Dot("On").Call(jen.Lit("ExtractOAuth2ClientFromRequest"), jen.Qual("github.com/stretchr/testify/mock", "Anything")).Dot("Return").Call(jen.Parens(jen.PointerTo().Qual(proj.ModelsV1Package(), "OAuth2Client")).Call(jen.Nil()), jen.Qual("errors", "New").Call(jen.Lit("blah"))),
				jen.ID("s").Dot("oauth2ClientsService").Equals().ID("ocv"),
				jen.Line(),
				jen.ID("h").Assign().VarPointer().ID("MockHTTPHandler").Values(),
				jen.ID("h").Dot("On").Call(jen.Lit("ServeHTTP"), jen.Qual("github.com/stretchr/testify/mock", "Anything"), jen.Qual("github.com/stretchr/testify/mock", "Anything")).Dot("Return").Call(),
			...
			utils.BuildSubTestWithoutContext(
				...
				jen.Line(),
				jen.ID("mockDB").Assign().Qual(proj.DatabaseV1Package(), "BuildMockDatabase").Call().Dot("UserDataManager"),
				jen.ID("mockDB").Dot("On").Call(jen.Lit("GetUser"), jen.Qual("github.com/stretchr/testify/mock", "Anything"), jen.ID("exampleClient").Dot("BelongsToUser")).Dot("Return").Call(jen.Parens(jen.PointerTo().Qual(proj.ModelsV1Package(), "User")).Call(jen.Nil()), jen.Qual("errors", "New").Call(jen.Lit("blah"))),
				jen.ID("s").Dot("userDB").Equals().ID("mockDB"),
				jen.Line(),
				jen.ID("h").Assign().VarPointer().ID("MockHTTPHandler").Values(),
				jen.ID("h").Dot("On").Call(jen.Lit("ServeHTTP"), jen.Qual("github.com/stretchr/testify/mock", "Anything"), jen.Qual("github.com/stretchr/testify/mock", "Anything")).Dot("Return").Call(),
			...
			utils.BuildSubTestWithoutContext(
				...
				jen.Line(),
				jen.ID("ocv").Assign().VarPointer().ID("mockOAuth2ClientValidator").Values(),
				jen.ID("ocv").Dot("On").Call(jen.Lit("ExtractOAuth2ClientFromRequest"), jen.Qual("github.com/stretchr/testify/mock", "Anything")).Dot("Return").Call(jen.Parens(jen.PointerTo().Qual(proj.ModelsV1Package(), "OAuth2Client")).Call(jen.Nil()), jen.Nil()),
				jen.ID("s").Dot("oauth2ClientsService").Equals().ID("ocv"),
				jen.Line(),
				jen.ID("h").Assign().VarPointer().ID("MockHTTPHandler").Values(),
				jen.ID("h").Dot("On").Call(jen.Lit("ServeHTTP"), jen.Qual("github.com/stretchr/testify/mock", "Anything"), jen.Qual("github.com/stretchr/testify/mock", "Anything")).Dot("Return").Call(),
			...
			utils.BuildSubTestWithoutContext(
				...
				jen.Line(),
				jen.ID("mockDB").Assign().Qual(proj.DatabaseV1Package(), "BuildMockDatabase").Call().Dot("UserDataManager"),
				jen.ID("mockDB").Dot("On").Call(jen.Lit("GetUser"), jen.Qual("github.com/stretchr/testify/mock", "Anything"), jen.ID("exampleClient").Dot("BelongsToUser")).Dot("Return").Call(jen.Parens(jen.PointerTo().Qual(proj.ModelsV1Package(), "User")).Call(jen.Nil()), jen.Nil()),
				jen.ID("s").Dot("userDB").Equals().ID("mockDB"),
				jen.Line(),
				jen.ID("h").Assign().VarPointer().ID("MockHTTPHandler").Values(),
				jen.ID("h").Dot("On").Call(jen.Lit("ServeHTTP"), jen.Qual("github.com/stretchr/testify/mock", "Anything"), jen.Qual("github.com/stretchr/testify/mock", "Anything")).Dot("Return").Call(),
	...
	ret.Add(
		jen.Func().ID("Test_parseLoginInputFromForm").Params(jen.ID("T").ParamPointer().Qual("testing", "T")).Block(
			...
			utils.BuildSubTestWithoutContext(
				...
				jen.Qual("github.com/stretchr/testify/require", "NotNil").Call(jen.ID("t"), jen.ID("req")),
				jen.Line(),
				jen.ID("expected").Assign().VarPointer().Qual(proj.ModelsV1Package(), "UserLoginInput").Valuesln(
			...
			utils.BuildSubTestWithoutContext(
				...
				jen.ID("req").Equals().ID("req").Dot("WithContext").Callln(
					jen.Qual("context", "WithValue").Callln(
						...
						jen.VarPointer().Qual(proj.ModelsV1Package(), "User").Values(jen.ID("IsAdmin").MapAssign().ID("true")),
					),
				),
				jen.Line(),
				jen.ID("s").Assign().ID("buildTestService").Call(jen.ID("t")),
			...
			utils.BuildSubTestWithoutContext(
				...
				jen.ID("req").Equals().ID("req").Dot("WithContext").Callln(
					jen.Qual("context", "WithValue").Callln(
						jen.ID("req").Dot("Context").Call(),
						jen.Qual(proj.ModelsV1Package(), "UserKey"),
						jen.VarPointer().Qual(proj.ModelsV1Package(), "User").Values(jen.ID("IsAdmin").MapAssign().ID("false")),
					),
				),
				jen.Line(),
				jen.ID("s").Assign().ID("buildTestService").Call(jen.ID("t")),
