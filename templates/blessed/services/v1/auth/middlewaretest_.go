...
func middlewareTestDotGo(pkg *models.Project) *jen.File {
	...
	ret.Add(
		jen.Func().ID("TestService_CookieAuthenticationMiddleware").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.Line(),
				jen.List(jen.ID("cookie"), jen.Err()).Op(":=").ID("s").Dot("buildAuthCookie").Call(jen.ID("exampleUser")),
	...
	ret.Add(
		jen.Func().ID("Test_parseLoginInputFromForm").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.Line(),
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				jen.List(jen.ID("req"), jen.Err()).Op(":=").Qual("net/http", "NewRequest").Call(jen.Qual("net/http", "MethodGet"), jen.Lit("http://todo.verygoodsoftwarenotvirus.ru"), jen.Nil()),
				jen.Qual("github.com/stretchr/testify/require", "NoError").Call(jen.ID("t"), jen.Err()),
				jen.Qual("github.com/stretchr/testify/require", "NotNil").Call(jen.ID("t"), jen.ID("req")),
				jen.Line(),
	...
	ret.Add(
		jen.Func().ID("TestService_UserLoginInputMiddleware").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.Qual("github.com/stretchr/testify/require", "NoError").Call(jen.ID("t"), jen.Qual("encoding/json", "NewEncoder").Call(jen.Op("&").ID("b")).Dot("Encode").Call(jen.ID("exampleInput"))),
				jen.Line(),
				jen.List(jen.ID("req"), jen.Err()).Op(":=").Qual("net/http", "NewRequest").Call(jen.Qual("net/http", "MethodPost"), jen.Lit("http://todo.verygoodsoftwarenotvirus.ru"), jen.Op("&").ID("b")),
				jen.Qual("github.com/stretchr/testify/require", "NoError").Call(jen.ID("t"), jen.Err()),
				jen.Qual("github.com/stretchr/testify/require", "NotNil").Call(jen.ID("t"), jen.ID("req")),
				jen.ID("res").Op(":=").ID("httptest").Dot("NewRecorder").Call(),
				jen.Line(),
