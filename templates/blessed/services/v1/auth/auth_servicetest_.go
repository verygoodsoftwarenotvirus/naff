...
func authServiceTestDotGo(pkgRoot string, types []models.DataType) *jen.File {
	...
	ret.Add(
		jen.Func().ID("buildTestService").Params(jen.ID("t").Op("*").Qual("testing", "T")).Params(jen.Op("*").ID("Service")).Block(
			...
			jen.ID("cfg").Op(":=").Op("&").Qual(filepath.Join(pkgRoot, "internal/v1/config"), "ServerConfig").Valuesln(
			...
			jen.ID("auth").Op(":=").Op("&").Qual(filepath.Join(pkgRoot, "internal/v1/auth/mock"), "Authenticator").Values(),
			jen.ID("userDB").Op(":=").Op("&").Qual(filepath.Join(pkgRoot, "models/v1/mock"), "UserDataManager").Values(),
			...
			jen.ID("ed").Op(":=").Qual(filepath.Join(pkgRoot, "internal/v1/encoding"), "ProvideResponseEncoder").Call(),
			jen.Line(),
