...
func httpRoutesDotGo(pkgRoot string) *jen.File {
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("DecodeCookieFromRequest").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.ID("ca").Op("*").Qual(filepath.Join(pkgRoot, "models/v1"), "CookieAuth"), jen.ID("err").ID("error")).Block(
			...
			jen.Line(),
			jen.List(jen.ID("cookie"), jen.ID("err")).Op(":=").ID("req").Dot("Cookie").Call(jen.ID("CookieName")),
			jen.If(jen.ID("err").Op("!=").Qual("net/http", "ErrNoCookie").Op("&&").ID("cookie").Op("!=").ID("nil")).Block(
				...
				jen.If(jen.ID("decodeErr").Op("!=").ID("nil")).Block(
					jen.ID("s").Dot("logger").Dot("Error").Call(jen.ID("err"), jen.Lit("decoding request cookie")),
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("WebsocketAuthFunction").Params(jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.ID("bool")).Block(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").Qual("go.opencensus.io/trace", "StartSpan").Call(jen.ID("req").Dot("Context").Call(), jen.Lit("WebsocketAuthFunction")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.Line(),
			...
			jen.List(jen.ID("cookieAuth"), jen.ID("cookieErr")).Op(":=").ID("s").Dot("DecodeCookieFromRequest").Call(jen.ID("ctx"), jen.ID("req")),
			...
			jen.ID("s").Dot("logger").Dot("Error").Call(jen.ID("err"), jen.Lit("error authenticated token-authenticated request")),
			jen.Return().ID("false"),
		),
		jen.Line(),
	)
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("FetchUserFromRequest").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.Op("*").Qual(filepath.Join(pkgRoot, "models/v1"), "User"), jen.ID("error")).Block(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").Qual("go.opencensus.io/trace", "StartSpan").Call(jen.ID("ctx"), jen.Lit("FetchUserFromRequest")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.Line(),
			jen.List(jen.ID("ca"), jen.ID("decodeErr")).Op(":=").ID("s").Dot("DecodeCookieFromRequest").Call(jen.ID("ctx"), jen.ID("req")),
			jen.If(jen.ID("decodeErr").Op("!=").ID("nil")).Block(
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("LoginHandler").Params().Params(jen.Qual("net/http", "HandlerFunc")).Block(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Block(
				jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").Qual("go.opencensus.io/trace", "StartSpan").Call(jen.ID("req").Dot("Context").Call(), jen.Lit("LoginHandler")),
				jen.Defer().ID("span").Dot("End").Call(),
				jen.Line(),
				...
				jen.List(jen.ID("cookie"), jen.ID("err")).Op(":=").ID("s").Dot("buildAuthCookie").Call(jen.ID("loginData").Dot("user")),
				jen.If(jen.ID("err").Op("!=").ID("nil")).Block(
					...
					jen.ID("response").Op(":=").Op("&").Qual(filepath.Join(pkgRoot, "models/v1"), "ErrorResponse").Valuesln(
						jen.ID("Code").Op(":").Qual("net/http", "StatusInternalServerError"),
						jen.ID("Message").Op(":").Lit("error encountered building cookie"),
					),
					jen.If(jen.ID("err").Op(":=").ID("s").Dot("encoderDecoder").Dot("EncodeResponse").Call(jen.ID("res"), jen.ID("response")), jen.ID("err").Op("!=").ID("nil")).Block(
						jen.ID("s").Dot("logger").Dot("Error").Call(jen.ID("err"), jen.Lit("encoding response")),
					),
					jen.Return(),
				),
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("LogoutHandler").Params().Params(jen.Qual("net/http", "HandlerFunc")).Block(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Block(
				...
				jen.Defer().ID("span").Dot("End").Call(),
				jen.Line(),
				jen.If(jen.List(jen.ID("cookie"), jen.ID("err")).Op(":=").ID("req").Dot("Cookie").Call(jen.ID("CookieName")), jen.ID("err").Op("==").ID("nil").Op("&&").ID("cookie").Op("!=").ID("nil")).Block(
	...
	ret.Add(
		jen.Type().ID("loginData").Struct(jen.ID("loginInput").Op("*").Qual(filepath.Join(pkgRoot, "models/v1"), "UserLoginInput"), jen.ID("user").Op("*").Qual(filepath.Join(pkgRoot, "models/v1"), "User")),
		jen.Line(),
	)
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("fetchLoginDataFromRequest").Params(jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.Op("*").ID("loginData"), jen.Op("*").Qual(filepath.Join(pkgRoot, "models/v1"), "ErrorResponse")).Block(
			...
			jen.Defer().ID("span").Dot("End").Call(),
			jen.Line(),
			jen.List(jen.ID("loginInput"), jen.ID("ok")).Op(":=").ID("ctx").Dot("Value").Call(jen.ID("UserLoginInputMiddlewareCtxKey")).Assert(jen.Op("*").Qual(filepath.Join(pkgRoot, "models/v1"), "UserLoginInput")),
			jen.If(jen.Op("!").ID("ok")).Block(
				jen.ID("s").Dot("logger").Dot("Debug").Call(jen.Lit("no UserLoginInput found for /login request")),
				jen.Return().List(jen.ID("nil"), jen.Op("&").Qual(filepath.Join(pkgRoot, "models/v1"), "ErrorResponse").Valuesln(
			...
			jen.If(jen.ID("err").Op("==").Qual("database/sql", "ErrNoRows")).Block(
				...
				jen.Return().List(jen.ID("nil"), jen.Op("&").Qual(filepath.Join(pkgRoot, "models/v1"), "ErrorResponse").Values(jen.ID("Code").Op(":").Qual("net/http", "StatusBadRequest"))),
			).Else().If(jen.ID("err").Op("!=").ID("nil")).Block(
				jen.ID("s").Dot("logger").Dot("Error").Call(jen.ID("err"), jen.Lit("error fetching user")),
				jen.Return().List(jen.ID("nil"), jen.Op("&").Qual(filepath.Join(pkgRoot, "models/v1"), "ErrorResponse").Values(jen.ID("Code").Op(":").Qual("net/http", "StatusInternalServerError"))),
			),
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("validateLogin").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("loginInfo").ID("loginData")).Params(jen.ID("bool"), jen.ID("error")).Block(
			...
			jen.List(jen.ID("loginValid"), jen.ID("err")).Op(":=").ID("s").Dot("authenticator").Dot("ValidateLogin").Callln(
				jen.ID("ctx"),
			...
			jen.If(jen.ID("err").Op("==").Qual(filepath.Join(pkgRoot, "internal/v1/auth"), "ErrPasswordHashTooWeak").Op("&&").ID("loginValid")).Block(
				...
				jen.List(jen.ID("updated"), jen.ID("hashErr")).Op(":=").ID("s").Dot("authenticator").Dot("HashPassword").Call(jen.ID("ctx"), jen.ID("loginInput").Dot("Password")),
			...
			).Else().If(jen.ID("err").Op("!=").ID("nil").Op("&&").ID("err").Op("!=").Qual(filepath.Join(pkgRoot, "internal/v1/auth"), "ErrPasswordHashTooWeak")).Block(
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("buildAuthCookie").Params(jen.ID("user").Op("*").Qual(filepath.Join(pkgRoot, "models/v1"), "User")).Params(jen.Op("*").Qual("net/http", "Cookie"), jen.ID("error")).Block(
			...
			jen.List(jen.ID("encoded"), jen.ID("err")).Op(":=").ID("s").Dot("cookieManager").Dot("Encode").Callln(
				jen.ID("CookieName"),
				jen.Qual(filepath.Join(pkgRoot, "models/v1"), "CookieAuth").Valuesln(
			...
			jen.If(jen.ID("err").Op("!=").ID("nil")).Block(
				...
				jen.ID("s").Dot("logger").Dot("WithName").Call(jen.ID("cookieErrorLogName")).Dot("WithValue").Call(jen.Lit("user_id"), jen.ID("user").Dot("ID")).Dot("Error").Call(jen.ID("err"), jen.Lit("error encoding cookie")),
				jen.Return().List(jen.ID("nil"), jen.ID("err")),
			),
			jen.Line(),
			jen.Return().List(jen.ID("s").Dot("buildCookie").Call(jen.ID("encoded")), jen.ID("nil")),
		),
		jen.Line(),
	)
