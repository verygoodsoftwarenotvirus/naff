...
func authServiceDotGo(pkg *models.Project) *jen.File {
	...
	ret.Add(
		jen.Type().Defs(
			...
			jen.ID("OAuth2ClientValidator").Interface(
				jen.ID("ExtractOAuth2ClientFromRequest").Params(utils.CtxParam(), jen.ID("req").ParamPointer().Qual("net/http", "Request")).Params(jen.Op("*").Qual(pkg.ModelsV1Package(), "OAuth2Client"), jen.ID("error")),
			),
			jen.Line(),
	...
	ret.Add(
		...
		jen.Func().ID("ProvideAuthService").Paramsln(
			...
			jen.ID("authenticator").Qual(pkg.InternalAuthV1Package(), "Authenticator"),
			jen.ID("database").Qual(pkg.ModelsV1Package(), "UserDataManager"),
			jen.ID("oauth2ClientsService").ID("OAuth2ClientValidator"),
			jen.ID("userIDFetcher").ID("UserIDFetcher"),
