...
func authServiceDotGo() *jen.File {
	...
	ret.Add(
		jen.Type().Defs(
			...
			jen.ID("OAuth2ClientValidator").Interface(
				jen.ID("ExtractOAuth2ClientFromRequest").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.Op("*").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/models/v1", "OAuth2Client"), jen.ID("error")),
			),
			jen.Line(),
			...
			jen.ID("Service").Struct(
				jen.ID("config").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/v1/config", "AuthSettings"),
				...
				jen.ID("userDB").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/models/v1", "UserDataManager"),
	...
	ret.Add(
		...
		jen.Func().ID("ProvideAuthService").Paramsln(
			...
			jen.ID("authenticator").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/v1/auth", "Authenticator"),
			jen.ID("database").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/models/v1", "UserDataManager"),
			jen.ID("oauth2ClientsService").ID("OAuth2ClientValidator"),
			jen.ID("userIDFetcher").ID("UserIDFetcher"),
