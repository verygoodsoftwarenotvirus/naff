...
func httpRoutesTestDotGo(proj *models.Project) *jen.File {
		...
		jen.Func().ID("TestService_WebsocketAuthFunction").Params(jen.ID("T").PointerTo().Qual("testing", "T")).Block(
			...
			utils.BuildSubTestWithoutContext(
				...
				jen.ID("oacv").Dot("On").Callln(
					...
					jen.Qual(utils.MockPkg, "Anything"),
					jen.Qual(utils.MockPkg, "Anything"),
				).Dot("Return").Call(jen.ID(utils.BuildFakeVarName("OAuth2Client")), utils.ObligatoryError()),
				jen.ID("s").Dot("oauth2ClientsService").Equals().ID("oacv"),
				jen.Line(),
				jen.List(jen.ID("req"), jen.Err()).Assign().Qual("net/http", "NewRequest").Call(jen.Qual("net/http", "MethodGet"), jen.Lit("http://todo.verygoodsoftwarenotvirus.ru/testing"), jen.Nil()),
				utils.RequireNotNil(jen.ID("req"), nil),
	...
	ret.Add(
		jen.Func().ID("TestService_FetchUserFromRequest").Params(jen.ID("T").PointerTo().Qual("testing", "T")).Block(
			jen.ID("T").Dot("Parallel").Call(),
			jen.Line(),
			utils.BuildSubTestWithoutContext(
				"happy path",
	...
	ret.Add(
		jen.Func().ID("TestService_LoginHandler").Params(jen.ID("T").PointerTo().Qual("testing", "T")).Block(
			...
			utils.BuildSubTestWithoutContext(
				...
				jen.ID("req").Equals().ID("req").Dot("WithContext").Call(jen.Qual("context", "WithValue").Call(jen.ID("req").Dot("Context").Call(), jen.ID("UserLoginInputMiddlewareCtxKey"), jen.ID(utils.BuildFakeVarName("LoginData")))),
				jen.Line(),
			...
			utils.BuildSubTestWithoutContext(
				...
				jen.ID("cb").Dot("On").Callln(
					jen.Lit("Encode"),
					jen.Qual(utils.MockPkg, "Anything"),
					jen.Qual(utils.MockPkg, "Anything")).Dot("Return").Call(jen.EmptyString(), utils.ObligatoryError()),
			...
			utils.BuildSubTestWithoutContext(
				...
				jen.ID("cb").Dot("On").Callln(
					...
					jen.Qual(utils.MockPkg, "Anything"),
					jen.Qual(utils.MockPkg, "Anything"),
				).Dot("Return").Call(jen.EmptyString(), utils.ObligatoryError()),
				jen.ID("s").Dot("cookieManager").Equals().ID("cb"),
				jen.Line(),
				jen.ID("ed").Assign().AddressOf().Qual(proj.InternalEncodingV1Package("mock"), "EncoderDecoder").Values(),
				jen.ID("ed").Dot("On").Callln(
	...
	ret.Add(
		jen.Func().ID("TestService_Logout").Params(jen.ID("T").PointerTo().Qual("testing", "T")).Block(
			jen.ID("T").Dot("Parallel").Call(),
			jen.Line(),
			utils.BuildSubTestWithoutContext(
				"happy path",
	...
	ret.Add(
		jen.Func().ID("TestService_validateLogin").Params(jen.ID("T").PointerTo().Qual("testing", "T")).Block(
			...
			utils.BuildSubTest(
				...
				jen.ID("s").Dot("authenticator").Equals().ID("authr"),
				jen.Line(),
				jen.List(jen.ID("actual"), jen.Err()).Assign().ID("s").Dot("validateLogin").Call(utils.CtxVar(), jen.ID(utils.BuildFakeVarName("Input"))),
				utils.AssertTrue(jen.ID("actual"), nil),
				utils.AssertNoError(jen.Err(), nil),
				jen.Line(),
	...
	ret.Add(
		jen.Func().ID("TestService_CycleSecret").Params(jen.ID("T").PointerTo().Qual("testing", "T")).Block(
			jen.ID("T").Dot("Parallel").Call(),
			jen.Line(),
			utils.BuildSubTestWithoutContext(
				"normal operation",
