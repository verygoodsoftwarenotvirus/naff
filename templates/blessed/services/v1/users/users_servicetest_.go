			...
			utils.BuildSubTest(
				...
				jen.List(jen.ID("service"), jen.Err()).Assign().ID("ProvideUsersService").Callln(
					utils.CtxVar(), jen.Qual(proj.InternalConfigV1Package(),
						...
						"ProvideNoopLogger",
					).Call(), jen.ID("mockDB"), jen.AddressOf().Qual(proj.InternalAuthV1Package("mock"), "Authenticator").Values(), jen.Nil(), jen.AddressOf().Qual(proj.InternalEncodingV1Package("mock"), "EncoderDecoder").Values(), jen.ID("ucp"), jen.Nil()),
