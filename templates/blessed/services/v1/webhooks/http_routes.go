...
func httpRoutesDotGo(pkg *models.Project) *jen.File {
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("ListHandler").Params().Params(jen.Qual("net/http", "HandlerFunc")).Block(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").ParamPointer().Qual("net/http", "Request")).Block(
				...
				jen.If(jen.Err().Op("==").Qual("database/sql", "ErrNoRows")).Block(
					jen.ID("webhooks").Equals().VarPointer().Qual(pkg.ModelsV1Package(), "WebhookList").Valuesln(
						jen.ID("Webhooks").MapAssign().Index().Qual(pkg.ModelsV1Package(), "Webhook").Values()),
				...
				jen.If(jen.Err().Equals().ID("s").Dot("encoderDecoder").Dot("EncodeResponse").Call(jen.ID("res"), jen.ID("webhooks")), jen.Err().DoesNotEqual().ID("nil")).Block(
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().ID("validateWebhook").Params(jen.ID("input").Op("*").Qual(pkg.ModelsV1Package(),
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("CreateHandler").Params().Params(jen.Qual("net/http", "HandlerFunc")).Block(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").ParamPointer().Qual("net/http", "Request")).Block(
				...
				jen.Comment("figure out who this is all for"),
				jen.ID("userID").Assign().ID("s").Dot("userIDFetcher").Call(jen.ID("req")),
				jen.ID("logger").Assign().ID("s").Dot("logger").Dot("WithValue").Call(jen.Lit("user"), jen.ID("userID")),
				jen.Qual(pkg.InternalTracingV1Package(), "AttachUserIDToSpan").Call(jen.ID("span"), jen.ID("userID")),
				jen.Line(),
				jen.Comment("try to pluck the parsed input from the request context"),
				jen.List(jen.ID("input"), jen.ID("ok")).Assign().ID(utils.ContextVarName).Dot("Value").Call(jen.ID("CreateMiddlewareCtxKey")).Assert(jen.Op("*").Qual(pkg.ModelsV1Package(), "WebhookCreationInput")),
				jen.If(jen.Op("!").ID("ok")).Block(
					jen.ID("logger").Dot("Info").Call(jen.Lit("valid input not attached to request")),
					utils.WriteXHeader("res", "StatusBadRequest"),
					jen.Return(),
				),
				jen.ID("input").Dot("BelongsToUser").Equals().ID("userID"),
				jen.Line(),
				...
				jen.List(jen.ID("wh"), jen.Err()).Assign().ID("s").Dot("webhookDatabase").Dot("CreateWebhook").Call(utils.CtxVar(), jen.ID("input")),
				jen.If(jen.Err().DoesNotEqual().ID("nil")).Block(
					jen.ID("logger").Dot("Error").Call(jen.Err(), jen.Lit("error creating webhook")),
					utils.WriteXHeader("res", "StatusInternalServerError"),
					jen.Return(),
				),
				jen.Line(),
				jen.Comment("notify the relevant parties"),
				jen.Qual(pkg.InternalTracingV1Package(), "AttachWebhookIDToSpan").Call(jen.ID("span"), jen.ID("wh").Dot("ID")),
				...
				jen.If(jen.Err().Equals().ID("s").Dot("encoderDecoder").Dot("EncodeResponse").Call(jen.ID("res"), jen.ID("wh")), jen.Err().DoesNotEqual().ID("nil")).Block(
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("ReadHandler").Params().Params(jen.Qual("net/http", "HandlerFunc")).Block(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").ParamPointer().Qual("net/http", "Request")).Block(
				...
				jen.ID("userID").Assign().ID("s").Dot("userIDFetcher").Call(jen.ID("req")),
				jen.ID("webhookID").Assign().ID("s").Dot("webhookIDFetcher").Call(jen.ID("req")),
				jen.Line(),
				jen.Comment("document it for posterity"),
				jen.Qual(pkg.InternalTracingV1Package(), "AttachUserIDToSpan").Call(jen.ID("span"), jen.ID("userID")),
				jen.Qual(pkg.InternalTracingV1Package(), "AttachWebhookIDToSpan").Call(jen.ID("span"), jen.ID("webhookID")),
				...
				jen.If(jen.Err().Op("==").Qual("database/sql", "ErrNoRows")).Block(
					...
					jen.ID("logger").Dot("Error").Call(jen.Err(), jen.Lit("Error fetching webhook from webhookDatabase")),
					utils.WriteXHeader("res", "StatusInternalServerError"),
					jen.Return(),
				),
				jen.Line(),
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("UpdateHandler").Params().Params(jen.Qual("net/http", "HandlerFunc")).Block(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").ParamPointer().Qual("net/http", "Request")).Block(
				...
				jen.List(jen.ID("input"), jen.ID("ok")).Assign().ID(utils.ContextVarName).Dot("Value").Call(jen.ID("UpdateMiddlewareCtxKey")).Assert(jen.Op("*").Qual(pkg.ModelsV1Package(), "WebhookUpdateInput")),
				jen.If(jen.Op("!").ID("ok")).Block(
				...
				jen.If(jen.Err().Op("==").Qual("database/sql", "ErrNoRows")).Block(
					...
					jen.ID("logger").Dot("Error").Call(jen.Err(), jen.Lit("error encountered getting webhook")),
					utils.WriteXHeader("res", "StatusInternalServerError"),
					jen.Return(),
				),
				jen.Line(),
				...
				jen.If(jen.Err().Equals().ID("s").Dot("webhookDatabase").Dot("UpdateWebhook").Call(utils.CtxVar(), jen.ID("wh")), jen.Err().DoesNotEqual().ID("nil")).Block(
					jen.ID("logger").Dot("Error").Call(jen.Err(), jen.Lit("error encountered updating webhook")),
					utils.WriteXHeader("res", "StatusInternalServerError"),
					jen.Return(),
				),
				jen.Line(),
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("ArchiveHandler").Params().Params(jen.Qual("net/http", "HandlerFunc")).Block(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").ParamPointer().Qual("net/http", "Request")).Block(
				jen.List(utils.CtxVar(), jen.ID("span")).Assign().Qual("go.opencensus.io/trace", "StartSpan").Call(jen.ID("req").Dot("Context").Call(), jen.Lit("delete_route")),
				jen.Defer().ID("span").Dot("End").Call(),
				jen.Line(),
				jen.Comment("figure out what this is for and who it belongs to"),
				jen.ID("userID").Assign().ID("s").Dot("userIDFetcher").Call(jen.ID("req")),
				...
				jen.If(jen.Err().Op("==").Qual("database/sql", "ErrNoRows")).Block(
					...
					jen.ID("logger").Dot("Error").Call(jen.Err(), jen.Lit("error encountered deleting webhook")),
					utils.WriteXHeader("res", "StatusInternalServerError"),
					jen.Return(),
				),
				jen.Line(),
				...
				jen.ID("s").Dot("eventManager").Dot("Report").Call(jen.Qual("gitlab.com/verygoodsoftwarenotvirus/newsman", "Event").Valuesln(
					...
					jen.ID("Data").MapAssign().Qual(pkg.ModelsV1Package(), "Webhook").Values(jen.ID("ID").MapAssign().ID("webhookID")),
					jen.ID("Topics").MapAssign().Index().ID("string").Values(jen.ID("topicName"))),
				),
				jen.Line(),
