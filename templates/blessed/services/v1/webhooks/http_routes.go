...
func httpRoutesDotGo(pkg *models.Project) *jen.File {
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("ListHandler").Params().Params(jen.Qual("net/http", "HandlerFunc")).Block(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Block(
				...
				jen.If(jen.Err().Op("=").ID("s").Dot("encoderDecoder").Dot("EncodeResponse").Call(jen.ID("res"), jen.ID("webhooks")), jen.Err().Op("!=").ID("nil")).Block(
	...
	ret.Add(
		...
		jen.Func().ID("validateWebhook").Params(jen.ID("input").Op("*").Qual(filepath.Join(pkg.OutputPath, "models/v1"),
			...
			jen.List(jen.ID("_"), jen.Err()).Op(":=").Qual("net/url", "Parse").Call(jen.ID("input").Dot("URL")),
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("CreateHandler").Params().Params(jen.Qual("net/http", "HandlerFunc")).Block(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Block(
				...
				jen.If(jen.Err().Op(":=").ID("validateWebhook").Call(jen.ID("input")), jen.Err().Op("!=").ID("nil")).Block(
				...
				jen.List(jen.ID("wh"), jen.Err()).Op(":=").ID("s").Dot("webhookDatabase").Dot("CreateWebhook").Call(utils.CtxVar(), jen.ID("input")),
				jen.If(jen.Err().Op("!=").ID("nil")).Block(
				...
				jen.If(jen.Err().Op("=").ID("s").Dot("encoderDecoder").Dot("EncodeResponse").Call(jen.ID("res"), jen.ID("wh")), jen.Err().Op("!=").ID("nil")).Block(
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("ReadHandler").Params().Params(jen.Qual("net/http", "HandlerFunc")).Block(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Block(
				...
				jen.List(jen.ID("x"), jen.Err()).Op(":=").ID("s").Dot("webhookDatabase").Dot("GetWebhook").Call(utils.CtxVar(), jen.ID("webhookID"), jen.ID("userID")),
				jen.If(jen.Err().Op("==").Qual("database/sql", "ErrNoRows")).Block(
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("UpdateHandler").Params().Params(jen.Qual("net/http", "HandlerFunc")).Block(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Block(
				...
				jen.List(jen.ID("input"), jen.ID("ok")).Op(":=").ID(utils.ContextVarName).Dot("Value").Call(jen.ID("UpdateMiddlewareCtxKey")).Assert(jen.Op("*").Qual(filepath.Join(pkg.OutputPath, "models/v1"), "WebhookUpdateInput")),
				jen.If(jen.Op("!").ID("ok")).Block(
				...
				jen.List(jen.ID("wh"), jen.Err()).Op(":=").ID("s").Dot("webhookDatabase").Dot("GetWebhook").Call(utils.CtxVar(), jen.ID("webhookID"), jen.ID("userID")),
				jen.If(jen.Err().Op("==").Qual("database/sql", "ErrNoRows")).Block(
				...
				jen.If(jen.Err().Op("=").ID("s").Dot("webhookDatabase").Dot("UpdateWebhook").Call(utils.CtxVar(), jen.ID("wh")), jen.Err().Op("!=").ID("nil")).Block(
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("ArchiveHandler").Params().Params(jen.Qual("net/http", "HandlerFunc")).Block(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Block(
				jen.List(utils.CtxVar(), jen.ID("span")).Op(":=").Qual("go.opencensus.io/trace", "StartSpan").Call(jen.ID("req").Dot("Context").Call(), jen.Lit("delete_route")),
				jen.Defer().ID("span").Dot("End").Call(),
				jen.Line(),
				jen.Comment("figure out what this is for and who it belongs to"),
				jen.ID("userID").Op(":=").ID("s").Dot("userIDFetcher").Call(jen.ID("req")),
				...
				jen.Err().Op(":=").ID("s").Dot("webhookDatabase").Dot("ArchiveWebhook").Call(utils.CtxVar(), jen.ID("webhookID"), jen.ID("userID")),
				jen.If(jen.Err().Op("==").Qual("database/sql", "ErrNoRows")).Block(
					jen.ID("logger").Dot("Debug").Call(jen.Lit("no rows found for webhook")),
					utils.WriteXHeader("res", "StatusNotFound"),
					jen.Return(),
