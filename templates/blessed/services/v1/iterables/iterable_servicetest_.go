...
func buildbuildTestServiceFuncDecl(pkg *models.Project, typ models.DataType) []jen.Code {
	...
	serviceValues := []jen.Code{
		...
		jen.ID(fmt.Sprintf("%sCounter", uvn)).MapAssign().VarPointer().Qual(filepath.Join(pkg.OutputPath, "internal/v1/metrics/mock"), "UnitCounter").Values(),
		jen.ID(fmt.Sprintf("%sDatabase", uvn)).MapAssign().VarPointer().Qual(filepath.Join(pkg.OutputPath, "models/v1/mock"), fmt.Sprintf("%sDataManager", sn)).Values(),
	}
	...
	lines := []jen.Code{
		jen.Func().ID(fmt.Sprintf("TestProvide%sService", pn)).Params(jen.ID("T").ParamPointer().Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").ParamPointer().Qual("testing", "T")).Block(
				...
				jen.Line(),
				jen.ID("idm").Assign().VarPointer().Qual(filepath.Join(pkg.OutputPath, "models/v1/mock"), fmt.Sprintf("%sDataManager", sn)).Values(),
