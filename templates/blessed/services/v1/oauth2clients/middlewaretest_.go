		...
		jen.Func().ID("TestService_RequestIsAuthenticated").Params(jen.ID("T").ParamPointer().Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").ParamPointer().Qual("testing", "T")).Block(
				...
				jen.ID("mh").Dot("On").Callln(
					...
					jen.Qual("github.com/stretchr/testify/mock", "AnythingOfType").Call(jen.Lit("*http.Request")),
				).Dot("Return").Call(jen.VarPointer().Qual("goproj.in/oauth2.v3/models", "Token").Values(jen.ID("ClientID").MapAssign().ID("expected").Dot("ClientID")), jen.Nil()),
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("with error validating token"), jen.Func().Params(jen.ID("t").ParamPointer().Qual("testing", "T")).Block(
				...
				jen.ID("mh").Dot("On").Callln(
					jen.Lit("ValidationBearerToken"),
					jen.Qual("github.com/stretchr/testify/mock", "AnythingOfType").Call(jen.Lit("*http.Request")),
				).Dot("Return").Call(jen.Parens(jen.ParamPointer().Qual("goproj.in/oauth2.v3/models", "Token")).Call(jen.Nil()), jen.Qual("errors", "New").Call(jen.Lit("blah"))),
				jen.ID("s").Dot("oauth2Handler").Equals().ID("mh"),
				jen.Line(),
				jen.ID("req").Assign().ID("buildRequest").Call(jen.ID("t")),
				jen.List(jen.ID("actual"), jen.Err()).Assign().ID("s").Dot("ExtractOAuth2ClientFromRequest").Call(jen.ID("req").Dot("Context").Call(), jen.ID("req")),
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("with error fetching from database"), jen.Func().Params(jen.ID("t").ParamPointer().Qual("testing", "T")).Block(
				...
				jen.ID("mockDB").Dot("OAuth2ClientDataManager").Dot("On").Callln(
					...
					jen.Qual("github.com/stretchr/testify/mock", "Anything"),
					jen.ID("expected").Dot("ClientID"),
				).Dot("Return").Call(jen.Parens(jen.Op("*").Qual(proj.ModelsV1Package(), "OAuth2Client")).Call(jen.Nil()), jen.Qual("errors", "New").Call(jen.Lit("blah"))),
				jen.ID("s").Dot("database").Equals().ID("mockDB"),
				jen.Line(),
				jen.ID("req").Assign().ID("buildRequest").Call(jen.ID("t")),
				jen.List(jen.ID("actual"), jen.Err()).Assign().ID("s").Dot("ExtractOAuth2ClientFromRequest").Call(jen.ID("req").Dot("Context").Call(), jen.ID("req")),
