	...
	ret.Add(
		jen.Func().ID("TestService_AuthorizeScopeHandler").Params(jen.ID("T").PointerTo().Qual("testing", "T")).Block(
			...
			utils.BuildSubTestWithoutContext(
				...
				jen.Line(),
				jen.ID("req").Equals().ID("req").Dot("WithContext").Callln(
					jen.Qual("context", "WithValue").Call(jen.ID("req").Dot("Context").Call(), jen.ID("clientIDKey"), jen.ID(utils.BuildFakeVarName("OAuth2Client")).Dot("ClientID")),
				),
				jen.ID("req").Dot("URL").Dot("Path").Equals().Qual("fmt", "Sprintf").Call(
					jen.Lit("%s/%s"),
					jen.ID("apiURLPrefix"),
	...
	ret.Add(
		jen.Func().ID("TestService_UserAuthorizationHandler").Params(jen.ID("T").PointerTo().Qual("testing", "T")).Block(
			...
			utils.BuildSubTestWithoutContext(
				...
				jen.Line(),
				utils.BuildFakeVar(proj, "OAuth2Client"),
				jen.ID("expected").Assign().Qual("fmt", "Sprintf").Call(jen.Lit("%d"), jen.ID(utils.BuildFakeVarName("OAuth2Client")).Dot("BelongsToUser")),
				jen.Line(),
				jen.ID("req").Assign().ID("buildRequest").Call(jen.ID("t")),
				jen.ID("res").Assign().ID("httptest").Dot("NewRecorder").Call(),
				jen.ID("req").Equals().ID("req").Dot("WithContext").Callln(
					jen.Qual("context", "WithValue").Call(jen.ID("req").Dot("Context").Call(), jen.Qual(proj.ModelsV1Package(), "OAuth2ClientKey"), jen.ID(utils.BuildFakeVarName("OAuth2Client"))),
				),
				jen.Line(),
			...
			utils.BuildSubTestWithoutContext(
				...
				jen.ID("res").Assign().ID("httptest").Dot("NewRecorder").Call(),
				jen.ID("req").Equals().ID("req").Dot("WithContext").Callln(
					jen.Qual("context", "WithValue").Call(jen.ID("req").Dot("Context").Call(), jen.Qual(proj.ModelsV1Package(), "UserKey"), jen.ID(utils.BuildFakeVarName("User"))),
				),
				jen.Line(),
				jen.List(jen.ID("actual"), jen.Err()).Assign().ID("s").Dot("UserAuthorizationHandler").Call(jen.ID("res"), jen.ID("req")),
				utils.AssertNoError(jen.Err(), nil),
