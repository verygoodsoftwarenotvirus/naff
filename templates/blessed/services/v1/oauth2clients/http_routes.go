...
func httpRoutesDotGo() *jen.File {
	...
	ret.Add(
		jen.Const().Defs(
			...
			jen.ID("clientIDKey").ID("models").Dot("ContextKey").Op("=").Lit("client_id"),
		),
		jen.Line(),
	)
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("fetchUserID").Params(jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.ID("uint64")).Block(
			jen.If(jen.List(jen.ID("id"), jen.ID("ok")).Op(":=").ID("req").Dot("Context").Call().Dot("Value").Call(jen.ID("models").Dot("UserIDKey")).Assert(jen.ID("uint64")), jen.ID("ok")).Block(
				jen.Return().ID("id"),
			),
			jen.Return().Lit(0),
		),
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("ListHandler").Params().Params(jen.Qual("net/http", "HandlerFunc")).Block(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Block(
				...
				jen.If(jen.ID("err").Op("==").Qual("database/sql", "ErrNoRows")).Block(
					...
					jen.ID("oauth2Clients").Op("=").Op("&").ID("models").Dot("OAuth2ClientList").Valuesln(
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("CreateHandler").Params().Params(jen.Qual("net/http", "HandlerFunc")).Block(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Block(
				...
				jen.List(jen.ID("input"), jen.ID("ok")).Op(":=").ID("ctx").Dot("Value").Call(jen.ID("CreationMiddlewareCtxKey")).Assert(jen.Op("*").ID("models").Dot("OAuth2ClientCreationInput")),
				...
				jen.ID("input").Dot("BelongsTo").Op("=").ID("user").Dot("ID"),
				jen.Line(),
				...
				jen.ID("input").Dot("BelongsTo").Op("=").ID("s").Dot("fetchUserID").Call(jen.ID("req")),
				jen.Line(),
