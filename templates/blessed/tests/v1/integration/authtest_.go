	...
	ret.Add(
		jen.Func().ID("TestAuth").Params(jen.ID("test").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("test").Dot("Run").Call(jen.Lit("should only allow users to see their own content"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.Qual("github.com/stretchr/testify/assert", "Error").Call(jen.ID("t"), jen.ID("err"), jen.Lit("should experience error trying to fetch entry they're not authorized for")),
				jen.Line(),
				jen.Comment("Clean up"),
				jen.Qual("github.com/stretchr/testify/assert", "NoError").Call(jen.ID("t"), jen.ID("todoClient").Dot("ArchiveWebhook").Call(jen.ID("tctx"), jen.ID("webhookA").Dot("ID"))),
				jen.Qual("github.com/stretchr/testify/assert", "NoError").Call(jen.ID("t"), jen.ID("todoClient").Dot("ArchiveWebhook").Call(jen.ID("tctx"), jen.ID("webhookB").Dot("ID"))),
			)),
			jen.Line(),
