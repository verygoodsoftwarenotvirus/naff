	...
	ret.Add(
		jen.Func().ID("loginUser").Params(jen.ID("t").Op("*").Qual("testing", "T"), jen.List(jen.ID("username"), jen.ID("password"), jen.ID("totpSecret")).ID("string")).Params(jen.Op("*").Qual("net/http", "Cookie")).Block(
			jen.ID("loginURL").Op(":=").Qual("fmt", "Sprintf").Call(
				...
				jen.ID("todoClient").Dot("URL").Dot("Scheme"),
				jen.ID("todoClient").Dot("URL").Dot("Hostname").Call(),
...
`), jen.ID("username"), jen.ID("password"), jen.ID("code")),
			...
			jen.List(jen.ID("resp"), jen.ID("err")).Op(":=").Qual("net/http", "DefaultClient").Dot("Do").Call(jen.ID("req")),
			jen.If(jen.ID("err").Op("!=").ID("nil")).Block(
				jen.Qual("log", "Fatal").Call(jen.ID("err")),
			),
			jen.Line(),
			jen.ID("assert").Dot("Equal").Call(jen.ID("t"), jen.Qual("net/http", "StatusNoContent"), jen.ID("resp").Dot("StatusCode"), jen.Lit("login should be successful")),
			jen.Line(),
	...
	ret.Add(
		jen.Func().ID("TestAuth").Params(jen.ID("test").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("test").Dot("Run").Call(jen.Lit("should be able to login"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.Line(),
				jen.List(jen.ID("u"), jen.ID("err")).Op(":=").Qual("net/url", "Parse").Call(jen.ID("todoClient").Dot("BuildURL").Call(jen.ID("nil"))),
				jen.ID("require").Dot("NoError").Call(jen.ID("t"), jen.ID("err")),
				jen.ID("u").Dot("Path").Op("=").Lit("/users/login"),
				jen.Line(),
				jen.List(jen.ID("req"), jen.ID("err")).Op("=").Qual("net/http", "NewRequest").Call(jen.Qual("net/http", "MethodPost"), jen.ID("u").Dot("String").Call(), jen.ID("body")),
				jen.ID("checkValueAndError").Call(jen.ID("t"), jen.ID("req"), jen.ID("err")),
				jen.Line(),
				jen.Comment("execute login request"),
				jen.List(jen.ID("res"), jen.ID("err")).Op("=").ID("todoClient").Dot("PlainClient").Call().Dot("Do").Call(jen.ID("req")),
				jen.ID("checkValueAndError").Call(jen.ID("t"), jen.ID("res"), jen.ID("err")),
				jen.ID("assert").Dot("Equal").Call(jen.ID("t"), jen.Qual("net/http", "StatusNoContent"), jen.ID("res").Dot("StatusCode")),
				jen.Line(),
				jen.ID("cookies").Op(":=").ID("res").Dot("Cookies").Call(),
				jen.ID("assert").Dot("Len").Call(jen.ID("t"), jen.ID("cookies"), jen.Lit(1)),
			)),
			jen.Line(),
			jen.ID("test").Dot("Run").Call(jen.Lit("should be able to logout"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("cookies").Op(":=").ID("res").Dot("Cookies").Call(),
				jen.ID("require").Dot("Len").Call(jen.ID("t"), jen.ID("cookies"), jen.Lit(1)),
				jen.ID("loginCookie").Op(":=").ID("cookies").Index(jen.Lit(0)),
				jen.Line(),
				jen.Comment("build logout request"),
				jen.List(jen.ID("u2"), jen.ID("err")).Op(":=").Qual("net/url", "Parse").Call(jen.ID("todoClient").Dot("BuildURL").Call(jen.ID("nil"))),
				jen.ID("require").Dot("NoError").Call(jen.ID("t"), jen.ID("err")),
				jen.ID("u2").Dot("Path").Op("=").Lit("/users/logout"),
				jen.Line(),
				jen.List(jen.ID("req"), jen.ID("err")).Op("=").Qual("net/http", "NewRequest").Call(jen.Qual("net/http", "MethodPost"), jen.ID("u2").Dot("String").Call(), jen.ID("nil")),
				jen.ID("checkValueAndError").Call(jen.ID("t"), jen.ID("req"), jen.ID("err")),
				...
				jen.List(jen.ID("res"), jen.ID("err")).Op("=").ID("todoClient").Dot("PlainClient").Call().Dot("Do").Call(jen.ID("req")),
				jen.ID("checkValueAndError").Call(jen.ID("t"), jen.ID("res"), jen.ID("err")),
				jen.ID("assert").Dot("Equal").Call(jen.ID("t"), jen.Qual("net/http", "StatusOK"), jen.ID("res").Dot("StatusCode")),
			)),
			jen.Line(),
			jen.ID("test").Dot("Run").Call(jen.Lit("login request without body fails"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("u").Dot("Path").Op("=").Lit("/users/login"),
				jen.Line(),
				jen.List(jen.ID("req"), jen.ID("err")).Op(":=").Qual("net/http", "NewRequest").Call(jen.Qual("net/http", "MethodPost"), jen.ID("u").Dot("String").Call(), jen.ID("nil")),
				jen.ID("checkValueAndError").Call(jen.ID("t"), jen.ID("req"), jen.ID("err")),
				jen.Line(),
				jen.Comment("execute login request"),
				jen.List(jen.ID("res"), jen.ID("err")).Op(":=").ID("todoClient").Dot("PlainClient").Call().Dot("Do").Call(jen.ID("req")),
				jen.ID("checkValueAndError").Call(jen.ID("t"), jen.ID("res"), jen.ID("err")),
				jen.ID("assert").Dot("Equal").Call(jen.ID("t"), jen.Qual("net/http", "StatusBadRequest"), jen.ID("res").Dot("StatusCode")),
			)),
			jen.Line(),
			jen.ID("test").Dot("Run").Call(jen.Lit("should not be able to log in with the wrong password"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.List(jen.ID("res"), jen.ID("err")).Op("=").ID("todoClient").Dot("PlainClient").Call().Dot("Do").Call(jen.ID("req")),
				jen.ID("checkValueAndError").Call(jen.ID("t"), jen.ID("res"), jen.ID("err")),
				jen.ID("assert").Dot("Equal").Call(jen.ID("t"), jen.Qual("net/http", "StatusUnauthorized"), jen.ID("res").Dot("StatusCode")),
			)),
			jen.Line(),
			jen.ID("test").Dot("Run").Call(jen.Lit("should not be able to login as someone that doesn't exist"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.Line(),
				jen.ID("cookies").Op(":=").ID("res").Dot("Cookies").Call(),
				jen.ID("assert").Dot("Len").Call(jen.ID("t"), jen.ID("cookies"), jen.Lit(0)),
			)),
			jen.Line(),
			jen.ID("test").Dot("Run").Call(jen.Lit("should reject an unauthenticated request"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				jen.List(jen.ID("req"), jen.ID("err")).Op(":=").Qual("net/http", "NewRequest").Call(jen.Qual("net/http", "MethodGet"), jen.ID("todoClient").Dot("BuildURL").Call(jen.ID("nil"), jen.Lit("webhooks")), jen.ID("nil")),
				jen.ID("assert").Dot("NoError").Call(jen.ID("t"), jen.ID("err")),
				jen.Line(),
				jen.List(jen.ID("res"), jen.ID("err")).Op(":=").ID("todoClient").Dot("PlainClient").Call().Dot("Do").Call(jen.ID("req")),
				jen.ID("assert").Dot("NoError").Call(jen.ID("t"), jen.ID("err")),
			...
			jen.ID("test").Dot("Run").Call(jen.Lit("should be able to change password"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.Line(),
				jen.List(jen.ID("req"), jen.ID("err")).Op(":=").Qual("net/http", "NewRequest").Call(jen.Qual("net/http", "MethodPut"), jen.ID("u").Dot("String").Call(), jen.ID("body")),
				jen.ID("checkValueAndError").Call(jen.ID("t"), jen.ID("req"), jen.ID("err")),
				...
				jen.Line(),
				jen.List(jen.ID("u3"), jen.ID("err")).Op(":=").Qual("net/url", "Parse").Call(jen.ID("todoClient").Dot("BuildURL").Call(jen.ID("nil"))),
				jen.ID("require").Dot("NoError").Call(jen.ID("t"), jen.ID("err")),
				jen.ID("u3").Dot("Path").Op("=").Lit("/users/login"),
				jen.Line(),
				jen.List(jen.ID("req"), jen.ID("err")).Op("=").Qual("net/http", "NewRequest").Call(jen.Qual("net/http", "MethodPost"), jen.ID("u3").Dot("String").Call(), jen.ID("body")),
				jen.ID("checkValueAndError").Call(jen.ID("t"), jen.ID("req"), jen.ID("err")),
				jen.Line(),
				jen.Comment("execute login request"),
				jen.List(jen.ID("res"), jen.ID("err")).Op("=").ID("todoClient").Dot("PlainClient").Call().Dot("Do").Call(jen.ID("req")),
			...
			jen.ID("test").Dot("Run").Call(jen.Lit("should accept a login cookie if a token is missing"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("req").Dot("AddCookie").Call(jen.ID("cookie")),
				jen.Line(),
				jen.List(jen.ID("res"), jen.ID("err")).Op(":=").Parens(jen.Op("&").Qual("net/http", "Client").Values(jen.ID("Timeout").Op(":").Lit(10).Op("*").Qual("time", "Second"))).Dot("Do").Call(jen.ID("req")),
				jen.ID("assert").Dot("NoError").Call(jen.ID("t"), jen.ID("err")),
				jen.ID("assert").Dot("Equal").Call(jen.ID("t"), jen.Qual("net/http", "StatusOK"), jen.ID("res").Dot("StatusCode")),
			)),
			jen.Line(),
			jen.ID("test").Dot("Run").Call(jen.Lit("should only allow users to see their own content"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.List(jen.ID("userA"), jen.ID("err")).Op(":=").Qual(filepath.Join(pkg.OutputPath, "tests/v1/testutil"), "CreateObligatoryUser").Call(jen.ID("urlToUse"), jen.ID("debug")),
				jen.ID("require").Dot("NoError").Call(jen.ID("t"), jen.ID("err")),
				jen.Line(),
				jen.List(jen.ID("ca"), jen.ID("err")).Op(":=").Qual(filepath.Join(pkg.OutputPath, "tests/v1/testutil"), "CreateObligatoryClient").Call(jen.ID("urlToUse"), jen.ID("userA")),
				jen.ID("require").Dot("NoError").Call(jen.ID("t"), jen.ID("err")),
				jen.Line(),
				jen.List(jen.ID("clientA"), jen.ID("err")).Op(":=").Qual(filepath.Join(pkg.OutputPath, "client/v1/http"), "NewClient").Callln(
					jen.ID("tctx"),
					jen.ID("ca").Dot("ClientID"),
					jen.ID("ca").Dot("ClientSecret"),
					jen.ID("todoClient").Dot("URL"),
					jen.ID("noop").Dot("ProvideNoopLogger").Call(), jen.ID("buildHTTPClient").Call(), jen.ID("ca").Dot("Scopes"),
					jen.ID("true"),
				),
				jen.ID("checkValueAndError").Call(jen.ID("test"), jen.ID("clientA"), jen.ID("err")),
				jen.Line(),
				jen.Comment("create user and oauth2 client B"),
				jen.List(jen.ID("userB"), jen.ID("err")).Op(":=").Qual(filepath.Join(pkg.OutputPath, "tests/v1/testutil"), "CreateObligatoryUser").Call(jen.ID("urlToUse"), jen.ID("debug")),
				jen.ID("require").Dot("NoError").Call(jen.ID("t"), jen.ID("err")),
				jen.Line(),
				jen.List(jen.ID("cb"), jen.ID("err")).Op(":=").Qual(filepath.Join(pkg.OutputPath, "tests/v1/testutil"), "CreateObligatoryClient").Call(jen.ID("urlToUse"), jen.ID("userB")),
				jen.ID("require").Dot("NoError").Call(jen.ID("t"), jen.ID("err")),
				jen.Line(),
				jen.List(jen.ID("clientB"), jen.ID("err")).Op(":=").Qual(filepath.Join(pkg.OutputPath, "client/v1/http"), "NewClient").Callln(
					jen.ID("tctx"),
					jen.ID("cb").Dot("ClientID"),
					jen.ID("cb").Dot("ClientSecret"),
					jen.ID("todoClient").Dot("URL"),
					jen.ID("noop").Dot("ProvideNoopLogger").Call(), jen.ID("buildHTTPClient").Call(), jen.ID("cb").Dot("Scopes"),
					jen.ID("true"),
				),
				jen.ID("checkValueAndError").Call(jen.ID("test"), jen.ID("clientA"), jen.ID("err")),
				jen.Line(),
				jen.Comment("create webhook for user A"),
				jen.List(jen.ID("webhookA"), jen.ID("err")).Op(":=").ID("clientA").Dot("CreateWebhook").Call(jen.ID("tctx"), jen.Op("&").Qual(filepath.Join(pkg.OutputPath, "models/v1"), "WebhookCreationInput").Valuesln(
				...
				jen.List(jen.ID("webhookB"), jen.ID("err")).Op(":=").ID("clientB").Dot("CreateWebhook").Call(jen.ID("tctx"), jen.Op("&").Qual(filepath.Join(pkg.OutputPath, "models/v1"), "WebhookCreationInput").Valuesln(
					jen.ID("Method").Op(":").Qual("net/http", "MethodPatch"),
					jen.ID("Name").Op(":").Lit("B"),
				)),
				jen.ID("checkValueAndError").Call(jen.ID("t"), jen.ID("webhookB"), jen.ID("err")),
				jen.Line(),
				jen.List(jen.ID("i"), jen.ID("err")).Op(":=").ID("clientB").Dot("GetWebhook").Call(jen.ID("tctx"), jen.ID("webhookA").Dot("ID")),
				...
				jen.ID("assert").Dot("NoError").Call(jen.ID("t"), jen.ID("todoClient").Dot("ArchiveWebhook").Call(jen.ID("tctx"), jen.ID("webhookA").Dot("ID"))),
				jen.ID("assert").Dot("NoError").Call(jen.ID("t"), jen.ID("todoClient").Dot("ArchiveWebhook").Call(jen.ID("tctx"), jen.ID("webhookB").Dot("ID"))),
			)),
			jen.Line(),
			jen.ID("test").Dot("Run").Call(jen.Lit("should only allow clients with a given scope to see that scope's content"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.List(jen.ID("c"), jen.ID("err")).Op(":=").Qual(filepath.Join(pkg.OutputPath, "client/v1/http"), "NewClient").Callln(
					...
					jen.ID("todoClient").Dot("URL"),
					jen.ID("noop").Dot("ProvideNoopLogger").Call(), jen.ID("buildHTTPClient").Call(), jen.ID("premade").Dot("Scopes"),
					jen.ID("true"),
				),
				jen.ID("checkValueAndError").Call(jen.ID("test"), jen.ID("c"), jen.ID("err")),
				jen.Line(),
				jen.List(jen.ID("i"), jen.ID("err")).Op(":=").ID("c").Dot("GetOAuth2Clients").Call(jen.ID("tctx"), jen.ID("nil")),
				jen.ID("assert").Dot("Nil").Call(jen.ID("t"), jen.ID("i")),
				jen.ID("assert").Dot("Error").Call(jen.ID("t"), jen.ID("err"), jen.Lit("should experience error trying to fetch entry they're not authorized for")),
			)),
		),
