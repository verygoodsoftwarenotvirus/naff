...
func oauth2ClientsDotGo(proj *models.Project) *jen.File {
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().ID("fetchRandomOAuth2Client").Params(jen.ID("c").Op("*").Qual(proj.HTTPClientV1Package(), "V1Client")).Params(jen.Op("*").Qual(proj.ModelsV1Package(),
			"OAuth2Client",
		)).Block(
			...
			jen.Line(),
			jen.Var().ID("selectedClient").Op("*").Qual(proj.ModelsV1Package(), "OAuth2Client"),
	...
	ret.Add(
		jen.Func().ID("buildOAuth2ClientActions").Params(jen.ID("c").Op("*").Qual(proj.HTTPClientV1Package(), "V1Client")).Params(jen.Map(jen.ID("string")).Op("*").ID("Action")).Block(
