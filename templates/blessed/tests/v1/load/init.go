...
func initDotGo(pkgRoot string, types []models.DataType) *jen.File {
	...
	ret.Add(
		jen.Var().Defs(
			...
			jen.ID("oa2Client").Op("*").Qual(filepath.Join(pkgRoot, "models/v1"), "OAuth2Client"),
		),
		jen.Line(),
	)
	...
	ret.Add(
		jen.Func().ID("init").Params().Block(
			...
			jen.Line(),
			jen.List(jen.ID("u"), jen.ID("err")).Op(":=").Qual(filepath.Join(pkgRoot, "tests/v1/testutil"), "CreateObligatoryUser").Call(jen.ID("urlToUse"), jen.ID("debug")),
			jen.If(jen.ID("err").Op("!=").ID("nil")).Block(
				jen.ID("logger").Dot("Fatal").Call(jen.ID("err")),
			),
			jen.Line(),
			jen.List(jen.ID("oa2Client"), jen.ID("err")).Op("=").Qual(filepath.Join(pkgRoot, "tests/v1/testutil"), "CreateObligatoryClient").Call(jen.ID("urlToUse"), jen.ID("u")),
			jen.If(jen.ID("err").Op("!=").ID("nil")).Block(
				jen.ID("logger").Dot("Fatal").Call(jen.ID("err")),
			),
			jen.Line(),
