	...
	ret.Add(
		jen.Type().Defs(
			...
			jen.ID("User").Struct(
				...
				jen.ID("PasswordLastChangedOn").PointerTo().ID("uint64").Tag(jsonTag("password_last_changed_on")),
			...
			jen.ID("UserDataManager").Interface(
				jen.ID("GetUser").Params(utils.CtxParam(), jen.ID("userID").ID("uint64")).Params(jen.PointerTo().ID("User"), jen.Error()),
				jen.ID("GetUserByUsername").Params(utils.CtxParam(), jen.ID("username").ID("string")).Params(jen.PointerTo().ID("User"), jen.Error()),
				jen.ID("GetUserCount").Params(utils.CtxParam(), jen.ID(utils.FilterVarName).PointerTo().ID("QueryFilter")).Params(jen.ID("uint64"), jen.Error()),
				jen.ID("GetUsers").Params(utils.CtxParam(), jen.ID(utils.FilterVarName).PointerTo().ID("QueryFilter")).Params(jen.PointerTo().ID("UserList"), jen.Error()),
				jen.ID("CreateUser").Params(utils.CtxParam(), jen.ID("input").PointerTo().ID("UserInput")).Params(jen.PointerTo().ID("User"), jen.Error()),
				jen.ID("UpdateUser").Params(utils.CtxParam(), jen.ID("updated").PointerTo().ID("User")).Params(jen.Error()),
				jen.ID("ArchiveUser").Params(utils.CtxParam(), jen.ID("userID").ID("uint64")).Params(jen.Error()),
			),
			jen.Line(),
