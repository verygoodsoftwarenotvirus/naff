	...
	ret.Add(
		jen.Type().Defs(
			...
			jen.ID("OAuth2ClientDataManager").Interface(
				...
				jen.ID("GetOAuth2ClientCount").Params(utils.CtxParam(), jen.ID("userID").ID("uint64"), jen.ID(utils.FilterVarName).Op("*").ID("QueryFilter")).Params(jen.ID("uint64"), jen.ID("error")),
				jen.ID("GetOAuth2Clients").Params(utils.CtxParam(), jen.ID("userID").ID("uint64"), jen.ID(utils.FilterVarName).Op("*").ID("QueryFilter")).Params(jen.Op("*").ID("OAuth2ClientList"), jen.ID("error")),
				...
				jen.ID("ArchiveOAuth2Client").Params(utils.CtxParam(), jen.List(jen.ID("clientID"), jen.ID("userID")).ID("uint64")).Params(jen.ID("error")),
			),
			jen.Line(),
