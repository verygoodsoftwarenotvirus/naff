...
func oauth2ClientDotGo(pkgRoot string, types []models.DataType) *jen.File {
	...
	ret.Add(
		jen.Var().ID("_").ID("oauth2").Dot("ClientInfo").Op("=").Parens(jen.Op("*").ID("OAuth2Client")).Call(jen.ID("nil")),
		jen.Line(),
	)
