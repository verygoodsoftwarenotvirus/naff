	...
	ret.Add(
		jen.Type().Defs(
			...
			jen.ID("OAuth2ClientDataServer").Interface(
				...
				jen.ID("ExtractOAuth2ClientFromRequest").Params(constants.CtxParam(), jen.ID("req").PointerTo().Qual("net/http", "Request")).Params(jen.PointerTo().ID("OAuth2Client"), jen.Error()),
				jen.Line(),
