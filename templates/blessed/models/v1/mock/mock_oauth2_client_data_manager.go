...
func mockOauth2ClientDataManagerDotGo(proj *models.Project) *jen.File {
	...
	ret.Add(
		jen.Var().ID("_").Qual(proj.ModelsV1Package(), "OAuth2ClientDataManager").Equals().Parens(jen.Op("*").ID("OAuth2ClientDataManager")).Call(jen.Nil()),
		jen.Line(),
	)
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("m").Op("*").ID("OAuth2ClientDataManager")).ID("GetOAuth2Client").Params(utils.CtxParam(), jen.List(jen.ID("clientID"), jen.ID("userID")).ID("uint64")).Params(jen.Op("*").Qual(proj.ModelsV1Package(), "OAuth2Client"),
			jen.ID("error")).Block(
			jen.ID("args").Assign().ID("m").Dot("Called").Call(utils.CtxVar(), jen.ID("clientID"), jen.ID("userID")),
			jen.Return().List(jen.ID("args").Dot("Get").Call(jen.Lit(0)).Assert(jen.Op("*").Qual(proj.ModelsV1Package(), "OAuth2Client")), jen.ID("args").Dot("Error").Call(jen.Lit(1))),
		),
		jen.Line(),
	)
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("m").Op("*").ID("OAuth2ClientDataManager")).ID("GetOAuth2ClientByClientID").Params(utils.CtxParam(), jen.ID("identifier").ID("string")).Params(jen.Op("*").Qual(proj.ModelsV1Package(), "OAuth2Client"),
			jen.ID("error")).Block(
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("m").Op("*").ID("OAuth2ClientDataManager")).ID("GetAllOAuth2Clients").Params(utils.CtxVar().Qual("context", "Context")).Params(jen.Index().Op("*").Qual(proj.ModelsV1Package(), "OAuth2Client"),
			jen.ID("error")).Block(
			jen.ID("args").Assign().ID("m").Dot("Called").Call(utils.CtxVar()),
			jen.Return().List(jen.ID("args").Dot("Get").Call(jen.Lit(0)).Assert(jen.Index().Op("*").Qual(proj.ModelsV1Package(), "OAuth2Client")), jen.ID("args").Dot("Error").Call(jen.Lit(1))),
		),
		jen.Line(),
	)
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("m").Op("*").ID("OAuth2ClientDataManager")).ID("GetAllOAuth2ClientsForUser").Params(utils.CtxParam(), jen.ID("userID").ID("uint64")).Params(jen.Index().Op("*").Qual(proj.ModelsV1Package(), "OAuth2Client"),
			jen.ID("error")).Block(
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("m").Op("*").ID("OAuth2ClientDataManager")).ID("GetOAuth2Clients").Params(
			...
			jen.ID("userID").ID("uint64"),
			jen.ID(utils.FilterVarName).Op("*").Qual(proj.ModelsV1Package(), "QueryFilter"),
		).Params(jen.Op("*").Qual(proj.ModelsV1Package(), "OAuth2ClientList"),
			jen.ID("error")).Block(
			jen.ID("args").Assign().ID("m").Dot("Called").Call(utils.CtxVar(), jen.ID("userID"), jen.ID(utils.FilterVarName)),
			jen.Return().List(jen.ID("args").Dot("Get").Call(jen.Lit(0)).Assert(jen.Op("*").Qual(proj.ModelsV1Package(), "OAuth2ClientList")), jen.ID("args").Dot("Error").Call(jen.Lit(1))),
		),
		jen.Line(),
	)
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("m").Op("*").ID("OAuth2ClientDataManager")).ID("CreateOAuth2Client").Params(utils.CtxParam(), jen.ID("input").Op("*").Qual(proj.ModelsV1Package(), "OAuth2ClientCreationInput")).Params(jen.Op("*").Qual(proj.ModelsV1Package(), "OAuth2Client"),
			jen.ID("error")).Block(
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("m").Op("*").ID("OAuth2ClientDataManager")).ID("UpdateOAuth2Client").Params(utils.CtxParam(), jen.ID("updated").Op("*").Qual(proj.ModelsV1Package(), "OAuth2Client")).Params(jen.ID("error")).Block(
