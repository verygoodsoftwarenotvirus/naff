...
func databaseDotGo(pkg *models.Project) *jen.File {
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("cfg").Op("*").ID("ServerConfig")).ID("ProvideDatabase").Params(utils.CtxParam(), jen.ID("logger").Qual("gitlab.com/verygoodsoftwarenotvirus/logging/v1", "Logger")).Params(jen.Qual(pkg.DatabaseV1Package(), "Database"), jen.ID("error")).Block(
			jen.Var().Defs(
			...
			jen.Switch(jen.ID("cfg").Dot("Database").Dot("Provider")).Block(
				jen.Case(jen.ID("postgresProviderKey")).Block(
					...
					jen.Line(),
					jen.ID("pgdb").Assign().Qual(pkg.DatabaseV1Package("queriers", "postgres"), "ProvidePostgres").Call(jen.ID("debug"), jen.ID("rawDB"), jen.ID("logger")),
					jen.Line(),
					jen.Return().Qual(pkg.DatabaseV1Package("client"), "ProvideDatabaseClient").Call(utils.CtxVar(), jen.ID("rawDB"), jen.ID("pgdb"), jen.ID("debug"), jen.ID("logger"))),
				jen.Case(jen.ID("mariaDBProviderKey")).Block(
					...
					jen.Qual("contrib.go.opencensus.io/integrations/ocsql", "RecordStats").Call(jen.ID("rawDB"), jen.ID("cfg").Dot("Metrics").Dot("DBMetricsCollectionInterval")),
					jen.Line(),
					jen.ID("mdb").Assign().Qual(pkg.DatabaseV1Package("queriers", "mariadb"), "ProvideMariaDB").Call(jen.ID("debug"), jen.ID("rawDB"), jen.ID("logger")),
					jen.Line(),
					jen.Return().Qual(pkg.DatabaseV1Package("client"), "ProvideDatabaseClient").Call(utils.CtxVar(), jen.ID("rawDB"), jen.ID("mdb"), jen.ID("debug"), jen.ID("logger"))),
				jen.Case(jen.ID("sqliteProviderKey")).Block(
					...
					jen.Qual("contrib.go.opencensus.io/integrations/ocsql", "RecordStats").Call(jen.ID("rawDB"), jen.ID("cfg").Dot("Metrics").Dot("DBMetricsCollectionInterval")),
					jen.Line(),
					jen.ID("sdb").Assign().Qual(pkg.DatabaseV1Package("queriers", "sqlite"), "ProvideSqlite").Call(jen.ID("debug"), jen.ID("rawDB"), jen.ID("logger")),
					jen.Line(),
					jen.Return().Qual(pkg.DatabaseV1Package("client"), "ProvideDatabaseClient").Call(utils.CtxVar(), jen.ID("rawDB"), jen.ID("sdb"), jen.ID("debug"), jen.ID("logger"))),
