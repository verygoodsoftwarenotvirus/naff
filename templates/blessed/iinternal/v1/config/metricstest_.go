...
func metricsTestDotGo(pkgRoot string, types []models.DataType) *jen.File {
	...
	ret.Add(
		jen.Func().ID("TestServerConfig_ProvideInstrumentationHandler").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.Line(),
				jen.List(jen.ID("ih"), jen.ID("err")).Op(":=").ID("c").Dot("ProvideInstrumentationHandler").Call(jen.ID("noop").Dot("ProvideNoopLogger").Call()),
				jen.ID("assert").Dot("NoError").Call(jen.ID("t"), jen.ID("err")),
				jen.ID("assert").Dot("NotNil").Call(jen.ID("t"), jen.ID("ih")),
			)),
		),
		jen.Line(),
	)
	...
	ret.Add(
		jen.Func().ID("TestServerConfig_ProvideTracing").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.Line(),
				jen.ID("assert").Dot("NoError").Call(jen.ID("t"), jen.ID("c").Dot("ProvideTracing").Call(jen.ID("noop").Dot("ProvideNoopLogger").Call())),
			)),
		),
		jen.Line(),
	)
