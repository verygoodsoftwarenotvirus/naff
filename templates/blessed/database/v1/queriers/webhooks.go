	...
	code.Add(
		jen.Var().Defs(
			jen.ID("webhooksTableColumns").Equals().Index().String().Valuesln(
				utils.FormatString("%s.id", jen.ID("webhooksTableName")),
				utils.FormatString("%s.name", jen.ID("webhooksTableName")),
				utils.FormatString("%s.content_type", jen.ID("webhooksTableName")),
				utils.FormatString("%s.url", jen.ID("webhooksTableName")),
				utils.FormatString("%s.method", jen.ID("webhooksTableName")),
				utils.FormatString("%s.events", jen.ID("webhooksTableName")),
				utils.FormatString("%s.data_types", jen.ID("webhooksTableName")),
				utils.FormatString("%s.topics", jen.ID("webhooksTableName")),
				utils.FormatString("%s.created_on", jen.ID("webhooksTableName")),
				utils.FormatString("%s.updated_on", jen.ID("webhooksTableName")),
				utils.FormatString("%s.archived_on", jen.ID("webhooksTableName")),
	...
	lines := []jen.Code{
		...
		jen.Func().Params(jen.ID(dbfl).PointerTo().ID(sn)).ID("scanWebhooks").Params(
			...
			jen.For(jen.ID("rows").Dot("Next").Call()).Block(
				jen.List(jen.ID("webhook"), jen.ID("c"), jen.Err()).Assign().ID(dbfl).Dot("scanWebhook").Call(jen.ID("rows"), jen.True()),
...
func buildBuildGetWebhookQuery(proj *models.Project, dbvendor wordsmith.SuperPalabra) []jen.Code {
	...
	lines := []jen.Code{
		...
		jen.Line(),
		jen.Func().Params(jen.ID(dbfl).PointerTo().ID(sn)).ID("buildGetWebhookQuery").Params(jen.List(jen.ID("webhookID"), jen.ID(constants.UserIDVarName)).Uint64()).Params(jen.ID("query").String(), jen.ID("args").Index().Interface()).Block(
			jen.Var().Err().Error(),
			jen.Line(),
			jen.List(jen.ID("query"), jen.ID("args"), jen.Err()).Equals().ID(dbfl).Dot("sqlBuilder").
				...
				utils.FormatString("%s.id", jen.ID("webhooksTableName")).MapAssign().ID("webhookID"),
...
func build_GetWebhook(proj *models.Project, dbvendor wordsmith.SuperPalabra) []jen.Code {
	...
	lines := []jen.Code{
		...
		jen.Func().Params(jen.ID(dbfl).PointerTo().ID(sn)).ID("GetWebhook").Params(constants.CtxParam(), jen.List(jen.ID("webhookID"), jen.ID(constants.UserIDVarName)).Uint64()).Params(jen.PointerTo().Qual(proj.ModelsV1Package(), "Webhook"),
			...
			jen.Line(),
			jen.List(jen.ID("webhook"), jen.Underscore(), jen.Err()).Assign().ID(dbfl).Dot("scanWebhook").Call(jen.ID("row"), jen.False()),
			jen.If(jen.Err().DoesNotEqual().ID("nil")).Block(
		...
		jen.Func().Params(jen.ID(dbfl).PointerTo().ID(sn)).ID("buildGetAllWebhooksCountQuery").Params().Params(jen.String()).Block(
			jen.ID("getAllWebhooksCountQueryBuilder").Dot("Do").Call(jen.Func().Params().Block(
				...
				jen.List(jen.ID("getAllWebhooksCountQuery"), jen.Underscore(), jen.Err()).Equals().ID(dbfl).Dot("sqlBuilder").
					...
					Dotln("From").Call(jen.ID("webhooksTableName")).
					Dotln("Where").Call(jen.Qual("github.com/Masterminds/squirrel", "Eq").Valuesln(
					utils.FormatString("%s.archived_on", jen.ID("webhooksTableName")).MapAssign().ID("nil"))).
					Dotln("ToSql").Call(),
				jen.Line(),
				jen.ID(dbfl).Dot("logQueryBuildingError").Call(jen.Err()),
			)),
...
func build_GetAllWebhooksCount(proj *models.Project, dbvendor wordsmith.SuperPalabra) []jen.Code {
	...
	lines := []jen.Code{
		...
		jen.Line(),
		jen.Func().Params(jen.ID(dbfl).PointerTo().ID(sn)).ID("GetAllWebhooksCount").Params(constants.CtxParam()).Params(jen.ID("count").Uint64(), jen.Err().Error()).Block(
...
func build_GetAllWebhooks(proj *models.Project, dbvendor wordsmith.SuperPalabra) []jen.Code {
	...
	lines := []jen.Code{
		...
		jen.Line(),
		jen.Func().Params(jen.ID(dbfl).PointerTo().ID(sn)).ID("GetAllWebhooks").Params(constants.CtxParam()).Params(jen.PointerTo().Qual(proj.ModelsV1Package(), "WebhookList"), jen.Error()).Block(
			...
			jen.Line(),
			jen.List(jen.ID("list"), jen.ID("count"), jen.Err()).Assign().ID(dbfl).Dot("scanWebhooks").Call(jen.ID("rows")),
			jen.If(jen.Err().DoesNotEqual().ID("nil")).Block(
...
func build_GetAllWebhooksForUser(proj *models.Project, dbvendor wordsmith.SuperPalabra) []jen.Code {
	...
	lines := []jen.Code{
		...
		jen.Line(),
		jen.Func().Params(jen.ID(dbfl).PointerTo().ID(sn)).ID("GetAllWebhooksForUser").Params(constants.CtxParam(), constants.UserIDParam()).Params(jen.Index().Qual(proj.ModelsV1Package(), "Webhook"), jen.Error()).Block(
			jen.List(jen.ID("query"), jen.ID("args")).Assign().ID(dbfl).Dot("buildGetWebhooksQuery").Call(jen.ID(constants.UserIDVarName), jen.Nil()),
			jen.Line(),
	...
	lines := []jen.Code{
		...
		jen.Func().Params(jen.ID(dbfl).PointerTo().ID(sn)).ID("buildGetWebhooksQuery").Params(
			...
			jen.ID("builder").Assign().ID(dbfl).Dot("sqlBuilder").
				...
				utils.FormatStringWithArg(jen.ID("countQuery"), jen.ID("webhooksTableName")),
			).Spread()).
				Dotln("From").Call(jen.ID("webhooksTableName")).
				Dotln("Where").Call(jen.Qual("github.com/Masterminds/squirrel", "Eq").Valuesln(
				utils.FormatString("%s.%s",
				...
				Dotln("GroupBy").Call(utils.FormatString("%s.id", jen.ID("webhooksTableName"))),
			jen.Line(),
...
func buildCreateWebhook(proj *models.Project, dbvendor wordsmith.SuperPalabra) []jen.Code {
	...
	buildCreateWebhookQuery := func() []jen.Code {
		...
		if isPostgres(dbvendor) {
			out = append(out,
				jen.If(jen.Err().Assign().ID(dbfl).Dot("db").Dot("QueryRowContext").Call(constants.CtxVar(), jen.ID("query"), jen.ID("args").Spread()).Dot("Scan").Call(jen.AddressOf().ID("x").Dot("ID"), jen.AddressOf().ID("x").Dot("CreatedOn")), jen.Err().DoesNotEqual().ID("nil")).Block(
	...
	lines := []jen.Code{
		...
		jen.Line(),
		jen.Func().Params(jen.ID(dbfl).PointerTo().ID(sn)).ID("CreateWebhook").Params(constants.CtxParam(), jen.ID("input").PointerTo().Qual(proj.ModelsV1Package(), "WebhookCreationInput")).Params(jen.PointerTo().Qual(proj.ModelsV1Package(), "Webhook"), jen.Error()).Block(
...
func buildUpdateWebhook(proj *models.Project, dbvendor wordsmith.SuperPalabra) []jen.Code {
	...
	lines := []jen.Code{
		...
		jen.Line(),
		jen.Func().Params(jen.ID(dbfl).PointerTo().ID(sn)).ID("UpdateWebhook").Params(constants.CtxParam(), jen.ID("input").PointerTo().Qual(proj.ModelsV1Package(), "Webhook")).Params(jen.Error()).Block(
...
func buildBuildArchiveWebhookQuery(proj *models.Project, dbvendor wordsmith.SuperPalabra) []jen.Code {
	...
	lines := []jen.Code{
		...
		jen.Line(),
		jen.Func().Params(jen.ID(dbfl).PointerTo().ID(sn)).ID("buildArchiveWebhookQuery").Params(jen.List(jen.ID("webhookID"), jen.ID(constants.UserIDVarName)).Uint64()).Params(jen.ID("query").String(), jen.ID("args").Index().Interface()).Block(
			jen.Var().Err().Error(),
			jen.Line(),
...
func buildArchiveWebhook(proj *models.Project, dbvendor wordsmith.SuperPalabra) []jen.Code {
	...
	lines := []jen.Code{
		...
		jen.Line(),
		jen.Func().Params(jen.ID(dbfl).PointerTo().ID(sn)).ID("ArchiveWebhook").Params(constants.CtxParam(), jen.List(jen.ID("webhookID"), jen.ID(constants.UserIDVarName)).Uint64()).Params(jen.Error()).Block(
