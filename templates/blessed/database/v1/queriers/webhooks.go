...
func build_GetAllWebhooks(proj *models.Project, dbvendor wordsmith.SuperPalabra) []jen.Code {
	...
	lines := []jen.Code{
		...
		jen.Func().Params(jen.ID(dbfl).PointerTo().ID(sn)).ID("GetAllWebhooks").Params(constants.CtxParam()).Params(jen.PointerTo().Qual(proj.ModelsV1Package(), "WebhookList"), jen.Error()).Block(
			...
			jen.Line(),
			jen.List(jen.ID("list"), jen.ID("count"), jen.Err()).Assign().ID("scanWebhooks").Call(jen.ID(dbfl).Dot(constants.LoggerVarName), jen.ID("rows")),
			jen.If(jen.Err().DoesNotEqual().ID("nil")).Block(
...
func build_GetAllWebhooksForUser(proj *models.Project, dbvendor wordsmith.SuperPalabra) []jen.Code {
	...
	lines := []jen.Code{
		...
		jen.Func().Params(jen.ID(dbfl).PointerTo().ID(sn)).ID("GetAllWebhooksForUser").Params(constants.CtxParam(), jen.ID("userID").Uint64()).Params(jen.Index().Qual(proj.ModelsV1Package(), "Webhook"), jen.Error()).Block(
			...
			jen.Line(),
			jen.List(jen.ID("list"), jen.Err()).Assign().ID("scanWebhooks").Call(jen.ID(dbfl).Dot(constants.LoggerVarName), jen.ID("rows")),
			jen.If(jen.Err().DoesNotEqual().ID("nil")).Block(
				jen.Return().List(jen.Nil(), jen.Qual("fmt", "Errorf").Call(jen.Lit("scanning response from database: %w"), jen.Err())),
			),
			jen.Line(),
