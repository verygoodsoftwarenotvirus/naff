		...
		jen.Func().ID("TestClient_GetOAuth2ClientByClientID").Params(jen.ID("T").PointerTo().Qual("testing", "T")).Block(
			...
			utils.BuildSubTest(
				...
				jen.List(jen.ID("actual"), jen.Err()).Assign().ID("c").Dot("GetOAuth2ClientByClientID").Call(
					utils.CtxVar(), jen.ID(utils.BuildFakeVarName("OAuth2Client")).Dot("ClientID")),
				utils.AssertNoError(jen.Err(), nil),
				utils.AssertEqual(jen.ID(utils.BuildFakeVarName("OAuth2Client")), jen.ID("actual"), nil),
				jen.Line(),
				utils.AssertExpectationsFor("mockDB"),
		...
		jen.Func().ID("TestClient_GetOAuth2Clients").Params(jen.ID("T").PointerTo().Qual("testing", "T")).Block(
			...
			utils.BuildSubTest(
				...
				jen.ID("mockDB").Dot("OAuth2ClientDataManager").Dot("On").Call(
					...
					jen.ID(utils.BuildFakeVarName("User")).Dot("ID"),
					jen.ID(utils.FilterVarName),
				).Dot("Return").Call(jen.ID(utils.BuildFakeVarName("OAuth2ClientList")), utils.ObligatoryError()),
				jen.Line(),
				jen.List(jen.ID("actual"), jen.Err()).Assign().ID("c").Dot("GetOAuth2Clients").Call(
					utils.CtxVar(),
					jen.ID(utils.BuildFakeVarName("User")).Dot("ID"),
	...
	lines := []jen.Code{
		jen.Func().ID("TestClient_CreateOAuth2Client").Params(jen.ID("T").PointerTo().Qual("testing", "T")).Block(
			...
			utils.BuildSubTest(
				...
				jen.Line(),
				jen.List(jen.ID("actual"), jen.Err()).Assign().ID("c").Dot("CreateOAuth2Client").Call(utils.CtxVar(), jen.ID(utils.BuildFakeVarName("Input"))),
				utils.AssertNoError(jen.Err(), nil),
				utils.AssertEqual(jen.ID(utils.BuildFakeVarName("OAuth2Client")), jen.ID("actual"), nil),
				jen.Line(),
				utils.AssertExpectationsFor("mockDB"),
	...
	lines := []jen.Code{
		jen.Func().ID("TestClient_UpdateOAuth2Client").Params(jen.ID("T").PointerTo().Qual("testing", "T")).Block(
			...
			utils.BuildSubTest(
				...
				jen.Line(),
				jen.ID("actual").Assign().ID("c").Dot("UpdateOAuth2Client").Call(utils.CtxVar(), jen.ID(utils.BuildFakeVarName("OAuth2Client"))),
