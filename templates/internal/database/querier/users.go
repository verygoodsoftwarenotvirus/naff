...
func buildVarDeclarations(proj *models.Project) []jen.Code {
	lines := []jen.Code{
		jen.Var().Defs(
			jen.Underscore().Qual(proj.TypesPackage(), "UserDataManager").Equals().Parens(jen.PointerTo().ID("Client")).Call(jen.Nil()),
			jen.Line(),
...
func buildGetUser(proj *models.Project) []jen.Code {
	lines := []jen.Code{
		...
		jen.Line(),
		jen.Func().Params(jen.ID("c").PointerTo().ID("Client")).ID("GetUser").Params(constants.CtxParam(), constants.UserIDParam()).Params(jen.PointerTo().Qual(proj.TypesPackage(),
			"User",
		),
			jen.Error()).Body(
			jen.List(constants.CtxVar(), jen.ID(constants.SpanVarName)).Assign().Qual(proj.InternalTracingPackage(), "StartSpan").Call(constants.CtxVar(), jen.Lit("GetUser")),
...
func buildGetUserWithUnverifiedTwoFactorSecret(proj *models.Project) []jen.Code {
	lines := []jen.Code{
		...
		jen.Line(),
		jen.Func().Params(jen.ID("c").PointerTo().ID("Client")).ID("GetUserWithUnverifiedTwoFactorSecret").Params(constants.CtxParam(), constants.UserIDParam()).Params(jen.PointerTo().Qual(proj.TypesPackage(),
			"User",
		),
			jen.Error()).Body(
			jen.List(constants.CtxVar(), jen.ID(constants.SpanVarName)).Assign().Qual(proj.InternalTracingPackage(), "StartSpan").Call(constants.CtxVar(), jen.Lit("GetUserWithUnverifiedTwoFactorSecret")),
			jen.Defer().ID(constants.SpanVarName).Dot("End").Call(),
			jen.Line(),
			jen.Qual(proj.InternalTracingPackage(), "AttachUserIDToSpan").Call(jen.ID(constants.SpanVarName), jen.ID(constants.UserIDVarName)),
...
func buildVerifyUserTwoFactorSecret(proj *models.Project) []jen.Code {
	lines := []jen.Code{
		...
		jen.Func().Params(jen.ID("c").PointerTo().ID("Client")).ID("VerifyUserTwoFactorSecret").Params(constants.CtxParam(), constants.UserIDParam()).Params(jen.Error()).Body(
			jen.List(constants.CtxVar(), jen.ID(constants.SpanVarName)).Assign().Qual(proj.InternalTracingPackage(), "StartSpan").Call(constants.CtxVar(), jen.Lit("VerifyUserTwoFactorSecret")),
			jen.Defer().ID(constants.SpanVarName).Dot("End").Call(),
			jen.Line(),
			jen.Qual(proj.InternalTracingPackage(), "AttachUserIDToSpan").Call(jen.ID(constants.SpanVarName), jen.ID(constants.UserIDVarName)),
...
func buildGetUserByUsername(proj *models.Project) []jen.Code {
	lines := []jen.Code{
		...
		jen.Line(),
		jen.Func().Params(jen.ID("c").PointerTo().ID("Client")).ID("GetUserByUsername").Params(constants.CtxParam(), jen.ID("username").String()).Params(jen.PointerTo().Qual(proj.TypesPackage(), "User"), jen.Error()).Body(
			jen.List(constants.CtxVar(), jen.ID(constants.SpanVarName)).Assign().Qual(proj.InternalTracingPackage(), "StartSpan").Call(constants.CtxVar(), jen.Lit("GetUserByUsername")),
			jen.Defer().ID(constants.SpanVarName).Dot("End").Call(),
			jen.Line(),
...
func buildGetAllUsersCount(proj *models.Project) []jen.Code {
	lines := []jen.Code{
		...
		jen.Func().Params(jen.ID("c").PointerTo().ID("Client")).ID("GetAllUsersCount").Params(constants.CtxParam()).Params(jen.ID("count").Uint64(), jen.Err().Error()).Body(
			jen.List(constants.CtxVar(), jen.ID(constants.SpanVarName)).Assign().Qual(proj.InternalTracingPackage(), "StartSpan").Call(constants.CtxVar(), jen.Lit("GetAllUsersCount")),
			jen.Defer().ID(constants.SpanVarName).Dot("End").Call(),
			jen.Line(),
...
func buildGetUsers(proj *models.Project) []jen.Code {
	lines := []jen.Code{
		...
		jen.Line(),
		jen.Func().Params(jen.ID("c").PointerTo().ID("Client")).ID("GetUsers").Params(constants.CtxParam(), jen.ID(constants.FilterVarName).PointerTo().Qual(proj.TypesPackage(), "QueryFilter")).Params(jen.PointerTo().Qual(proj.TypesPackage(), "UserList"), jen.Error()).Body(
			jen.List(constants.CtxVar(), jen.ID(constants.SpanVarName)).Assign().Qual(proj.InternalTracingPackage(), "StartSpan").Call(constants.CtxVar(), jen.Lit("GetUsers")),
			jen.Defer().ID(constants.SpanVarName).Dot("End").Call(),
			jen.Line(),
...
func buildCreateUser(proj *models.Project) []jen.Code {
	lines := []jen.Code{
		...
		jen.Func().Params(jen.ID("c").PointerTo().ID("Client")).ID("CreateUser").Params(
			...
			jen.List(constants.CtxVar(), jen.ID(constants.SpanVarName)).Assign().Qual(proj.InternalTracingPackage(), "StartSpan").Call(constants.CtxVar(), jen.Lit("CreateUser")),
			jen.Defer().ID(constants.SpanVarName).Dot("End").Call(),
			jen.Line(),
			jen.Qual(proj.InternalTracingPackage(), "AttachUsernameToSpan").Call(jen.ID(constants.SpanVarName), jen.ID("input").Dot("Username")),
...
func buildUpdateUser(proj *models.Project) []jen.Code {
	lines := []jen.Code{
		...
		jen.Line(),
		jen.Func().Params(jen.ID("c").PointerTo().ID("Client")).ID("UpdateUser").Params(constants.CtxParam(), jen.ID("updated").PointerTo().Qual(proj.TypesPackage(), "User")).Params(jen.Error()).Body(
			jen.List(constants.CtxVar(), jen.ID(constants.SpanVarName)).Assign().Qual(proj.InternalTracingPackage(), "StartSpan").Call(constants.CtxVar(), jen.Lit("UpdateUser")),
			jen.Defer().ID(constants.SpanVarName).Dot("End").Call(),
			jen.Line(),
			jen.Qual(proj.InternalTracingPackage(), "AttachUsernameToSpan").Call(jen.ID(constants.SpanVarName), jen.ID("updated").Dot("Username")),
...
func buildUpdateUserPassword(proj *models.Project) []jen.Code {
	lines := []jen.Code{
		...
		jen.Func().Params(jen.ID("c").PointerTo().ID("Client")).ID("UpdateUserPassword").Params(
			...
			jen.List(constants.CtxVar(), jen.ID(constants.SpanVarName)).Assign().Qual(proj.InternalTracingPackage(), "StartSpan").Call(constants.CtxVar(), jen.Lit("UpdateUserPassword")),
			jen.Defer().ID(constants.SpanVarName).Dot("End").Call(),
			jen.Line(),
			jen.Qual(proj.InternalTracingPackage(), "AttachUserIDToSpan").Call(jen.ID(constants.SpanVarName), jen.ID("userID")),
...
func buildArchiveUser(proj *models.Project) []jen.Code {
	lines := []jen.Code{
		...
		jen.Func().Params(jen.ID("c").PointerTo().ID("Client")).ID("ArchiveUser").Params(constants.CtxParam(), constants.UserIDParam()).Params(jen.Error()).Body(
			jen.List(constants.CtxVar(), jen.ID(constants.SpanVarName)).Assign().Qual(proj.InternalTracingPackage(), "StartSpan").Call(constants.CtxVar(), jen.Lit("ArchiveUser")),
			jen.Defer().ID(constants.SpanVarName).Dot("End").Call(),
			jen.Line(),
			jen.Qual(proj.InternalTracingPackage(), "AttachUserIDToSpan").Call(jen.ID(constants.SpanVarName), jen.ID(constants.UserIDVarName)),
