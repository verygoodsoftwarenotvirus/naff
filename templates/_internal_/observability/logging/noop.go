...
func noopDotGo(proj *models.Project) *jen.File {
	code := jen.NewFile(packageName)

	utils.AddImports(proj, code, false)

	code.Add(
		jen.Type().ID("noopLogger").Struct(),
		jen.Line(),
	)

	code.Add(
		jen.Var().ID("logger").Op("=").ID("new").Call(jen.ID("noopLogger")),
		jen.Line(),
	)

	code.Add(
		jen.Comment("NewNonOperationalLogger provides our noop logger to dependency managers."),
		jen.Line(),
		jen.Func().ID("NewNonOperationalLogger").Params().Params(jen.ID("Logger")).Body(
