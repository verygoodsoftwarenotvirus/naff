...
func configDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("c").Op("*").ID("Config")).ID("Initialize").Params(jen.ID("l").ID("logging").Dot("Logger")).Params(jen.ID("flushFunc").Func().Params(), jen.ID("err").ID("error")).Body(
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("_").Qual("github.com/go-ozzo/ozzo-validation/v4", "ValidatableWithContext").Op("=").Parens(jen.Op("*").ID("JaegerConfig")).Call(jen.ID("nil")),
		),
		jen.Line(),
	)
