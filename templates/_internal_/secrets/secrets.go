...
func secretsDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		...
		jen.Line(),
		jen.Comment("\t\t1. JSON encodes a given value"),
		jen.Line(),
		jen.Comment("\t\t2. encrypts that encoded data"),
		jen.Line(),
		jen.Comment("\t\t3. base64 URL encodes that encrypted data"),
		jen.Line(),
		jen.Func().Params(jen.ID("sm").Op("*").ID("secretManager")).ID("Encrypt").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("value").Interface()).Params(jen.ID("string"), jen.ID("error")).Body(
	...
	code.Add(
		...
		jen.Line(),
		jen.Comment("\t\t1. base64 URL decodes the provided data"),
		jen.Line(),
		jen.Comment("\t\t2. decrypts that encoded data"),
		jen.Line(),
		jen.Comment("\t\t3. JSON decodes that decrypted data into the target variable."),
		jen.Line(),
		jen.Func().Params(jen.ID("sm").Op("*").ID("secretManager")).ID("Decrypt").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("content").ID("string"), jen.ID("v").Interface()).Params(jen.ID("error")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("sm").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
