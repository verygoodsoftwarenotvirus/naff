		...
		jen.Func().ID("buildJPEGBytes").Params(jen.ID("t").Op("*").Qual("testing", "T")).Params(jen.Op("*").Qual("bytes", "Buffer")).Body(
			...
			jen.ID("require").Dot("NoError").Call(
				...
				jen.Qual("image/jpeg", "Encode").Call(
					jen.ID("b"),
					jen.ID("exampleImage"),
					jen.Op("&").Qual("image/jpeg", "Options").Valuesln(
						jen.ID("Quality").Op(":").Qual("image/jpeg", "DefaultQuality")),
				),
			),
			jen.ID("expected").Op(":=").ID("b").Dot("Bytes").Call(),
			jen.Return().Qual("bytes", "NewBuffer").Call(jen.ID("expected")),
		...
		jen.Func().ID("buildGIFBytes").Params(jen.ID("t").Op("*").Qual("testing", "T")).Params(jen.Op("*").Qual("bytes", "Buffer")).Body(
			...
			jen.ID("require").Dot("NoError").Call(
				...
				jen.Qual("image/gif", "Encode").Call(
					jen.ID("b"),
					jen.ID("exampleImage"),
					jen.Op("&").Qual("image/gif", "Options").Valuesln(
						jen.ID("NumColors").Op(":").Lit(256)),
				),
			),
			jen.ID("expected").Op(":=").ID("b").Dot("Bytes").Call(),
			jen.Return().Qual("bytes", "NewBuffer").Call(jen.ID("expected")),
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("i").Op(":=").Op("&").ID("Image").Valuesln(
						jen.ID("Filename").Op(":").ID("t").Dot("Name").Call(), jen.ID("ContentType").Op(":").Lit("things/stuff"), jen.ID("Data").Op(":").Index().ID("byte").Call(jen.ID("t").Dot("Name").Call()), jen.ID("Size").Op(":").Lit(12345)),
		...
		jen.Func().ID("TestImage_Thumbnail").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("i").Op(":=").Op("&").ID("Image").Valuesln(
						jen.ID("Filename").Op(":").ID("t").Dot("Name").Call(), jen.ID("ContentType").Op(":").ID("imagePNG"), jen.ID("Data").Op(":").ID("imgBytes"), jen.ID("Size").Op(":").ID("len").Call(jen.ID("imgBytes"))),
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					jen.ID("t").Dot("Parallel").Call(),
					jen.ID("i").Op(":=").Op("&").ID("Image").Valuesln(
						jen.ID("ContentType").Op(":").ID("t").Dot("Name").Call()),
					jen.List(jen.ID("actual"), jen.ID("err")).Op(":=").ID("i").Dot("Thumbnail").Call(
						jen.Lit(123),
						jen.Lit(123),
