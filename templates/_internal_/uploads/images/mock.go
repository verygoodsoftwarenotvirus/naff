...
func mockDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("_").ID("ImageUploadProcessor").Op("=").Parens(jen.Op("*").ID("MockImageUploadProcessor")).Call(jen.ID("nil")),
		jen.Line(),
	)

	code.Add(
		jen.Type().ID("MockImageUploadProcessor").Struct(jen.ID("mock").Dot("Mock")),
		jen.Line(),
	)
