		...
		jen.Func().ID("Test_newThumbnailer").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					jen.ID("t").Dot("Parallel").Call(),
					jen.For(jen.List(jen.ID("_"), jen.ID("ct")).Op(":=").Range().Index().ID("string").Valuesln(
						jen.ID("imagePNG"), jen.ID("imageJPEG"), jen.ID("imageGIF"))).Body(
		...
		jen.Func().ID("Test_preprocess").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("i").Op(":=").Op("&").ID("Image").Valuesln(
						jen.ID("Filename").Op(":").Lit("whatever.png"), jen.ID("ContentType").Op(":").ID("imagePNG"), jen.ID("Data").Op(":").ID("imgBytes"), jen.ID("Size").Op(":").ID("len").Call(jen.ID("imgBytes"))),
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					jen.ID("t").Dot("Parallel").Call(),
					jen.ID("i").Op(":=").Op("&").ID("Image").Valuesln(
						jen.ID("Filename").Op(":").Lit("whatever.png"), jen.ID("ContentType").Op(":").ID("imagePNG"), jen.ID("Data").Op(":").Index().ID("byte").Call(jen.ID("t").Dot("Name").Call()), jen.ID("Size").Op(":").Lit(1024)),
					jen.List(jen.ID("img"), jen.ID("err")).Op(":=").ID("preprocess").Call(
						jen.ID("i"),
						jen.Lit(128),
						jen.Lit(128),
		...
		jen.Func().ID("Test_jpegThumbnailer_Thumbnail").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.List(jen.ID("actual"), jen.ID("err")).Op(":=").Parens(jen.Op("&").ID("jpegThumbnailer").Values()).Dot("Thumbnail").Call(
						jen.ID("i"),
						jen.Lit(128),
						jen.Lit(128),
		...
		jen.Func().ID("Test_pngThumbnailer_Thumbnail").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.List(jen.ID("actual"), jen.ID("err")).Op(":=").Parens(jen.Op("&").ID("pngThumbnailer").Values()).Dot("Thumbnail").Call(
						jen.ID("i"),
						jen.Lit(128),
						jen.Lit(128),
						jen.ID("tempFile").Dot("Name").Call(),
		...
		jen.Func().ID("Test_gifThumbnailer_Thumbnail").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.List(jen.ID("actual"), jen.ID("err")).Op(":=").Parens(jen.Op("&").ID("gifThumbnailer").Values()).Dot("Thumbnail").Call(
						jen.ID("i"),
						jen.Lit(128),
						jen.Lit(128),
						jen.ID("tempFile").Dot("Name").Call(),
