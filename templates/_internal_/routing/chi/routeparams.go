...
func routeparamsDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("r").ID("chiRouteParamManager")).ID("BuildRouteParamIDFetcher").Params(
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("r").ID("chiRouteParamManager")).ID("BuildRouteParamStringIDFetcher").Params(jen.ID("key").ID("string")).Params(jen.Func().Params(jen.ID("req").Op("*").Qual("net/http", "Request")).ID("string")).Body(
