...
func routeParamManagerDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("m").Op("*").ID("RouteParamManager")).ID("BuildRouteParamIDFetcher").Params(jen.ID("logger").ID("logging").Dot("Logger"), jen.List(jen.ID("key"), jen.ID("logDescription")).ID("string")).Params(jen.Op("*").Qual("net/http", "Request"), jen.ID("uint64")).Body(
			...
			).Dot("Get").Call(jen.Lit(0)).Assert(jen.Op("*").Qual("net/http", "Request"))),
		jen.Line(),
	)
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("m").Op("*").ID("RouteParamManager")).ID("BuildRouteParamStringIDFetcher").Params(jen.ID("key").ID("string")).Params(jen.Op("*").Qual("net/http", "Request"), jen.ID("string")).Body(
			jen.Return().ID("m").Dot("Called").Call(jen.ID("key")).Dot("Get").Call(jen.Lit(0)).Assert(jen.Op("*").Qual("net/http", "Request"))),
		jen.Line(),
	)
