...
func buildWebhooksServiceConstDefs(proj *models.Project) []jen.Code {
	lines := []jen.Code{
		jen.Const().Defs(
			...
			jen.Line(),
			jen.ID("counterName").Qual(proj.MetricsPackage(), "CounterName").Equals().Lit("webhooks"),
	...
	lines := []jen.Code{
		jen.Type().Defs(
			...
			jen.ID("Service").Struct(
				proj.LoggerParam(),
				jen.ID("webhookCounter").Qual(proj.MetricsPackage(), "UnitCounter"),
...
func buildProvideWebhooksService(proj *models.Project) []jen.Code {
	lines := []jen.Code{
		...
		jen.Func().ID("ProvideWebhooksService").Paramsln(
			...
			jen.ID("webhookCounterProvider").Qual(proj.MetricsPackage(), "UnitCounterProvider"),
