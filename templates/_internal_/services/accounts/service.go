...
func serviceDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().ID("ProvideService").Params(
			jen.ID("logger").ID("logging").Dot("Logger"),
			jen.ID("accountDataManager").ID("types").Dot("AccountDataManager"),
			jen.ID("accountMembershipDataManager").ID("types").Dot("AccountUserMembershipDataManager"),
			jen.ID("encoder").ID("encoding").Dot("ServerEncoderDecoder"),
			jen.ID("counterProvider").ID("metrics").Dot("UnitCounterProvider"),
			...
			jen.Return().Op("&").ID("service").Valuesln(
				...
				jen.ID("sessionContextDataFetcher").Op(":").ID("authentication").Dot("FetchContextFromRequest"),
				jen.ID("accountDataManager").Op(":").ID("accountDataManager"),
				jen.ID("accountMembershipDataManager").Op(":").ID("accountMembershipDataManager"),
				jen.ID("encoderDecoder").Op(":").ID("encoder"),
				jen.ID("accountCounter").Op(":").ID("metrics").Dot("EnsureUnitCounter").Call(
