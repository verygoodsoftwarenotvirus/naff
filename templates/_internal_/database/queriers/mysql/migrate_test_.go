			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("accountCreationArgs").Op(":=").Index().Interface().Valuesln(
						jen.Op("&").ID("idMatcher").Values(), jen.ID("accountCreationInput").Dot("Name"), jen.ID("types").Dot("UnpaidAccountBillingStatus"), jen.ID("accountCreationInput").Dot("ContactEmail"), jen.ID("accountCreationInput").Dot("ContactPhone"), jen.Lit(""), jen.Op("&").ID("idMatcher").Values()),
