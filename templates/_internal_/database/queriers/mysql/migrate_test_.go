		...
		jen.Func().ID("TestQuerier_Migrate").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					jen.ID("t").Dot("Parallel").Call(),
					jen.Newline(),
					jen.ID("exampleCreationTime").Assign().Qual(proj.FakeTypesPackage(), "BuildFakeTime").Call(),
					jen.Newline(),
					...
					jen.Newline(),
					jen.ID("exampleAccount").Assign().Qual(proj.FakeTypesPackage(), "BuildFakeAccountForUser").Call(jen.ID("exampleUser")),
					jen.Newline(),
					jen.ID("exampleTestUserConfig").Assign().Op("&").Qual(proj.TypesPackage(), "TestUserCreationConfig").Valuesln(
					...
					jen.ID("testUserExistenceArgs").Assign().Index().Interface().Values(jen.ID("exampleTestUserConfig").Dot("Username")),
					...
					jen.ID("testUserCreationArgs").Assign().Index().Interface().Valuesln(
						jen.Op("&").ID("idMatcher").Values(), jen.ID("exampleTestUserConfig").Dot("Username"), jen.ID("exampleTestUserConfig").Dot("HashedPassword"), jen.ID("defaultTestUserTwoFactorSecret"), jen.Lit(""), jen.Qual(proj.TypesPackage(), "GoodStandingAccountStatus"), jen.Lit(""), jen.ID("authorization").Dot("ServiceAdminRole").Dot("String").Call()),
					jen.Newline(),
					...
					jen.ID("accountCreationInput").Assign().Qual(proj.TypesPackage(), "AccountCreationInputForNewUser").Call(jen.ID("exampleUser")),
					jen.ID("accountCreationArgs").Assign().Index().Interface().Valuesln(
					...
					jen.ID("createAccountMembershipForNewUserArgs").Assign().Index().Interface().Valuesln(
