		...
		jen.Func().ID("TestQuerier_Migrate").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("exampleTestUserConfig").Op(":=").Op("&").ID("types").Dot("TestUserCreationConfig").Valuesln(jen.ID("Username").MapAssign().ID("exampleUser").Dot("Username"), jen.ID("Password").MapAssign().ID("exampleUser").Dot("HashedPassword"), jen.ID("HashedPassword").MapAssign().ID("exampleUser").Dot("HashedPassword"), jen.ID("IsServiceAdmin").MapAssign().ID("true")),
					...
					jen.ID("testUserExistenceArgs").Op(":=").Index().Interface().Valuesln(jen.ID("exampleTestUserConfig").Dot("Username")),
					...
					jen.ID("testUserCreationArgs").Op(":=").Index().Interface().Valuesln(jen.Op("&").ID("idMatcher").Values(), jen.ID("exampleTestUserConfig").Dot("Username"), jen.ID("exampleTestUserConfig").Dot("HashedPassword"), jen.ID("defaultTestUserTwoFactorSecret"), jen.Lit(""), jen.ID("types").Dot("GoodStandingAccountStatus"), jen.Lit(""), jen.ID("authorization").Dot("ServiceAdminRole").Dot("String").Call()),
					...
					jen.ID("accountCreationArgs").Op(":=").Index().Interface().Valuesln(jen.Op("&").ID("idMatcher").Values(), jen.ID("accountCreationInput").Dot("Name"), jen.ID("types").Dot("UnpaidAccountBillingStatus"), jen.ID("accountCreationInput").Dot("ContactEmail"), jen.ID("accountCreationInput").Dot("ContactPhone"), jen.Lit(""), jen.Op("&").ID("idMatcher").Values()),
					...
					jen.ID("createAccountMembershipForNewUserArgs").Op(":=").Index().Interface().Valuesln(jen.Op("&").ID("idMatcher").Values(), jen.Op("&").ID("idMatcher").Values(), jen.Op("&").ID("idMatcher").Values(), jen.ID("true"), jen.ID("authorization").Dot("AccountAdminRole").Dot("String").Call()),
