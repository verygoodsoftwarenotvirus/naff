			...
			jen.If(jen.ID("testUserConfig").DoesNotEqual().Nil()).Body(
				...
				jen.If(jen.ID("err").DoesNotEqual().Nil()).Body(
					...
					jen.ID("testUserCreationArgs").Op(":=").Index().Interface().Valuesln(
						jen.ID("testUserConfig").Dot("ID"), jen.ID("testUserConfig").Dot("Username"), jen.ID("testUserConfig").Dot("HashedPassword"), jen.ID("defaultTestUserTwoFactorSecret"), jen.Lit(""), jen.ID("types").Dot("GoodStandingAccountStatus"), jen.Lit(""), jen.ID("authorization").Dot("ServiceAdminRole").Dot("String").Call()),
