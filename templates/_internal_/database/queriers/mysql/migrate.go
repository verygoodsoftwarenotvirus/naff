...
func migrateDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("driverName").Op("=").Lit("instrumented-mysql"),
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("testUserExistenceQuery").Op("=").Lit(`
...
`),
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("testUserCreationQuery").Op("=").Lit(`
			...
			jen.If(jen.ID("testUserConfig").Op("!=").ID("nil")).Body(
				...
				jen.If(jen.ID("err").Op("!=").ID("nil")).Body(
					jen.If(jen.ID("testUserConfig").Dot("ID").Op("==").Lit("")).Body(
						jen.ID("testUserConfig").Dot("ID").Op("=").ID("ksuid").Dot("New").Call().Dot("String").Call()),
					...
					jen.ID("user").Op(":=").Op("&").ID("types").Dot("User").Valuesln(jen.ID("ID").Op(":").ID("testUserConfig").Dot("ID"), jen.ID("Username").Op(":").ID("testUserConfig").Dot("Username")),
					jen.ID("account").Op(":=").Op("&").ID("types").Dot("Account").Valuesln(jen.ID("ID").Op(":").ID("ksuid").Dot("New").Call().Dot("String").Call()),
