...
func itemsDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("itemExistenceQuery").Equals().Lit("SELECT EXISTS ( SELECT items.id FROM items WHERE items.archived_on IS NULL AND items.belongs_to_account = $1 AND items.id = $2 )"),
		),
		jen.Newline(),
	)
