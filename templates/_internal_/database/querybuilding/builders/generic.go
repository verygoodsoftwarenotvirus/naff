...
func genericDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Func().ID("joinUint64s").Params(jen.ID("in").Index().ID("uint64")).Params(jen.ID("string")).Body(
			jen.ID("out").Op(":=").Index().ID("string").Values(),
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("b").Op("*").ID("Postgres")).ID("buildQueryOnly").Params(jen.ID("span").ID("tracing").Dot("Span"), jen.ID("builder").ID("squirrel").Dot("Sqlizer")).Params(jen.ID("string")).Body(
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("b").Op("*").ID("Postgres")).ID("buildQuery").Params(jen.ID("span").ID("tracing").Dot("Span"), jen.ID("builder").ID("squirrel").Dot("Sqlizer")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Body(
	...
	code.Add(
		jen.Func().Params(jen.ID("b").Op("*").ID("Postgres")).ID("buildTotalCountQuery").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("tableName"), jen.ID("ownershipColumn")).ID("string"), jen.ID("userID").ID("uint64"), jen.List(jen.ID("forAdmin"), jen.ID("includeArchived")).ID("bool")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Body(
			jen.List(jen.ID("_"), jen.ID("span")).Op(":=").ID("b").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.ID("where").Op(":=").ID("squirrel").Dot("Eq").Values(),
			jen.ID("totalCountQueryBuilder").Op(":=").ID("b").Dot("sqlBuilder").Dot("PlaceholderFormat").Call(jen.ID("squirrel").Dot("Question")).Dot("Select").Call(jen.Qual("fmt", "Sprintf").Call(
	...
	code.Add(
		jen.Func().Params(jen.ID("b").Op("*").ID("Postgres")).ID("buildFilteredCountQuery").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("tableName"), jen.ID("ownershipColumn")).ID("string"), jen.ID("userID").ID("uint64"), jen.List(jen.ID("forAdmin"), jen.ID("includeArchived")).ID("bool"), jen.ID("filter").Op("*").ID("types").Dot("QueryFilter")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Body(
			jen.List(jen.ID("_"), jen.ID("span")).Op(":=").ID("b").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			...
			jen.ID("where").Op(":=").ID("squirrel").Dot("Eq").Values(),
			jen.ID("filteredCountQueryBuilder").Op(":=").ID("b").Dot("sqlBuilder").Dot("PlaceholderFormat").Call(jen.ID("squirrel").Dot("Question")).Dot("Select").Call(jen.Qual("fmt", "Sprintf").Call(
				jen.ID("columnCountQueryTemplate"),
				jen.ID("tableName"),
			)).Dot("From").Call(jen.ID("tableName")),
			jen.If(jen.Op("!").ID("forAdmin")).Body(
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("b").Op("*").ID("Postgres")).ID("buildListQuery").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("tableName"), jen.ID("ownershipColumn")).ID("string"), jen.ID("columns").Index().ID("string"), jen.ID("userID").ID("uint64"), jen.ID("forAdmin").ID("bool"), jen.ID("filter").Op("*").ID("types").Dot("QueryFilter")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Body(
			...
			jen.If(jen.Op("!").ID("forAdmin")).Body(
				...
				jen.If(jen.ID("ownershipColumn").Op("!=").Lit("").Op("&&").ID("userID").Op("!=").Lit(0)).Body(
