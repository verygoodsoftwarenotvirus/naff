...
func genericTestDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Func().ID("TestPostgres_BuildListQuery").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			jen.ID("T").Dot("Parallel").Call(),
			jen.Var().Defs(
				jen.ID("exampleTableName").Op("=").Lit("example_table"),
				jen.ID("exampleOwnershipColumn").Op("=").Lit("belongs_to_account"),
			),
