	...
	return []jen.Code{
		...
		jen.Func().Params(jen.ID("b").PointerTo().ID(dbvsn)).ID("BuildMigrationFunc").Params(jen.ID("db").PointerTo().Qual("database/sql", "DB")).Params(jen.Func().Params()).Body(
			jen.Return().Func().Params().Body(
				jen.ID("d").Assign().Qual("github.com/GuiaBolso/darwin", "NewGenericDriver").Call(jen.ID("db"), jen.Qual("github.com/GuiaBolso/darwin", dialectName).Values()),
				jen.If(jen.Err().Assign().Qual("github.com/GuiaBolso/darwin", "Migrate").Call(jen.ID("d"), jen.ID("migrations"), jen.Nil()), jen.Err().DoesNotEqual().ID("nil")).Body(
