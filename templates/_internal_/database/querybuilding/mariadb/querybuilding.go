...
func buildListQuery(tableName, ownershipColumn string, columns []string, userID uint64, forAdmin bool) (query string, args []interface{}) {
	sqlBuilder := squirrel.StatementBuilder.PlaceholderFormat(squirrel.Question)
	...
	if !forAdmin {
		w := squirrel.Eq{fmt.Sprintf("%s.%s", tableName, "archived_on"): nil}
