...
func serverEncoderDecoderDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("ContentTypeHeaderKey").Op("=").Lit("RawHTML-type"),
			jen.ID("contentTypeXML").Op("=").Lit("application/xml"),
			jen.ID("contentTypeJSON").Op("=").Lit("application/json"),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("defaultContentType").Op("=").ID("ContentTypeJSON"),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Type().Defs(
			jen.ID("ServerEncoderDecoder").Interface(
			...
			jen.ID("encoder").Interface(jen.ID("Encode").Params(jen.Interface()).Params(jen.ID("error"))),
			jen.ID("decoder").Interface(jen.ID("Decode").Params(jen.ID("v").Interface()).Params(jen.ID("error"))),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Comment("EncodeInvalidInputResponse encodes a generic 400 error to a response."),
		jen.Line(),
		jen.Func().Params(jen.ID("e").Op("*").ID("serverEncoderDecoder")).ID("EncodeInvalidInputResponse").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("res").Qual("net/http", "ResponseWriter")).Body(
	...
	code.Add(
		jen.Comment("EncodeNotFoundResponse encodes a generic 404 error to a response."),
		jen.Line(),
		jen.Func().Params(jen.ID("e").Op("*").ID("serverEncoderDecoder")).ID("EncodeNotFoundResponse").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("res").Qual("net/http", "ResponseWriter")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("e").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.ID("e").Dot("EncodeErrorResponse").Call(
				jen.ID("ctx"),
	...
	code.Add(
		jen.Comment("EncodeUnspecifiedInternalServerErrorResponse encodes a generic 500 error to a response."),
		jen.Line(),
		jen.Func().Params(jen.ID("e").Op("*").ID("serverEncoderDecoder")).ID("EncodeUnspecifiedInternalServerErrorResponse").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("res").Qual("net/http", "ResponseWriter")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("e").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.ID("e").Dot("EncodeErrorResponse").Call(
				jen.ID("ctx"),
	...
	code.Add(
		jen.Comment("EncodeUnauthorizedResponse encodes a generic 401 error to a response."),
		jen.Line(),
		jen.Func().Params(jen.ID("e").Op("*").ID("serverEncoderDecoder")).ID("EncodeUnauthorizedResponse").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("res").Qual("net/http", "ResponseWriter")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("e").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.ID("e").Dot("EncodeErrorResponse").Call(
				jen.ID("ctx"),
	...
	code.Add(
		jen.Comment("EncodeInvalidPermissionsResponse encodes a generic 403 error to a response."),
		jen.Line(),
		jen.Func().Params(jen.ID("e").Op("*").ID("serverEncoderDecoder")).ID("EncodeInvalidPermissionsResponse").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("res").Qual("net/http", "ResponseWriter")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("e").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.ID("e").Dot("EncodeErrorResponse").Call(
				jen.ID("ctx"),
