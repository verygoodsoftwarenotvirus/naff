...
func actionsDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().ID("RandomAction").Params(jen.ID("c").Op("*").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/pkg/client/httpclient", "Client"), jen.ID("builder").Op("*").ID("requests").Dot("Builder")).Params(jen.Op("*").ID("Action")).Body(
			jen.ID("allActions").Op(":=").Map(jen.ID("string")).Op("*").ID("Action").Valuesln(jen.Lit("GetHealthCheck").Op(":").Valuesln(jen.ID("Name").Op(":").Lit("GetHealthCheck"), jen.ID("Action").Op(":").Func().Params().Params(jen.Op("*").Qual("net/http", "Request"), jen.ID("error")).Body(
				...
				jen.ID("ctx").Op(":=").Qual("context", "Background").Call(),
				jen.ID("ui").Op(":=").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/pkg/types/fakes", "BuildFakeUserCreationInput").Call(),
