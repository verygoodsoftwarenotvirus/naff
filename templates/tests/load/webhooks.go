...
func webhooksDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().ID("fetchRandomWebhook").Params(jen.ID("c").Op("*").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/pkg/client/httpclient", "Client")).Params(jen.Op("*").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/pkg/types", "Webhook")).Body(
	...
	code.Add(
		jen.Func().ID("buildWebhookActions").Params(jen.ID("c").Op("*").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/pkg/client/httpclient", "Client"), jen.ID("builder").Op("*").ID("requests").Dot("Builder")).Params(jen.Map(jen.ID("string")).Op("*").ID("Action")).Body(
			jen.Return().Map(jen.ID("string")).Op("*").ID("Action").Valuesln(jen.Lit("GetWebhooks").Op(":").Valuesln(jen.ID("Name").Op(":").Lit("GetWebhooks"), jen.ID("Action").Op(":").Func().Params().Params(jen.Op("*").Qual("net/http", "Request"), jen.ID("error")).Body(
				...
				jen.ID("ctx").Op(":=").Qual("context", "Background").Call(),
				jen.ID("exampleInput").Op(":=").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/pkg/types/fakes", "BuildFakeWebhookCreationInput").Call(),
				...
				jen.ID("ctx").Op(":=").Qual("context", "Background").Call(),
				jen.If(jen.ID("randomWebhook").Op(":=").ID("fetchRandomWebhook").Call(jen.ID("c")), jen.ID("randomWebhook").Op("!=").ID("nil")).Body(
					jen.ID("randomWebhook").Dot("Name").Op("=").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/pkg/types/fakes", "BuildFakeWebhook").Call().Dot("Name"),
