			...
			jen.ID("s").Dot("runForEachClientExcept").Call(
				...
				jen.Func().Params(jen.ID("testClients").Op("*").ID("testClientWrapper")).Params(jen.Func().Params()).Body(
					jen.Return().Func().Params().Body(
						...
						jen.ID("expectedAuditLogEntries").Op(":=").Index().Op("*").ID("types").Dot("AuditLogEntry").Valuesln(jen.Valuesln(jen.ID("EventType").Op(":").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/audit", "APIClientCreationEvent"))),
			...
			jen.ID("s").Dot("runForEachClientExcept").Call(
				...
				jen.Func().Params(jen.ID("testClients").Op("*").ID("testClientWrapper")).Params(jen.Func().Params()).Body(
					jen.Return().Func().Params().Body(
						...
						jen.ID("expectedAuditLogEntries").Op(":=").Index().Op("*").ID("types").Dot("AuditLogEntry").Valuesln(jen.Valuesln(jen.ID("EventType").Op(":").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/audit", "APIClientCreationEvent")), jen.Valuesln(jen.ID("EventType").Op(":").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/audit", "APIClientArchiveEvent"))),
						jen.ID("validateAuditLogEntries").Call(
							jen.ID("t"),
							jen.ID("expectedAuditLogEntries"),
							jen.ID("auditLogEntries"),
