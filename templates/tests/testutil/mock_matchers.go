...
func mockMatchersDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().ID("AuditLogEntryCreationInputMatcher").Params(jen.ID("eventType").ID("string")).Params(jen.Func().Params(jen.Op("*").ID("types").Dot("AuditLogEntryCreationInput")).Params(jen.ID("bool"))).Body(
