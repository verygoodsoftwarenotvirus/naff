...
func runtimeTestDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Func().ID("TestRecordRuntimeStats").Params(jen.ID("T").PointerTo().Qual("testprojects", "T")).Block(
	...
	code.Add(
		jen.Func().ID("TestRegisterDefaultViews").Params(jen.ID("t").PointerTo().Qual("testprojects", "T")).Block(
			jen.ID("t").Dot("Parallel").Call(),
			jen.Line(),
