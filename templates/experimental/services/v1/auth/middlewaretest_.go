	...
	ret.Add(
		jen.Func().ID("TestService_CookieAuthenticationMiddleware").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("md").Dot("On").Call(jen.Lit("GetUser"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				),
				...
				jen.ID("ms").Dot("On").Call(jen.Lit("ServeHTTP"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				),
					jen.Qual("github.com/stretchr/testify/mock",
						"Anything",
		...
		jen.Func().ID("TestService_AuthenticationMiddleware").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("exampleClient").Op(":=").Op("&").ID("models").Dot(
					...
					jen.ID("ClientID").Op(":").Lit("PRETEND_THIS_IS_A_REAL_CLIENT_ID"), jen.ID("ClientSecret").Op(":").Lit("PRETEND_THIS_IS_A_REAL_CLIENT_SECRET"), jen.ID("BelongsTo").Op(":").ID("exampleUser").Dot("ID")),
				jen.ID("ocv").Op(":=").Op("&").ID("mockOAuth2ClientValidator").Values(),
				jen.ID("ocv").Dot("On").Call(jen.Lit("ExtractOAuth2ClientFromRequest"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				)).Dot(
					"Return",
				...
				jen.ID("mockDB").Dot("On").Call(jen.Lit("GetUser"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				),
					jen.ID("exampleClient").Dot("BelongsTo")).Dot(
					"Return",
				).Call(jen.ID("exampleUser"), jen.ID("nil")),
				jen.ID("s").Dot(
					"userDB",
				...
				jen.ID("h").Dot("On").Call(jen.Lit("ServeHTTP"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				),
					jen.Qual("github.com/stretchr/testify/mock",
						"Anything",
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("with error fetching client but able to use cookie but unable to decode cookie"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("cb").Dot("On").Call(jen.Lit("Decode"), jen.ID("CookieName"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				),
					jen.Qual("github.com/stretchr/testify/mock",
						"Anything",
	...
	ret.Add(
		jen.Func().ID("Test_parseLoginInputFromForm").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("req").Dot(
					"Form",
				).Op("=").Map(jen.ID("string")).Index().ID("string").Valuesln(
					jen.ID("UsernameFormKey").Op(":").Valuesln(
						jen.ID("expected").Dot(
							"Username",
						)), jen.ID("PasswordFormKey").Op(":").Valuesln(
						jen.ID("expected").Dot(
							"Password",
						)), jen.ID("TOTPTokenFormKey").Op(":").Valuesln(
						jen.ID("expected").Dot(
							"TOTPToken",
						))),
		...
		jen.Func().ID("TestService_UserLoginInputMiddleware").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				jen.ID("exampleInput").Op(":=").Op("&").ID("models").Dot(
					...
					jen.ID("Username").Op(":").Lit("username"), jen.ID("Password").Op(":").Lit("password"), jen.ID("TOTPToken").Op(":").Lit("1233456")),

				jen.Var().ID("b").Qual("bytes", "Buffer"),
				jen.ID("require").Dot("NoError").Call(jen.ID("t"), jen.Qual("encoding/json", "NewEncoder").Call(jen.Op("&").ID("b")).Dot(
					"Encode",
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("with error decoding request but valid value attached to form"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("req").Dot(
					"Header",
				).Dot("Set").Call(jen.Lit("Content-type"), jen.Lit("application/x-www-form-urlencoded")),
				jen.ID("s").Op(":=").ID("buildTestService").Call(jen.ID("t")),
				jen.ID("ed").Op(":=").Op("&").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/v1/encoding/mock", "EncoderDecoder").Values(),
				jen.ID("ed").Dot("On").Call(jen.Lit("DecodeRequest"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
