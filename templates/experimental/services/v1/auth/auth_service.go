...
func authServiceDotGo() *jen.File {
	...
	ret.Add(
		jen.Type().Defs(
			jen.ID("OAuth2ClientValidator").Interface(
				jen.ID("ExtractOAuth2ClientFromRequest").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.Op("*").ID("models").Dot("OAuth2Client"), jen.ID("error")),
			),
			jen.ID("cookieEncoderDecoder").Interface(
				jen.ID("Encode").Params(jen.ID("name").ID("string"), jen.ID("value").Interface()).Params(jen.ID("string"), jen.ID("error")), jen.ID("Decode").Params(jen.List(jen.ID("name"), jen.ID("value")).ID("string"), jen.ID("dst").Interface()).Params(jen.ID("error")),
			),
			jen.ID("UserIDFetcher").Func().Params(jen.Op("*").Qual("net/http", "Request")).Params(jen.ID("uint64")),
			jen.ID("Service").Struct(
				jen.ID("config").ID("config").Dot("AuthSettings"),
				jen.ID("logger").Qual("gitlab.com/verygoodsoftwarenotvirus/logging/v1", "Logger"),
				jen.ID("authenticator").ID("auth").Dot("Authenticator"),
				jen.ID("userIDFetcher").ID("UserIDFetcher"),
				jen.ID("userDB").ID("models").Dot("UserDataManager"),
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().ID("ProvideAuthService").Params(
			jen.ID("logger").Qual("gitlab.com/verygoodsoftwarenotvirus/logging/v1", "Logger"),
			jen.ID("cfg").Op("*").ID("config").Dot("ServerConfig"),
			jen.ID("authenticator").ID("auth").Dot("Authenticator"),
			jen.ID("database").ID("models").Dot("UserDataManager"),
			jen.ID("oauth2ClientsService").ID("OAuth2ClientValidator"),
			jen.ID("userIDFetcher").ID("UserIDFetcher"),
			...
			jen.ID("svc").Op(":=").Op("&").ID("Service").Valuesln(
				...
				jen.ID("cookieManager").Op(":").ID("securecookie").Dot("New").Call(jen.ID("securecookie").Dot("GenerateRandomKey").Call(jen.Lit(64)), jen.Index().ID("byte").Call(jen.ID("cfg").Dot("Auth").Dot("CookieSecret"))),
			),
			jen.Return().ID("svc"),
		),
		jen.Line(),
