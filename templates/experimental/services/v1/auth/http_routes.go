...
func httpRoutesDotGo() *jen.File {
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("DecodeCookieFromRequest").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.ID("ca").Op("*").ID("models").Dot(
			"CookieAuth",
		),
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("FetchUserFromRequest").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.Op("*").ID("models").Dot(
			"User",
		),
			jen.ID("error")).Block(
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("LoginHandler").Params().Params(jen.Qual("net/http", "HandlerFunc")).Block(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Block(
				...
				jen.ID("logger").Op(":=").ID("s").Dot("logger").Dot("WithValue").Call(jen.Lit("user"), jen.ID("loginData").Dot(
					"user",
				).Dot("ID")),
				...
				jen.If(jen.ID("err").Op("!=").ID("nil")).Block(
					...
					jen.ID("response").Op(":=").Op("&").ID("models").Dot(
						...
						jen.ID("Code").Op(":").Qual("net/http", "StatusInternalServerError"), jen.ID("Message").Op(":").Lit("error encountered building cookie")),
					jen.If(jen.ID("err").Op(":=").ID("s").Dot(
						"encoderDecoder",
					).Dot(
						"EncodeResponse",
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("CycleSecretHandler").Params().Params(jen.Qual("net/http", "HandlerFunc")).Block(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Block(
				...
				jen.ID("s").Dot(
					"cookieManager",
				).Op("=").ID("securecookie").Dot("New").Call(jen.ID("securecookie").Dot(
					"GenerateRandomKey",
	...
	ret.Add(
		jen.Type().ID("loginData").Struct(jen.ID("loginInput").Op("*").ID("models").Dot(
			"UserLoginInput",
		),
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("fetchLoginDataFromRequest").Params(jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.Op("*").ID("loginData"), jen.Op("*").ID("models").Dot(
			"ErrorResponse",
		)).Block(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").Qual("go.opencensus.io/trace", "StartSpan").Call(jen.ID("req").Dot(
				"Context",
			...
			jen.If(jen.ID("err").Op("==").Qual("database/sql", "ErrNoRows")).Block(
				...
				jen.Return().List(jen.ID("nil"), jen.Op("&").ID("models").Dot(
					...
					jen.ID("Code").Op(":").Qual("net/http", "StatusBadRequest"))),
			...
			jen.ID("ld").Op(":=").Op("&").ID("loginData").Valuesln(
				jen.ID("loginInput").Op(":").ID("loginInput"), jen.ID("user").Op(":").ID("user")),
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("validateLogin").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("loginInfo").ID("loginData")).Params(jen.ID("bool"), jen.ID("error")).Block(
			...
			jen.ID("logger").Op(":=").ID("s").Dot("logger").Dot("WithValue").Call(jen.Lit("username"), jen.ID("user").Dot(
				"Username",
			)),
			jen.List(jen.ID("loginValid"), jen.ID("err")).Op(":=").ID("s").Dot(
				"authenticator",
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("buildAuthCookie").Params(jen.ID("user").Op("*").ID("models").Dot(
			"User",
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("buildCookie").Params(jen.ID("value").ID("string")).Params(jen.Op("*").Qual("net/http", "Cookie")).Block(
			jen.Return().Op("&").Qual("net/http", "Cookie").Valuesln(
				jen.ID("Name").Op(":").ID("CookieName"), jen.ID("Value").Op(":").ID("value"), jen.ID("Path").Op(":").Lit("/"), jen.ID("HttpOnly").Op(":").ID("true"), jen.ID("Secure").Op(":").ID("s").Dot(
					"config",
				).Dot(
					"SecureCookiesOnly",
				),
