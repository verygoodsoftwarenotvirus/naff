...
func httpRoutesDotGo() *jen.File {
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("DecodeCookieFromRequest").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.ID("ca").Op("*").ID("models").Dot(
			...
			jen.If(jen.ID("err").Op("!=").Qual("net/http", "ErrNoCookie").Op("&&").ID("cookie").Op("!=").ID("nil")).Block(
				...
				jen.If(jen.ID("decodeErr").Op("!=").ID("nil")).Block(
					jen.ID("s").Dot(
						"logger",
					).Dot("Error").Call(jen.ID("err"), jen.Lit("decoding request cookie")),
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("WebsocketAuthFunction").Params(jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.ID("bool")).Block(
			...
			jen.ID("s").Dot(
				"logger",
			).Dot("Error").Call(jen.ID("err"), jen.Lit("error authenticated token-authenticated request")),
			jen.Return().ID("false"),
		),
		jen.Line(),
	)
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("LoginHandler").Params().Params(jen.Qual("net/http", "HandlerFunc")).Block(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Block(
				...
				jen.If(jen.ID("errRes").Op("!=").ID("nil")).Block(
					jen.ID("s").Dot(
						"logger",
					).Dot("Error").Call(jen.ID("errRes"), jen.Lit("error encountered fetching login data from request")),
					jen.ID("res").Dot(
						"WriteHeader",
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("fetchLoginDataFromRequest").Params(jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.Op("*").ID("loginData"), jen.Op("*").ID("models").Dot(
			...
			jen.If(jen.ID("err").Op("==").Qual("database/sql", "ErrNoRows")).Block(
				jen.ID("s").Dot(
					"logger",
				).Dot("Error").Call(jen.ID("err"), jen.Lit("no matching user")),
				jen.Return().List(jen.ID("nil"), jen.Op("&").ID("models").Dot(
					"ErrorResponse",
				).Valuesln(
				...
				jen.ID("s").Dot(
					"logger",
				).Dot("Error").Call(jen.ID("err"), jen.Lit("error fetching user")),
				jen.Return().List(jen.ID("nil"), jen.Op("&").ID("models").Dot(
					"ErrorResponse",
				).Valuesln(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Service")).ID("buildAuthCookie").Params(jen.ID("user").Op("*").ID("models").Dot(
			...
			jen.If(jen.ID("err").Op("!=").ID("nil")).Block(
				jen.ID("s").Dot(
					...
					"ID",
				)).Dot("Error").Call(jen.ID("err"), jen.Lit("error encoding cookie")),
