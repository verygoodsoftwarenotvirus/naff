		...
		jen.Func().ID("TestService_DecodeCookieFromRequest").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.List(jen.ID("c"), jen.ID("err")).Op(":=").ID("s").Dot(
					...
					jen.ID("ID").Op(":").Lit(1), jen.ID("Username").Op(":").Lit("username"))),
				jen.ID("require").Dot("NoError").Call(jen.ID("t"), jen.ID("err")),
				jen.ID("req").Dot(
					"AddCookie",
				).Call(jen.ID("c")),
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("with invalid cookie"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("c").Op(":=").Op("&").Qual("net/http", "Cookie").Valuesln(
					jen.ID("Name").Op(":").ID("CookieName"), jen.ID("Value").Op(":").Lit("blah blah blah this is not a real cookie"), jen.ID("Path").Op(":").Lit("/"), jen.ID("HttpOnly").Op(":").ID("true")),
				jen.ID("req").Dot(
					"AddCookie",
				).Call(jen.ID("c")),
				jen.List(jen.ID("cookie"), jen.ID("err")).Op(":=").ID("s").Dot(
	...
	ret.Add(
		jen.Func().ID("TestService_WebsocketAuthFunction").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("with valid oauth2 client"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				).Assert(jen.Op("*").ID("mockOAuth2ClientValidator")).Dot("On").Call(jen.Lit("ExtractOAuth2ClientFromRequest"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				)).Dot(
					"Return",
		...
		jen.Func().ID("TestService_FetchUserFromRequest").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("expectedUser").Op(":=").Op("&").ID("models").Dot(
					...
					jen.ID("ID").Op(":").ID("userID"), jen.ID("Username").Op(":").Lit("username")),
				jen.List(jen.ID("req"), jen.ID("err")).Op(":=").Qual("net/http", "NewRequest").Call(jen.Qual("net/http", "MethodGet"), jen.Lit("http://todo.verygoodsoftwarenotvirus.ru/testing"), jen.ID("nil")),
				jen.ID("require").Dot("NotNil").Call(jen.ID("t"), jen.ID("req")),
				jen.ID("require").Dot("NoError").Call(jen.ID("t"), jen.ID("err")),
				jen.List(jen.ID("c"), jen.ID("err")).Op(":=").ID("s").Dot(
				...
				jen.ID("s").Dot(
					"userDB",
				).Assert(jen.Op("*").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/models/v1/mock", "UserDataManager")).Dot("On").Call(jen.Lit("GetUser"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				),
					jen.ID("userID")).Dot(
					"Return",
	...
	ret.Add(
		jen.Func().ID("TestService_Login").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("s").Dot(
					"userDB",
				).Assert(jen.Op("*").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/models/v1/mock", "UserDataManager")).Dot("On").Call(jen.Lit("GetUserByUsername"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				),
				...
				jen.ID("s").Dot(
					"authenticator",
				).Assert(jen.Op("*").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/v1/auth/mock", "Authenticator")).Dot("On").Call(jen.Lit("ValidateLogin"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				),
			...
			jen.Line(),
			jen.ID("T").Dot("Run").Call(jen.Lit("with error encoding error fetching login dataa"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				jen.ID("s").Op(":=").ID("buildTestService").Call(jen.ID("t")),
	...
	ret.Add(
		jen.Func().ID("TestService_fetchLoginDataFromRequest").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("assert").Dot("Equal").Call(jen.ID("t"), jen.ID("loginData").Dot(
					"user",
				),
					jen.ID("expectedUser")),
	...
	ret.Add(
		jen.Func().ID("TestService_validateLogin").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("with too weak a password hash"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("s").Dot(
					"authenticator",
				).Assert(jen.Op("*").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/v1/auth/mock", "Authenticator")).Dot("On").Call(jen.Lit("HashPassword"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				),
					jen.Qual("github.com/stretchr/testify/mock",
						"Anything",
				...
				jen.ID("s").Dot(
					"userDB",
				).Assert(jen.Op("*").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/models/v1/mock", "UserDataManager")).Dot("On").Call(jen.Lit("UpdateUser"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				),
					jen.Qual("github.com/stretchr/testify/mock",
						"Anything",
