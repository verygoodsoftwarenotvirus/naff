...
func webhooksServiceTestDotGo() *jen.File {
	...
	ret.Add(
		...
		jen.Return().Op("&").ID("Service").Valuesln(jen.ID("logger").Op(":").ID("noop").Dot(
			...
			jen.Return().Lit(0),
		), jen.ID("encoderDecoder").Op(":").Op("&").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/v1/encoding/mock", "EncoderDecoder").Valuesln(), jen.ID("eventManager").Op(":").ID("newsman").Dot(
			"NewNewsman",
