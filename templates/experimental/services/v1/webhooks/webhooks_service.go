...
func webhooksServiceDotGo() *jen.File {
	...
	ret.Add(jen.Null().Type().ID("eventManager").Interface(jen.ID("newsman").Dot(
		...
		"Listener",
	))).Type().ID("Service").Struct(jen.ID("logger").ID("logging").Dot(
		"Logger",
	...
	ret.Add(jen.Func().Comment("// ProvideWebhooksService builds a new WebhooksService").ID("ProvideWebhooksService").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("logger").ID("logging").Dot(
		"Logger",
	), jen.ID("webhookDatabase").ID("models").Dot(
		"WebhookDataManager",
