	...
	ret.Add(
		jen.Func().ID("TestProvideItemsService").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("idm").Dot("On").Call(jen.Lit("GetAllItemsCount"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				)).Dot(
					"Return",
				).Call(jen.ID("expectation"), jen.ID("nil")),
				jen.ID("uc").Dot("On").Call(jen.Lit("IncrementBy"), jen.ID("expectation")).Dot(
					"Return",
				).Call(),
