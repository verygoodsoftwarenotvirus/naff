	...
	ret.Add(
		jen.Func().ID("buildTestService").Params(jen.ID("t").Op("*").Qual("testing", "T")).Params(jen.Op("*").ID("Service")).Block(
			...
			jen.ID("service").Op(":=").Op("&").ID("Service").Valuesln(
				jen.ID("database").Op(":").ID("database").Dot(
					"BuildMockDatabase",
	...
	ret.Add(
		jen.Func().ID("TestProvideOAuth2ClientsService").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("uc").Dot("On").Call(jen.Lit("IncrementBy"), jen.ID("expected")).Dot(
					"Return",
				).Call(),
		...
		jen.Func().ID("Test_clientStore_GetByID").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("mockDB").Dot(
					...
					jen.ID("ClientID").Op(":").ID("exampleID")), jen.ID("nil")),
				jen.ID("c").Op(":=").Op("&").ID("clientStore").Valuesln(
					jen.ID("database").Op(":").ID("mockDB")),
				jen.List(jen.ID("actual"), jen.ID("err")).Op(":=").ID("c").Dot(
					"GetByID",
				).Call(jen.ID("exampleID")),
				jen.ID("assert").Dot("NoError").Call(jen.ID("t"), jen.ID("err")),
				jen.ID("assert").Dot("Equal").Call(jen.ID("t"), jen.ID("exampleID"), jen.ID("actual").Dot(
					"GetID",
				).Call()),
			)),
			jen.Line(),
	...
	ret.Add(
		jen.Func().ID("TestService_HandleAuthorizeRequest").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("moah").Dot("On").Call(jen.Lit("HandleAuthorizeRequest"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				),
				...
				jen.ID("assert").Dot("NoError").Call(jen.ID("t"), jen.ID("s").Dot(
	...
	ret.Add(
		jen.Func().ID("TestService_HandleTokenRequest").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				jen.ID("s").Op(":=").ID("buildTestService").Call(jen.ID("t")),
				jen.ID("moah").Op(":=").Op("&").ID("mockOauth2Handler").Values(),
				jen.ID("moah").Dot("On").Call(jen.Lit("HandleTokenRequest"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				),
					jen.Qual("github.com/stretchr/testify/mock",
						"Anything",
