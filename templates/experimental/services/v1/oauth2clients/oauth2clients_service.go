...
func oauth2ClientsServiceDotGo() *jen.File {
	...
	ret.Add(jen.Null().Var().ID("CreationMiddlewareCtxKey").ID("models").Dot(
		"ContextKey",
	...
	ret.Add(jen.Func().ID("newClientStore").Params(jen.ID("db").ID("database").Dot(
		"Database",
	...
	ret.Add(jen.Func().Comment("// GetByID implements oauth2.ClientStorage").Params(jen.ID("s").Op("*").ID("clientStore")).ID("GetByID").Params(jen.ID("id").ID("string")).Params(jen.ID("oauth2").Dot(
		"ClientInfo",
	), jen.ID("error")).Block(
	...
	ret.Add(jen.Func().Comment("// ProvideOAuth2ClientsService builds a new OAuth2ClientsService").ID("ProvideOAuth2ClientsService").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("logger").ID("logging").Dot(
		"Logger",
	...
	ret.Add(jen.Func().Comment("// initializeOAuth2Handler").ID("initializeOAuth2Handler").Params(jen.ID("handler").ID("oauth2Handler"), jen.ID("s").Op("*").ID("Service")).Block(
		jen.ID("handler").Dot(
			"SetAllowGetAccessRequest",
		).Call(jen.ID("true")),
		jen.ID("handler").Dot(
	...
	ret.Add(jen.Func().Comment("// HandleAuthorizeRequest is a simple wrapper around the internal server's HandleAuthorizeRequest").Params(jen.ID("s").Op("*").ID("Service")).ID("HandleAuthorizeRequest").Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.ID("error")).Block(
		jen.Return().ID("s").Dot(
			"oauth2Handler",
		).Dot(
	...
	ret.Add(jen.Func().Comment("// HandleTokenRequest is a simple wrapper around the internal server's HandleTokenRequest").Params(jen.ID("s").Op("*").ID("Service")).ID("HandleTokenRequest").Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.ID("error")).Block(
		jen.Return().ID("s").Dot(
			"oauth2Handler",
		).Dot(
			"HandleTokenRequest",
