...
func httpRoutesTestDotGo() *jen.File {
	...
	jen.ID("ID").Op(":").ID("uint64").Call(jen.Lit(123)), jen.ID("HashedPassword").Op(":").Lit("not really lol"), jen.ID("Salt").Op(":").Index().ID("byte").Call(jen.Lit(`nah`)), jen.ID("TwoFactorSecret").Op(":").Lit("still no")),
	...
	jen.ID("expected").Dot(
			...
			jen.ID("auth").Dot(
				"On",
			).Call(jen.Lit("ValidateLogin"), jen.Qual("github.com/stretchr/testify/mock",
				"Anything",
	),
	jen.ID("expected").Dot(
				"HashedPassword",
	...
	jen.Qual("github.com/stretchr/testify/mock",
			...
			jen.ID("ed").Dot(
				"On",
			).Call(jen.Lit("EncodeResponse"), jen.Qual("github.com/stretchr/testify/mock",
				"Anything",
	),
	jen.Qual("github.com/stretchr/testify/mock",
				"Anything",
	...
	jen.ID("Username").Op(":").Lit("username"), jen.ID("Password").Op(":").Lit("password")),
			jen.ID("expectedUser").Op(":=").Op("&").ID("models").Dot(
				"User",
			).Valuesln(
	jen.ID("Username").Op(":").ID("exampleInput").Dot(
				"Username",
	),
	jen.ID("HashedPassword").Op(":").Lit("blahblah")),
			...
			jen.ID("auth").Dot(
				"On",
			).Call(jen.Lit("HashPassword"), jen.Qual("github.com/stretchr/testify/mock",
				"Anything",
	),
	...
	jen.ID("exampleInput")).Dot(
			...
			jen.ID("mc").Dot(
				"On",
			).Call(jen.Lit("Increment"), jen.Qual("github.com/stretchr/testify/mock",
				"Anything",
			)),
			jen.ID("s").Dot(
				"userCounter",
			...
			jen.ID("r").Dot(
				"On",
			).Call(jen.Lit("Report"), jen.Qual("github.com/stretchr/testify/mock",
				"Anything",
			)).Dot(
				"Return",
			).Call(),
	...
	jen.Qual("github.com/stretchr/testify/mock",
			...
			jen.ID("assert").Dot("Equal").Call(jen.ID("t"), jen.Qual("net/http", "StatusForbidden"), jen.ID("res").Dot(
				"Code",
			)),
		)),
			jen.Line(),
	...
	jen.Qual("github.com/stretchr/testify/mock",
			...
			jen.ID("assert").Dot("Equal").Call(jen.ID("t"), jen.Qual("net/http", "StatusAccepted"), jen.ID("res").Dot(
				"Code",
			)),
		)),
			jen.Line(),
	...
	jen.ID("NewPassword").Op(":").Lit("new_password"), jen.ID("CurrentPassword").Op(":").Lit("old_password"), jen.ID("TOTPToken").Op(":").Lit("123456")),
			jen.ID("req").Op("=").ID("req").Dot(
				"WithContext",
			).Call(jen.Qual("context", "WithValue").Call(jen.ID("req").Dot(
				"Context",
	...
	jen.ID("expectedUserID")).Dot(
			...
			jen.ID("mc").Dot(
				"On",
			).Call(jen.Lit("Decrement"), jen.Qual("github.com/stretchr/testify/mock",
				"Anything",
			)),
			jen.ID("s").Dot(
				"userCounter",
