...
func initDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Func().ID("initializeClient").Params().Params(jen.Op("*").ID("httpclient").Dot("Client")).Body(
			jen.List(jen.ID("uri"), jen.ID("err")).Op(":=").Qual("net/url", "Parse").Call(jen.ID("urlToUse")),
			jen.If(jen.ID("err").Op("!=").ID("nil")).Body(
				jen.ID("panic").Call(jen.ID("err"))),
			jen.List(jen.ID("c"), jen.ID("err")).Op(":=").ID("httpclient").Dot("NewClient").Call(jen.ID("uri")),
			jen.If(jen.ID("err").Op("!=").ID("nil")).Body(
				jen.ID("panic").Call(jen.ID("err"))),
			jen.Return().ID("c"),
		),
