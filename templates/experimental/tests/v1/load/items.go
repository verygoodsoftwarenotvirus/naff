...
func itemsDotGo() *jen.File {
	...
	ret.Add(jen.Func().Comment("// fetchRandomItem retrieves a random item from the list of available items").ID("fetchRandomItem").Params(jen.ID("c").Op("*").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/client/v1/http", "V1Client")).Params(jen.Op("*").ID("models").Dot(
		"Item",
	)).Block(
		jen.List(jen.ID("itemsRes"), jen.ID("err")).Op(":=").ID("c").Dot(
			"GetItems",
		).Call(jen.Qual("context", "Background").Call(), jen.ID("nil")),
		jen.If(jen.ID("err").Op("!=").ID("nil").Op("||").ID("itemsRes").Op("==").ID("nil").Op("||").ID("len").Call(jen.ID("itemsRes").Dot(
			"Items",
		)).Op("==").Lit(0)).Block(
			jen.Return().ID("nil"),
		),
		jen.ID("randIndex").Op(":=").Qual("math/rand", "Intn").Call(jen.ID("len").Call(jen.ID("itemsRes").Dot(
			"Items",
		))),
		jen.Return().Op("&").ID("itemsRes").Dot(
			"Items",
