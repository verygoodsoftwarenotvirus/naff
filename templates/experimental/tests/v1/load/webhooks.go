...
func webhooksDotGo() *jen.File {
	...
	ret.Add(
		...
		jen.If(jen.ID("err").Op("!=").ID("nil").Op("||").ID("webhooks").Op("==").ID("nil").Op("||").ID("len").Call(jen.ID("webhooks").Dot(
			"Webhooks",
		)).Op("==").Lit(0)).Block(
			jen.Return().ID("nil"),
		),
		jen.ID("randIndex").Op(":=").Qual("math/rand", "Intn").Call(jen.ID("len").Call(jen.ID("webhooks").Dot(
			"Webhooks",
		))),
		jen.Return().Op("&").ID("webhooks").Dot(
			"Webhooks",
