...
func webhooksTestDotGo() *jen.File {
	...
	ret.Add(
		jen.Func().ID("checkWebhookEquality").Params(jen.ID("t").Op("*").Qual("testing", "T"), jen.List(jen.ID("expected"), jen.ID("actual")).Op("*").ID("models").Dot("Webhook")).Block(
			jen.ID("t").Dot(
				"Helper",
			).Call(),
			jen.ID("assert").Dot(
	...
	ret.Add(
		jen.Func().ID("buildDummyWebhook").Params(jen.ID("t").Op("*").Qual("testing", "T")).Params(jen.Op("*").ID("models").Dot("Webhook")).Block(
			jen.ID("t").Dot(
				"Helper",
			).Call(),
		...
		jen.Func().ID("TestWebhooks").Params(jen.ID("test").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("test").Dot(
				...
				jen.ID("T").Dot(
					...
					jen.Var().ID("expected").Index().Op("*").ID("models").Dot("Webhook"),
					...
					jen.ID("assert").Dot(
						"True",
					).Call(jen.ID("t"), jen.ID("len").Call(jen.ID("expected")).Op("<=").ID("len").Call(jen.ID("actual").Dot("Webhooks"))),
					jen.For(jen.List(jen.ID("_"), jen.ID("webhook")).Op(":=").Range().ID("actual").Dot("Webhooks")).Block(
						jen.ID("err").Op("=").ID("todoClient").Dot(
							"ArchiveWebhook",
						).Call(jen.ID("tctx"), jen.ID("webhook").Dot("ID")),
						jen.ID("assert").Dot("NoError").Call(jen.ID("t"), jen.ID("err")),
					),
				)),
			)),
			...
			jen.ID("test").Dot(
				...
				jen.ID("T").Dot(
					...
					jen.ID("err").Op(":=").ID("todoClient").Dot(
						"UpdateWebhook",
					).Call(jen.ID("tctx"), jen.Op("&").ID("models").Dot("Webhook").Valuesln(
