		...
		jen.Func().ID("buildDummyOAuth2ClientInput").Params(jen.ID("t").Op("*").Qual("testing", "T"), jen.List(jen.ID("username"), jen.ID("password"), jen.ID("totpToken")).ID("string")).Params(jen.Op("*").ID("models").Dot(
			...
			jen.ID("x").Op(":=").Op("&").ID("models").Dot(
				...
				jen.ID("UserLoginInput").Op(":").ID("models").Dot(
					...
					jen.ID("Username").Op(":").ID("username"), jen.ID("Password").Op(":").ID("password"), jen.ID("TOTPToken").Op(":").ID("mustBuildCode").Call(jen.ID("t"), jen.ID("totpToken"))), jen.ID("Scopes").Op(":").Index().ID("string").Valuesln(
					jen.Lit("*")), jen.ID("RedirectURI").Op(":").Lit("http://localhost")),
			jen.Return().ID("x"),
		),
		jen.Line(),
	)
		...
		jen.Func().ID("TestOAuth2Clients").Params(jen.ID("test").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("test").Dot(
				...
				jen.ID("T").Dot(
					...
					jen.ID("err").Op("=").ID("testClient").Dot(
						"ArchiveOAuth2Client",
					).Call(jen.ID("tctx"), jen.ID("actual").Dot("ID")),
			...
			jen.ID("test").Dot(
				...
				jen.ID("T").Dot(
					...
					jen.List(jen.ID("actual"), jen.ID("err")).Op(":=").ID("testClient").Dot(
						"GetOAuth2Client",
					).Call(jen.ID("tctx"), jen.ID("c").Dot("ID")),
					jen.ID("checkValueAndError").Call(jen.ID("t"), jen.ID("actual"), jen.ID("err")),
					jen.ID("checkOAuth2ClientEquality").Call(jen.ID("t"), jen.ID("convertInputToClient").Call(jen.ID("input")), jen.ID("actual")),
					jen.ID("err").Op("=").ID("testClient").Dot(
						"ArchiveOAuth2Client",
			...
			jen.ID("test").Dot(
				...
				jen.ID("T").Dot(
					...
					jen.ID("err").Op("=").ID("testClient").Dot(
						"ArchiveOAuth2Client",
					).Call(jen.ID("tctx"), jen.ID("premade").Dot("ID")),
					jen.ID("assert").Dot("NoError").Call(jen.ID("t"), jen.ID("err")),
				)),
				jen.ID("T").Dot(
					"Run",
					...
					jen.ID("assert").Dot("Error").Call(jen.ID("t"), jen.ID("err"), jen.Lit("expected error from what should be an unauthorized client")),
				)),
			)),
			jen.ID("test").Dot(
				"Run",
				...
				jen.ID("T").Dot(
					...
					jen.For(jen.List(jen.ID("_"), jen.ID("oAuth2Client")).Op(":=").Range().ID("expected")).Block(
						...
						jen.ID("assert").Dot(
							"True",
						).Call(jen.ID("t"), jen.ID("clientFound"), jen.Lit("expected oAuth2Client ID %d to be present in results"), jen.ID("oAuth2Client").Dot("ID")),
					),
					jen.For(jen.List(jen.ID("_"), jen.ID("oa2c")).Op(":=").Range().ID("expected")).Block(
						jen.ID("err").Op("=").ID("testClient").Dot(
							"ArchiveOAuth2Client",
						).Call(jen.ID("tctx"), jen.ID("oa2c").Dot("ID")),
						jen.ID("assert").Dot("NoError").Call(jen.ID("t"), jen.ID("err"), jen.Lit("error deleting client %d: %v"), jen.ID("oa2c").Dot("ID"),
							jen.ID("err")),
					),
				)),
			)),
