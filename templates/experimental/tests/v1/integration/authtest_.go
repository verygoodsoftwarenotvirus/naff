...
`), jen.ID("username"), jen.ID("password"), jen.ID("code")),
			...
			jen.ID("assert").Dot("Equal").Call(jen.ID("t"), jen.Qual("net/http", "StatusNoContent"), jen.ID("resp").Dot(
				"StatusCode",
			),
	...
	ret.Add(
		jen.Func().ID("TestAuth").Params(jen.ID("test").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("test").Dot(
				...
				jen.ID("require").Dot("NoError").Call(jen.ID("t"), jen.Qual("encoding/json", "NewDecoder").Call(jen.ID("res").Dot(
					"Body",
				)).Dot(
					"Decode",
				).Call(jen.ID("ucr"))),
			...
			jen.ID("test").Dot(
				...
				jen.ID("r").Op(":=").Op("&").ID("models").Dot(
					...
					jen.ID("CurrentPassword").Op(":").ID("ui").Dot(
						"Password",
					),
				...
				jen.ID("assert").Dot("NotEqual").Call(jen.ID("t"), jen.ID("cookie"), jen.ID("cookies").Index(jen.Lit(0))),
			)),
			jen.ID("test").Dot(
				"Run",
				...
				jen.ID("require").Dot("NotEqual").Call(jen.ID("t"), jen.ID("user").Dot(
					"TwoFactorSecret",
				),
					jen.ID("r").Dot(
						"TwoFactorSecret",
			...
			jen.ID("test").Dot(
				...
				jen.List(jen.ID("res"), jen.ID("err")).Op(":=").Parens(jen.Op("&").Qual("net/http", "Client").Valuesln(
					jen.ID("Timeout").Op(":").Lit(10).Op("*").Qual("time", "Second"))).Dot(
					"Do",
				).Call(jen.ID("req")),
				jen.ID("assert").Dot("NoError").Call(jen.ID("t"), jen.ID("err")),
				jen.ID("assert").Dot("Equal").Call(jen.ID("t"), jen.Qual("net/http", "StatusOK"), jen.ID("res").Dot(
			...
			jen.ID("test").Dot(
				...
				jen.List(jen.ID("webhookA"), jen.ID("err")).Op(":=").ID("clientA").Dot(
					...
					jen.ID("Method").Op(":").Qual("net/http", "MethodPatch"), jen.ID("Name").Op(":").Lit("A"))),
				...
				jen.List(jen.ID("webhookB"), jen.ID("err")).Op(":=").ID("clientB").Dot(
					...
					jen.ID("Method").Op(":").Qual("net/http", "MethodPatch"), jen.ID("Name").Op(":").Lit("B"))),
			...
			jen.ID("test").Dot(
				...
				jen.ID("input").Dot(
					"Scopes",
				).Op("=").Index().ID("string").Valuesln(
					jen.Lit("absolutelynevergonnaexistascopelikethis")),
