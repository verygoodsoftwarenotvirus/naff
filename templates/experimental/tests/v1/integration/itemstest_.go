...
func itemsTestDotGo() *jen.File {
	...
	ret.Add(
		jen.Func().ID("checkItemEquality").Params(jen.ID("t").Op("*").Qual("testing", "T"), jen.List(jen.ID("expected"), jen.ID("actual")).Op("*").ID("models").Dot("Item")).Block(
			jen.ID("t").Dot(
				"Helper",
			).Call(),
			jen.ID("assert").Dot(
				"NotZero",
			).Call(jen.ID("t"), jen.ID("actual").Dot("ID")),
			...
			jen.ID("assert").Dot(
				"NotZero",
			).Call(jen.ID("t"), jen.ID("actual").Dot("CreatedOn")),
		),
		jen.Line(),
	)

	ret.Add(
		jen.Func().ID("buildDummyItem").Params(jen.ID("t").Op("*").Qual("testing", "T")).Params(jen.Op("*").ID("models").Dot("Item")).Block(
			jen.ID("t").Dot(
				"Helper",
			).Call(),
		...
		jen.Func().ID("TestItems").Params(jen.ID("test").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("test").Dot(
				...
				jen.ID("T").Dot(
					...
					jen.ID("err").Op("=").ID("todoClient").Dot(
						"ArchiveItem",
					).Call(jen.ID("ctx"), jen.ID("premade").Dot("ID")),
					...
					jen.ID("assert").Dot(
						"NotZero",
					).Call(jen.ID("t"), jen.ID("actual").Dot("ArchivedOn")),
				)),
			)),
			jen.ID("test").Dot(
				"Run",
				...
				jen.ID("T").Dot(
					...
					jen.Var().ID("expected").Index().Op("*").ID("models").Dot("Item"),
					...
					jen.ID("assert").Dot(
						"True",
					).Call(jen.ID("t"), jen.ID("len").Call(jen.ID("expected")).Op("<=").ID("len").Call(jen.ID("actual").Dot("Items")), jen.Lit("expected %d to be <= %d"), jen.ID("len").Call(jen.ID("expected")), jen.ID("len").Call(jen.ID("actual").Dot("Items"))),
					jen.For(jen.List(jen.ID("_"), jen.ID("item")).Op(":=").Range().ID("actual").Dot("Items")).Block(
						jen.ID("err").Op("=").ID("todoClient").Dot(
							"ArchiveItem",
						).Call(jen.ID("ctx"), jen.ID("item").Dot("ID")),
						jen.ID("assert").Dot("NoError").Call(jen.ID("t"), jen.ID("err")),
					),
				)),
			)),
			jen.ID("test").Dot(
				...
				jen.ID("T").Dot(
					...
					jen.ID("err").Op("=").ID("todoClient").Dot(
						"ArchiveItem",
					).Call(jen.ID("ctx"), jen.ID("actual").Dot("ID")),
					jen.ID("assert").Dot("NoError").Call(jen.ID("t"), jen.ID("err")),
				)),
			)),
			jen.ID("test").Dot(
				...
				jen.ID("T").Dot(
					...
					jen.ID("err").Op(":=").ID("todoClient").Dot(
						"UpdateItem",
					).Call(jen.ID("ctx"), jen.Op("&").ID("models").Dot("Item").Valuesln(
				...
				jen.ID("T").Dot(
					...
					jen.ID("checkValueAndError").Call(jen.ID("t"), jen.ID("premade"), jen.ID("err")),
					jen.List(jen.ID("premade").Dot("Name"),
						jen.ID("premade").Dot(
							"Details",
						)).Op("=").List(jen.ID("expected").Dot("Name"),
						jen.ID("expected").Dot(
							"Details",
						)),
					jen.ID("err").Op("=").ID("todoClient").Dot(
