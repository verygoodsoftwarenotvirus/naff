...
func suiteTestDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("TestSuite")).ID("runForEachClientExcept").Params(jen.ID("name").ID("string"), jen.ID("subtestBuilder").Params(jen.Op("*").ID("testClientWrapper")).Params(jen.Params()), jen.ID("exceptions").Op("...").ID("string")).Body(
