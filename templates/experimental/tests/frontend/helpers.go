...
func helpersDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Func().Params(jen.ID("h").Op("*").ID("testHelper")).ID("runForAllBrowsers").Params(jen.ID("t").Op("*").Qual("testing", "T"), jen.ID("testName").ID("string"), jen.ID("testFunc").Params(jen.ID("playwright").Dot("Browser")).Params(jen.Params(jen.Op("*").Qual("testing", "T")))).Body(
			jen.If(jen.Op("!").ID("chromeDisabled")).Body(
