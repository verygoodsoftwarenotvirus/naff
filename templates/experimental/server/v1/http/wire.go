...
func wireDotGo() *jen.File {
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().ID("ProvideNamespace").Params().Params(jen.Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/v1/metrics",
			"Namespace",
		)).Block(
	...
	ret.Add(
		...
		jen.Func().ID("ProvideNewsmanTypeNameManipulationFunc").Params(jen.ID("logger").Qual("gitlab.com/verygoodsoftwarenotvirus/logging/v1",
			...
			jen.Return().Func().Params(jen.ID("s").ID("string")).Params(jen.ID("string")).Block(
				jen.ID("logger").Dot("WithName").Call(jen.Lit("events")).Dot("WithValue").Call(jen.Lit("type_name"), jen.ID("s")).Dot(
					"Info",
	...
	ret.Add(
		...
		jen.Func().ID("provideHTTPServer").Params().Params(jen.Op("*").Qual("net/http", "Server")).Block(
			jen.ID("srv").Op(":=").Op("&").Qual("net/http", "Server").Valuesln(
				jen.ID("ReadTimeout").Op(":").Lit(5).Op("*").Qual("time", "Second"), jen.ID("WriteTimeout").Op(":").Lit(10).Op("*").Qual("time", "Second"), jen.ID("IdleTimeout").Op(":").Lit(120).Op("*").Qual("time", "Second"), jen.ID("TLSConfig").Op(":").Op("&").Qual("crypto/tls", "Config").Valuesln(
					jen.ID("PreferServerCipherSuites").Op(":").ID("true"), jen.ID("CurvePreferences").Op(":").Index().Qual("crypto/tls", "CurveID").Valuesln(
						jen.Qual("crypto/tls", "CurveP256"), jen.Qual("crypto/tls", "X25519")), jen.ID("MinVersion").Op(":").Qual("crypto/tls", "VersionTLS12"), jen.ID("CipherSuites").Op(":").Index().ID("uint16").Valuesln(
