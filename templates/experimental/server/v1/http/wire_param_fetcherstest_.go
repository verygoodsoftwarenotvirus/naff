		...
		jen.Func().ID("Test_buildChiUserIDFetcher").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("req").Op("=").ID("req").Dot(
					...
					"Context",
				).Call(), jen.Qual("github.com/go-chi/chi",
					"RouteCtxKey",
				),
					jen.Op("&").Qual("github.com/go-chi/chi",
						"Context",
					).Valuesln(
						jen.ID("URLParams").Op(":").Qual("github.com/go-chi/chi",
							"RouteParams",
						).Valuesln(
	...
	ret.Add(
		jen.Func().ID("Test_buildChiWebhookIDFetcher").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("expected").Op(":=").ID("uint64").Call(jen.Lit(123)),
				jen.ID("req").Op(":=").ID("buildRequest").Call(jen.ID("t")),
				jen.ID("req").Op("=").ID("req").Dot("WithContext").Call(jen.Qual("context", "WithValue").Call(jen.ID("req").Dot("Context").Call(), jen.Qual("github.com/go-chi/chi", "RouteCtxKey"),
					jen.Op("&").Qual("github.com/go-chi/chi", "Context").Valuesln(
						jen.ID("URLParams").Op(":").Qual("github.com/go-chi/chi", "RouteParams").Valuesln(
							jen.ID("Keys").Op(":").Index().ID("string").Values(
								jen.ID("webhooks").Dot("URIParamKey"),
							),
							jen.ID("Values").Op(":").Index().ID("string").Values(
								jen.Qual("fmt", "Sprintf").Call(jen.Lit("%d"), jen.ID("expected")),
							),
						),
					),
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("with invalid value somehow"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("expected").Op(":=").ID("uint64").Call(jen.Lit(0)),
				jen.ID("req").Op(":=").ID("buildRequest").Call(jen.ID("t")),
				jen.ID("req").Op("=").ID("req").Dot("WithContext").Call(jen.Qual("context", "WithValue").Call(jen.ID("req").Dot("Context").Call(), jen.Qual("github.com/go-chi/chi", "RouteCtxKey"), jen.Op("&").Qual("github.com/go-chi/chi", "Context").Valuesln(
					jen.ID("URLParams").Op(":").Qual("github.com/go-chi/chi", "RouteParams").Valuesln(
						jen.ID("Keys").Op(":").Index().ID("string").Values(
							jen.ID("webhooks").Dot("URIParamKey"),
						),
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("happy path"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("req").Op("=").ID("req").Dot("WithContext").Call(jen.Qual("context", "WithValue").Call(jen.ID("req").Dot("Context").Call(), jen.Qual("github.com/go-chi/chi", "RouteCtxKey"),
					jen.Op("&").Qual("github.com/go-chi/chi", "Context").Valuesln(
						jen.ID("URLParams").Op(":").Qual("github.com/go-chi/chi", "RouteParams").Valuesln(
							jen.ID("Keys").Op(":").Index().ID("string").Values(
								jen.ID("oauth2clients").Dot("URIParamKey"),
							),
							jen.ID("Values").Op(":").Index().ID("string").Values(
								jen.Qual("fmt", "Sprintf").Call(jen.Lit("%d"), jen.ID("expected")),
							),
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("with invalid value somehow"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("req").Op("=").ID("req").Dot("WithContext").Call(
					jen.Qual("context", "WithValue").Call(
						jen.ID("req").Dot("Context").Call(), jen.Qual("github.com/go-chi/chi", "RouteCtxKey"), jen.Op("&").Qual("github.com/go-chi/chi", "Context").Valuesln(
							jen.ID("URLParams").Op(":").Qual("github.com/go-chi/chi", "RouteParams").Valuesln(
								jen.ID("Keys").Op(":").Index().ID("string").Values(
									jen.ID("oauth2clients").Dot("URIParamKey"),
								),
