...
func configDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("cfg").Op("*").ID("Config")).ID("ProvideDatabaseConnection").Params(jen.ID("logger").ID("logging").Dot("Logger")).Params(jen.Op("*").Qual("database/sql", "DB"), jen.ID("error")).Body(
			jen.Switch(jen.ID("cfg").Dot("Provider")).Body(
				...
				jen.Case(jen.ID("SqliteProvider")).Body(
					jen.Return().Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/database/querybuilding/sqlite", "ProvideSqliteDB").Call(
						jen.ID("logger"),
						jen.ID("cfg").Dot("ConnectionDetails"),
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().ID("ProvideSessionManager").Params(jen.ID("cookieConfig").ID("authentication").Dot("CookieConfig"), jen.ID("dbConf").ID("Config"), jen.ID("db").Op("*").Qual("database/sql", "DB")).Params(jen.Op("*").ID("scs").Dot("SessionManager"), jen.ID("error")).Body(
