...
func mockUserSQLQueryBuilderDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("_").ID("querybuilding").Dot("UserSQLQueryBuilder").Op("=").Parens(jen.Op("*").ID("UserSQLQueryBuilder")).Call(jen.ID("nil")),
		jen.Line(),
	)

	code.Add(
		jen.Type().ID("UserSQLQueryBuilder").Struct(jen.ID("mock").Dot("Mock")),
		jen.Line(),
	)
