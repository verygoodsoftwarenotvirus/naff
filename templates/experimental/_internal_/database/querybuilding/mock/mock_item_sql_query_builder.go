...
func mockItemSQLQueryBuilderDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("_").ID("querybuilding").Dot("ItemSQLQueryBuilder").Op("=").Parens(jen.Op("*").ID("ItemSQLQueryBuilder")).Call(jen.ID("nil")),
		jen.Line(),
	)

	code.Add(
		jen.Type().ID("ItemSQLQueryBuilder").Struct(jen.ID("mock").Dot("Mock")),
		jen.Line(),
	)
