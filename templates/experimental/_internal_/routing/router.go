...
func routerDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Type().ID("Middleware").Params(jen.Qual("net/http", "Handler")).Params(jen.Qual("net/http", "Handler")).Type().ID("Router").Interface(
		...
		).Type().ID("RouteParamManager").Interface(
			jen.ID("BuildRouteParamIDFetcher").Params(jen.ID("logger").ID("logging").Dot("Logger"), jen.List(jen.ID("key"), jen.ID("logDescription")).ID("string")).Params(jen.ID("req").Op("*").Qual("net/http", "Request"), jen.ID("uint64")),
			jen.ID("BuildRouteParamStringIDFetcher").Params(jen.ID("key").ID("string")).Params(jen.ID("req").Op("*").Qual("net/http", "Request"), jen.ID("string")),
		),
		jen.Line(),
	)
