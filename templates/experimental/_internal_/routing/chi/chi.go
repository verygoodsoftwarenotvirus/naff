...
func chiDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("roughlyOneYear").Op("=").Qual("time", "Hour").Op("*").Lit(24).Op("*").Lit(365),
	...
	code.Add(
		jen.Func().ID("convertMiddleware").Params(jen.ID("in").Op("...").ID("routing").Dot("Middleware")).Params(jen.Index().Params(jen.ID("handler").Qual("net/http", "Handler")).Params(jen.Qual("net/http", "Handler"))).Body(
			jen.ID("out").Op(":=").Index().Params(jen.ID("handler").Qual("net/http", "Handler")).Params(jen.Qual("net/http", "Handler")).Values(),
			...
			jen.If(jen.ID("err").Op(":=").ID("chi").Dot("Walk").Call(
				...
				jen.Func().Params(jen.ID("method").ID("string"), jen.ID("route").ID("string"), jen.ID("_").Qual("net/http", "Handler"), jen.ID("_").Op("...").Params(jen.Qual("net/http", "Handler")).Params(jen.Qual("net/http", "Handler"))).Params(jen.ID("error")).Body(
					jen.ID("r").Dot("logger").Dot("WithValues").Call(jen.Map(jen.ID("string")).Interface().Valuesln(
						jen.Lit("method").Op(":").ID("method"), jen.Lit("route").Op(":").ID("route"))).Dot("Debug").Call(jen.Lit("route found")),
					jen.Return().ID("nil"),
				),
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("r").Op("*").ID("router")).ID("BuildRouteParamIDFetcher").Params(jen.ID("logger").ID("logging").Dot("Logger"), jen.List(jen.ID("key"), jen.ID("logDescription")).ID("string")).Params(jen.ID("req").Op("*").Qual("net/http", "Request"), jen.ID("uint64")).Body(
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("r").Op("*").ID("router")).ID("BuildRouteParamStringIDFetcher").Params(jen.ID("key").ID("string")).Params(jen.ID("req").Op("*").Qual("net/http", "Request"), jen.ID("string")).Body(
