...
func loggingDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Type().ID("level").ID("int").Type().ID("Level").Op("*").ID("level").Type().ID("RequestIDFunc").Params(jen.Op("*").Qual("net/http", "Request")).Params(jen.ID("string")),
		jen.Line(),
	)

	code.Add(
		jen.Var().ID("InfoLevel").ID("Level").Op("=").ID("new").Call(jen.ID("level")),
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().ID("BuildLoggingMiddleware").Params(jen.ID("logger").ID("Logger")).Params(jen.ID("next").Qual("net/http", "Handler"), jen.Qual("net/http", "Handler")).Body(
