...
func httpHelpersTestDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Type().ID("accountsServiceHTTPRoutesTestHelper").Struct(
	...
	code.Add(
		jen.Func().ID("buildTestHelper").Params(jen.ID("t").Op("*").Qual("testing", "T")).Params(jen.Op("*").ID("accountsServiceHTTPRoutesTestHelper")).Body(
			jen.ID("t").Dot("Helper").Call(),
			jen.ID("helper").Op(":=").Op("&").ID("accountsServiceHTTPRoutesTestHelper").Values(),
			...
			jen.ID("helper").Dot("service").Dot("accountIDFetcher").Op("=").Func().Params(jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.ID("uint64")).Body(
				jen.Return().ID("helper").Dot("exampleAccount").Dot("ID")),
