...
func itemsDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("itemIDURLParamKey").Op("=").Lit("item"),
		jen.Line(),
	)
	...
	code.Add(
		jen.Var().ID("itemCreatorTemplate").ID("string"),
		jen.Line(),
	)

	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("service")).ID("buildItemCreatorView").Params(jen.ID("includeBaseTemplate").ID("bool")).Params(jen.Qual("net/http", "ResponseWriter"), jen.Op("*").Qual("net/http", "Request")).Body(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Body(
				...
				jen.ID("item").Op(":=").Op("&").ID("types").Dot("Item").Values(),
				jen.If(jen.ID("includeBaseTemplate")).Body(
					...
					jen.ID("page").Op(":=").Op("&").ID("pageData").Valuesln(
						jen.ID("IsLoggedIn").Op(":").ID("sessionCtxData").Op("!=").ID("nil"), jen.ID("Title").Op(":").Lit("New Item"), jen.ID("ContentData").Op(":").ID("item")),
	...
	code.Add(
		jen.Var().ID("itemCreationInputNameFormKey").Op("=").Lit("name"),
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("service")).ID("parseFormEncodedItemCreationInput").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("req").Op("*").Qual("net/http", "Request"), jen.ID("sessionCtxData").Op("*").ID("types").Dot("SessionContextData")).Params(jen.ID("creationInput").Op("*").ID("types").Dot("ItemCreationInput")).Body(
			...
			jen.ID("creationInput").Op("=").Op("&").ID("types").Dot("ItemCreationInput").Valuesln(
				jen.ID("Name").Op(":").ID("form").Dot("Get").Call(jen.ID("itemCreationInputNameFormKey")), jen.ID("Details").Op(":").ID("form").Dot("Get").Call(jen.ID("itemCreationInputDetailsFormKey")), jen.ID("BelongsToAccount").Op(":").ID("sessionCtxData").Dot("ActiveAccountID")),
	...
	code.Add(
		jen.Var().ID("itemEditorTemplate").ID("string"),
		jen.Line(),
	)

	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("service")).ID("buildItemEditorView").Params(jen.ID("includeBaseTemplate").ID("bool")).Params(jen.Qual("net/http", "ResponseWriter"), jen.Op("*").Qual("net/http", "Request")).Body(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Body(
				jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("s").Dot("tracer").Dot("StartSpan").Call(jen.ID("req").Dot("Context").Call()),
				jen.Defer().ID("span").Dot("End").Call(),
				jen.ID("logger").Op(":=").ID("s").Dot("logger").Dot("WithRequest").Call(jen.ID("req")),
				...
				jen.ID("tmplFuncMap").Op(":=").Map(jen.ID("string")).Interface().Valuesln(
					jen.Lit("componentTitle").Op(":").Func().Params(jen.ID("x").Op("*").ID("types").Dot("Item")).Params(jen.ID("string")).Body(
	...
	code.Add(
		jen.Var().ID("itemsTableTemplate").ID("string"),
		jen.Line(),
	)

	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("service")).ID("buildItemsTableView").Params(jen.ID("includeBaseTemplate").ID("bool")).Params(jen.Qual("net/http", "ResponseWriter"), jen.Op("*").Qual("net/http", "Request")).Body(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Body(
				jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("s").Dot("tracer").Dot("StartSpan").Call(jen.ID("req").Dot("Context").Call()),
				jen.Defer().ID("span").Dot("End").Call(),
				jen.ID("logger").Op(":=").ID("s").Dot("logger").Dot("WithRequest").Call(jen.ID("req")),
				...
				jen.ID("tmplFuncMap").Op(":=").Map(jen.ID("string")).Interface().Valuesln(
					jen.Lit("individualURL").Op(":").Func().Params(jen.ID("x").Op("*").ID("types").Dot("Item")).Params(jen.Qual("html/template", "URL")).Body(
				...
				jen.If(jen.ID("includeBaseTemplate")).Body(
					...
					jen.ID("page").Op(":=").Op("&").ID("pageData").Valuesln(
						jen.ID("IsLoggedIn").Op(":").ID("sessionCtxData").Op("!=").ID("nil"), jen.ID("Title").Op(":").Lit("Items"), jen.ID("ContentData").Op(":").ID("items")),
					jen.If(jen.ID("sessionCtxData").Op("!=").ID("nil")).Body(
						jen.ID("page").Dot("IsServiceAdmin").Op("=").ID("sessionCtxData").Dot("Requester").Dot("ServicePermissions").Dot("IsServiceAdmin").Call()),
					jen.ID("s").Dot("renderTemplateToResponse").Call(
						jen.ID("ctx"),
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("service")).ID("parseFormEncodedItemUpdateInput").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("req").Op("*").Qual("net/http", "Request"), jen.ID("sessionCtxData").Op("*").ID("types").Dot("SessionContextData")).Params(jen.ID("updateInput").Op("*").ID("types").Dot("ItemUpdateInput")).Body(
			...
			jen.ID("updateInput").Op("=").Op("&").ID("types").Dot("ItemUpdateInput").Valuesln(
				jen.ID("Name").Op(":").ID("form").Dot("Get").Call(jen.ID("itemCreationInputNameFormKey")), jen.ID("Details").Op(":").ID("form").Dot("Get").Call(jen.ID("itemCreationInputDetailsFormKey")), jen.ID("BelongsToAccount").Op(":").ID("sessionCtxData").Dot("ActiveAccountID")),
