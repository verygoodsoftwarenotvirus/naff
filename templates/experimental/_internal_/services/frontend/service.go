...
func serviceDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Type().ID("AuthService").Interface(
			jen.ID("UserAttributionMiddleware").Params(jen.ID("next").Qual("net/http", "Handler")).Params(jen.Qual("net/http", "Handler")),
			jen.ID("PermissionFilterMiddleware").Params(jen.ID("permissions").Op("...").ID("authorization").Dot("Permission")).Params(jen.ID("next").Qual("net/http", "Handler"), jen.Qual("net/http", "Handler")),
		...
		).Type().ID("UsersService").Interface(
		...
		).Type().ID("Service").Interface(jen.ID("SetupRoutes").Params(jen.ID("router").ID("routing").Dot("Router"))).Type().ID("service").Struct(
