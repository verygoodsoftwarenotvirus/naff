...
func thumbnailsDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("allSupportedColors").Op("=").Lit(2).Op("<<").Lit(7),
		jen.Line(),
	)

	code.Add(
		jen.Type().ID("thumbnailer").Interface(jen.ID("Thumbnail").Params(jen.ID("i").Op("*").ID("Image"), jen.List(jen.ID("width"), jen.ID("height")).ID("uint"), jen.ID("filename").ID("string")).Params(jen.Op("*").ID("Image"), jen.ID("error"))),
		jen.Line(),
	)
		...
		jen.Func().ID("newThumbnailer").Params(jen.ID("contentType").ID("string")).Params(jen.ID("thumbnailer"), jen.ID("error")).Body(
			jen.Switch(jen.Qual("strings", "TrimSpace").Call(jen.Qual("strings", "ToLower").Call(jen.ID("contentType")))).Body(
				jen.Case(jen.ID("imagePNG")).Body(
					jen.Return().List(jen.Op("&").ID("pngThumbnailer").Values(), jen.ID("nil"))),
				jen.Case(jen.ID("imageJPEG")).Body(
					jen.Return().List(jen.Op("&").ID("jpegThumbnailer").Values(), jen.ID("nil"))),
				jen.Case(jen.ID("imageGIF")).Body(
					jen.Return().List(jen.Op("&").ID("gifThumbnailer").Values(), jen.ID("nil"))),
				jen.Default().Body(
	...
	code.Add(
		jen.Type().ID("jpegThumbnailer").Struct(),
		jen.Line(),
	)
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("t").Op("*").ID("gifThumbnailer")).ID("Thumbnail").Params(jen.ID("img").Op("*").ID("Image"), jen.List(jen.ID("width"), jen.ID("height")).ID("uint"), jen.ID("filename").ID("string")).Params(jen.Op("*").ID("Image"), jen.ID("error")).Body(
			...
			jen.ID("i").Op(":=").Op("&").ID("Image").Valuesln(
				jen.ID("Filename").Op(":").Qual("fmt", "Sprintf").Call(
					jen.Lit("%s.png"),
					jen.ID("filename"),
	...
	code.Add(
		jen.Type().ID("pngThumbnailer").Struct(),
		jen.Line(),
	)
	...
	code.Add(
		jen.Type().ID("gifThumbnailer").Struct(),
		jen.Line(),
	)
