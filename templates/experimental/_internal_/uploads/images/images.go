...
func imagesDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Type().ID("Image").Struct(
			jen.ID("Filename").ID("string"),
			jen.ID("ContentType").ID("string"),
			jen.ID("Data").Index().ID("byte"),
			jen.ID("Size").ID("int"),
		).Type().ID("ImageUploadProcessor").Interface(jen.ID("Process").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("req").Op("*").Qual("net/http", "Request"), jen.ID("filename").ID("string")).Params(jen.Op("*").ID("Image"), jen.ID("error"))).Type().ID("uploadProcessor").Struct(
