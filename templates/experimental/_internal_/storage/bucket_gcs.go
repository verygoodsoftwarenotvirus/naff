...
func bucketGcsDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("GCSProvider").Op("=").Lit("gcs"),
		jen.Line(),
	)

	code.Add(
		jen.Type().ID("GCSBlobConfig").Struct(jen.ID("GoogleAccessID").ID("string")).Type().ID("GCSConfig").Struct(
	...
	code.Add(
		jen.Func().ID("buildGCSBucket").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("cfg").Op("*").ID("GCSConfig")).Params(jen.Op("*").Qual("gocloud.dev/blob", "Bucket"), jen.ID("error")).Body(
			jen.Var().ID("creds").Op("*").ID("google").Dot("Credentials"),
			...
			jen.If(jen.ID("cfg").Dot("ServiceAccountKeyFilepath").Op("!=").Lit("")).Body(
				...
				jen.If(jen.List(jen.ID("creds"), jen.ID("err")).Op("=").Qual("gocloud.dev/gcp", "DefaultCredentials").Call(jen.ID("ctx")), jen.ID("err").Op("!=").ID("nil")).Body(
					jen.Return().List(jen.ID("nil"), jen.Qual("fmt", "Errorf").Call(
						jen.Lit("constructing GCP credentials: %w"),
						jen.ID("err"),
					))),
			),
			...
			jen.If(jen.ID("gcsClientErr").Op("!=").ID("nil")).Body(
				jen.Return().List(jen.ID("nil"), jen.Qual("fmt", "Errorf").Call(
					jen.Lit("constructing GCP client: %w"),
					jen.ID("gcsClientErr"),
				))),
			jen.ID("blobOpts").Op(":=").Op("&").ID("gcsblob").Dot("Options").Valuesln(
				jen.ID("GoogleAccessID").Op(":").ID("cfg").Dot("BlobSettings").Dot("GoogleAccessID")),
	...
	code.Add(
		jen.Var().ID("_").Qual("github.com/go-ozzo/ozzo-validation/v4", "ValidatableWithContext").Op("=").Parens(jen.Op("*").ID("GCSConfig")).Call(jen.ID("nil")),
		jen.Line(),
	)
