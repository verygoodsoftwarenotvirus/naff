...
func clientDotGo() *jen.File {
	...
	jen.ID("db").Op(":").ID("db"), jen.ID("querier").Op(":").ID("querier"), jen.ID("debug").Op(":").ID("debug"), jen.ID("logger").Op(":").ID("logger").Dot(
			...
			jen.If(jen.ID("debug")).Block(
				jen.ID("c").Dot(
					...
					"SetLevel",
				).Call(jen.Qual("gitlab.com/verygoodsoftwarenotvirus/logging/v1",
					"DebugLevel",
				)),
			),
			jen.ID("c").Dot(
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().ID("attachFilterToSpan").Params(jen.ID("span").Op("*").Qual("go.opencensus.io/trace", "Span"), jen.ID("filter").Op("*").ID("models").Dot(
			"QueryFilter",
		)).Block(
