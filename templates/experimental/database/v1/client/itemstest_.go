	...
	ret.Add(
		jen.Func().ID("TestClient_GetItem").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("obligatory"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("mockDB").Dot(
					"ItemDataManager",
				).Dot("On").Call(jen.Lit("GetItem"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				),
	...
	ret.Add(
		jen.Func().ID("TestClient_GetItemCount").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("obligatory"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("mockDB").Dot(
					"ItemDataManager",
				).Dot("On").Call(jen.Lit("GetItemCount"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				),
					jen.ID("models").Dot(
						"DefaultQueryFilter",
	...
	ret.Add(
		jen.Func().ID("TestClient_GetAllItemsCount").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("obligatory"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("mockDB").Dot(
					"ItemDataManager",
				).Dot("On").Call(jen.Lit("GetAllItemsCount"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				)).Dot(
					"Return",
				).Call(jen.ID("expected"), jen.ID("nil")),
	...
	ret.Add(
		jen.Func().ID("TestClient_GetItems").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("obligatory"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("mockDB").Dot(
					"ItemDataManager",
				).Dot("On").Call(jen.Lit("GetItems"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				),
					jen.ID("models").Dot(
						"DefaultQueryFilter",
	...
	ret.Add(
		jen.Func().ID("TestClient_CreateItem").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("obligatory"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("mockDB").Dot(
					"ItemDataManager",
				).Dot("On").Call(jen.Lit("CreateItem"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				),
	...
	ret.Add(
		jen.Func().ID("TestClient_UpdateItem").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("obligatory"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("mockDB").Dot(
					"ItemDataManager",
				).Dot("On").Call(jen.Lit("UpdateItem"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				),
					jen.ID("exampleInput")).Dot(
					"Return",
	...
	ret.Add(
		jen.Func().ID("TestClient_ArchiveItem").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("obligatory"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("mockDB").Dot(
					"ItemDataManager",
				).Dot("On").Call(jen.Lit("ArchiveItem"), jen.Qual("github.com/stretchr/testify/mock",
					"Anything",
				),
					jen.ID("exampleItemID"), jen.ID("exampleUserID")).Dot(
					"Return",
