...
func usersDotGo() *jen.File {
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("c").Op("*").ID("Client")).ID("CreateUser").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("input").Op("*").ID("models").Dot(
			...
			jen.ID("c").Dot("logger").Dot("WithValue").Call(jen.Lit("username"), jen.ID("input").Dot(
				"Username",
			)).Dot("Debug").Call(jen.Lit("CreateUser called")),
			jen.Return().ID("c").Dot("querier").Dot(
				"CreateUser",
	...
	ret.Add(
		...
		jen.Func().Comment("// NOTE: this function uses the ID provided in the input to make its query.").Params(jen.ID("c").Op("*").ID("Client")).ID("UpdateUser").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("updated").Op("*").ID("models").Dot(
			...
			jen.ID("c").Dot("logger").Dot("WithValue").Call(jen.Lit("username"), jen.ID("updated").Dot(
				"Username",
			)).Dot("Debug").Call(jen.Lit("UpdateUser called")),
			jen.Return().ID("c").Dot("querier").Dot(
				"UpdateUser",
