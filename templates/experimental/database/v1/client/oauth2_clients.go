...
func oauth2ClientsDotGo() *jen.File {
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("c").Op("*").ID("Client")).ID("GetOAuth2ClientCount").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("models").Dot(
			"QueryFilter",
	...
	ret.Add(
		jen.Func().Comment("// GetAllOAuth2Clients returns all OAuth2 clients, irrespective of ownership.").Params(jen.ID("c").Op("*").ID("Client")).ID("GetAllOAuth2Clients").Params(jen.ID("ctx").Qual("context", "Context")).Params(jen.Index().Op("*").ID("models").Dot(
		"OAuth2Client",
	), jen.ID("error")).Block(
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("c").Op("*").ID("Client")).ID("GetOAuth2Clients").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("models").Dot(
			"QueryFilter",
	...
	jen.Line(),
	jen.Func().Comment("// ID field to be valid.").Params(jen.ID("c").Op("*").ID("Client")).ID("UpdateOAuth2Client").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("updated").Op("*").ID("models").Dot(
		"OAuth2Client",
