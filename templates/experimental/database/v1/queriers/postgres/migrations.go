...
func migrationsDotGo() *jen.File {
	...
	ret.Add(
		jen.Var().ID("migrations").Op("=").Index().ID("darwin").Dot(
			...
			CREATE TABLE IF NOT EXISTS users (
				...
				jen.ID("Version").Op(":").Lit(2), jen.ID("Description").Op(":").Lit("Add oauth2_clients table"), jen.ID("Script").Op(":").Lit(`
			CREATE TABLE IF NOT EXISTS oauth2_clients (
				...
				jen.ID("Version").Op(":").Lit(3), jen.ID("Description").Op(":").Lit("create webhooks table"), jen.ID("Script").Op(":").Lit(`
			CREATE TABLE IF NOT EXISTS webhooks (
				...
				jen.ID("Version").Op(":").Lit(4), jen.ID("Description").Op(":").Lit("create items table"), jen.ID("Script").Op(":").Lit(`
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Comment("// safe (as in idempotent, though not recommended) to call this function multiple times.").Params(jen.ID("p").Op("*").ID("Postgres")).ID("Migrate").Params(jen.ID("ctx").Qual("context", "Context")).Params(jen.ID("error")).Block(
			...
			jen.ID("p").Dot("migrateOnce").Dot("Do").Call(jen.ID("buildMigrationFunc").Call(jen.ID("p").Dot(
				"db",
			))),
			jen.Return().ID("nil"),
		),
