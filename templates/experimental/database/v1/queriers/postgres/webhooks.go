...
func webhooksDotGo() *jen.File {
	...
	ret.Add(jen.Func().Comment("// buildGetWebhookQuery returns a SQL query (and arguments) for retrieving a given webhook").Params(jen.ID("p").Op("*").ID("Postgres")).ID("buildGetWebhookQuery").Params(jen.List(jen.ID("webhookID"), jen.ID("userID")).ID("uint64")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
	...
	ret.Add(jen.Func().Comment("// GetWebhook fetches a webhook from the database").Params(jen.ID("p").Op("*").ID("Postgres")).ID("GetWebhook").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("webhookID"), jen.ID("userID")).ID("uint64")).Params(jen.Op("*").ID("models").Dot(
		"Webhook",
	), jen.ID("error")).Block(
	...
	ret.Add(jen.Func().Comment("// buildGetWebhookCountQuery returns a SQL query (and arguments) that returns a list of webhooks").Comment("// meeting a given filter's criteria and belonging to a given user.").Params(jen.ID("p").Op("*").ID("Postgres")).ID("buildGetWebhookCountQuery").Params(jen.ID("filter").Op("*").ID("models").Dot(
		"QueryFilter",
	...
	ret.Add(jen.Func().Comment("// GetWebhookCount will fetch the count of webhooks from the database that meet a particular filter,").Comment("// and belong to a particular user.").Params(jen.ID("p").Op("*").ID("Postgres")).ID("GetWebhookCount").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("models").Dot(
		"QueryFilter",
	...
	ret.Add(jen.Func().Comment("// buildGetAllWebhooksCountQuery returns a query which would return the count of webhooks regardless of ownership.").Params(jen.ID("p").Op("*").ID("Postgres")).ID("buildGetAllWebhooksCountQuery").Params().Params(jen.ID("string")).Block(
	...
	ret.Add(jen.Func().Comment("// GetAllWebhooksCount will fetch the count of every active webhook in the database").Params(jen.ID("p").Op("*").ID("Postgres")).ID("GetAllWebhooksCount").Params(jen.ID("ctx").Qual("context", "Context")).Params(jen.ID("count").ID("uint64"), jen.ID("err").ID("error")).Block(
		jen.ID("err").Op("=").ID("p").Dot(
			"db",
		).Dot(
			"QueryRowContext",
	...
	ret.Add(jen.Func().Comment("// buildGetAllWebhooksQuery returns a SQL query which will return all webhooks, regardless of ownership").Params(jen.ID("p").Op("*").ID("Postgres")).ID("buildGetAllWebhooksQuery").Params().Params(jen.ID("string")).Block(
	...
	ret.Add(jen.Func().Comment("// GetAllWebhooks fetches a list of all webhooks from the database").Params(jen.ID("p").Op("*").ID("Postgres")).ID("GetAllWebhooks").Params(jen.ID("ctx").Qual("context", "Context")).Params(jen.Op("*").ID("models").Dot(
		"WebhookList",
	), jen.ID("error")).Block(
		...
		jen.List(jen.ID("list"), jen.ID("err")).Op(":=").ID("scanWebhooks").Call(jen.ID("p").Dot(
			"logger",
		), jen.ID("rows")),
		jen.If(jen.ID("err").Op("!=").ID("nil")).Block(
	...
	ret.Add(jen.Func().Comment("// GetAllWebhooksForUser fetches a list of all webhooks from the database").Params(jen.ID("p").Op("*").ID("Postgres")).ID("GetAllWebhooksForUser").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64")).Params(jen.Index().ID("models").Dot(
		"Webhook",
	), jen.ID("error")).Block(
		jen.List(jen.ID("query"), jen.ID("args")).Op(":=").ID("p").Dot(
	...
	ret.Add(jen.Func().Comment("// buildGetWebhooksQuery returns a SQL query (and arguments) that would return a").Params(jen.ID("p").Op("*").ID("Postgres")).ID("buildGetWebhooksQuery").Params(jen.ID("filter").Op("*").ID("models").Dot(
		"QueryFilter",
	), jen.ID("userID").ID("uint64")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.ID("builder").Op(":=").ID("p").Dot(
	...
	ret.Add(jen.Func().Comment("// GetWebhooks fetches a list of webhooks from the database that meet a particular filter").Params(jen.ID("p").Op("*").ID("Postgres")).ID("GetWebhooks").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("models").Dot(
		"QueryFilter",
	...
	ret.Add(jen.Func().Comment("// buildWebhookCreationQuery returns a SQL query (and arguments) that would create a given webhook").Params(jen.ID("p").Op("*").ID("Postgres")).ID("buildWebhookCreationQuery").Params(jen.ID("x").Op("*").ID("models").Dot(
		"Webhook",
	...
	ret.Add(jen.Func().Comment("// CreateWebhook creates a webhook in the database").Params(jen.ID("p").Op("*").ID("Postgres")).ID("CreateWebhook").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("input").Op("*").ID("models").Dot(
	...
	ret.Add(jen.Func().Comment("// buildUpdateWebhookQuery takes a given webhook and returns a SQL query to update").Params(jen.ID("p").Op("*").ID("Postgres")).ID("buildUpdateWebhookQuery").Params(jen.ID("input").Op("*").ID("models").Dot(
		"Webhook",
	)).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.List(jen.ID("query"), jen.ID("args"), jen.ID("err")).Op("=").ID("p").Dot(
	...
	ret.Add(jen.Func().Comment("// UpdateWebhook updates a particular webhook. Note that UpdateWebhook expects the provided input to have a valid ID.").Params(jen.ID("p").Op("*").ID("Postgres")).ID("UpdateWebhook").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("input").Op("*").ID("models").Dot(
		"Webhook",
	...
	ret.Add(jen.Func().Comment("// buildArchiveWebhookQuery returns a SQL query (and arguments) that will mark a webhook as archived.").Params(jen.ID("p").Op("*").ID("Postgres")).ID("buildArchiveWebhookQuery").Params(jen.List(jen.ID("webhookID"), jen.ID("userID")).ID("uint64")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.List(jen.ID("query"), jen.ID("args"), jen.ID("err")).Op("=").ID("p").Dot(
			"sqlBuilder",
		).Dot(
	...
	ret.Add(jen.Func().Comment("// ArchiveWebhook archives a webhook from the database by its ID").Params(jen.ID("p").Op("*").ID("Postgres")).ID("ArchiveWebhook").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("webhookID"), jen.ID("userID")).ID("uint64")).Params(jen.ID("error")).Block(
		jen.List(jen.ID("query"), jen.ID("args")).Op(":=").ID("p").Dot(
