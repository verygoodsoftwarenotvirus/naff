	...
	ret.Add(
		jen.Func().ID("buildMockRowFromItem").Params(jen.ID("item").Op("*").ID("models").Dot("Item")).Params(jen.Op("*").ID("sqlmock").Dot(
			...
			jen.ID("exampleRows").Op(":=").ID("sqlmock").Dot(
				...
				"AddRow",
			).Call(jen.ID("item").Dot("ID"),
				jen.ID("item").Dot("Name"),
				jen.ID("item").Dot(
					"Details",
				),
				jen.ID("item").Dot("CreatedOn"),
				jen.ID("item").Dot("UpdatedOn"),
				jen.ID("item").Dot("ArchivedOn"),
				jen.ID("item").Dot("BelongsTo")),
	...
	ret.Add(
		jen.Func().ID("buildErroneousMockRowFromItem").Params(jen.ID("item").Op("*").ID("models").Dot("Item")).Params(jen.Op("*").ID("sqlmock").Dot(
			...
			jen.ID("exampleRows").Op(":=").ID("sqlmock").Dot(
				...
				"AddRow",
			).Call(jen.ID("item").Dot("ArchivedOn"),
				jen.ID("item").Dot("Name"),
				jen.ID("item").Dot(
					"Details",
				),
		...
		jen.Func().ID("TestPostgres_CreateItem").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("with error writing to database"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("mockDB").Dot(
					...
					"WithArgs",
				).Call(jen.ID("example").Dot("Name"),
					jen.ID("example").Dot(
						"Details",
					),
					jen.ID("example").Dot("BelongsTo")).Dot(
					"WillReturnError",
				).Call(jen.Qual("errors", "New").Call(jen.Lit("blah"))),
				jen.List(jen.ID("actual"), jen.ID("err")).Op(":=").ID("p").Dot(
					"CreateItem",
		...
		jen.Func().ID("TestPostgres_ArchiveItem").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			...
			jen.ID("T").Dot("Run").Call(jen.Lit("with error writing to database"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
				...
				jen.ID("mockDB").Dot(
					...
					"WithArgs",
				).Call(jen.ID("example").Dot("BelongsTo"),
					jen.ID("example").Dot("ID")).Dot(
					"WillReturnError",
				).Call(jen.Qual("errors", "New").Call(jen.Lit("blah"))),
				jen.ID("err").Op(":=").ID("p").Dot(
					"ArchiveItem",
				).Call(jen.Qual("context", "Background").Call(), jen.ID("example").Dot("ID"),
					jen.ID("expectedUserID")),
				jen.ID("assert").Dot("Error").Call(jen.ID("t"), jen.ID("err")),
				jen.ID("assert").Dot("NoError").Call(jen.ID("t"), jen.ID("mockDB").Dot(
					"ExpectationsWereMet",
