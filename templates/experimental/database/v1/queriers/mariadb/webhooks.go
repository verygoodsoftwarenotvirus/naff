...
func webhooksDotGo() *jen.File {
	...
	ret.Add(
		jen.Func().Comment("// GetWebhookCount will fetch the count of webhooks from the database that meet a particular filter,").Comment("// and belong to a particular user.").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("GetWebhookCount").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("models").Dot(
		"QueryFilter",
	...
	ret.Add(
		jen.Func().Comment("// buildGetAllWebhooksQuery returns a SQL query which will return all webhooks, regardless of ownership").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("buildGetAllWebhooksQuery").Params().Params(jen.ID("string")).Block(
