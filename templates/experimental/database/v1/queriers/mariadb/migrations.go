		...
		jen.Var().Defs(
			jen.ID("migrations").Op("=").Index().Qual("github.com/GuiaBolso/darwin", "Migration").Valuesln(
				jen.Valuesln(
					...
					jen.ID("Script").Op(":").Qual("strings", "Join").Call(jen.Index().ID("string").Valuesln(
				...
				jen.Valuesln(
					jen.ID("Version").Op(":").Lit(2),
					jen.ID("Description").Op(":").Lit("create users table creation trigger"),
					jen.ID("Script").Op(":").Qual("strings", "Join").Call(jen.Index().ID("string").Valuesln(
				...
				jen.Valuesln(
					jen.ID("Version").Op(":").Lit(3),
					jen.ID("Description").Op(":").Lit("Add oauth2_clients table"),
					jen.ID("Script").Op(":").Qual("strings", "Join").Call(jen.Index().ID("string").Valuesln(
				...
				jen.Valuesln(
					jen.ID("Version").Op(":").Lit(4),
					jen.ID("Description").Op(":").Lit("create oauth2_clients table creation trigger"),
					jen.ID("Script").Op(":").Qual("strings", "Join").Call(jen.Index().ID("string").Valuesln(
				...
				jen.Valuesln(
					jen.ID("Version").Op(":").Lit(6),
					jen.ID("Description").Op(":").Lit("create webhooks table creation trigger"),
					jen.ID("Script").Op(":").Qual("strings", "Join").Call(jen.Index().ID("string").Valuesln(
				...
				jen.Valuesln(
					jen.ID("Version").Op(":").Lit(7),
					jen.ID("Description").Op(":").Lit("create items table"),
					jen.ID("Script").Op(":").Qual("strings", "Join").Call(jen.Index().ID("string").Valuesln(
						jen.Lit("CREATE TABLE IF NOT EXISTS items ("),
						jen.Lit("    `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,"),
						jen.Lit("    `name` VARCHAR(256) NOT NULL,"),
						jen.Lit("    `details` VARCHAR(4096) NOT NULL DEFAULT '',"),
						jen.Lit("    `created_on` BIGINT UNSIGNED,"),
						jen.Lit("    `updated_on` BIGINT UNSIGNED DEFAULT NULL,"),
						jen.Lit("  	 `archived_on` BIGINT UNSIGNED DEFAULT NULL,"),
						jen.Lit("    `belongs_to` BIGINT UNSIGNED NOT NULL,"),
				...
				jen.Valuesln(
					jen.ID("Version").Op(":").Lit(8),
					jen.ID("Description").Op(":").Lit("create items table creation trigger"),
					jen.ID("Script").Op(":").Qual("strings", "Join").Call(jen.Index().ID("string").Valuesln(
						jen.Lit("CREATE TRIGGER IF NOT EXISTS webhooks_creation_trigger BEFORE INSERT ON webhooks FOR EACH ROW"),
						jen.Lit("BEGIN"), jen.Lit("  IF (new.created_on is null)"),
						jen.Lit("  THEN"), jen.Lit("    SET new.created_on = UNIX_TIMESTAMP();"),
						jen.Lit("  END IF;"), jen.Lit("END;"),
					), jen.Lit("\n")),
				),
			),
		),
