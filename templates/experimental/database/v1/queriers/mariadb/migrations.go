...
func migrationsDotGo() *jen.File {
	...
	ret.Add(jen.Func().Comment("// buildMigrationFunc returns a sync.Once compatible function closure that will").Comment("// migrate a MariaDB database").ID("buildMigrationFunc").Params(jen.ID("db").Op("*").Qual("database/sql", "DB")).Params(jen.Params()).Block(
	...
	ret.Add(jen.Func().Comment("// Migrate migrates the database. It does so by invoking the migrateOnce function via sync.Once, so it should be").Comment("// safe (as in idempotent, though not recommended) to call this function multiple times.").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("Migrate").Params(jen.ID("ctx").Qual("context", "Context")).Params(jen.ID("error")).Block(
		jen.ID("m").Dot(
			"logger",
		).Dot(
			"Info",
		).Call(jen.Lit("migrating db")),
		jen.If(jen.Op("!").ID("m").Dot(
			"IsReady",
		...
		jen.ID("m").Dot(
			...
			"Do",
		).Call(jen.ID("buildMigrationFunc").Call(jen.ID("m").Dot(
			"db",
		))),
		jen.Return().ID("nil"),
	),
