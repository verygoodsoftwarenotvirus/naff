...
func itemsDotGo() *jen.File {
	...
	ret.Add(jen.Func().Comment("// GetItem fetches an item from the database").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("GetItem").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("itemID"), jen.ID("userID")).ID("uint64")).Params(jen.Op("*").ID("models").Dot(
		"Item",
	), jen.ID("error")).Block(
	...
	ret.Add(jen.Func().Comment("// GetItemCount will fetch the count of items from the database that meet a particular filter and belong to a particular user.").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("GetItemCount").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("models").Dot(
		"QueryFilter",
	...
	ret.Add(jen.Func().Comment("// buildGetAllItemsCountQuery returns a query that fetches the total number of items in the database.").Comment("// This query only gets generated once, and is otherwise returned from cache.").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("buildGetAllItemsCountQuery").Params().Params(jen.ID("string")).Block(
	...
	ret.Add(jen.Func().Comment("// GetAllItemsCount will fetch the count of items from the database").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("GetAllItemsCount").Params(jen.ID("ctx").Qual("context", "Context")).Params(jen.ID("count").ID("uint64"), jen.ID("err").ID("error")).Block(
		jen.ID("err").Op("=").ID("m").Dot(
			"db",
		).Dot(
			"QueryRowContext",
	...
	ret.Add(jen.Func().Comment("// GetItems fetches a list of items from the database that meet a particular filter").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("GetItems").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("models").Dot(
		"QueryFilter",
	...
	ret.Add(jen.Func().Comment("// GetAllItemsForUser fetches every item belonging to a user").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("GetAllItemsForUser").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64")).Params(jen.Index().ID("models").Dot(
		"Item",
	), jen.ID("error")).Block(
		jen.List(jen.ID("query"), jen.ID("args")).Op(":=").ID("m").Dot(
			"buildGetItemsQuery",
	...
	ret.Add(jen.Func().Comment("// buildCreateItemQuery takes an item and returns a creation query for that item and the relevant arguments.").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("buildCreateItemQuery").Params(jen.ID("input").Op("*").ID("models").Dot(
		"Item",
	...
	ret.Add(jen.Func().Comment("// buildCreateItemQuery takes an item and returns a creation query for that item and the relevant arguments.").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("buildItemCreationTimeQuery").Params(jen.ID("itemID").ID("uint64")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.List(jen.ID("query"), jen.ID("args"), jen.ID("err")).Op("=").ID("m").Dot(
			"sqlBuilder",
		).Dot(
	...
	ret.Add(jen.Func().Comment("// CreateItem creates an item in the database").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("CreateItem").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("input").Op("*").ID("models").Dot(
		"ItemCreationInput",
	)).Params(jen.Op("*").ID("models").Dot(
		"Item",
	), jen.ID("error")).Block(
	...
	ret.Add(jen.Func().Comment("// buildUpdateItemQuery takes an item and returns an update SQL query, with the relevant query parameters").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("buildUpdateItemQuery").Params(jen.ID("input").Op("*").ID("models").Dot(
		"Item",
	)).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.List(jen.ID("query"), jen.ID("args"), jen.ID("err")).Op("=").ID("m").Dot(
	...
	ret.Add(jen.Func().Comment("// UpdateItem updates a particular item. Note that UpdateItem expects the provided input to have a valid ID.").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("UpdateItem").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("input").Op("*").ID("models").Dot(
		"Item",
	...
	ret.Add(jen.Func().Comment("// buildArchiveItemQuery returns a SQL query which marks a given item belonging to a given user as archived.").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("buildArchiveItemQuery").Params(jen.List(jen.ID("itemID"), jen.ID("userID")).ID("uint64")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.List(jen.ID("query"), jen.ID("args"), jen.ID("err")).Op("=").ID("m").Dot(
			"sqlBuilder",
		).Dot(
	...
	ret.Add(jen.Func().Comment("// ArchiveItem marks an item as archived in the database").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("ArchiveItem").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("itemID"), jen.ID("userID")).ID("uint64")).Params(jen.ID("error")).Block(
		jen.List(jen.ID("query"), jen.ID("args")).Op(":=").ID("m").Dot(
