...
func usersDotGo() *jen.File {
	...
	ret.Add(
		jen.Func().Comment("// buildGetUserCountQuery returns a SQL query (and arguments) for retrieving the number of users who adhere").Comment("// to a given filter's criteria.").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("buildGetUserCountQuery").Params(jen.ID("filter").Op("*").ID("models").Dot(
			"QueryFilter",
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("m").Op("*").ID("MariaDB")).ID("GetUserCount").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("models").Dot(
			"QueryFilter",
	...
	ret.Add(
		jen.Func().Comment("// buildGetUserCountQuery returns a SQL query (and arguments) for retrieving a slice of users who adhere").Comment("// to a given filter's criteria.").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("buildGetUsersQuery").Params(jen.ID("filter").Op("*").ID("models").Dot(
			"QueryFilter",
		)).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("m").Op("*").ID("MariaDB")).ID("GetUsers").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("models").Dot(
			"QueryFilter",
		)).Params(jen.Op("*").ID("models").Dot(
			"UserList",
		), jen.ID("error")).Block(
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("m").Op("*").ID("MariaDB")).ID("CreateUser").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("input").Op("*").ID("models").Dot(
			"UserInput",
		)).Params(jen.Op("*").ID("models").Dot(
			"User",
		), jen.ID("error")).Block(
