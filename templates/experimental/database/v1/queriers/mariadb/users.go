...
func usersDotGo() *jen.File {
	...
	ret.Add(jen.Func().Comment("// scanUsers takes database rows and loads them into a slice of User structs").ID("scanUsers").Params(jen.ID("logger").ID("logging").Dot(
		"Logger",
	...
	ret.Add(jen.Func().Comment("// buildGetUserQuery returns a SQL query (and argument) for retrieving a user by their database ID").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("buildGetUserQuery").Params(jen.ID("userID").ID("uint64")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
	...
	ret.Add(jen.Func().Comment("// GetUser fetches a user").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("GetUser").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64")).Params(jen.Op("*").ID("models").Dot(
		"User",
	), jen.ID("error")).Block(
	...
	ret.Add(jen.Func().Comment("// buildGetUserByUsernameQuery returns a SQL query (and argument) for retrieving a user by their username").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("buildGetUserByUsernameQuery").Params(jen.ID("username").ID("string")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.List(jen.ID("query"), jen.ID("args"), jen.ID("err")).Op("=").ID("m").Dot(
			"sqlBuilder",
		).Dot(
	...
	ret.Add(jen.Func().Comment("// GetUserByUsername fetches a user by their username").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("GetUserByUsername").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("username").ID("string")).Params(jen.Op("*").ID("models").Dot(
		"User",
	), jen.ID("error")).Block(
		jen.List(jen.ID("query"), jen.ID("args")).Op(":=").ID("m").Dot(
	...
	ret.Add(jen.Func().Comment("// buildGetUserCountQuery returns a SQL query (and arguments) for retrieving the number of users who adhere").Comment("// to a given filter's criteria.").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("buildGetUserCountQuery").Params(jen.ID("filter").Op("*").ID("models").Dot(
		"QueryFilter",
	...
	ret.Add(jen.Func().Comment("// GetUserCount fetches a count of users from the database that meet a particular filter").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("GetUserCount").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("models").Dot(
		"QueryFilter",
	...
	ret.Add(jen.Func().Comment("// buildGetUserCountQuery returns a SQL query (and arguments) for retrieving a slice of users who adhere").Comment("// to a given filter's criteria.").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("buildGetUsersQuery").Params(jen.ID("filter").Op("*").ID("models").Dot(
		"QueryFilter",
	)).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.ID("builder").Op(":=").ID("m").Dot(
	...
	ret.Add(jen.Func().Comment("// GetUsers fetches a list of users from the database that meet a particular filter").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("GetUsers").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("models").Dot(
		"QueryFilter",
	)).Params(jen.Op("*").ID("models").Dot(
		"UserList",
	), jen.ID("error")).Block(
	...
	ret.Add(jen.Func().Comment("// buildCreateUserQuery returns a SQL query (and arguments) that would create a given User").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("buildCreateUserQuery").Params(jen.ID("input").Op("*").ID("models").Dot(
		"UserInput",
	)).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.List(jen.ID("query"), jen.ID("args"), jen.ID("err")).Op("=").ID("m").Dot(
	...
	ret.Add(jen.Func().Comment("// buildCreateUserQuery returns a SQL query (and arguments) that would create a given User").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("buildUserCreationTimeQuery").Params(jen.ID("userID").ID("uint64")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.List(jen.ID("query"), jen.ID("args"), jen.ID("err")).Op("=").ID("m").Dot(
			"sqlBuilder",
		).Dot(
	...
	ret.Add(jen.Func().Comment("// CreateUser creates a user").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("CreateUser").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("input").Op("*").ID("models").Dot(
		"UserInput",
	)).Params(jen.Op("*").ID("models").Dot(
		"User",
	), jen.ID("error")).Block(
		...
		jen.If(jen.ID("err").Op("!=").ID("nil")).Block(
			jen.Switch(jen.ID("e").Op(":=").ID("err").Assert(jen.Type())).Block(
				jen.Case(jen.Op("*").ID("pq").Dot(
					"Error",
				)).Block(jen.If(jen.ID("e").Dot(
					"Code",
				).Op("==").ID("pq").Dot(
					"ErrorCode",
				...
				jen.Default().Block(jen.Return().List(jen.ID("nil"), jen.Qual("fmt", "Errorf").Call(jen.Lit("error executing user creation query: %w"), jen.ID("err")))),
			),
		),
	...
	ret.Add(jen.Func().Comment("// buildUpdateUserQuery returns a SQL query (and arguments) that would update the given user's row").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("buildUpdateUserQuery").Params(jen.ID("input").Op("*").ID("models").Dot(
		"User",
	)).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.List(jen.ID("query"), jen.ID("args"), jen.ID("err")).Op("=").ID("m").Dot(
	...
	ret.Add(jen.Func().Params(jen.ID("m").Op("*").ID("MariaDB")).ID("buildArchiveUserQuery").Params(jen.ID("userID").ID("uint64")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.List(jen.ID("query"), jen.ID("args"), jen.ID("err")).Op("=").ID("m").Dot(
			"sqlBuilder",
		).Dot(
	...
	ret.Add(jen.Func().Comment("// ArchiveUser archives a user by their username").Params(jen.ID("m").Op("*").ID("MariaDB")).ID("ArchiveUser").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64")).Params(jen.ID("error")).Block(
		jen.List(jen.ID("query"), jen.ID("args")).Op(":=").ID("m").Dot(
