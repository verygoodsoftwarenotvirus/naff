...
func mariadbTestDotGo() *jen.File {
	...
	ret.Add(
		jen.Func().ID("buildTestService").Params(jen.ID("t").Op("*").Qual("testing", "T")).Params(jen.Op("*").ID("MariaDB"), jen.Qual("github.com/DATA-DOG/go-sqlmock",
			"Sqlmock",
		)).Block(
	...
	ret.Add(
		jen.Func().ID("TestProvideMariaDB").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
	...
	ret.Add(
		jen.Func().ID("TestMariaDB_IsReady").Params(jen.ID("T").Op("*").Qual("testing", "T")).Block(
			jen.ID("T").Dot("Parallel").Call(),
			jen.Line(),
			jen.ID("T").Dot("Run").Call(jen.Lit("obligatory"), jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Block(
