...
func sqliteDotGo() *jen.File {
	...
	ret.Add(jen.Null().Type().ID("Sqlite").Struct(jen.ID("logger").ID("logging").Dot(
		"Logger",
	...
	ret.Add(jen.Func().Comment("// ProvideSqliteDB provides an instrumented sqlite db").ID("ProvideSqliteDB").Params(jen.ID("logger").ID("logging").Dot(
		"Logger",
	...
	ret.Add(jen.Func().Comment("// ProvideSqlite provides a sqlite db controller").ID("ProvideSqlite").Params(jen.ID("debug").ID("bool"), jen.ID("db").Op("*").Qual("database/sql", "DB"), jen.ID("logger").ID("logging").Dot(
		"Logger",
	...
	ret.Add(jen.Func().Comment("// IsReady reports whether or not the db is ready").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("IsReady").Params(jen.ID("ctx").Qual("context", "Context")).Params(jen.ID("ready").ID("bool")).Block(
		jen.Return().ID("true"),
	),

		jen.Line(),
	)
	ret.Add(jen.Func().Comment("// s.logQueryBuildingError logs errors that may occur during query construction.").Comment("// Such errors should be few and far between, as the generally only occur with").Comment("// type discrepancies or other misuses of SQL. An alert should be set up for").Comment("// any log entries with the given name, and those alerts should be investigated").Comment("// with the utmost priority.").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("logQueryBuildingError").Params(jen.ID("err").ID("error")).Block(
