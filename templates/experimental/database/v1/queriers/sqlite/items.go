...
func itemsDotGo() *jen.File {
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Sqlite")).ID("GetItems").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/models/v1", "QueryFilter"),
			...
			jen.List(jen.ID("list"), jen.ID("err")).Op(":=").ID("scanItems").Call(jen.ID("s").Dot("logger"),
				jen.ID("rows")),
			jen.If(jen.ID("err").Op("!=").ID("nil")).Block(
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("Sqlite")).ID("buildArchiveItemQuery").Params(jen.List(jen.ID("itemID"), jen.ID("userID")).ID("uint64")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
			...
			jen.List(jen.ID("query"), jen.ID("args"), jen.ID("err")).Op("=").ID("s").Dot(
				...
				"Update",
			).Call(jen.ID("itemsTableName")).Dot("Set").Call(jen.Lit("updated_on"), jen.Qual("github.com/Masterminds/squirrel", "Expr").Call(jen.ID("CurrentUnixTimeQuery"))).Dot("Set").Call(jen.Lit("archived_on"), jen.Qual("github.com/Masterminds/squirrel", "Expr").Call(jen.ID("CurrentUnixTimeQuery"))).Dot(
				"Where",
			).Call(jen.Qual("github.com/Masterminds/squirrel", "Eq").Valuesln(
