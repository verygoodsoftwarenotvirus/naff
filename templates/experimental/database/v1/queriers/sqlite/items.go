...
func itemsDotGo() *jen.File {
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("s").Op("*").ID("Sqlite")).ID("buildGetAllItemsCountQuery").Params().Params(jen.ID("string")).Block(
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("s").Op("*").ID("Sqlite")).ID("buildGetItemsQuery").Params(jen.ID("filter").Op("*").ID("models").Dot(
			"QueryFilter",
		),
			jen.ID("userID").ID("uint64")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("s").Op("*").ID("Sqlite")).ID("buildUpdateItemQuery").Params(jen.ID("input").Op("*").ID("models").Dot(
			"Item",
		)).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
			...
			jen.List(jen.ID("query"), jen.ID("args"), jen.ID("err")).Op("=").ID("s").Dot(
				...
				"Update",
			).Call(jen.ID("itemsTableName")).Dot("Set").Call(jen.Lit("name"), jen.ID("input").Dot(
				"Name",
			)).Dot("Set").Call(jen.Lit("details"), jen.ID("input").Dot(
				"Details",
