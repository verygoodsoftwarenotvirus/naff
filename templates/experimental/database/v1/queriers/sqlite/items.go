...
func itemsDotGo() *jen.File {
	...
	ret.Add(jen.Func().Comment("// GetItem fetches an item from the sqlite database").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("GetItem").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("itemID"), jen.ID("userID")).ID("uint64")).Params(jen.Op("*").ID("models").Dot(
		"Item",
	), jen.ID("error")).Block(
	...
	ret.Add(jen.Func().Comment("// GetItemCount will fetch the count of items from the database that meet a particular filter and belong to a particular user.").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("GetItemCount").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("models").Dot(
		"QueryFilter",
	...
	ret.Add(jen.Func().Comment("// buildGetAllItemsCountQuery returns a query that fetches the total number of items in the database.").Comment("// This query only gets generated once, and is otherwise returned from cache.").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("buildGetAllItemsCountQuery").Params().Params(jen.ID("string")).Block(
		jen.ID("allItemsCountQueryBuilder").Dot(
			"Do",
		).Call(jen.Func().Params().Block(
			jen.Null().Var().ID("err").ID("error"),
			jen.List(jen.ID("allItemsCountQuery"), jen.ID("_"), jen.ID("err")).Op("=").ID("s").Dot(
				"sqlBuilder",
			).Dot(
				"Select",
			).Call(jen.ID("CountQuery")).Dot(
	...
	ret.Add(jen.Func().Comment("// GetAllItemsCount will fetch the count of items from the database").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("GetAllItemsCount").Params(jen.ID("ctx").Qual("context", "Context")).Params(jen.ID("count").ID("uint64"), jen.ID("err").ID("error")).Block(
		jen.ID("err").Op("=").ID("s").Dot(
			"db",
		).Dot(
			"QueryRowContext",
	...
	ret.Add(jen.Func().Comment("// buildGetItemsQuery builds a SQL query selecting items that adhere to a given QueryFilter and belong to a given user,").Comment("// and returns both the query and the relevant args to pass to the query executor.").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("buildGetItemsQuery").Params(jen.ID("filter").Op("*").ID("models").Dot(
		"QueryFilter",
	), jen.ID("userID").ID("uint64")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.ID("builder").Op(":=").ID("s").Dot(
	...
	ret.Add(jen.Func().Comment("// GetItems fetches a list of items from the database that meet a particular filter").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("GetItems").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("models").Dot(
		"QueryFilter",
		...
		jen.List(jen.ID("list"), jen.ID("err")).Op(":=").ID("scanItems").Call(jen.ID("s").Dot(
			"logger",
		), jen.ID("rows")),
		jen.If(jen.ID("err").Op("!=").ID("nil")).Block(
	...
	ret.Add(jen.Func().Comment("// GetAllItemsForUser fetches every item belonging to a user").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("GetAllItemsForUser").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64")).Params(jen.Index().ID("models").Dot(
		"Item",
	), jen.ID("error")).Block(
		jen.List(jen.ID("query"), jen.ID("args")).Op(":=").ID("s").Dot(
			"buildGetItemsQuery",
	...
	ret.Add(jen.Func().Comment("// buildCreateItemQuery takes an item and returns a creation query for that item and the relevant arguments.").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("buildCreateItemQuery").Params(jen.ID("input").Op("*").ID("models").Dot(
		"Item",
	...
	ret.Add(jen.Func().Comment("// buildCreateItemQuery takes an item and returns a creation query for that item and the relevant arguments.").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("buildItemCreationTimeQuery").Params(jen.ID("itemID").ID("uint64")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.List(jen.ID("query"), jen.ID("args"), jen.ID("err")).Op("=").ID("s").Dot(
			"sqlBuilder",
		).Dot(
	...
	ret.Add(jen.Func().Comment("// CreateItem creates an item in the database").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("CreateItem").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("input").Op("*").ID("models").Dot(
		"ItemCreationInput",
	)).Params(jen.Op("*").ID("models").Dot(
		"Item",
	), jen.ID("error")).Block(
	...
	ret.Add(jen.Func().Comment("// buildUpdateItemQuery takes an item and returns an update SQL query, with the relevant query parameters").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("buildUpdateItemQuery").Params(jen.ID("input").Op("*").ID("models").Dot(
		"Item",
	)).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.List(jen.ID("query"), jen.ID("args"), jen.ID("err")).Op("=").ID("s").Dot(
	...
	ret.Add(jen.Func().Comment("// UpdateItem updates a particular item. Note that UpdateItem expects the provided input to have a valid ID.").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("UpdateItem").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("input").Op("*").ID("models").Dot(
		"Item",
	...
	ret.Add(jen.Func().Comment("// buildArchiveItemQuery returns a SQL query which marks a given item belonging to a given user as archived.").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("buildArchiveItemQuery").Params(jen.List(jen.ID("itemID"), jen.ID("userID")).ID("uint64")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.List(jen.ID("query"), jen.ID("args"), jen.ID("err")).Op("=").ID("s").Dot(
			"sqlBuilder",
		).Dot(
	...
	ret.Add(jen.Func().Comment("// ArchiveItem marks an item as archived in the database").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("ArchiveItem").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("itemID"), jen.ID("userID")).ID("uint64")).Params(jen.ID("error")).Block(
		jen.List(jen.ID("query"), jen.ID("args")).Op(":=").ID("s").Dot(
