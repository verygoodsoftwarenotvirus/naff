...
func oauth2ClientsDotGo() *jen.File {
	...
	ret.Add(jen.Func().Comment("// scanOAuth2Clients takes sql rows and turns them into a slice of OAuth2Clients").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("scanOAuth2Clients").Params(jen.ID("rows").Op("*").Qual("database/sql", "Rows")).Params(jen.Index().Op("*").ID("models").Dot(
		"OAuth2Client",
	), jen.ID("error")).Block(
	...
	ret.Add(jen.Func().Comment("// buildGetOAuth2ClientByClientIDQuery builds a SQL query for fetching an OAuth2 client by its ClientID").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("buildGetOAuth2ClientByClientIDQuery").Params(jen.ID("clientID").ID("string")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
	...
	ret.Add(jen.Func().Comment("// GetOAuth2ClientByClientID gets an OAuth2 client").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("GetOAuth2ClientByClientID").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("clientID").ID("string")).Params(jen.Op("*").ID("models").Dot(
		"OAuth2Client",
	), jen.ID("error")).Block(
	...
	ret.Add(jen.Func().Comment("// buildGetAllOAuth2ClientsQuery builds a SQL query").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("buildGetAllOAuth2ClientsQuery").Params().Params(jen.ID("query").ID("string")).Block(
	...
	ret.Add(jen.Func().Comment("// GetAllOAuth2Clients gets a list of OAuth2 clients regardless of ownership").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("GetAllOAuth2Clients").Params(jen.ID("ctx").Qual("context", "Context")).Params(jen.Index().Op("*").ID("models").Dot(
		"OAuth2Client",
	), jen.ID("error")).Block(
	...
	ret.Add(jen.Func().Comment("// GetAllOAuth2ClientsForUser gets a list of OAuth2 clients belonging to a given user").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("GetAllOAuth2ClientsForUser").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64")).Params(jen.Index().Op("*").ID("models").Dot(
		"OAuth2Client",
	), jen.ID("error")).Block(
		jen.List(jen.ID("query"), jen.ID("args")).Op(":=").ID("s").Dot(
	...
	ret.Add(jen.Func().Comment("// buildGetOAuth2ClientQuery returns a SQL query which requests a given OAuth2 client by its database ID").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("buildGetOAuth2ClientQuery").Params(jen.List(jen.ID("clientID"), jen.ID("userID")).ID("uint64")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.List(jen.ID("query"), jen.ID("args"), jen.ID("err")).Op("=").ID("s").Dot(
			"sqlBuilder",
		).Dot(
	...
	ret.Add(jen.Func().Comment("// GetOAuth2Client retrieves an OAuth2 client from the database").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("GetOAuth2Client").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("clientID"), jen.ID("userID")).ID("uint64")).Params(jen.Op("*").ID("models").Dot(
		"OAuth2Client",
	), jen.ID("error")).Block(
		jen.List(jen.ID("query"), jen.ID("args")).Op(":=").ID("s").Dot(
	...
	ret.Add(jen.Func().Comment("// GetOAuth2ClientCount will get the count of OAuth2 clients that match the given filter and belong to the user").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("GetOAuth2ClientCount").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("models").Dot(
		"QueryFilter",
	...
	ret.Add(jen.Func().Comment("// buildGetAllOAuth2ClientCountQuery returns a SQL query for the number of OAuth2 clients").Comment("// in the database, regardless of ownership.").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("buildGetAllOAuth2ClientCountQuery").Params().Params(jen.ID("string")).Block(
	...
	ret.Add(jen.Func().Comment("// GetAllOAuth2ClientCount will get the count of OAuth2 clients that match the current filter").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("GetAllOAuth2ClientCount").Params(jen.ID("ctx").Qual("context", "Context")).Params(jen.ID("uint64"), jen.ID("error")).Block(
	...
	ret.Add(jen.Func().Comment("// GetOAuth2Clients gets a list of OAuth2 clients").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("GetOAuth2Clients").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("models").Dot(
		"QueryFilter",
	...
	ret.Add(jen.Func().Comment("// buildCreateOAuth2ClientQuery returns a SQL query (and args) that will create the given OAuth2Client in the database").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("buildCreateOAuth2ClientQuery").Params(jen.ID("input").Op("*").ID("models").Dot(
		"OAuth2Client",
	...
	ret.Add(jen.Func().Comment("// buildCreateItemQuery takes an item and returns a creation query for that item and the relevant arguments.").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("buildOAuth2ClientCreationTimeQuery").Params(jen.ID("clientID").ID("uint64")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.List(jen.ID("query"), jen.ID("args"), jen.ID("err")).Op("=").ID("s").Dot(
			"sqlBuilder",
		).Dot(
	...
	ret.Add(jen.Func().Comment("// CreateOAuth2Client creates an OAuth2 client").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("CreateOAuth2Client").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("input").Op("*").ID("models").Dot(
	...
	ret.Add(jen.Func().Comment("// buildUpdateOAuth2ClientQuery returns a SQL query (and args) that will update a given OAuth2 client in the database").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("buildUpdateOAuth2ClientQuery").Params(jen.ID("input").Op("*").ID("models").Dot(
		"OAuth2Client",
	)).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.List(jen.ID("query"), jen.ID("args"), jen.ID("err")).Op("=").ID("s").Dot(
	...
	ret.Add(jen.Func().Comment("// UpdateOAuth2Client updates a OAuth2 client.").Comment("// NOTE: this function expects the input's ID field to be valid and non-zero.").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("UpdateOAuth2Client").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("input").Op("*").ID("models").Dot(
		"OAuth2Client",
	...
	ret.Add(jen.Func().Comment("// buildArchiveOAuth2ClientQuery returns a SQL query (and arguments) that will mark an OAuth2 client as archived.").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("buildArchiveOAuth2ClientQuery").Params(jen.List(jen.ID("clientID"), jen.ID("userID")).ID("uint64")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Block(
		jen.Null().Var().ID("err").ID("error"),
		jen.List(jen.ID("query"), jen.ID("args"), jen.ID("err")).Op("=").ID("s").Dot(
			"sqlBuilder",
		).Dot(
	...
	ret.Add(jen.Func().Comment("// ArchiveOAuth2Client archives an OAuth2 client").Params(jen.ID("s").Op("*").ID("Sqlite")).ID("ArchiveOAuth2Client").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("clientID"), jen.ID("userID")).ID("uint64")).Params(jen.ID("error")).Block(
		jen.List(jen.ID("query"), jen.ID("args")).Op(":=").ID("s").Dot(
