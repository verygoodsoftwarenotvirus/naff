...
func roundtripperBaseDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("userAgentHeader").Op("=").Lit("User-Agent").Var().ID("userAgent").Op("=").Lit("TODO Service Client").Var().ID("maxRetryCount").Op("=").Lit(5).Var().ID("minRetryWait").Op("=").Lit(100).Op("*").Qual("time", "Millisecond").Var().ID("maxRetryWait").Op("=").Qual("time", "Second").Var().ID("keepAlive").Op("=").Lit(30).Op("*").Qual("time", "Second").Var().ID("tlsHandshakeTimeout").Op("=").Lit(10).Op("*").Qual("time", "Second").Var().ID("expectContinueTimeout").Op("=").Lit(2).Op("*").ID("defaultTimeout").Var().ID("idleConnTimeout").Op("=").Lit(3).Op("*").ID("defaultTimeout").Var().ID("maxIdleConns").Op("=").Lit(100),
		jen.Line(),
	)

	code.Add(
		jen.Null().Type().ID("defaultRoundTripper").Struct(jen.ID("baseRoundTripper").Qual("net/http", "RoundTripper")),
		jen.Line(),
	)
	...
	code.Add(
		jen.Func().ID("buildRequestLogHook").Params(jen.ID("logger").ID("logging").Dot("Logger")).Params(jen.Params(jen.ID("retryablehttp").Dot("Logger"), jen.Op("*").Qual("net/http", "Request"), jen.ID("int"))).Body(
	...
	code.Add(
		jen.Func().ID("buildResponseLogHook").Params(jen.ID("logger").ID("logging").Dot("Logger")).Params(jen.Params(jen.ID("retryablehttp").Dot("Logger"), jen.Op("*").Qual("net/http", "Response"))).Body(
			jen.ID("logger").Op("=").ID("logging").Dot("EnsureLogger").Call(jen.ID("logger")),
	...
	code.Add(
		jen.Func().ID("buildCheckRetryFunc").Params(jen.ID("tracer").ID("tracing").Dot("Tracer")).Params(jen.Params(jen.Qual("context", "Context"), jen.Op("*").Qual("net/http", "Response"), jen.ID("error")).Params(jen.ID("bool"), jen.ID("error"))).Body(
	...
	code.Add(
		jen.Func().ID("buildErrorHandler").Params(jen.ID("logger").ID("logging").Dot("Logger")).Params(jen.Params(jen.ID("res").Op("*").Qual("net/http", "Response"), jen.ID("err").ID("error"), jen.ID("numTries").ID("int")).Params(jen.Op("*").Qual("net/http", "Response"), jen.ID("error"))).Body(
			jen.ID("logger").Op("=").ID("logging").Dot("EnsureLogger").Call(jen.ID("logger")),
