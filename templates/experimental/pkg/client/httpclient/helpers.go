...
func helpersDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Func().Comment("errorFromResponse returns library errors according to a response's status code.").ID("errorFromResponse").Params(jen.ID("res").Op("*").Qual("net/http", "Response")).Params(jen.ID("error")).Body(
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Comment("if a destination value is valid before decoding an HTTP response, for instance.").ID("argIsNotPointerOrNil").Params(jen.ID("i").Interface()).Params(jen.ID("error")).Body(
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Comment("should only ever be received in testing, and should never be encountered by an end-user.").Params(jen.ID("c").Op("*").ID("Client")).ID("unmarshalBody").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("res").Op("*").Qual("net/http", "Response"), jen.ID("dest").Interface()).Params(jen.ID("error")).Body(
