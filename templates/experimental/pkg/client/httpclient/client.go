...
func clientDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("defaultTimeout").Op("=").Lit(30).Op("*").Qual("time", "Second").Var().ID("clientName").Op("=").Lit("todo_client_v1"),
		jen.Line(),
	)

	code.Add(
		jen.Null().Type().ID("authMethod").Struct(),
		jen.Line(),
	)

	code.Add(
		jen.Var().ID("cookieAuthMethod").Op("=").ID("new").Call(jen.ID("authMethod")).Var().ID("pasetoAuthMethod").Op("=").ID("new").Call(jen.ID("authMethod")).Var().ID("defaultContentType").Op("=").ID("encoding").Dot("ContentTypeJSON").Var().ID("errInvalidResponseCode").Op("=").Qual("errors", "New").Call(jen.Lit("invalid response code")),
		jen.Line(),
	)

	code.Add(
		jen.Null().Type().ID("Client").Struct(
	...
	code.Add(
		jen.Func().Comment("AuthenticatedClient returns the authenticated *httpclient.Client that we use to make most requests.").Params(jen.ID("c").Op("*").ID("Client")).ID("AuthenticatedClient").Params().Params(jen.Op("*").Qual("net/http", "Client")).Body(
			jen.Return().ID("c").Dot("authedClient")),
		jen.Line(),
	)

	code.Add(
		jen.Func().Comment("PlainClient returns the unauthenticated *httpclient.Client that we use to make certain requests.").Params(jen.ID("c").Op("*").ID("Client")).ID("PlainClient").Params().Params(jen.Op("*").Qual("net/http", "Client")).Body(
			jen.Return().ID("c").Dot("unauthenticatedClient")),
		jen.Line(),
	)

	code.Add(
		jen.Func().Comment("URL provides the client's URL.").Params(jen.ID("c").Op("*").ID("Client")).ID("URL").Params().Params(jen.Op("*").Qual("net/url", "URL")).Body(
			jen.Return().ID("c").Dot("url")),
		jen.Line(),
	)

	code.Add(
		jen.Func().Comment("RequestBuilder provides the client's *requests.Builder.").Params(jen.ID("c").Op("*").ID("Client")).ID("RequestBuilder").Params().Params(jen.Op("*").ID("requests").Dot("Builder")).Body(
	...
	code.Add(
		jen.Func().Comment("buildVersionlessURL builds a url without the `/api/v1/` prefix. It should otherwise be identical to buildRawURL.").Params(jen.ID("c").Op("*").ID("Client")).ID("buildVersionlessURL").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("qp").Qual("net/url", "Values"), jen.ID("parts").Op("...").ID("string")).Params(jen.ID("string")).Body(
			jen.List(jen.ID("_"), jen.ID("span")).Op(":=").ID("c").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.ID("tu").Op(":=").Op("*").ID("c").Dot("url"),
	...
	code.Add(
		jen.Func().Comment("IsUp returns whether the service's health endpoint is returning 200s.").Params(jen.ID("c").Op("*").ID("Client")).ID("IsUp").Params(jen.ID("ctx").Qual("context", "Context")).Params(jen.ID("bool")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("c").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
	...
	code.Add(
		jen.Func().Comment("fetchResponseToRequest takes a given *http.Request and executes it with the provided.").Comment("client, alongside some debugging logging.").Params(jen.ID("c").Op("*").ID("Client")).ID("fetchResponseToRequest").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("client").Op("*").Qual("net/http", "Client"), jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.Op("*").Qual("net/http", "Response"), jen.ID("error")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("c").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			...
			jen.Var().ID("bdump").Index().ID("byte"),
