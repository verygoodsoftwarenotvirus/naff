...
func authDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Func().Comment("UserStatus fetches a user's status.").Params(jen.ID("c").Op("*").ID("Client")).ID("UserStatus").Params(jen.ID("ctx").Qual("context", "Context")).Params(jen.Op("*").ID("types").Dot("UserStatusResponse"), jen.ID("error")).Body(
			...
			jen.Var().ID("output").Op("*").ID("types").Dot("UserStatusResponse"),
	...
	code.Add(
		jen.Func().Comment("ChangePassword changes a user's password.").Params(jen.ID("c").Op("*").ID("Client")).ID("ChangePassword").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("cookie").Op("*").Qual("net/http", "Cookie"), jen.ID("input").Op("*").ID("types").Dot("PasswordUpdateInput")).Params(jen.ID("error")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("c").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
	...
	code.Add(
		jen.Func().Comment("CycleTwoFactorSecret cycles a user's 2FA secret.").Params(jen.ID("c").Op("*").ID("Client")).ID("CycleTwoFactorSecret").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("cookie").Op("*").Qual("net/http", "Cookie"), jen.ID("input").Op("*").ID("types").Dot("TOTPSecretRefreshInput")).Params(jen.Op("*").ID("types").Dot("TOTPSecretRefreshResponse"), jen.ID("error")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("c").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.If(jen.ID("cookie").Op("==").ID("nil")).Body(
			...
			jen.Var().ID("output").Op("*").ID("types").Dot("TOTPSecretRefreshResponse"),
			jen.If(jen.ID("err").Op("=").ID("c").Dot("fetchAndUnmarshal").Call(
				jen.ID("ctx"),
				jen.ID("req"),
				jen.Op("&").ID("output"),
	...
	code.Add(
		jen.Func().Comment("VerifyTOTPSecret verifies a 2FA secret.").Params(jen.ID("c").Op("*").ID("Client")).ID("VerifyTOTPSecret").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64"), jen.ID("token").ID("string")).Params(jen.ID("error")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("c").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
