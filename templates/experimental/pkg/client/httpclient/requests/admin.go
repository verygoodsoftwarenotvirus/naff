...
func adminDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("adminBasePath").Op("=").Lit("admin"),
		jen.Line(),
	)

	code.Add(
		jen.Func().Comment("BuildUserReputationUpdateInputRequest builds a request to change a user's reputation.").Params(jen.ID("b").Op("*").ID("Builder")).ID("BuildUserReputationUpdateInputRequest").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("input").Op("*").ID("types").Dot("UserReputationUpdateInput")).Params(jen.Op("*").Qual("net/http", "Request"), jen.ID("error")).Body(
