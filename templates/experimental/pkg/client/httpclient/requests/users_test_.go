...
func usersTestDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		...
		jen.Func().ID("buildArbitraryImage").Params(jen.ID("widthAndHeight").ID("int")).Params(jen.Qual("image", "Image")).Body(
			jen.ID("img").Op(":=").Qual("image", "NewRGBA").Call(jen.Qual("image", "Rectangle").Valuesln(jen.ID("Min").Op(":").Qual("image", "Point").Valuesln(), jen.ID("Max").Op(":").Qual("image", "Point").Valuesln(jen.ID("X").Op(":").ID("widthAndHeight"), jen.ID("Y").Op(":").ID("widthAndHeight")))),
	...
	code.Add(
		jen.Func().ID("TestBuilder_BuildGetAuditLogForUserRequest").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			jen.ID("T").Dot("Parallel").Call(),
			jen.Var().ID("expectedPath").Op("=").Lit("/api/v1/users/%d/audit"),
			jen.ID("T").Dot("Run").Call(
				jen.Lit("standard"),
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					jen.ID("t").Dot("Parallel").Call(),
