...
func authDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("authBasePath").Op("=").Lit("auth"),
		jen.Line(),
	)

	code.Add(
		jen.Func().Comment("BuildUserStatusRequest builds an HTTP request that fetches a user's status.").Params(jen.ID("b").Op("*").ID("Builder")).ID("BuildUserStatusRequest").Params(jen.ID("ctx").Qual("context", "Context")).Params(jen.Op("*").Qual("net/http", "Request"), jen.ID("error")).Body(
	...
	code.Add(
		jen.Func().Comment("BuildLogoutRequest builds an HTTP request that clears the user's session.").Params(jen.ID("b").Op("*").ID("Builder")).ID("BuildLogoutRequest").Params(jen.ID("ctx").Qual("context", "Context")).Params(jen.Op("*").Qual("net/http", "Request"), jen.ID("error")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("b").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.ID("uri").Op(":=").ID("b").Dot("buildUnversionedURL").Call(
				jen.ID("ctx"),
	...
	code.Add(
		jen.Func().Comment("BuildChangePasswordRequest builds a request to change a user's password.").Params(jen.ID("b").Op("*").ID("Builder")).ID("BuildChangePasswordRequest").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("cookie").Op("*").Qual("net/http", "Cookie"), jen.ID("input").Op("*").ID("types").Dot("PasswordUpdateInput")).Params(jen.Op("*").Qual("net/http", "Request"), jen.ID("error")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("b").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.If(jen.ID("input").Op("==").ID("nil")).Body(
				jen.Return().List(jen.ID("nil"), jen.ID("ErrNilInputProvided"))),
	...
	code.Add(
		jen.Func().Comment("BuildCycleTwoFactorSecretRequest builds a request to change a user's 2FA secret.").Params(jen.ID("b").Op("*").ID("Builder")).ID("BuildCycleTwoFactorSecretRequest").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("cookie").Op("*").Qual("net/http", "Cookie"), jen.ID("input").Op("*").ID("types").Dot("TOTPSecretRefreshInput")).Params(jen.Op("*").Qual("net/http", "Request"), jen.ID("error")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("b").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
	...
	code.Add(
		jen.Func().Comment("BuildVerifyTOTPSecretRequest builds a request to validate a 2FA secret.").Params(jen.ID("b").Op("*").ID("Builder")).ID("BuildVerifyTOTPSecretRequest").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64"), jen.ID("token").ID("string")).Params(jen.Op("*").Qual("net/http", "Request"), jen.ID("error")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("b").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
