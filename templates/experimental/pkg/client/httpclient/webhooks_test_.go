...
func webhooksTestDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Null().Type().ID("webhooksTestSuite").Struct(
	...
	code.Add(
		jen.Var().ID("_").ID("suite").Dot("SetupTestSuite").Op("=").Parens(jen.Op("*").ID("webhooksTestSuite")).Call(jen.ID("nil")),
		jen.Line(),
	)
	...
	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("webhooksTestSuite")).ID("TestClient_GetWebhook").Params().Body(
			jen.Var().ID("expectedPathFormat").Op("=").Lit("/api/v1/webhooks/%d"),
	...
	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("webhooksTestSuite")).ID("TestClient_GetWebhooks").Params().Body(
			jen.Var().ID("expectedPath").Op("=").Lit("/api/v1/webhooks"),
			jen.ID("s").Dot("Run").Call(
				jen.Lit("standard"),
				jen.Func().Params().Body(
					jen.ID("t").Op(":=").ID("s").Dot("T").Call(),
	...
	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("webhooksTestSuite")).ID("TestClient_GetAuditLogForWebhook").Params().Body(
			jen.Var().ID("expectedPath").Op("=").Lit("/api/v1/webhooks/%d/audit").Var().ID("expectedMethod").Op("=").Qual("net/http", "MethodGet"),
			jen.ID("s").Dot("Run").Call(
				jen.Lit("standard"),
				jen.Func().Params().Body(
					jen.ID("t").Op(":=").ID("s").Dot("T").Call(),
