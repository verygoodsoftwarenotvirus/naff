...
func adminDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Func().Comment("UpdateUserReputation updates a user's reputation.").Params(jen.ID("c").Op("*").ID("Client")).ID("UpdateUserReputation").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("input").Op("*").ID("types").Dot("UserReputationUpdateInput")).Params(jen.ID("error")).Body(
