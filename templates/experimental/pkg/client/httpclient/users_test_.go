...
func usersTestDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Null().Type().ID("usersBaseSuite").Struct(
	...
	code.Add(
		jen.Var().ID("_").ID("suite").Dot("SetupTestSuite").Op("=").Parens(jen.Op("*").ID("usersBaseSuite")).Call(jen.ID("nil")),
		jen.Line(),
	)
	...
	code.Add(
		jen.Null().Type().ID("usersTestSuite").Struct(
			jen.ID("suite").Dot("Suite"),
	...
	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("usersTestSuite")).ID("TestClient_GetUser").Params().Body(
			jen.Var().ID("expectedPathFormat").Op("=").Lit("/api/v1/users/%d"),
	...
	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("usersTestSuite")).ID("TestClient_GetUsers").Params().Body(
			jen.Var().ID("expectedPath").Op("=").Lit("/api/v1/users"),
			jen.ID("s").Dot("Run").Call(
				jen.Lit("standard"),
				jen.Func().Params().Body(
					jen.ID("t").Op(":=").ID("s").Dot("T").Call(),
	...
	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("usersTestSuite")).ID("TestClient_SearchForUsersByUsername").Params().Body(
			jen.Var().ID("expectedPath").Op("=").Lit("/api/v1/users/search"),
	...
	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("usersTestSuite")).ID("TestClient_CreateUser").Params().Body(
			jen.Var().ID("expectedPath").Op("=").Lit("/users"),
			jen.ID("s").Dot("Run").Call(
				jen.Lit("standard"),
				jen.Func().Params().Body(
					jen.ID("t").Op(":=").ID("s").Dot("T").Call(),
					...
					jen.ID("c").Op(":=").ID("buildTestClientWithRequestBodyValidation").Call(
						jen.ID("t"),
						jen.ID("spec"),
						jen.Op("&").ID("types").Dot("UserRegistrationInput").Valuesln(),
						jen.ID("exampleInput"),
						jen.ID("expected"),
					),
	...
	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("usersTestSuite")).ID("TestClient_GetAuditLogForUser").Params().Body(
			jen.Var().ID("expectedPath").Op("=").Lit("/api/v1/users/%d/audit").Var().ID("expectedMethod").Op("=").Qual("net/http", "MethodGet"),
			jen.ID("s").Dot("Run").Call(
				jen.Lit("standard"),
				jen.Func().Params().Body(
					jen.ID("t").Op(":=").ID("s").Dot("T").Call(),
	...
	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("usersTestSuite")).ID("TestClient_UploadNewAvatar").Params().Body(
			jen.Var().ID("expectedPath").Op("=").Lit("/api/v1/users/avatar/upload"),
			jen.ID("s").Dot("Run").Call(
				jen.Lit("standard"),
				jen.Func().Params().Body(
					jen.ID("t").Op(":=").ID("s").Dot("T").Call(),
