...
func testHelpersTestDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Null().Type().ID("argleBargle").Struct(jen.ID("Name").ID("string")).Type().ID("valuer").Map(jen.ID("string")).Index().ID("string"),
		jen.Line(),
	)
	...
	code.Add(
		jen.Func().Comment("createBodyFromStruct takes any value in and returns an io.Reader for placement within http.NewRequest's last argument.").ID("createBodyFromStruct").Params(jen.ID("t").Op("*").Qual("testing", "T"), jen.ID("in").Interface()).Params(jen.Qual("io", "Reader")).Body(
			jen.ID("t").Dot("Helper").Call(),
