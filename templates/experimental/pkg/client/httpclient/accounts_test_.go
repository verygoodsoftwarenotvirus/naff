...
func accountsTestDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Null().Type().ID("accountsTestSuite").Struct(
	...
	code.Add(
		jen.Var().ID("_").ID("suite").Dot("SetupTestSuite").Op("=").Parens(jen.Op("*").ID("accountsTestSuite")).Call(jen.ID("nil")),
		jen.Line(),
	)
	...
	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("accountsTestSuite")).ID("TestClient_SwitchActiveAccount").Params().Body(
			jen.Var().ID("expectedPath").Op("=").Lit("/users/account/select"),
	...
	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("accountsTestSuite")).ID("TestClient_GetAuditLogForAccount").Params().Body(
			jen.Var().ID("expectedPath").Op("=").Lit("/api/v1/accounts/%d/audit").Var().ID("expectedMethod").Op("=").Qual("net/http", "MethodGet"),
			jen.ID("s").Dot("Run").Call(
				jen.Lit("standard"),
				jen.Func().Params().Body(
					jen.ID("t").Op(":=").ID("s").Dot("T").Call(),
