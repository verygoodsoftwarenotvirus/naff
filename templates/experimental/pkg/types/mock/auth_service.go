...
func authServiceDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("m").Op("*").ID("AuthService")).ID("PermissionFilterMiddleware").Params(jen.ID("perms").Op("...").ID("authorization").Dot("Permission")).Params(jen.Params(jen.ID("next").Qual("net/http", "Handler")).Params(jen.Qual("net/http", "Handler"))).Body(
			jen.Return().ID("m").Dot("Called").Call(jen.ID("perms")).Dot("Get").Call(jen.Lit(0)).Assert(jen.Params(jen.Qual("net/http", "Handler")).Params(jen.Qual("net/http", "Handler")))),
		jen.Line(),
	)
