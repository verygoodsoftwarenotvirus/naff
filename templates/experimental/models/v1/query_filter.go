...
func queryFilterDotGo() *jen.File {
	...
	ret.Add(jen.Null().Type().ID("QueryFilter").Struct(jen.ID("Page").ID("uint64"), jen.ID("Limit").ID("uint64"), jen.ID("CreatedAfter").ID("uint64"), jen.ID("CreatedBefore").ID("uint64"), jen.ID("UpdatedAfter").ID("uint64"), jen.ID("UpdatedBefore").ID("uint64"), jen.ID("SortBy").ID("sortType")),

		jen.Line(),
	)
	ret.Add(jen.Func().Comment("// DefaultQueryFilter builds the default query filter").ID("DefaultQueryFilter").Params().Params(jen.Op("*").ID("QueryFilter")).Block(
	...
	ret.Add(jen.Func().Comment("// FromParams overrides the core QueryFilter values with values retrieved from url.Params").Params(jen.ID("qf").Op("*").ID("QueryFilter")).ID("FromParams").Params(jen.ID("params").Qual("net/url", "Values")).Block(
	...
	ret.Add(jen.Func().Comment("// SetPage sets the current page with certain constraints").Params(jen.ID("qf").Op("*").ID("QueryFilter")).ID("SetPage").Params(jen.ID("page").ID("uint64")).Block(
		jen.ID("qf").Dot(
			"Page",
	...
	ret.Add(jen.Func().Comment("// QueryPage calculates a query page from the current filter values").Params(jen.ID("qf").Op("*").ID("QueryFilter")).ID("QueryPage").Params().Params(jen.ID("uint64")).Block(
	...
	ret.Add(jen.Func().Comment("// ToValues returns a url.Values from a QueryFilter").Params(jen.ID("qf").Op("*").ID("QueryFilter")).ID("ToValues").Params().Params(jen.Qual("net/url", "Values")).Block(
	...
	ret.Add(jen.Func().Comment("// ApplyToQueryBuilder applies the query filter to a query builder").Params(jen.ID("qf").Op("*").ID("QueryFilter")).ID("ApplyToQueryBuilder").Params(jen.ID("queryBuilder").ID("squirrel").Dot(
		"SelectBuilder",
	...
	ret.Add(jen.Func().Comment("// ExtractQueryFilter can extract a QueryFilter from a request").ID("ExtractQueryFilter").Params(jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.Op("*").ID("QueryFilter")).Block(
