...
func itemDotGo() *jen.File {
	...
	ret.Add(
		jen.Type().Defs(
			...
			jen.ID("ItemDataManager").Interface(
				jen.ID("GetItem").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("itemID"), jen.ID("userID")).ID("uint64")).Params(jen.Op("*").ID("Item"), jen.ID("error")),
				jen.ID("GetItemCount").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("QueryFilter"), jen.ID("userID").ID("uint64")).Params(jen.ID("uint64"), jen.ID("error")),
				jen.ID("GetAllItemsCount").Params(jen.ID("ctx").Qual("context", "Context")).Params(jen.ID("uint64"), jen.ID("error")),
				jen.ID("GetItems").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("QueryFilter"), jen.ID("userID").ID("uint64")).Params(jen.Op("*").ID("ItemList"), jen.ID("error")),
				jen.ID("GetAllItemsForUser").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64")).Params(jen.Index().ID("Item"), jen.ID("error")),
				jen.ID("CreateItem").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("input").Op("*").ID("ItemCreationInput")).Params(jen.Op("*").ID("Item"), jen.ID("error")),
				jen.ID("UpdateItem").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("updated").Op("*").ID("Item")).Params(jen.ID("error")),
				jen.ID("ArchiveItem").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("id"), jen.ID("userID")).ID("uint64")).Params(jen.ID("error")),
			),
			jen.ID("ItemDataServer").Interface(
	...
	ret.Add(
		...
		jen.Func().Params(jen.ID("x").Op("*").ID("Item")).ID("Update").Params(jen.ID("input").Op("*").ID("ItemUpdateInput")).Block(
			...
			jen.If(jen.ID("input").Dot("Details").Op("!=").Lit("").Op("||").ID("input").Dot("Details").Op("!=").ID("x").Dot("Details")).Block(
