...
func mockItemDataManagerDotGo() *jen.File {
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("m").Op("*").ID("ItemDataManager")).ID("GetItem").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("itemID"), jen.ID("userID")).ID("uint64")).Params(jen.Op("*").ID("models").Dot("Item"),
			jen.ID("error")).Block(
			...
			jen.Return().List(jen.ID("args").Dot(
				"Get",
			).Call(jen.Lit(0)).Assert(jen.Op("*").ID("models").Dot("Item")), jen.ID("args").Dot("Error").Call(jen.Lit(1))),
		),
		jen.Line(),
	)
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("m").Op("*").ID("ItemDataManager")).ID("GetItemCount").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("models").Dot("QueryFilter"),
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("m").Op("*").ID("ItemDataManager")).ID("GetItems").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("models").Dot("QueryFilter"),
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("m").Op("*").ID("ItemDataManager")).ID("GetAllItemsForUser").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64")).Params(jen.Index().ID("models").Dot("Item"),
			jen.ID("error")).Block(
			jen.ID("args").Op(":=").ID("m").Dot(
				"Called",
			...
			jen.Return().List(jen.ID("args").Dot(
				"Get",
			).Call(jen.Lit(0)).Assert(jen.Index().ID("models").Dot("Item")), jen.ID("args").Dot("Error").Call(jen.Lit(1))),
		),
		jen.Line(),
	)
	...
	ret.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("m").Op("*").ID("ItemDataManager")).ID("UpdateItem").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("updated").Op("*").ID("models").Dot("Item")).Params(jen.ID("error")).Block(
			jen.Return().ID("m").Dot(
				"Called",
