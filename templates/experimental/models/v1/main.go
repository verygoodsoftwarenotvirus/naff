...
func mainDotGo() *jen.File {
	...
	ret.Add(
		jen.Var().ID("SortAscending").ID("sortType").Op("=").Lit("asc").Var().ID("SortDescending").ID("sortType").Op("=").Lit("desc"),
	jen.Line(),
	)

	ret.Add(
		jen.Type().ID("ContextKey").ID("string").Type().ID("sortType").ID("string").Type().ID("Pagination").Struct(jen.ID("Page").ID("uint64"), jen.ID("Limit").ID("uint64"), jen.ID("TotalCount").ID("uint64")).Type().ID("CountResponse").Struct(jen.ID("Count").ID("uint64")),
	jen.Line(),
	)
