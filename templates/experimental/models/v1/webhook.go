...
func webhookDotGo() *jen.File {
	...
	ret.Add(jen.Func().Comment("// Update merges an WebhookCreationInput with an Webhook").Params(jen.ID("w").Op("*").ID("Webhook")).ID("Update").Params(jen.ID("input").Op("*").ID("WebhookUpdateInput")).Block(
	...
	ret.Add(jen.Func().Comment("// ToListener creates a newsman Listener from a Webhook").Params(jen.ID("w").Op("*").ID("Webhook")).ID("ToListener").Params(jen.ID("logger").ID("logging").Dot(
		"Logger",
