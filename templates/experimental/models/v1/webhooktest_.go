...
func webhookTestDotGo() *jen.File {
	...
	jen.ID("Name").Op(":").Lit("name"), jen.ID("ContentType").Op(":").Lit("application/json"), jen.ID("URL").Op(":").Lit("https://verygoodsoftwarenotvirus.ru"), jen.ID("Method").Op(":").Qual("net/http", "MethodPost"), jen.ID("Events").Op(":").Index().ID("string").Valuesln(
	jen.Lit("things")), jen.ID("DataTypes").Op(":").Index().ID("string").Valuesln(
	jen.Lit("stuff")), jen.ID("Topics").Op(":").Index().ID("string").Valuesln(
	jen.Lit("blah"))),
				jen.ID("exampleInput").Op(":=").Op("&").ID("WebhookUpdateInput").Valuesln(
	jen.ID("Name").Op(":").Lit("new name"), jen.ID("ContentType").Op(":").Lit("application/xml"), jen.ID("URL").Op(":").Lit("https://blah.verygoodsoftwarenotvirus.ru"), jen.ID("Method").Op(":").Qual("net/http", "MethodPatch"), jen.ID("Events").Op(":").Index().ID("string").Valuesln(
	jen.Lit("more_things")), jen.ID("DataTypes").Op(":").Index().ID("string").Valuesln(
	jen.Lit("new_stuff")), jen.ID("Topics").Op(":").Index().ID("string").Valuesln(
	jen.Lit("blah-blah"))),
