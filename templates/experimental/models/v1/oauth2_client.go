...
func oauth2ClientDotGo() *jen.File {
	...
	ret.Add(
		jen.Func().Comment("// GetID returns the client ID. NOTE: I believe this is implemented for the above interface spec (oauth2.ClientInfo)").Params(jen.ID("c").Op("*").ID("OAuth2Client")).ID("GetID").Params().Params(jen.ID("string")).Block(
			jen.Return().ID("c").Dot(
				"ClientID",
			),
		),
