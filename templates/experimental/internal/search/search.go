...
func searchDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Type().Defs(
			...
			jen.ID("IndexManagerProvider").Params(jen.ID("path").ID("IndexPath"), jen.ID("name").ID("IndexName"), jen.ID("logger").ID("logging").Dot("Logger")).Params(jen.ID("IndexManager"), jen.ID("error")),
		),
		jen.Line(),
	)
