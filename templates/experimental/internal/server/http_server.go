...
func httpServerDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("serverNamespace").Op("=").Lit("todo_service").Var().ID("loggerName").Op("=").Lit("api_server"),
		jen.Line(),
	)

	code.Add(
		jen.Null().Type().ID("HTTPServer").Struct(
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("HTTPServer")).ID("Serve").Params().Body(
			...
			jen.ID("http2ServerConf").Op(":=").Op("&").ID("http2").Dot("Server").Valuesln(),
	...
	code.Add(
		jen.Var().ID("maxTimeout").Op("=").Lit(120).Op("*").Qual("time", "Second").Var().ID("readTimeout").Op("=").Lit(5).Op("*").Qual("time", "Second").Var().ID("writeTimeout").Op("=").Lit(2).Op("*").ID("readTimeout").Var().ID("idleTimeout").Op("=").ID("maxTimeout"),
		jen.Line(),
	)
