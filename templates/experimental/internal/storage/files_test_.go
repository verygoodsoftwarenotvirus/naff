		...
		jen.Func().ID("TestUploader_ReadFile").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("b").Op(":=").ID("memblob").Dot("OpenBucket").Call(jen.Op("&").ID("memblob").Dot("Options").Valuesln()),
					...
					jen.ID("u").Op(":=").Op("&").ID("Uploader").Valuesln(jen.ID("bucket").Op(":").ID("b"), jen.ID("logger").Op(":").ID("logging").Dot("NewNoopLogger").Call(), jen.ID("tracer").Op(":").ID("tracing").Dot("NewTracer").Call(jen.ID("t").Dot("Name").Call()), jen.ID("filenameFetcher").Op(":").Func().Params(jen.Op("*").Qual("net/http", "Request")).Params(jen.ID("string")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("ctx").Op(":=").Qual("context", "Background").Call(),
					jen.ID("exampleFilename").Op(":=").Lit("hello_world.txt"),
					jen.ID("u").Op(":=").Op("&").ID("Uploader").Valuesln(jen.ID("bucket").Op(":").ID("memblob").Dot("OpenBucket").Call(jen.Op("&").ID("memblob").Dot("Options").Valuesln()), jen.ID("logger").Op(":").ID("logging").Dot("NewNoopLogger").Call(), jen.ID("tracer").Op(":").ID("tracing").Dot("NewTracer").Call(jen.ID("t").Dot("Name").Call()), jen.ID("filenameFetcher").Op(":").Func().Params(jen.Op("*").Qual("net/http", "Request")).Params(jen.ID("string")).Body(
						jen.Return().ID("t").Dot("Name").Call())),
					jen.List(jen.ID("x"), jen.ID("err")).Op(":=").ID("u").Dot("ReadFile").Call(
						jen.ID("ctx"),
						jen.ID("exampleFilename"),
