...
func bucketAzureDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("AzureProvider").Op("=").Lit("azure"),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("_").Qual("github.com/go-ozzo/ozzo-validation/v4", "ValidatableWithContext").Op("=").Parens(jen.Op("*").ID("AzureConfig")).Call(jen.ID("nil")),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Func().ID("provideAzureBucket").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("cfg").Op("*").ID("AzureConfig"), jen.ID("logger").ID("logging").Dot("Logger")).Params(jen.Op("*").Qual("gocloud.dev/blob", "Bucket"), jen.ID("error")).Body(
			...
			jen.Var().Defs(
				jen.ID("cred").ID("azblob").Dot("Credential"),
			...
			jen.If(jen.List(jen.ID("bucket"), jen.ID("err")).Op("=").ID("azureblob").Dot("OpenBucket").Call(
				...
				jen.Op("&").ID("azureblob").Dot("Options").Valuesln(jen.ID("Protocol").Op(":").Lit("https")),
			), jen.ID("err").Op("!=").ID("nil")).Body(
				jen.Return().List(jen.ID("nil"), jen.Qual("fmt", "Errorf").Call(
	...
	code.Add(
		jen.Func().ID("buildPipelineLogFunc").Params(jen.ID("logger").ID("logging").Dot("Logger")).Params(jen.Params(jen.ID("pipeline").Dot("LogLevel"), jen.ID("string"))).Body(
			jen.ID("logger").Op("=").ID("logging").Dot("EnsureLogger").Call(jen.ID("logger")),
	...
	code.Add(
		jen.Func().ID("buildPipelineOptions").Params(jen.ID("logger").ID("logging").Dot("Logger"), jen.ID("retrying").Op("*").ID("AzureRetryConfig")).Params(jen.ID("azblob").Dot("PipelineOptions")).Body(
			jen.ID("logger").Op("=").ID("logging").Dot("EnsureLogger").Call(jen.ID("logger")),
			jen.ID("options").Op(":=").ID("azblob").Dot("PipelineOptions").Valuesln(jen.ID("Log").Op(":").ID("pipeline").Dot("LogOptions").Valuesln(jen.ID("Log").Op(":").ID("buildPipelineLogFunc").Call(jen.ID("logger")), jen.ID("ShouldLog").Op(":").Func().Params(jen.ID("level").ID("pipeline").Dot("LogLevel")).Params(jen.ID("bool")).Body(
