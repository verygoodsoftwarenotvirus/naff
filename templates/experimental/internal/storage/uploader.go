...
func uploaderDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("MemoryProvider").Op("=").Lit("memory"),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Type().Defs(
			jen.ID("Uploader").Struct(
				...
				jen.ID("filenameFetcher").Params(jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.ID("string")),
			),
	...
	code.Add(
		...
		jen.Func().ID("NewUploadManager").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("logger").ID("logging").Dot("Logger"), jen.ID("cfg").Op("*").ID("Config"), jen.ID("routeParamManager").ID("routing").Dot("RouteParamManager")).Params(jen.Op("*").ID("Uploader"), jen.ID("error")).Body(
			...
			jen.ID("u").Op(":=").Op("&").ID("Uploader").Valuesln(jen.ID("logger").Op(":").ID("logging").Dot("EnsureLogger").Call(jen.ID("logger")).Dot("WithName").Call(jen.ID("serviceName")), jen.ID("tracer").Op(":").ID("tracing").Dot("NewTracer").Call(jen.ID("serviceName")), jen.ID("filenameFetcher").Op(":").ID("routeParamManager").Dot("BuildRouteParamStringIDFetcher").Call(jen.ID("cfg").Dot("UploadFilenameKey"))),
			...
			jen.Switch(jen.Qual("strings", "TrimSpace").Call(jen.Qual("strings", "ToLower").Call(jen.ID("cfg").Dot("Provider")))).Body(
				...
				jen.Case(jen.ID("S3Provider")).Body(
					jen.If(jen.ID("cfg").Dot("S3Config").Op("==").ID("nil")).Body(
						...
						jen.Op("&").ID("s3blob").Dot("Options").Valuesln(jen.ID("UseLegacyList").Op(":").ID("false")),
					), jen.ID("err").Op("!=").ID("nil")).Body(
						jen.Return().Qual("fmt", "Errorf").Call(
				...
				jen.Case(jen.ID("MemoryProvider")).Body(
					jen.ID("u").Dot("bucket").Op("=").ID("memblob").Dot("OpenBucket").Call(jen.Op("&").ID("memblob").Dot("Options").Valuesln())),
				jen.Default().Body(
					jen.If(jen.ID("cfg").Dot("FilesystemConfig").Op("==").ID("nil")).Body(
						...
						jen.Op("&").ID("fileblob").Dot("Options").Valuesln(jen.ID("URLSigner").Op(":").ID("nil"), jen.ID("CreateDir").Op(":").ID("true")),
					), jen.ID("err").Op("!=").ID("nil")).Body(
						jen.Return().Qual("fmt", "Errorf").Call(
