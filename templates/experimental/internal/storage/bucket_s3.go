...
func bucketS3DotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Type().Defs(
			jen.ID("S3Config").Struct(jen.ID("BucketName").ID("string")),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("_").Qual("github.com/go-ozzo/ozzo-validation/v4", "ValidatableWithContext").Op("=").Parens(jen.Op("*").ID("S3Config")).Call(jen.ID("nil")),
		),
		jen.Line(),
	)
