...
func tracerDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Null().Type().ID("errorHandler").Struct(jen.ID("logger").ID("logging").Dot("Logger")),
		jen.Line(),
	)
	...
	code.Add(
		jen.Func().ID("init").Params().Body(
			jen.Qual("go.opentelemetry.io/otel", "SetErrorHandler").Call(jen.ID("errorHandler").Valuesln(jen.ID("logger").Op(":").ID("logging").Dot("NewNonOperationalLogger").Call().Dot("WithName").Call(jen.Lit("otel_errors"))))),
		jen.Line(),
	)
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("c").Op("*").ID("Config")).ID("SetupJaeger").Params().Params(jen.Params(), jen.ID("error")).Body(
	...
	code.Add(
		jen.Null().Type().ID("Tracer").Interface(
