...
func instrumentedsqlDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("_").ID("instrumentedsql").Dot("Tracer").Op("=").Parens(jen.Op("*").ID("instrumentedSQLTracerWrapper")).Call(jen.ID("nil")),
		jen.Line(),
	)

	code.Add(
		jen.Null().Type().ID("instrumentedSQLTracerWrapper").Struct(jen.ID("tracer").ID("Tracer")),
		jen.Line(),
	)
	...
	code.Add(
		jen.Null().Type().ID("instrumentedSQLLoggerWrapper").Struct(jen.ID("logger").ID("logging").Dot("Logger")),
		jen.Line(),
	)
