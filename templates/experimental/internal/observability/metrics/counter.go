...
func counterDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("_").ID("UnitCounter").Op("=").Parens(jen.Op("*").ID("unitCounter")).Call(jen.ID("nil")),
		jen.Line(),
	)

	code.Add(
		jen.Null().Type().ID("unitCounter").Struct(jen.ID("counter").ID("metric").Dot("Int64Counter")),
		jen.Line(),
	)
