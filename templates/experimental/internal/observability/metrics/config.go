...
func configDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("defaultNamespace").Op("=").Lit("todo_server").Var().ID("instrumentationVersion").Op("=").Lit("1.0.0").Var().ID("minimumRuntimeCollectionInterval").Op("=").Qual("time", "Second").Var().ID("DefaultMetricsCollectionInterval").Op("=").Lit(2).Op("*").Qual("time", "Second").Var().ID("Prometheus").Op("=").Lit("prometheus"),
		jen.Line(),
	)
	...
	code.Add(
		jen.Var().ID("prometheusExporterInitOnce").Qual("sync", "Once").Var().ID("prometheusExporter").Op("*").Qual("go.opentelemetry.io/otel/exporters/metric/prometheus", "Exporter"),
		jen.Line(),
	)
	...
	code.Add(
		jen.Func().ID("initiatePrometheusExporter").Params().Body(
			jen.ID("prometheusExporterInitOnce").Dot("Do").Call(jen.Func().Params().Body(
				...
				jen.If(jen.List(jen.ID("prometheusExporter"), jen.ID("err")).Op("=").Qual("go.opentelemetry.io/otel/exporters/metric/prometheus", "InstallNewPipeline").Call(jen.Qual("go.opentelemetry.io/otel/exporters/metric/prometheus", "Config").Valuesln()), jen.ID("err").Op("!=").ID("nil")).Body(
