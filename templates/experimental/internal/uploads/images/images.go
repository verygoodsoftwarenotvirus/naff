...
func imagesDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		...
		jen.Func().ID("NewImageUploadProcessor").Params(jen.ID("logger").ID("logging").Dot("Logger")).Params(jen.ID("ImageUploadProcessor")).Body(
			jen.Return().Op("&").ID("uploadProcessor").Valuesln(jen.ID("logger").Op(":").ID("logging").Dot("EnsureLogger").Call(jen.ID("logger")).Dot("WithName").Call(jen.Lit("image_upload_processor")), jen.ID("tracer").Op(":").ID("tracing").Dot("NewTracer").Call(jen.Lit("image_upload_processor")))),
		jen.Line(),
	)
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("p").Op("*").ID("uploadProcessor")).ID("Process").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("req").Op("*").Qual("net/http", "Request"), jen.ID("filename").ID("string")).Params(jen.Op("*").ID("Image"), jen.ID("error")).Body(
			...
			jen.ID("i").Op(":=").Op("&").ID("Image").Valuesln(jen.ID("Filename").Op(":").ID("info").Dot("Filename"), jen.ID("ContentType").Op(":").ID("contentTypeFromFilename").Call(jen.ID("filename")), jen.ID("Data").Op(":").ID("bs"), jen.ID("Size").Op(":").ID("len").Call(jen.ID("bs"))),
