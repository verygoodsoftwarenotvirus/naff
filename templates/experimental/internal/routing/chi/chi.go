...
func chiDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("errInvalidMethod").Op("=").Qual("errors", "New").Call(jen.Lit("invalid method")),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("_").ID("routing").Dot("Router").Op("=").Parens(jen.Op("*").ID("router")).Call(jen.ID("nil")),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Func().ID("buildChiMux").Params(jen.ID("logger").ID("logging").Dot("Logger")).Params(jen.ID("chi").Dot("Router")).Body(
			jen.ID("ch").Op(":=").ID("cors").Dot("New").Call(jen.ID("cors").Dot("Options").Valuesln(jen.ID("AllowedOrigins").Op(":").Index().ID("string").Valuesln(jen.Lit("*")), jen.ID("AllowedMethods").Op(":").Index().ID("string").Valuesln(jen.Qual("net/http", "MethodGet"), jen.Qual("net/http", "MethodPost"), jen.Qual("net/http", "MethodPut"), jen.Qual("net/http", "MethodDelete"), jen.Qual("net/http", "MethodOptions")), jen.ID("AllowedHeaders").Op(":").Index().ID("string").Valuesln(jen.Lit("Accept"), jen.Lit("Authorization"), jen.Lit("RawHTML-Provider"), jen.Lit("X-CSRF-Token")), jen.ID("ExposedHeaders").Op(":").Index().ID("string").Valuesln(jen.Lit("Link")), jen.ID("AllowCredentials").Op(":").ID("true"), jen.ID("MaxAge").Op(":").ID("maxCORSAge"))),
			jen.ID("sec").Op(":=").ID("secure").Dot("New").Call(jen.ID("secure").Dot("Options").Valuesln(jen.ID("AllowedHosts").Op(":").Index().ID("string").Valuesln(jen.Lit("")), jen.ID("AllowedHostsAreRegex").Op(":").ID("false"), jen.ID("HostsProxyHeaders").Op(":").Index().ID("string").Valuesln(jen.Lit("X-Forwarded-Hosts")), jen.ID("SSLRedirect").Op(":").ID("true"), jen.ID("SSLTemporaryRedirect").Op(":").ID("false"), jen.ID("SSLHost").Op(":").Lit(""), jen.ID("SSLHostFunc").Op(":").ID("nil"), jen.ID("SSLProxyHeaders").Op(":").Map(jen.ID("string")).ID("string").Valuesln(jen.Lit("X-Forwarded-Proto").Op(":").Lit("https")), jen.ID("STSSeconds").Op(":").ID("int64").Call(jen.ID("roughlyOneYear").Dot("Seconds").Call()), jen.ID("STSIncludeSubdomains").Op(":").ID("true"), jen.ID("STSPreload").Op(":").ID("true"), jen.ID("ForceSTSHeader").Op(":").ID("false"), jen.ID("FrameDeny").Op(":").ID("true"), jen.ID("CustomFrameOptionsValue").Op(":").Lit(""), jen.ID("ContentTypeNosniff").Op(":").ID("true"), jen.ID("BrowserXssFilter").Op(":").ID("true"), jen.ID("CustomBrowserXssValue").Op(":").Lit(""), jen.ID("ContentSecurityPolicy").Op(":").Lit(""), jen.ID("PublicKey").Op(":").Lit(""), jen.ID("ReferrerPolicy").Op(":").Lit(""), jen.ID("FeaturePolicy").Op(":").Lit(""), jen.ID("ExpectCTHeader").Op(":").Lit(""), jen.ID("SecureContextKey").Op(":").Lit("secureContext"), jen.ID("IsDevelopment").Op(":").ID("true"))),
	...
	code.Add(
		jen.Func().ID("buildRouter").Params(jen.ID("mux").ID("chi").Dot("Router"), jen.ID("logger").ID("logging").Dot("Logger")).Params(jen.Op("*").ID("router")).Body(
			...
			jen.ID("r").Op(":=").Op("&").ID("router").Valuesln(jen.ID("router").Op(":").ID("mux"), jen.ID("tracer").Op(":").ID("tracing").Dot("NewTracer").Call(jen.Lit("router")), jen.ID("logger").Op(":").ID("logger")),
			jen.Return().ID("r"),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Func().ID("convertMiddleware").Params(jen.ID("in").Op("...").ID("routing").Dot("Middleware")).Params(jen.Index().Params(jen.ID("handler").Qual("net/http", "Handler")).Params(jen.Qual("net/http", "Handler"))).Body(
			jen.ID("out").Op(":=").Index().Params(jen.ID("handler").Qual("net/http", "Handler")).Params(jen.Qual("net/http", "Handler")).Valuesln(),
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("r").Op("*").ID("router")).ID("LogRoutes").Params().Body(
			jen.If(jen.ID("err").Op(":=").ID("chi").Dot("Walk").Call(
				jen.ID("r").Dot("router"),
				jen.Func().Params(jen.ID("method").ID("string"), jen.ID("route").ID("string"), jen.ID("_").Qual("net/http", "Handler"), jen.ID("_").Op("...").Params(jen.Qual("net/http", "Handler")).Params(jen.Qual("net/http", "Handler"))).Params(jen.ID("error")).Body(
					jen.ID("r").Dot("logger").Dot("WithValues").Call(jen.Map(jen.ID("string")).Interface().Valuesln(jen.Lit("method").Op(":").ID("method"), jen.Lit("route").Op(":").ID("route"))).Dot("Debug").Call(jen.Lit("route found")),
					jen.Return().ID("nil"),
				),
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("r").Op("*").ID("router")).ID("Route").Params(jen.ID("pattern").ID("string"), jen.ID("fn").Params(jen.ID("r").ID("routing").Dot("Router"))).Params(jen.ID("routing").Dot("Router")).Body(
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("r").Op("*").ID("router")).ID("BuildRouteParamIDFetcher").Params(jen.ID("logger").ID("logging").Dot("Logger"), jen.List(jen.ID("key"), jen.ID("logDescription")).ID("string")).Params(jen.Params(jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.ID("uint64"))).Body(
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("r").Op("*").ID("router")).ID("BuildRouteParamStringIDFetcher").Params(jen.ID("key").ID("string")).Params(jen.Params(jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.ID("string"))).Body(
