	...
	code.Add(
		jen.Func().ID("Test_BuildRouteParamIDFetcher").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					jen.ID("t").Dot("Parallel").Call(),
					jen.ID("r").Op(":=").Op("&").ID("chiRouteParamManager").Valuesln(),
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("req").Op("=").ID("req").Dot("WithContext").Call(jen.Qual("context", "WithValue").Call(
						jen.ID("ctx"),
						jen.ID("chi").Dot("RouteCtxKey"),
						jen.Op("&").ID("chi").Dot("Context").Valuesln(jen.ID("URLParams").Op(":").ID("chi").Dot("RouteParams").Valuesln(jen.ID("Keys").Op(":").Index().ID("string").Valuesln(jen.ID("exampleKey")), jen.ID("Values").Op(":").Index().ID("string").Valuesln(jen.Lit("expected")))),
					)),
					jen.ID("actual").Op(":=").ID("fn").Call(jen.ID("req")),
					jen.ID("assert").Dot("Equal").Call(
						jen.ID("t"),
