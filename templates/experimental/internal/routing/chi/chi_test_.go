...
func chiTestDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Func().ID("buildRouterForTest").Params().Params(jen.ID("routing").Dot("Router")).Body(
			jen.Return().ID("NewRouter").Call(jen.ID("logging").Dot("NewNoopLogger").Call())),
		jen.Line(),
	)
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("assert").Dot("NotNil").Call(
						jen.ID("t"),
						jen.ID("NewRouter").Call(jen.ID("logging").Dot("NewNoopLogger").Call()),
					),
				),
			),
		),
		...
		jen.Func().ID("Test_buildChiMux").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("assert").Dot("NotNil").Call(
						jen.ID("t"),
						jen.ID("buildChiMux").Call(jen.ID("logging").Dot("NewNoopLogger").Call()),
					),
				),
			),
		),
		...
		jen.Func().ID("Test_router_AddRoute").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("methods").Op(":=").Index().ID("string").Valuesln(jen.Qual("net/http", "MethodGet"), jen.Qual("net/http", "MethodHead"), jen.Qual("net/http", "MethodPost"), jen.Qual("net/http", "MethodPut"), jen.Qual("net/http", "MethodPatch"), jen.Qual("net/http", "MethodDelete"), jen.Qual("net/http", "MethodConnect"), jen.Qual("net/http", "MethodOptions"), jen.Qual("net/http", "MethodTrace")),
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("req").Op("=").ID("req").Dot("WithContext").Call(jen.Qual("context", "WithValue").Call(
						...
						jen.Op("&").ID("chi").Dot("Context").Valuesln(jen.ID("URLParams").Op(":").ID("chi").Dot("RouteParams").Valuesln(jen.ID("Keys").Op(":").Index().ID("string").Valuesln(jen.ID("exampleKey")), jen.ID("Values").Op(":").Index().ID("string").Valuesln(jen.Qual("strconv", "FormatUint").Call(
							jen.ID("expected"),
							jen.Lit(10),
						)))),
					)),
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("req").Op("=").ID("req").Dot("WithContext").Call(jen.Qual("context", "WithValue").Call(
						jen.ID("req").Dot("Context").Call(),
						jen.ID("chi").Dot("RouteCtxKey"),
						jen.Op("&").ID("chi").Dot("Context").Valuesln(jen.ID("URLParams").Op(":").ID("chi").Dot("RouteParams").Valuesln(jen.ID("Keys").Op(":").Index().ID("string").Valuesln(jen.ID("exampleKey")), jen.ID("Values").Op(":").Index().ID("string").Valuesln(jen.ID("expected")))),
					)),
					jen.ID("actual").Op(":=").ID("rf").Call(jen.ID("req")),
					jen.ID("assert").Dot("Equal").Call(
						jen.ID("t"),
