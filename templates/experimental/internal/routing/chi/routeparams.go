...
func routeparamsDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Type().Defs(
			jen.ID("chiRouteParamManager").Struct(),
		),
		jen.Line(),
	)
	...
	code.Add(
		...
		jen.Func().ID("NewRouteParamManager").Params().Params(jen.ID("routing").Dot("RouteParamManager")).Body(
			jen.Return().Op("&").ID("chiRouteParamManager").Valuesln()),
		jen.Line(),
	)
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("r").ID("chiRouteParamManager")).ID("BuildRouteParamIDFetcher").Params(jen.ID("logger").ID("logging").Dot("Logger"), jen.List(jen.ID("key"), jen.ID("logDescription")).ID("string")).Params(jen.Params(jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.ID("uint64"))).Body(
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("r").ID("chiRouteParamManager")).ID("BuildRouteParamStringIDFetcher").Params(jen.ID("key").ID("string")).Params(jen.Params(jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.ID("string"))).Body(
