...
func routeParamManagerDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		...
		jen.Func().ID("NewRouteParamManager").Params().Params(jen.Op("*").ID("RouteParamManager")).Body(
			jen.Return().Op("&").ID("RouteParamManager").Valuesln()),
		jen.Line(),
	)

	code.Add(
		jen.Type().Defs(
			jen.ID("RouteParamManager").Struct(jen.ID("mock").Dot("Mock")),
		),
		jen.Line(),
	)
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("m").Op("*").ID("RouteParamManager")).ID("BuildRouteParamIDFetcher").Params(jen.ID("logger").ID("logging").Dot("Logger"), jen.List(jen.ID("key"), jen.ID("logDescription")).ID("string")).Params(jen.Params(jen.Op("*").Qual("net/http", "Request")).Params(jen.ID("uint64"))).Body(
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("m").Op("*").ID("RouteParamManager")).ID("BuildRouteParamStringIDFetcher").Params(jen.ID("key").ID("string")).Params(jen.Params(jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.ID("string"))).Body(
