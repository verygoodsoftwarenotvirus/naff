...
func serviceDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("counterName").ID("metrics").Dot("CounterName").Op("=").Lit("webhooks"),
			jen.ID("counterDescription").ID("string").Op("=").Lit("the number of webhooks managed by the webhooks service"),
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("_").ID("types").Dot("WebhookDataService").Op("=").Parens(jen.Op("*").ID("service")).Call(jen.ID("nil")),
		),
		jen.Line(),
	)
	...
	code.Add(
		...
		jen.Func().ID("ProvideWebhooksService").Params(jen.ID("logger").ID("logging").Dot("Logger"), jen.ID("webhookDataManager").ID("types").Dot("WebhookDataManager"), jen.ID("encoder").ID("encoding").Dot("ServerEncoderDecoder"), jen.ID("counterProvider").ID("metrics").Dot("UnitCounterProvider"), jen.ID("routeParamManager").ID("routing").Dot("RouteParamManager")).Params(jen.ID("types").Dot("WebhookDataService")).Body(
			...
			), jen.ID("sessionContextDataFetcher").Op(":").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/services/authentication", "FetchContextFromRequest"), jen.ID("webhookIDFetcher").Op(":").ID("routeParamManager").Dot("BuildRouteParamIDFetcher").Call(
				jen.ID("logger"),
