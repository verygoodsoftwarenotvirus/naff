...
func serviceDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("_").ID("types").Dot("AccountDataService").Op("=").Parens(jen.Op("*").ID("service")).Call(jen.ID("nil")),
		),
		jen.Line(),
	)
	...
	code.Add(
		...
		jen.Func().ID("ProvideService").Params(jen.ID("logger").ID("logging").Dot("Logger"), jen.ID("accountDataManager").ID("types").Dot("AccountDataManager"), jen.ID("accountMembershipDataManager").ID("types").Dot("AccountUserMembershipDataManager"), jen.ID("encoder").ID("encoding").Dot("ServerEncoderDecoder"), jen.ID("counterProvider").ID("metrics").Dot("UnitCounterProvider"), jen.ID("routeParamManager").ID("routing").Dot("RouteParamManager")).Params(jen.ID("types").Dot("AccountDataService")).Body(
			jen.Return().Op("&").ID("service").Valuesln(jen.ID("logger").Op(":").ID("logging").Dot("EnsureLogger").Call(jen.ID("logger")).Dot("WithName").Call(jen.ID("serviceName")), jen.ID("accountIDFetcher").Op(":").ID("routeParamManager").Dot("BuildRouteParamIDFetcher").Call(
				jen.ID("logger"),
				...
				jen.Lit("user"),
			), jen.ID("sessionContextDataFetcher").Op(":").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/services/authentication", "FetchContextFromRequest"), jen.ID("accountDataManager").Op(":").ID("accountDataManager"), jen.ID("accountMembershipDataManager").Op(":").ID("accountMembershipDataManager"), jen.ID("encoderDecoder").Op(":").ID("encoder"), jen.ID("accountCounter").Op(":").ID("metrics").Dot("EnsureUnitCounter").Call(
