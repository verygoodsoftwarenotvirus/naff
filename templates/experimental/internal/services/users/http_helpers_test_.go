...
func httpHelpersTestDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Null().Type().ID("usersServiceHTTPRoutesTestHelper").Struct(
	...
	code.Add(
		jen.Func().ID("newTestHelper").Params(jen.ID("t").Op("*").Qual("testing", "T")).Params(jen.Op("*").ID("usersServiceHTTPRoutesTestHelper")).Body(
			jen.ID("t").Dot("Helper").Call(),
			jen.ID("helper").Op(":=").Op("&").ID("usersServiceHTTPRoutesTestHelper").Valuesln(),
			...
			jen.ID("helper").Dot("service").Dot("userIDFetcher").Op("=").Func().Params(jen.Op("*").Qual("net/http", "Request")).Params(jen.ID("uint64")).Body(
