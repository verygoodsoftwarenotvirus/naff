...
func serviceDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			...
			jen.ID("counterDescription").Op("=").Lit("number of users managed by the users service"),
			jen.ID("counterName").Op("=").ID("metrics").Dot("CounterName").Call(jen.Lit("users")),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("_").ID("types").Dot("UserDataService").Op("=").Parens(jen.Op("*").ID("service")).Call(jen.ID("nil")),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Type().Defs(
			jen.ID("RequestValidator").Interface(jen.ID("Validate").Params(jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.ID("bool"), jen.ID("error"))),
			jen.ID("service").Struct(
				jen.ID("userDataManager").ID("types").Dot("UserDataManager"),
				jen.ID("accountDataManager").ID("types").Dot("AccountDataManager"),
				jen.ID("authSettings").Op("*").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/services/authentication", "Config"),
	...
	code.Add(
		...
		jen.Func().ID("ProvideUsersService").Params(jen.ID("authSettings").Op("*").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/services/authentication", "Config"), jen.ID("logger").ID("logging").Dot("Logger"), jen.ID("userDataManager").ID("types").Dot("UserDataManager"), jen.ID("accountDataManager").ID("types").Dot("AccountDataManager"), jen.ID("authenticator").ID("authentication").Dot("Authenticator"), jen.ID("encoder").ID("encoding").Dot("ServerEncoderDecoder"), jen.ID("counterProvider").ID("metrics").Dot("UnitCounterProvider"), jen.ID("imageUploadProcessor").ID("images").Dot("ImageUploadProcessor"), jen.ID("uploadManager").ID("uploads").Dot("UploadManager"), jen.ID("routeParamManager").ID("routing").Dot("RouteParamManager")).Params(jen.ID("types").Dot("UserDataService")).Body(
			jen.Return().Op("&").ID("service").Valuesln(jen.ID("logger").Op(":").ID("logging").Dot("EnsureLogger").Call(jen.ID("logger")).Dot("WithName").Call(jen.ID("serviceName")), jen.ID("userDataManager").Op(":").ID("userDataManager"), jen.ID("accountDataManager").Op(":").ID("accountDataManager"), jen.ID("authenticator").Op(":").ID("authenticator"), jen.ID("userIDFetcher").Op(":").ID("routeParamManager").Dot("BuildRouteParamIDFetcher").Call(
				...
				jen.Lit("user"),
			), jen.ID("sessionContextDataFetcher").Op(":").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/services/authentication", "FetchContextFromRequest"), jen.ID("encoderDecoder").Op(":").ID("encoder"), jen.ID("authSettings").Op(":").ID("authSettings"), jen.ID("userCounter").Op(":").ID("metrics").Dot("EnsureUnitCounter").Call(
				jen.ID("counterProvider"),
				jen.ID("logger"),
				jen.ID("counterName"),
				jen.ID("counterDescription"),
			), jen.ID("secretGenerator").Op(":").ID("random").Dot("NewGenerator").Call(jen.ID("logger")), jen.ID("tracer").Op(":").ID("tracing").Dot("NewTracer").Call(jen.ID("serviceName")), jen.ID("imageUploadProcessor").Op(":").ID("imageUploadProcessor"), jen.ID("uploadManager").Op(":").ID("uploadManager"))),
		jen.Line(),
	)
