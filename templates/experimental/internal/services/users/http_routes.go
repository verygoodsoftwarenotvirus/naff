...
func httpRoutesDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("UserIDURIParamKey").Op("=").Lit("userID").Var().ID("totpIssuer").Op("=").Lit("todoService").Var().ID("base64ImagePrefix").Op("=").Lit("data:image/jpeg;base64,").Var().ID("minimumPasswordEntropy").Op("=").Lit(75).Var().ID("totpSecretSize").Op("=").Lit(64),
		jen.Line(),
	)

	code.Add(
		jen.Func().Comment("validateCredentialChangeRequest takes a user's credentials and determines").Comment("if they match what is on record.").Params(jen.ID("s").Op("*").ID("service")).ID("validateCredentialChangeRequest").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64"), jen.List(jen.ID("password"), jen.ID("totpToken")).ID("string")).Params(jen.ID("user").Op("*").ID("types").Dot("User"), jen.ID("httpStatus").ID("int")).Body(
	...
	code.Add(
		jen.Var().ID("errSecretAlreadyVerified").Op("=").Qual("errors", "New").Call(jen.Lit("secret already verified")),
		jen.Line(),
	)
	...
	code.Add(
		jen.Func().Comment("TOTPSecretVerificationHandler accepts a TOTP token as input and returns 200 if the TOTP token").Comment("is validated by the user's TOTP secret.").Params(jen.ID("s").Op("*").ID("service")).ID("TOTPSecretVerificationHandler").Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("s").Dot("tracer").Dot("StartSpan").Call(jen.ID("req").Dot("Context").Call()),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.ID("logger").Op(":=").ID("s").Dot("logger").Dot("WithRequest").Call(jen.ID("req")),
			jen.ID("tracing").Dot("AttachRequestToSpan").Call(
	...
	code.Add(
		jen.Func().Comment("UpdatePasswordHandler updates a user's password.").Params(jen.ID("s").Op("*").ID("service")).ID("UpdatePasswordHandler").Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("s").Dot("tracer").Dot("StartSpan").Call(jen.ID("req").Dot("Context").Call()),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.ID("logger").Op(":=").ID("s").Dot("logger").Dot("WithRequest").Call(jen.ID("req")),
			jen.ID("tracing").Dot("AttachRequestToSpan").Call(
	...
	code.Add(
		jen.Func().Comment("AvatarUploadHandler updates a user's avatar.").Params(jen.ID("s").Op("*").ID("service")).ID("AvatarUploadHandler").Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("s").Dot("tracer").Dot("StartSpan").Call(jen.ID("req").Dot("Context").Call()),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.ID("logger").Op(":=").ID("s").Dot("logger").Dot("WithRequest").Call(jen.ID("req")),
			jen.ID("tracing").Dot("AttachRequestToSpan").Call(
