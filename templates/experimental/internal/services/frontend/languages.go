...
func languagesDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Type().Defs(
			jen.ID("displayLanguage").Op("*").ID("string"),
			jen.ID("languageDetail").Struct(
	...
	code.Add(
		jen.Var().Defs(
			...
			jen.ID("englishDetails").Op("=").Op("&").ID("languageDetail").Valuesln(jen.ID("Name").Op(":").Lit("English"), jen.ID("Abbreviation").Op(":").Lit("en-US"), jen.ID("Tag").Op(":").ID("language").Dot("AmericanEnglish")),
			jen.ID("spanish").Op("=").ID("new").Call(jen.ID("displayLanguage")),
			jen.ID("spanishDetails").Op("=").Op("&").ID("languageDetail").Valuesln(jen.ID("Name").Op(":").Lit("Spanish"), jen.ID("Abbreviation").Op(":").Lit("es-419"), jen.ID("Tag").Op(":").ID("language").Dot("LatinAmericanSpanish")),
			jen.ID("languageDetails").Op("=").Map(jen.Op("*").ID("displayLanguage")).Op("*").ID("languageDetail").Valuesln(jen.ID("english").Op(":").ID("englishDetails"), jen.ID("spanish").Op(":").ID("spanishDetails")),
			jen.ID("supportedLanguages").Op("=").Index().Op("*").ID("displayLanguage").Valuesln(jen.ID("english"), jen.ID("spanish")),
