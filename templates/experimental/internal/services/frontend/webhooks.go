...
func webhooksDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("webhookIDURLParamKey").Op("=").Lit("webhook"),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("webhookEditorTemplate").ID("string"),
		),
		jen.Line(),
	)

	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("service")).ID("buildWebhookEditorView").Params(jen.ID("includeBaseTemplate").ID("bool")).Params(jen.Params(jen.Qual("net/http", "ResponseWriter"), jen.Op("*").Qual("net/http", "Request"))).Body(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Body(
				...
				jen.ID("tmplFuncMap").Op(":=").Map(jen.ID("string")).Interface().Valuesln(jen.Lit("componentTitle").Op(":").Func().Params(jen.ID("x").Op("*").ID("types").Dot("Webhook")).Params(jen.ID("string")).Body(
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("webhooksTableTemplate").ID("string"),
		),
		jen.Line(),
	)

	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("service")).ID("buildWebhooksTableView").Params(jen.ID("includeBaseTemplate").ID("bool")).Params(jen.Params(jen.Qual("net/http", "ResponseWriter"), jen.Op("*").Qual("net/http", "Request"))).Body(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Body(
				jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("s").Dot("tracer").Dot("StartSpan").Call(jen.ID("req").Dot("Context").Call()),
				jen.Defer().ID("span").Dot("End").Call(),
				jen.ID("logger").Op(":=").ID("s").Dot("logger").Dot("WithRequest").Call(jen.ID("req")),
				...
				jen.ID("tmplFuncMap").Op(":=").Map(jen.ID("string")).Interface().Valuesln(jen.Lit("individualURL").Op(":").Func().Params(jen.ID("x").Op("*").ID("types").Dot("Webhook")).Params(jen.Qual("html/template", "URL")).Body(
				...
				jen.If(jen.ID("includeBaseTemplate")).Body(
					...
					jen.ID("page").Op(":=").Op("&").ID("pageData").Valuesln(jen.ID("IsLoggedIn").Op(":").ID("sessionCtxData").Op("!=").ID("nil"), jen.ID("Title").Op(":").Lit("Webhooks"), jen.ID("ContentData").Op(":").ID("webhooks")),
					jen.If(jen.ID("sessionCtxData").Op("!=").ID("nil")).Body(
						jen.ID("page").Dot("IsServiceAdmin").Op("=").ID("sessionCtxData").Dot("Requester").Dot("ServicePermissions").Dot("IsServiceAdmin").Call()),
					jen.ID("s").Dot("renderTemplateToResponse").Call(
						jen.ID("ctx"),
