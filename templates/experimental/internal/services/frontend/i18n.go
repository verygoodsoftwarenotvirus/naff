...
func i18NDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("translationsDir").Qual("embed", "FS"),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("service")).ID("getSimpleLocalizedString").Params(jen.ID("messageID").ID("string")).Params(jen.ID("string")).Body(
			jen.Return().ID("s").Dot("localizer").Dot("MustLocalize").Call(jen.Op("&").ID("i18n").Dot("LocalizeConfig").Valuesln(jen.ID("MessageID").Op(":").ID("messageID"), jen.ID("DefaultMessage").Op(":").ID("nil"), jen.ID("TemplateData").Op(":").ID("nil"), jen.ID("Funcs").Op(":").ID("nil")))),
		jen.Line(),
	)
