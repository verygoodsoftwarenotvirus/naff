...
func itemsTestDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Func().ID("attachItemCreationInputToRequest").Params(jen.ID("input").Op("*").ID("types").Dot("ItemCreationInput")).Params(jen.Op("*").Qual("net/http", "Request")).Body(
			jen.ID("form").Op(":=").Qual("net/url", "Values").Valuesln(jen.ID("itemCreationInputNameFormKey").Op(":").Valuesln(jen.ID("input").Dot("Name")), jen.ID("itemCreationInputDetailsFormKey").Op(":").Valuesln(jen.ID("input").Dot("Details"))),
		...
		jen.Func().ID("TestService_parseFormEncodedItemCreationInput").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("sessionCtxData").Op(":=").Op("&").ID("types").Dot("SessionContextData").Valuesln(jen.ID("ActiveAccountID").Op(":").ID("expected").Dot("BelongsToAccount")),
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("badBody").Op(":=").Op("&").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/tests/utils", "MockReadCloser").Valuesln(),
					jen.ID("badBody").Dot("On").Call(
						jen.Lit("Read"),
						jen.ID("mock").Dot("IsType").Call(jen.Index().ID("byte").Valuesln()),
					).Dot("Return").Call(
						jen.Lit(0),
						jen.Qual("errors", "New").Call(jen.Lit("blah")),
					),
					...
					jen.ID("sessionCtxData").Op(":=").Op("&").ID("types").Dot("SessionContextData").Valuesln(jen.ID("ActiveAccountID").Op(":").ID("exampleInput").Dot("BelongsToAccount")),
					jen.ID("actual").Op(":=").ID("s").Dot("parseFormEncodedItemCreationInput").Call(
						jen.ID("ctx"),
						jen.ID("req"),
						jen.ID("sessionCtxData"),
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("s").Op(":=").ID("buildTestService").Call(jen.ID("t")),
					jen.ID("ctx").Op(":=").Qual("context", "Background").Call(),
					jen.ID("exampleInput").Op(":=").Op("&").ID("types").Dot("ItemCreationInput").Valuesln(),
		...
		jen.Func().ID("TestService_handleItemCreationRequest").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("exampleInput").Dot("BelongsToAccount").Op("=").ID("exampleSessionContextData").Dot("ActiveAccountID"),
					jen.ID("res").Op(":=").ID("httptest").Dot("NewRecorder").Call(),
					jen.ID("req").Op(":=").ID("attachItemCreationInputToRequest").Call(jen.Op("&").ID("types").Dot("ItemCreationInput").Valuesln()),
					jen.ID("mockDB").Op(":=").ID("database").Dot("BuildMockDatabase").Call(),
					jen.ID("mockDB").Dot("ItemDataManager").Dot("On").Call(
						jen.Lit("CreateItem"),
						jen.Qual("gitlab.com/verygoodsoftwarenotvirus/todo/tests/utils", "ContextMatcher"),
		...
		jen.Func().ID("TestService_parseFormEncodedItemUpdateInput").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("s").Op(":=").ID("buildTestService").Call(jen.ID("t")),
					jen.ID("ctx").Op(":=").Qual("context", "Background").Call(),
					jen.ID("exampleInput").Op(":=").Op("&").ID("types").Dot("ItemUpdateInput").Valuesln(),
					jen.ID("sessionCtxData").Op(":=").ID("fakes").Dot("BuildFakeSessionContextData").Call(),
