		...
		jen.Func().ID("Test_renderPrice").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					jen.ID("t").Dot("Parallel").Call(),
					jen.ID("inputsAndExpectations").Op(":=").Map(jen.ID("uint32")).ID("string").Valuesln(jen.Lit(12345).Op(":").Lit("$123.45"), jen.Lit(42069).Op(":").Lit("$420.69"), jen.Lit(666).Op(":").Lit("$6.66")),
		...
		jen.Func().ID("Test_service_handleCheckoutSessionStart").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("mpm").Op(":=").Op("&").ID("capitalism").Dot("MockPaymentManager").Valuesln(),
