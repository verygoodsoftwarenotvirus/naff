...
func usersDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("usersTableTemplate").ID("string"),
		),
		jen.Line(),
	)

	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("service")).ID("buildUsersTableView").Params(jen.List(jen.ID("includeBaseTemplate"), jen.ID("forSearch")).ID("bool")).Params(jen.Params(jen.Qual("net/http", "ResponseWriter"), jen.Op("*").Qual("net/http", "Request"))).Body(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Body(
				...
				jen.If(jen.ID("forSearch")).Body(
					...
					jen.ID("users").Op("=").Op("&").ID("types").Dot("UserList").Valuesln(jen.ID("Users").Op(":").ID("searchResults")),
				).Else().Body(
				...
				jen.If(jen.ID("includeBaseTemplate")).Body(
					...
					jen.ID("page").Op(":=").Op("&").ID("pageData").Valuesln(jen.ID("IsLoggedIn").Op(":").ID("sessionCtxData").Op("!=").ID("nil"), jen.ID("Title").Op(":").Lit("Users"), jen.ID("ContentData").Op(":").ID("users")),
