...
func authDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Type().Defs(
			jen.ID("loginPromptData").Struct(jen.ID("RedirectTo").ID("string")),
		),
		jen.Line(),
	)

	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("service")).ID("buildLoginView").Params(jen.ID("includeBaseTemplate").ID("bool")).Params(jen.Params(jen.Qual("net/http", "ResponseWriter"), jen.Op("*").Qual("net/http", "Request"))).Body(
			jen.Return().Func().Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Body(
				...
				jen.ID("contentData").Op(":=").Op("&").ID("loginPromptData").Valuesln(jen.ID("RedirectTo").Op(":").ID("pluckRedirectURL").Call(jen.ID("req"))),
				jen.If(jen.ID("includeBaseTemplate")).Body(
					jen.ID("tmpl").Op(":=").ID("s").Dot("renderTemplateIntoBaseTemplate").Call(
						jen.ID("loginPrompt"),
						jen.ID("nil"),
					),
					jen.ID("data").Op(":=").ID("pageData").Valuesln(jen.ID("IsLoggedIn").Op(":").ID("false"), jen.ID("Title").Op(":").Lit("BeginSession"), jen.ID("ContentData").Op(":").ID("contentData")),
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("service")).ID("parseFormEncodedLoginRequest").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.ID("loginData").Op("*").ID("types").Dot("UserLoginInput"), jen.ID("redirectTo").ID("string")).Body(
			...
			jen.ID("loginData").Op("=").Op("&").ID("types").Dot("UserLoginInput").Valuesln(jen.ID("Username").Op(":").ID("form").Dot("Get").Call(jen.ID("usernameFormKey")), jen.ID("Password").Op(":").ID("form").Dot("Get").Call(jen.ID("passwordFormKey")), jen.ID("TOTPToken").Op(":").ID("form").Dot("Get").Call(jen.ID("totpTokenFormKey"))),
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("registrationPrompt").ID("string"),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("service")).ID("registrationView").Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Body(
			...
			jen.ID("data").Op(":=").ID("pageData").Valuesln(jen.ID("IsLoggedIn").Op(":").ID("false"), jen.ID("Title").Op(":").Lit("Register"), jen.ID("ContentData").Op(":").ID("nil")),
			jen.ID("s").Dot("renderTemplateToResponse").Call(
				jen.ID("ctx"),
				jen.ID("tmpl"),
				jen.ID("data"),
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("successfulRegistrationResponse").ID("string"),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Type().Defs(
			jen.ID("totpVerificationPrompt").Struct(
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("service")).ID("parseFormEncodedRegistrationRequest").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.Op("*").ID("types").Dot("UserRegistrationInput")).Body(
			...
			jen.If(jen.ID("err").Op("!=").ID("nil")).Body(
				jen.Return().ID("nil")),
			jen.ID("input").Op(":=").Op("&").ID("types").Dot("UserRegistrationInput").Valuesln(jen.ID("Username").Op(":").ID("form").Dot("Get").Call(jen.ID("usernameFormKey")), jen.ID("Password").Op(":").ID("form").Dot("Get").Call(jen.ID("passwordFormKey"))),
	...
	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("service")).ID("handleRegistrationSubmission").Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Body(
			...
			jen.Var().Defs(
				jen.ID("ucr").Op("*").ID("types").Dot("UserCreationResponse"),
			),
			jen.If(jen.Op("!").ID("s").Dot("useFakeData")).Body(
				jen.Var().Defs(
					jen.ID("err").ID("error"),
				...
				jen.ID("ucr").Op("=").Op("&").ID("types").Dot("UserCreationResponse").Valuesln(jen.ID("TwoFactorQRCode").Op(":").Lit(""))),
			jen.ID("tmpl").Op(":=").ID("s").Dot("parseTemplate").Call(
				jen.ID("ctx"),
				jen.Lit(""),
			...
			jen.ID("tmplData").Op(":=").Op("&").ID("totpVerificationPrompt").Valuesln(jen.ID("TwoFactorQRCode").Op(":").Qual("html/template", "URL").Call(jen.ID("ucr").Dot("TwoFactorQRCode")), jen.ID("UserID").Op(":").ID("ucr").Dot("CreatedUserID")),
			jen.ID("s").Dot("renderTemplateToResponse").Call(
				jen.ID("ctx"),
				jen.ID("tmpl"),
				jen.ID("tmplData"),
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("service")).ID("parseFormEncodedTOTPSecretVerificationRequest").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("req").Op("*").Qual("net/http", "Request")).Params(jen.Op("*").ID("types").Dot("TOTPSecretVerificationInput")).Body(
			...
			jen.If(jen.ID("err").Op("!=").ID("nil")).Body(
				jen.Return().ID("nil")),
			jen.ID("input").Op(":=").Op("&").ID("types").Dot("TOTPSecretVerificationInput").Valuesln(jen.ID("UserID").Op(":").ID("userID"), jen.ID("TOTPToken").Op(":").ID("form").Dot("Get").Call(jen.ID("totpTokenFormKey"))),
