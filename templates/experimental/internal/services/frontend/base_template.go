...
func baseTemplateDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("baseTemplateSrc").ID("string"),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Func().Params(jen.ID("s").Op("*").ID("service")).ID("homepage").Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Body(
			...
			jen.ID("x").Op(":=").Op("&").ID("pageData").Valuesln(jen.ID("IsLoggedIn").Op(":").ID("sessionCtxData").Op("!=").ID("nil"), jen.ID("Title").Op(":").Lit("Home"), jen.ID("ContentData").Op(":").Lit("")),
