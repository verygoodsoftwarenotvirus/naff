...
func billingDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("pricePanicker").Op("=").ID("panicking").Dot("NewProductionPanicker").Call(),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("priceDivisor").Op("=").Lit(0.01),
	...
	code.Add(
		jen.Func().Comment("NOTE: this function panics when it receives a number > 100,000, as it is not meant to handle those kinds of prices.").ID("renderPrice").Params(jen.ID("p").ID("uint32")).Params(jen.ID("string")).Body(
