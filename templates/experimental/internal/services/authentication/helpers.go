...
func helpersDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Comment("In the event that there's an error, this function will return false and the error.").Params(jen.ID("s").Op("*").ID("service")).ID("validateLogin").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("user").Op("*").ID("types").Dot("User"), jen.ID("loginInput").Op("*").ID("types").Dot("UserLoginInput")).Params(jen.ID("bool"), jen.ID("error")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("s").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
