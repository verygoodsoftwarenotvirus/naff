...
func httpRoutesDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("LogEntryURIParamKey").Op("=").Lit("entryID"),
		),
		jen.Line(),
	)
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("s").Op("*").ID("service")).ID("ListHandler").Params(jen.ID("res").Qual("net/http", "ResponseWriter"), jen.ID("req").Op("*").Qual("net/http", "Request")).Body(
			...
			jen.If(jen.Qual("errors", "Is").Call(
				...
				jen.ID("entries").Op("=").Op("&").ID("types").Dot("AuditLogEntryList").Valuesln(jen.ID("Entries").Op(":").Index().Op("*").ID("types").Dot("AuditLogEntry").Valuesln())).Else().If(jen.ID("err").Op("!=").ID("nil")).Body(
				jen.ID("observability").Dot("AcknowledgeError").Call(
					jen.ID("err"),
					jen.ID("logger"),
					jen.ID("span"),
