...
func serviceDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("serviceName").Op("=").Lit("audit_log_entries_service"),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("_").ID("types").Dot("AuditLogEntryDataService").Op("=").Parens(jen.Op("*").ID("service")).Call(jen.ID("nil")),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Type().Defs(
			jen.ID("service").Struct(
				...
				jen.ID("auditLogEntryIDFetcher").Params(jen.Op("*").Qual("net/http", "Request")).Params(jen.ID("uint64")),
	...
	code.Add(
		...
		jen.Func().ID("ProvideService").Params(jen.ID("logger").ID("logging").Dot("Logger"), jen.ID("auditLog").ID("types").Dot("AuditLogEntryDataManager"), jen.ID("encoder").ID("encoding").Dot("ServerEncoderDecoder"), jen.ID("routeParamManager").ID("routing").Dot("RouteParamManager")).Params(jen.ID("types").Dot("AuditLogEntryDataService")).Body(
			jen.Return().Op("&").ID("service").Valuesln(jen.ID("logger").Op(":").ID("logging").Dot("EnsureLogger").Call(jen.ID("logger")).Dot("WithName").Call(jen.ID("serviceName")), jen.ID("auditLog").Op(":").ID("auditLog"), jen.ID("auditLogEntryIDFetcher").Op(":").ID("routeParamManager").Dot("BuildRouteParamIDFetcher").Call(
				jen.ID("logger"),
				jen.ID("LogEntryURIParamKey"),
				jen.Lit("audit log entry"),
			), jen.ID("sessionContextDataFetcher").Op(":").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/services/authentication", "FetchContextFromRequest"), jen.ID("encoderDecoder").Op(":").ID("encoder"), jen.ID("tracer").Op(":").ID("tracing").Dot("NewTracer").Call(jen.ID("serviceName")))),
		jen.Line(),
	)
