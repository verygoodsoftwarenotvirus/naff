...
func serviceDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Type().Defs(
			jen.ID("service").Struct(
				jen.ID("config").Op("*").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/services/authentication", "Config"),
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().ID("ProvideService").Params(jen.ID("logger").ID("logging").Dot("Logger"), jen.ID("cfg").Op("*").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/services/authentication", "Config"), jen.ID("authenticator").ID("authentication").Dot("Authenticator"), jen.ID("userDataManager").ID("types").Dot("AdminUserDataManager"), jen.ID("auditLog").ID("types").Dot("AdminAuditManager"), jen.ID("sessionManager").Op("*").ID("scs").Dot("SessionManager"), jen.ID("encoder").ID("encoding").Dot("ServerEncoderDecoder"), jen.ID("routeParamManager").ID("routing").Dot("RouteParamManager")).Params(jen.ID("types").Dot("AdminService")).Body(
			jen.ID("svc").Op(":=").Op("&").ID("service").Valuesln(jen.ID("logger").Op(":").ID("logging").Dot("EnsureLogger").Call(jen.ID("logger")).Dot("WithName").Call(jen.ID("serviceName")), jen.ID("encoderDecoder").Op(":").ID("encoder"), jen.ID("config").Op(":").ID("cfg"), jen.ID("userDB").Op(":").ID("userDataManager"), jen.ID("auditLog").Op(":").ID("auditLog"), jen.ID("authenticator").Op(":").ID("authenticator"), jen.ID("sessionManager").Op(":").ID("sessionManager"), jen.ID("sessionContextDataFetcher").Op(":").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/services/authentication", "FetchContextFromRequest"), jen.ID("userIDFetcher").Op(":").ID("routeParamManager").Dot("BuildRouteParamIDFetcher").Call(
				jen.ID("logger"),
