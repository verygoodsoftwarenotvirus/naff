	...
	code.Add(
		jen.Func().ID("buildTestService").Params(jen.ID("t").Op("*").Qual("testing", "T")).Params(jen.Op("*").ID("service")).Body(
			...
			jen.ID("s").Op(":=").ID("ProvideService").Call(
				jen.ID("logger"),
				jen.Op("&").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/services/authentication", "Config").Valuesln(jen.ID("Cookies").Op(":").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/services/authentication", "CookieConfig").Valuesln(jen.ID("SigningKey").Op(":").Lit("BLAHBLAHBLAHPRETENDTHISISSECRET!"))),
				jen.Op("&").ID("authentication").Dot("MockAuthenticator").Valuesln(),
				jen.Op("&").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/pkg/types/mock", "AdminUserDataManager").Valuesln(),
				jen.Op("&").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/pkg/types/mock", "AuditLogEntryDataManager").Valuesln(),
