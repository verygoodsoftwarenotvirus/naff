...
func mockRandDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("_").ID("Generator").Op("=").Parens(jen.Op("*").ID("MockGenerator")).Call(jen.ID("nil")),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Type().Defs(
			jen.ID("MockGenerator").Struct(jen.Qual("github.com/stretchr/testify/mock", "Mock")),
		),
		jen.Line(),
	)

	code.Add(
		jen.Comment("GenerateBase32EncodedString implements our interface."),
		jen.Line(),
