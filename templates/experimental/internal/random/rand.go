...
func randDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("arbitrarySize").ID("uint16").Op("=").Lit(128),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("_").ID("Generator").Op("=").Parens(jen.Op("*").ID("standardGenerator")).Call(jen.ID("nil")),
			jen.ID("defaultGenerator").Op("=").ID("NewGenerator").Call(jen.ID("logging").Dot("NewNoopLogger").Call()),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Type().Defs(
			...
			jen.ID("standardGenerator").Struct(
	...
	code.Add(
		...
		jen.Func().ID("NewGenerator").Params(jen.ID("logger").ID("logging").Dot("Logger")).Params(jen.ID("Generator")).Body(
			jen.Return().Op("&").ID("standardGenerator").Valuesln(jen.ID("logger").Op(":").ID("logging").Dot("EnsureLogger").Call(jen.ID("logger")).Dot("WithName").Call(jen.Lit("random_string_generator")), jen.ID("tracer").Op(":").ID("tracing").Dot("NewTracer").Call(jen.Lit("secret_generator")), jen.ID("randReader").Op(":").Qual("crypto/rand", "Reader"))),
		jen.Line(),
	)
