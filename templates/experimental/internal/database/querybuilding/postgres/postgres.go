...
func postgresDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("loggerName").Op("=").Lit("postgres").Var().ID("driverName").Op("=").Lit("wrapped-postgres-driver").Var().ID("columnCountQueryTemplate").Op("=").Lit(`COUNT(%s.id)`).Var().ID("allCountQuery").Op("=").Lit(`COUNT(*)`).Var().ID("jsonPluckQuery").Op("=").Lit(`%s.%s->'%s'`),
		jen.Line(),
	)

	code.Add(
		jen.Var().ID("currentUnixTimeQuery").Op("=").ID("squirrel").Dot("Expr").Call(jen.Lit(`extract(epoch FROM NOW())`)),
		jen.Line(),
	)

	code.Add(
		jen.Var().ID("_").ID("querybuilding").Dot("SQLQueryBuilder").Op("=").Parens(jen.Op("*").ID("Postgres")).Call(jen.ID("nil")),
		jen.Line(),
	)

	code.Add(
		jen.Null().Type().ID("Postgres").Struct(
	...
	code.Add(
		jen.Var().ID("instrumentedDriverRegistration").Qual("sync", "Once"),
		jen.Line(),
	)
			...
			jen.ID("instrumentedDriverRegistration").Dot("Do").Call(jen.Func().Params().Body(
				jen.Qual("database/sql", "Register").Call(
					...
					jen.ID("instrumentedsql").Dot("WrapDriver").Call(
						jen.Op("&").Qual("github.com/lib/pq", "Driver").Valuesln(),
	...
	code.Add(
		...
		jen.Func().ID("ProvidePostgres").Params(jen.ID("logger").ID("logging").Dot("Logger")).Params(jen.Op("*").ID("Postgres")).Body(
			jen.ID("pg").Op(":=").Op("&").ID("Postgres").Valuesln(jen.ID("logger").Op(":").ID("logging").Dot("EnsureLogger").Call(jen.ID("logger")).Dot("WithName").Call(jen.ID("loggerName")), jen.ID("tracer").Op(":").ID("tracing").Dot("NewTracer").Call(jen.Lit("postgres_query_builder")), jen.ID("sqlBuilder").Op(":").ID("squirrel").Dot("StatementBuilder").Dot("PlaceholderFormat").Call(jen.ID("squirrel").Dot("Dollar")), jen.ID("externalIDGenerator").Op(":").ID("querybuilding").Dot("UUIDExternalIDGenerator").Valuesln()),
			jen.Return().ID("pg"),
		),
		jen.Line(),
	)
