...
func sqliteTestDotGo(proj *models.Project) *jen.File {
	code := jen.NewFile(packageName)

	utils.AddImports(proj, code, false)

	code.Add(
		jen.Var().ID("defaultLimit").Op("=").ID("uint8").Call(jen.Lit(20)),
		jen.Line(),
	)

	code.Add(
		jen.Func().ID("buildTestService").Params(jen.ID("t").Op("*").Qual("testing", "T")).Params(jen.Op("*").ID("Sqlite"), jen.ID("sqlmock").Dot("Sqlmock")).Body(
			...
			jen.ID("q").Op(":=").ID("ProvideSqlite").Call(jen.ID("logging").Dot("NewNonOperationalLogger").Call()),
			jen.Return().List(jen.ID("q"), jen.ID("mock")),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Func().ID("TestProvideSqlite").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
	...
	code.Add(
		jen.Func().ID("TestSqlite_logQueryBuildingError").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			jen.ID("T").Dot("Parallel").Call(),
			jen.ID("T").Dot("Run").Call(
				jen.Lit("standard"),
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
	...
	code.Add(
		jen.Func().ID("TestProvideSqliteDB").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			jen.ID("T").Dot("Parallel").Call(),
			jen.ID("T").Dot("Run").Call(
				jen.Lit("standard"),
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					jen.ID("t").Dot("Parallel").Call(),
					jen.List(jen.ID("_"), jen.ID("err")).Op(":=").ID("ProvideSqliteDB").Call(
