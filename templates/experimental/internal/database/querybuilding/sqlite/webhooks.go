...
func webhooksDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("_").ID("querybuilding").Dot("WebhookSQLQueryBuilder").Op("=").Parens(jen.Op("*").ID("Sqlite")).Call(jen.ID("nil")),
		jen.Line(),
	)

	code.Add(
		jen.Func().Comment("BuildGetWebhookQuery returns a SQL query (and arguments) for retrieving a given webhook.").Params(jen.ID("b").Op("*").ID("Sqlite")).ID("BuildGetWebhookQuery").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("webhookID"), jen.ID("accountID")).ID("uint64")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Body(
	...
	code.Add(
		jen.Func().Comment("BuildGetWebhooksQuery returns a SQL query (and arguments) that would return a list of webhooks.").Params(jen.ID("b").Op("*").ID("Sqlite")).ID("BuildGetWebhooksQuery").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("accountID").ID("uint64"), jen.ID("filter").Op("*").ID("types").Dot("QueryFilter")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Body(
			jen.List(jen.ID("_"), jen.ID("span")).Op(":=").ID("b").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
