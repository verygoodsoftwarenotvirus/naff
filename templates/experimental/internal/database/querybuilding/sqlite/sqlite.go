...
func sqliteDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("loggerName").Op("=").Lit("sqlite").Var().ID("driverName").Op("=").Lit("wrapped-sqlite-driverName").Var().ID("columnCountQueryTemplate").Op("=").Lit(`COUNT(%s.id)`).Var().ID("allCountQuery").Op("=").Lit(`COUNT(*)`).Var().ID("jsonPluckQuery").Op("=").Lit(`json_extract(%s.%s, '$.%s')`),
		jen.Line(),
	)

	code.Add(
		jen.Var().ID("currentUnixTimeQuery").Op("=").ID("squirrel").Dot("Expr").Call(jen.Lit(`(strftime('%s','now'))`)),
		jen.Line(),
	)

	code.Add(
		jen.Var().ID("_").ID("querybuilding").Dot("SQLQueryBuilder").Op("=").Parens(jen.Op("*").ID("Sqlite")).Call(jen.ID("nil")),
		jen.Line(),
	)

	code.Add(
		jen.Null().Type().ID("Sqlite").Struct(
