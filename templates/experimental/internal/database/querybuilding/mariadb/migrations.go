...
func migrationsDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("migrations").Op("=").Index().ID("darwin").Dot("Migration").Valuesln(jen.Valuesln(jen.ID("Version").Op(":").Lit(0.00), jen.ID("Description").Op(":").Lit("create sessions table for session manager"), jen.ID("Script").Op(":").Qual("strings", "Join").Call(
