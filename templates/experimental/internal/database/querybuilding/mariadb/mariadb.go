...
func mariadbDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("loggerName").Op("=").Lit("mariadb").Var().ID("driverName").Op("=").Lit("wrapped-mariadb-driverName").Var().ID("columnCountQueryTemplate").Op("=").Lit(`COUNT(%s.id)`).Var().ID("allCountQuery").Op("=").Lit(`COUNT(*)`).Var().ID("jsonPluckQuery").Op("=").Lit(`JSON_CONTAINS(%s.%s, '%d', '$.%s')`),
		jen.Line(),
	)

	code.Add(
		jen.Var().ID("currentUnixTimeQuery").Op("=").ID("squirrel").Dot("Expr").Call(jen.Lit(`UNIX_TIMESTAMP()`)),
		jen.Line(),
	)

	code.Add(
		jen.Var().ID("_").ID("querybuilding").Dot("SQLQueryBuilder").Op("=").Parens(jen.Op("*").ID("MariaDB")).Call(jen.ID("nil")),
		jen.Line(),
	)

	code.Add(
		jen.Null().Type().ID("MariaDB").Struct(
