...
func genericDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("b").Op("*").ID("MariaDB")).ID("buildQueryOnly").Params(jen.ID("span").ID("tracing").Dot("Span"), jen.ID("builder").ID("squirrel").Dot("Sqlizer")).Params(jen.ID("string")).Body(
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Params(jen.ID("b").Op("*").ID("MariaDB")).ID("buildQuery").Params(jen.ID("span").ID("tracing").Dot("Span"), jen.ID("builder").ID("squirrel").Dot("Sqlizer")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Body(
	...
	code.Add(
		jen.Func().Params(jen.ID("b").Op("*").ID("MariaDB")).ID("buildTotalCountQuery").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("tableName"), jen.ID("ownershipColumn")).ID("string"), jen.ID("userID").ID("uint64"), jen.List(jen.ID("forAdmin"), jen.ID("includeArchived")).ID("bool")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Body(
	...
	code.Add(
		jen.Func().Params(jen.ID("b").Op("*").ID("MariaDB")).ID("buildFilteredCountQuery").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("tableName"), jen.ID("ownershipColumn")).ID("string"), jen.ID("userID").ID("uint64"), jen.List(jen.ID("forAdmin"), jen.ID("includeArchived")).ID("bool"), jen.ID("filter").Op("*").ID("types").Dot("QueryFilter")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Body(
			jen.List(jen.ID("_"), jen.ID("span")).Op(":=").ID("b").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Comment("and returns both the query and the relevant args to pass to the query executor.").Params(jen.ID("b").Op("*").ID("MariaDB")).ID("buildListQuery").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("tableName"), jen.ID("ownershipColumn")).ID("string"), jen.ID("columns").Index().ID("string"), jen.ID("userID").ID("uint64"), jen.ID("forAdmin").ID("bool"), jen.ID("filter").Op("*").ID("types").Dot("QueryFilter")).Params(jen.ID("query").ID("string"), jen.ID("args").Index().Interface()).Body(
			jen.List(jen.ID("_"), jen.ID("span")).Op(":=").ID("b").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.If(jen.ID("filter").Op("!=").ID("nil")).Body(
				jen.ID("tracing").Dot("AttachFilterToSpan").Call(
