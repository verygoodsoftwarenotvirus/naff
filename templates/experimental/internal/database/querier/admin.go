...
func adminDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().Defs(
			jen.ID("_").ID("types").Dot("AdminUserDataManager").Op("=").Parens(jen.Op("*").ID("SQLQuerier")).Call(jen.ID("nil")),
		),
		jen.Line(),
	)

	code.Add(
		jen.Comment("UpdateUserReputation updates a user's account status."),
		jen.Line(),
		jen.Func().Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("UpdateUserReputation").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64"), jen.ID("input").Op("*").ID("types").Dot("UserReputationUpdateInput")).Params(jen.ID("error")).Body(
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("LogUserBanEvent").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("banGiver"), jen.ID("banRecipient")).ID("uint64"), jen.ID("reason").ID("string")).Body(
			...
			jen.ID("q").Dot("createAuditLogEntry").Call(
				jen.ID("ctx"),
				jen.ID("q").Dot("db"),
				jen.ID("audit").Dot("BuildUserBanEventEntry").Call(
					jen.ID("banGiver"),
					jen.ID("banRecipient"),
					jen.ID("reason"),
				),
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("LogAccountTerminationEvent").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("terminator"), jen.ID("terminee")).ID("uint64"), jen.ID("reason").ID("string")).Body(
			...
			jen.ID("q").Dot("createAuditLogEntry").Call(
				jen.ID("ctx"),
				jen.ID("q").Dot("db"),
				jen.ID("audit").Dot("BuildAccountTerminationEventEntry").Call(
					jen.ID("terminator"),
					jen.ID("terminee"),
					jen.ID("reason"),
				),
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("LogCycleCookieSecretEvent").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64")).Body(
			...
			jen.ID("q").Dot("createAuditLogEntry").Call(
				jen.ID("ctx"),
				jen.ID("q").Dot("db"),
				jen.ID("audit").Dot("BuildCycleCookieSecretEvent").Call(jen.ID("userID")),
			),
		),
		jen.Line(),
	)
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("LogSuccessfulLoginEvent").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64")).Body(
			...
			jen.ID("q").Dot("createAuditLogEntry").Call(
				jen.ID("ctx"),
				jen.ID("q").Dot("db"),
				jen.ID("audit").Dot("BuildSuccessfulLoginEventEntry").Call(jen.ID("userID")),
			),
		),
		jen.Line(),
	)
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("LogBannedUserLoginAttemptEvent").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64")).Body(
			...
			jen.ID("q").Dot("createAuditLogEntry").Call(
				jen.ID("ctx"),
				jen.ID("q").Dot("db"),
				jen.ID("audit").Dot("BuildBannedUserLoginAttemptEventEntry").Call(jen.ID("userID")),
			),
		),
		jen.Line(),
	)
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("LogUnsuccessfulLoginBadPasswordEvent").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64")).Body(
			...
			jen.ID("q").Dot("createAuditLogEntry").Call(
				jen.ID("ctx"),
				jen.ID("q").Dot("db"),
				jen.ID("audit").Dot("BuildUnsuccessfulLoginBadPasswordEventEntry").Call(jen.ID("userID")),
			),
		),
		jen.Line(),
	)
	...
	code.Add(
		jen.Comment("LogUnsuccessfulLoginBad2FATokenEvent implements our AuditLogEntryDataManager interface."),
		jen.Line(),
		jen.Func().Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("LogUnsuccessfulLoginBad2FATokenEvent").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("q").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.ID("tracing").Dot("AttachUserIDToSpan").Call(
				jen.ID("span"),
			...
			jen.ID("q").Dot("createAuditLogEntry").Call(
				jen.ID("ctx"),
				jen.ID("q").Dot("db"),
				jen.ID("audit").Dot("BuildUnsuccessfulLoginBad2FATokenEventEntry").Call(jen.ID("userID")),
			),
		),
		jen.Line(),
	)
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("LogLogoutEvent").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64")).Body(
			...
			jen.ID("q").Dot("createAuditLogEntry").Call(
				jen.ID("ctx"),
				jen.ID("q").Dot("db"),
				jen.ID("audit").Dot("BuildLogoutEventEntry").Call(jen.ID("userID")),
			),
		),
		jen.Line(),
	)
