...
func accountUserMembershipsDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("_").ID("types").Dot("AccountUserMembershipDataManager").Op("=").Parens(jen.Op("*").ID("SQLQuerier")).Call(jen.ID("nil")),
		jen.Line(),
	)
	...
	code.Add(
		jen.Func().Comment("scanAccountUserMembership takes a database Scanner (i.e. *sql.Row) and scans the result into an AccountUserMembership struct.").Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("scanAccountUserMembership").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("scan").ID("database").Dot("Scanner")).Params(jen.ID("x").Op("*").ID("types").Dot("AccountUserMembership"), jen.ID("err").ID("error")).Body(
			jen.List(jen.ID("_"), jen.ID("span")).Op(":=").ID("q").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.ID("x").Op("=").Op("&").ID("types").Dot("AccountUserMembership").Valuesln(),
			jen.Var().ID("rawAccountRoles").ID("string"),
			...
			jen.If(jen.ID("roles").Op(":=").Qual("strings", "Split").Call(
				...
				jen.ID("x").Dot("AccountRoles").Op("=").Index().ID("string").Valuesln()),
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("scanAccountUserMemberships").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("rows").ID("database").Dot("ResultIterator")).Params(jen.ID("defaultAccount").ID("uint64"), jen.ID("accountRolesMap").Map(jen.ID("uint64")).Index().ID("string"), jen.ID("err").ID("error")).Body(
			jen.List(jen.ID("_"), jen.ID("span")).Op(":=").ID("q").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.ID("accountRolesMap").Op("=").Map(jen.ID("uint64")).Index().ID("string").Valuesln(),
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("BuildSessionContextDataForUser").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64")).Params(jen.Op("*").ID("types").Dot("SessionContextData"), jen.ID("error")).Body(
			...
			jen.ID("actualAccountRolesMap").Op(":=").Map(jen.ID("uint64")).ID("authorization").Dot("AccountRolePermissionsChecker").Valuesln(),
	...
	code.Add(
		jen.Func().Comment("RemoveUserFromAccount removes a user's membership to an account.").Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("RemoveUserFromAccount").Params(jen.ID("ctx").Qual("context", "Context"), jen.List(jen.ID("userID"), jen.ID("accountID"), jen.ID("removedByUser")).ID("uint64"), jen.ID("reason").ID("string")).Params(jen.ID("error")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("q").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
