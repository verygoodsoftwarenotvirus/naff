...
func usersDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("_").ID("types").Dot("UserDataManager").Op("=").Parens(jen.Op("*").ID("SQLQuerier")).Call(jen.ID("nil")),
		jen.Line(),
	)

	code.Add(
		jen.Var().ID("serviceRolesSeparator").Op("=").Lit(","),
		jen.Line(),
	)

	code.Add(
		jen.Func().Comment("scanUser provides a consistent way to scan something like a *sql.Row into a Requester struct.").Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("scanUser").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("scan").ID("database").Dot("Scanner"), jen.ID("includeCounts").ID("bool")).Params(jen.ID("user").Op("*").ID("types").Dot("User"), jen.List(jen.ID("filteredCount"), jen.ID("totalCount")).ID("uint64"), jen.ID("err").ID("error")).Body(
			...
			jen.If(jen.ID("roles").Op(":=").Qual("strings", "Split").Call(
				...
				jen.ID("user").Dot("ServiceRoles").Op("=").Index().ID("string").Valuesln()),
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("getUser").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64"), jen.ID("withVerifiedTOTPSecret").ID("bool")).Params(jen.Op("*").ID("types").Dot("User"), jen.ID("error")).Body(
			...
			jen.Var().ID("query").ID("string").Var().ID("args").Index().Interface(),
	...
	code.Add(
		jen.Func().Comment("createUser creates a user. The `user` and `account` parameters are meant to be filled out.").Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("createUser").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("user").Op("*").ID("types").Dot("User"), jen.ID("account").Op("*").ID("types").Dot("Account"), jen.ID("userCreationQuery").ID("string"), jen.ID("userCreationArgs").Index().Interface()).Params(jen.ID("error")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("q").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.ID("logger").Op(":=").ID("q").Dot("logger").Dot("WithValue").Call(
				jen.Lit("username"),
	...
	code.Add(
		jen.Func().Comment("GetUserWithUnverifiedTwoFactorSecret fetches a user with an unverified 2FA secret.").Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("GetUserWithUnverifiedTwoFactorSecret").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64")).Params(jen.Op("*").ID("types").Dot("User"), jen.ID("error")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("q").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.If(jen.ID("userID").Op("==").Lit(0)).Body(
				jen.Return().List(jen.ID("nil"), jen.ID("ErrInvalidIDProvided"))),
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("SearchForUsersByUsername").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("usernameQuery").ID("string")).Params(jen.Index().Op("*").ID("types").Dot("User"), jen.ID("error")).Body(
			...
			jen.If(jen.ID("usernameQuery").Op("==").Lit("")).Body(
				jen.Return().List(jen.Index().Op("*").ID("types").Dot("User").Valuesln(), jen.ID("ErrEmptyInputProvided"))),
	...
	code.Add(
		...
		jen.Func().Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("GetUsers").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("filter").Op("*").ID("types").Dot("QueryFilter")).Params(jen.ID("x").Op("*").ID("types").Dot("UserList"), jen.ID("err").ID("error")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("q").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.ID("x").Op("=").Op("&").ID("types").Dot("UserList").Valuesln(),
	...
	code.Add(
		...
		jen.Line(),
		jen.Func().Comment("NOTE: this function uses the ID provided in the input to make its query.").Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("UpdateUser").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("updated").Op("*").ID("types").Dot("User"), jen.ID("changes").Index().Op("*").ID("types").Dot("FieldChangeSummary")).Params(jen.ID("error")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("q").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
	...
	code.Add(
		jen.Func().Comment("UpdateUserPassword updates a user's passwords hash in the database.").Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("UpdateUserPassword").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64"), jen.ID("newHash").ID("string")).Params(jen.ID("error")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("q").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.If(jen.ID("userID").Op("==").Lit(0)).Body(
	...
	code.Add(
		jen.Func().Comment("UpdateUserTwoFactorSecret marks a user's two factor secret as validated.").Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("UpdateUserTwoFactorSecret").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64"), jen.ID("newSecret").ID("string")).Params(jen.ID("error")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("q").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.If(jen.ID("userID").Op("==").Lit(0)).Body(
				jen.Return().ID("ErrInvalidIDProvided")),
	...
	code.Add(
		jen.Func().Comment("MarkUserTwoFactorSecretAsVerified marks a user's two factor secret as validated.").Params(jen.ID("q").Op("*").ID("SQLQuerier")).ID("MarkUserTwoFactorSecretAsVerified").Params(jen.ID("ctx").Qual("context", "Context"), jen.ID("userID").ID("uint64")).Params(jen.ID("error")).Body(
			jen.List(jen.ID("ctx"), jen.ID("span")).Op(":=").ID("q").Dot("tracer").Dot("StartSpan").Call(jen.ID("ctx")),
			jen.Defer().ID("span").Dot("End").Call(),
			jen.If(jen.ID("userID").Op("==").Lit(0)).Body(
				jen.Return().ID("ErrInvalidIDProvided")),
