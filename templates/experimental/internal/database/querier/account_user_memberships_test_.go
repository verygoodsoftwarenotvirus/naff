		...
		jen.Func().ID("TestQuerier_BuildSessionContextDataForUser").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("examplePermsMap").Op(":=").Map(jen.ID("uint64")).Op("*").ID("types").Dot("UserAccountMembershipInfo").Valuesln(),
					...
					jen.ID("exampleAccountPermissionsMap").Op(":=").Map(jen.ID("uint64")).ID("authorization").Dot("AccountRolePermissionsChecker").Valuesln(),
					jen.For(jen.List(jen.ID("_"), jen.ID("membership")).Op(":=").Range().ID("exampleAccount").Dot("Members")).Body(
