	...
	code.Add(
		jen.Func().ID("TestProvideSessionManager").Params(jen.ID("T").Op("*").Qual("testing", "T")).Body(
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					jen.ID("t").Dot("Parallel").Call(),
					jen.ID("cookieConfig").Op(":=").Qual("gitlab.com/verygoodsoftwarenotvirus/todo/internal/services/authentication", "CookieConfig").Valuesln(),
			...
			jen.ID("T").Dot("Run").Call(
				...
				jen.Func().Params(jen.ID("t").Op("*").Qual("testing", "T")).Body(
					...
					jen.ID("mockDB").Dot("ExpectQuery").Call(jen.Lit("SELECT VERSION()")).Dot("WillReturnRows").Call(jen.ID("sqlmock").Dot("NewRows").Call(jen.Index().ID("string").Valuesln(jen.Lit("version"))).Dot("AddRow").Call(jen.Lit("1.2.3"))),
					jen.List(jen.ID("sessionManager"), jen.ID("err")).Op(":=").ID("ProvideSessionManager").Call(
						jen.ID("cookieConfig"),
						jen.ID("cfg"),
						jen.ID("db"),
