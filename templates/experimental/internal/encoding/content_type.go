...
func contentTypeDotGo(proj *models.Project) *jen.File {
	...
	code.Add(
		jen.Var().ID("ContentTypeJSON").ID("ContentType").Op("=").ID("buildContentType").Call(jen.ID("contentTypeJSON")).Var().ID("ContentTypeXML").ID("ContentType").Op("=").ID("buildContentType").Call(jen.ID("contentTypeXML")),
		jen.Line(),
	)

	code.Add(
		jen.Null().Type().ID("ContentType").Op("*").ID("contentType").Type().ID("contentType").Op("*").ID("string"),
		jen.Line(),
	)
