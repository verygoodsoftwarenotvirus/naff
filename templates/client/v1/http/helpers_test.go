...
import (
	...
	"errors"
	"fmt"
	mock "gitlab.com/verygoodsoftwarenotvirus/todo/tests/v1/testutil/mock"
	"io/ioutil"
	"net/http"
	"strings"
	"testing"
...
func TestUnmarshalBody(T *testing.T) {
	...
	T.Run("expected use", func(t *testing.T) {
		expected := "example"
		res := &http.Response{Body: ioutil.NopCloser(strings.NewReader(fmt.Sprintf("{\"name\": %q}", expected))), StatusCode: http.StatusOK}
		var out testingType
	...
	T.Run("with good status but unmarshallable response", func(t *testing.T) {
		res := &http.Response{Body: ioutil.NopCloser(strings.NewReader("\n\n\t\t\t\t\tBLAH\n\n\t\t\t\t")), StatusCode: http.StatusOK}
		var out testingType
		err := unmarshalBody(res, &out)
	...
	T.Run("with an erroneous error code", func(t *testing.T) {
		res := &http.Response{Body: ioutil.NopCloser(strings.NewReader(func() string {
	...
	T.Run("with erroneous reader", func(t *testing.T) {
		...
		rc := mock.NewMockReadCloser()
		rc.On("Read", mock.Anything).Return(0, expected)
		res := &http.Response{Body: rc, StatusCode: http.StatusOK}
		var out testingType
		err := unmarshalBody(res, &out)
