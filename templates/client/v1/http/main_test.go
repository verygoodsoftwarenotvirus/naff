...
var exampleURI = "https://todo.verygoodsoftwarenotvirus.ru"
...
func buildTestClient(t *testing.T, ts *httptest.Server) *V1Client {
	...
	return &V1Client{URL: u, plainClient: ts.Client(), logger: l, Debug: true, authedClient: ts.Client()}
}
...
func TestV1Client_TokenSource(T *testing.T) {
	...
	T.Run("obligatory", func(t *testing.T) {
		ts := httptest.NewTLSServer(nil)
		c, err := NewClient(context.Background(), "", "", mustParseURL(exampleURI), noop.ProvideNoopLogger(), ts.Client(), []string{"*"}, false)
...
func TestNewClient(T *testing.T) {
	...
	T.Run("with client but invalid timeout", func(t *testing.T) {
		c, err := NewClient(context.Background(), "", "", mustParseURL(exampleURI), noop.ProvideNoopLogger(), &http.Client{Timeout: 0}, []string{"*"}, true)
		require.NotNil(t, c)
		require.NoError(t, err)
...
func TestNewSimpleClient(T *testing.T) {
	T.Parallel()
	T.Run("obligatory", func(t *testing.T) {
		c, err := NewSimpleClient(context.Background(), mustParseURL(exampleURI), true)
		require.NotNil(t, c)
		require.NoError(t, err)
	})
}
...
func TestBuildURL(T *testing.T) {
	...
	T.Run("various urls", func(t *testing.T) {
		...
		testCases := []struct {
			...
			inputParts  []string
			inputQuery  valuer
		}{{expectation: "https://todo.verygoodsoftwarenotvirus.ru/api/v1/things", inputParts: []string{"things"}}, {expectation: "https://todo.verygoodsoftwarenotvirus.ru/api/v1/stuff?key=value", inputQuery: map[string][]string{"key": {"value"}}, inputParts: []string{"stuff"}}, {expectation: "https://todo.verygoodsoftwarenotvirus.ru/api/v1/things/and/stuff?key=value1&key=value2&yek=eulav", inputQuery: map[string][]string{"key": {"value1", "value2"}, "yek": {"eulav"}}, inputParts: []string{"things", "and", "stuff"}}}
