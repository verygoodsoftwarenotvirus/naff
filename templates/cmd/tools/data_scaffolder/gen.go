			...
			initializers = append(initializers, jen.Newline(), jen.Newline(),
				...
				jen.Go().Func().Params(jen.ID("wg").PointerTo().Qual("sync", "WaitGroup")).Body(
					jen.For(jen.ID("j").Assign().Zero(), jen.ID("j").LessThan().Int().Call(jen.ID("dataCount")), jen.ID("j").Increment()).Body(
						...
						jen.List(jen.IDf("created%sID", typ.Name.Singular()), jen.IDf("%sCreationErr", typ.Name.UnexportedVarName())).Assign().ID("userClient").Dotf("Create%s", typ.Name.Singular()).Call(constants.CtxVar(), jen.Qual(proj.FakeTypesPackage(), fmt.Sprintf("BuildFake%sCreationRequestInput", typ.Name.Singular())).Call()),
						...
						jen.Newline(),
						jen.ID("iterationLogger").Dot("WithValue").Call(jen.Qual(proj.ConstantKeysPackage(), fmt.Sprintf("%sIDKey", typ.Name.Singular())), jen.IDf("created%sID", typ.Name.Singular())).Dot("Debug").Call(jen.Litf("created %s", typ.Name.SingularCommonName())),
					),
